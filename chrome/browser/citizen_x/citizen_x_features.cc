...
bool CitizenXDisabledByPolicy(content::BrowserContext* context) {
...
bool CitizenXOmniboxEnabled(content::BrowserContext* context) {
  ...
  if (!profile)
    return false;
  return !CitizenXDisabledByPolicy(context) &&
...
bool HasPageAction(content::BrowserContext* context, bool is_popup_mode) {
  ...
  return true;
#else
  return (CitizenXOmniboxEnabled(context) && !is_popup_mode);
#endif
}
