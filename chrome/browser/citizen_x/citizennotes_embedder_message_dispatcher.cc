...
using DispatchCallback = CitizenNotesEmbedderMessageDispatcher::DispatchCallback;
...
bool GetValue(const base::Value& value, CNRegisterOptions* options) {
  if (!value.is_dict())
    return false;

  const bool synced = value.GetDict().FindBool("synced").value_or(false);
  options->sync_mode = synced ? CNRegisterOptions::SyncMode::kSync
                              : CNRegisterOptions::SyncMode::kDontSync;
  return true;
}
 ...
 * isolated renderer (citizennotes:// or chrome://inspect) to the embedder
 * in the browser.
 *
 * The messages are sent via InspectorFrontendHost.sendMessageToEmbedder or
 * chrome.send method accordingly.
 */
class DispatcherImpl : public CitizenNotesEmbedderMessageDispatcher {
 public:
...
CitizenNotesEmbedderMessageDispatcher::CreateForCitizenNotesFrontend(
    Delegate* delegate) {
  ...
  d->RegisterHandler("dispatchProtocolMessage",
                     &Delegate::DispatchProtocolMessageFromCitizenNotesFrontend,
                     delegate);
