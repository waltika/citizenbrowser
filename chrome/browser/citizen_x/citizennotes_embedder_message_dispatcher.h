...
#ifndef CHROME_BROWSER_CITIZENNOTES_CITIZENNOTES_EMBEDDER_MESSAGE_DISPATCHER_H_
#define CHROME_BROWSER_CITIZENNOTES_CITIZENNOTES_EMBEDDER_MESSAGE_DISPATCHER_H_
 ...
 * isolated renderer (on citizenotes://) to the embedder in the browser.
 *
 * The messages are sent via InspectorFrontendHost.sendMessageToEmbedder method.
 */
class CitizenNotesEmbedderMessageDispatcher {
 public:
  class Delegate {
   public:
    ...
    virtual void RegisterPreference(const std::string& name,
                                    const CNRegisterOptions& options) = 0;
    ...
    virtual void DispatchProtocolMessageFromCitizenNotesFrontend(
  ...
  virtual ~CitizenNotesEmbedderMessageDispatcher() = default;
  virtual bool Dispatch(DispatchCallback callback,
                        const std::string& method,
                        const base::Value::List& params) = 0;

  static std::unique_ptr<CitizenNotesEmbedderMessageDispatcher>
  CreateForCitizenNotesFrontend(Delegate* delegate);
};

#endif  // CHROME_BROWSER_CITIZENNOTES_CITIZENNOTES_EMBEDDER_MESSAGE_DISPATCHER_H_
