...
CitizenNotesFileHelper::FileSystem CreateFileSystemStruct(
    WebContents* web_contents,
  ...
  return CitizenNotesFileHelper::FileSystem(type, file_system_name, root_url,
                                        file_system_path);
}
...
PathToType GetAddedFileSystemPaths(Profile* profile) {
  const base::Value::Dict& file_systems_paths_value =
      profile->GetPrefs()->GetDict(prefs::kCitizenNotesFileSystemPaths);
  PathToType result;
...
CitizenNotesFileHelper::FileSystem::FileSystem() = default;

CitizenNotesFileHelper::FileSystem::~FileSystem() = default;

CitizenNotesFileHelper::FileSystem::FileSystem(const FileSystem& other) = default;

CitizenNotesFileHelper::FileSystem::FileSystem(const std::string& type,
...
CitizenNotesFileHelper::CitizenNotesFileHelper(WebContents* web_contents,
                                       Profile* profile,
                                       Delegate* delegate)
...
CitizenNotesFileHelper::~CitizenNotesFileHelper() = default;

void CitizenNotesFileHelper::Save(const std::string& url,
  ...
  const base::Value::Dict& file_map =
      profile_->GetPrefs()->GetDict(prefs::kCitizenNotesEditedFiles);
  ...
  SelectFileDialog::Show(base::BindOnce(&CitizenNotesFileHelper::SaveAsFileSelected,
...
void CitizenNotesFileHelper::Append(const std::string& url,
                                const std::string& content,
...
void CitizenNotesFileHelper::SaveAsFileSelected(const std::string& url,
                                            const std::string& content,
  ...
  *g_last_save_path.Pointer() = path;
  ...
  ScopedDictPrefUpdate update(profile_->GetPrefs(),
                              prefs::kCitizenNotesEditedFiles);
...
void CitizenNotesFileHelper::AddFileSystem(
    const std::string& type,
    const ShowInfoBarCallback& show_info_bar_callback) {
  SelectFileDialog::Show(
      base::BindOnce(&CitizenNotesFileHelper::InnerAddFileSystem,
                     weak_factory_.GetWeakPtr(), show_info_bar_callback, type),
      base::BindOnce(&CitizenNotesFileHelper::FailedToAddFileSystem,
                     weak_factory_.GetWeakPtr(), kSelectionCancelled),
      web_contents_, ui::SelectFileDialog::SELECT_FOLDER, base::FilePath());
}

void CitizenNotesFileHelper::UpgradeDraggedFileSystemPermissions(
...
void CitizenNotesFileHelper::InnerAddFileSystem(
    const ShowInfoBarCallback& show_info_bar_callback,
    const std::string& type,
    const base::FilePath& path) {
  std::string file_system_path = path.AsUTF8Unsafe();
  ...
  std::u16string message =
      l10n_util::GetStringFUTF16(IDS_CITIZEN_NOTES_CONFIRM_ADD_FILE_SYSTEM_MESSAGE,
                                 base::UTF8ToUTF16(path_display_name));
  show_info_bar_callback.Run(
      message, BindOnce(&CitizenNotesFileHelper::AddUserConfirmedFileSystem,
                        weak_factory_.GetWeakPtr(), type, path));
}

void CitizenNotesFileHelper::AddUserConfirmedFileSystem(const std::string& type,
                                                    const base::FilePath& path,
                                                    bool allowed) {
  if (!allowed) {
  ...
  ScopedDictPrefUpdate update(profile_->GetPrefs(),
                              prefs::kCitizenNotesFileSystemPaths);
  base::Value::Dict& file_systems_paths_value = update.Get();
  file_systems_paths_value.Set(file_system_path, type);
}

void CitizenNotesFileHelper::FailedToAddFileSystem(const std::string& error) {
...
std::vector<CitizenNotesFileHelper::FileSystem>
CitizenNotesFileHelper::GetFileSystems() {
  file_system_paths_ = GetAddedFileSystemPaths(profile_);
  std::vector<FileSystem> file_systems;
  if (!file_watcher_) {
    file_watcher_.reset(new CitizenNotesFileWatcher(
        base::BindRepeating(&CitizenNotesFileHelper::FilePathsChanged,
                            weak_factory_.GetWeakPtr()),
        base::SequencedTaskRunner::GetCurrentDefault()));
    auto change_handler_on_ui = base::BindRepeating(
        &CitizenNotesFileHelper::FileSystemPathsSettingChangedOnUI,
        weak_factory_.GetWeakPtr());
...
void CitizenNotesFileHelper::RemoveFileSystem(const std::string& file_system_path) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
bool CitizenNotesFileHelper::IsFileSystemAdded(
    const std::string& file_system_path) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);

  const base::Value::Dict& file_systems_paths_value =
      profile_->GetPrefs()->GetDict(prefs::kCitizenNotesFileSystemPaths);
  return file_systems_paths_value.Find(file_system_path);
}

void CitizenNotesFileHelper::OnOpenItemComplete(
    const base::FilePath& path,
...
void CitizenNotesFileHelper::ShowItemInFolder(const std::string& file_system_path) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  ...
  platform_util::OpenItem(
      ...
      base::BindOnce(&CitizenNotesFileHelper::OnOpenItemComplete,
                     weak_factory_.GetWeakPtr(), path));
}

void CitizenNotesFileHelper::FileSystemPathsSettingChangedOnUI() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void CitizenNotesFileHelper::FilePathsChanged(
