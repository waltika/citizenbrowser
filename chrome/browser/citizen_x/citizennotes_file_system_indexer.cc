...
CitizenNotesFileSystemIndexer::FileSystemIndexingJob::FileSystemIndexingJob(
...
void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::Start() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  impl_task_runner()->PostTask(
      FROM_HERE, BindOnce(&FileSystemIndexingJob::CollectFilesToIndex, this));
}

void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::Stop() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  impl_task_runner()->PostTask(
      FROM_HERE, BindOnce(&FileSystemIndexingJob::StopOnImplSequence, this));
}

void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::StopOnImplSequence() {
  stopped_ = true;
}

void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::CollectFilesToIndex() {
...
void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::IndexFiles() {
  DCHECK(impl_task_runner()->RunsTasksInCurrentSequence());
  if (stopped_)
    return;
...
void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::ReadFromFile() {
  if (stopped_) {
    CloseFile();
    return;
  }
...
void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::FinishFileIndexing(
    bool success) {
  DCHECK(impl_task_runner()->RunsTasksInCurrentSequence());
  CloseFile();
  if (success) {
...
void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::CloseFile() {
  if (current_file_.IsValid())
    current_file_.Close();
}

void CitizenNotesFileSystemIndexer::FileSystemIndexingJob::ReportWorked() {
...
CitizenNotesFileSystemIndexer::CitizenNotesFileSystemIndexer() {
...
CitizenNotesFileSystemIndexer::IndexPath(
...
void CitizenNotesFileSystemIndexer::SearchInPath(
  ...
  impl_task_runner()->PostTask(
      FROM_HERE,
      BindOnce(&CitizenNotesFileSystemIndexer::SearchInPathOnImplSequence, this,
               file_system_path, query, std::move(callback)));
}

void CitizenNotesFileSystemIndexer::SearchInPathOnImplSequence(
    const std::string& file_system_path,
    const std::string& query,
    SearchCallback callback) {
  DCHECK(impl_task_runner()->RunsTasksInCurrentSequence());
