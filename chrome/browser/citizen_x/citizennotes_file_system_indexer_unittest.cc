...
class CitizenNotesFileSystemIndexerTest : public testing::Test {
 public:
  void SetDone() {
    indexing_done_ = true;
 ...
 protected:
  void SetUp() override {
    indexer_ = new CitizenNotesFileSystemIndexer();
    indexing_done_ = false;
  }

  content::BrowserTaskEnvironment task_environment_;
  scoped_refptr<CitizenNotesFileSystemIndexer> indexer_;
  std::set<std::string> search_results_;
  bool indexing_done_;
};

TEST_F(CitizenNotesFileSystemIndexerTest, BasicUsage) {
  base::FilePath base_test_path;
  base::PathService::Get(chrome::DIR_TEST_DATA, &base_test_path);
  base::FilePath index_path =
      base_test_path.Append(FILE_PATH_LITERAL("citizennotes"))
          .Append(FILE_PATH_LITERAL("indexer"));

  std::vector<std::string> excluded_folders;
  scoped_refptr<CitizenNotesFileSystemIndexer::FileSystemIndexingJob> job =
      indexer_->IndexPath(index_path.AsUTF8Unsafe(), excluded_folders,
                          base::DoNothing(), base::DoNothing(),
                          base::BindOnce(&CitizenNotesFileSystemIndexerTest::SetDone,
  ...
  indexer_->SearchInPath(
      ...
      base::BindOnce(&CitizenNotesFileSystemIndexerTest::SearchCallback,
                     base::Unretained(this)));
  base::RunLoop().Run();
