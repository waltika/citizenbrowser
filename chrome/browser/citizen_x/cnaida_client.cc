...
CNAidaClient::CNAidaClient(
    Profile* profile,
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory)
    : profile_(*profile),
      url_loader_factory_(url_loader_factory),
      aida_endpoint_(features::kCitizenNotesConsoleInsightsAidaEndpoint.Get()),
      aida_scope_(features::kCitizenNotesConsoleInsightsAidaScope.Get()) {}

CNAidaClient::~CNAidaClient() = default;

void CNAidaClient::OverrideAidaEndpointAndScopeForTesting(
...
void CNAidaClient::DoConversation(
    std::string request,
  ...
  access_token_fetcher_ = identity_manager->CreateAccessTokenFetcherForAccount(
      ...
      base::BindOnce(&CNAidaClient::AccessTokenFetchFinished,
                     base::Unretained(this), std::move(request),
                     std::move(callback)),
      signin::AccessTokenFetcher::Mode::kImmediate);
}

void CNAidaClient::AccessTokenFetchFinished(
    std::string request,
    base::OnceCallback<void(const std::string&)> callback,
...
void CNAidaClient::SendAidaRequest(
    std::string request,
    base::OnceCallback<void(const std::string&)> callback) {
  ...
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("citizennotes_cdp_console_insights", R"(
        semantics {
          sender: "Citizen Notes via CDP"
          description:
            "In Chrome CitizenNotes, the user can ask for additional insights "
            ...
            "backend via CitizenNotes UI bindings, which in turn queries an AIDA "
            "endpoint."
          trigger: "User asks for more insights on a CitizenNotes error message."
          data: "Prompt for AIDA endpoint, containing instructions, error and "
            "sometimes some additional context information."
          destination: GOOGLE_OWNED_SERVICE
          internal {
            contacts {
              email: "chrome-citizennotes@google.com"
            }
          }
          user_data {
            type: WEB_CONTENT
        ...
        policy {
          ...
          chrome_policy {
            CitizenNotesAvailability {
              policy_options {mode: MANDATORY}
              CitizenNotesAvailability: 2
            }
          }
        })");
  ...
  simple_url_loader_ptr->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      ...
      base::BindOnce(&CNAidaClient::OnSimpleLoaderComplete,
                     base::Unretained(this), std::move(request),
                     std::move(callback), std::move(simple_url_loader)));
}

void CNAidaClient::OnSimpleLoaderComplete(
    std::string request,
    base::OnceCallback<void(const std::string&)> callback,
