...
#include "chrome/browser/citizen_x/device/adb/cnadb_device_provider.h"
#include "chrome/browser/citizen_x/device/cnport_forwarding_controller.h"
...
#include "chrome/browser/citizen_x/device/cncast_device_provider.h"
#endif
...
// static
CitizenNotesAndroidBridge::Factory* CitizenNotesAndroidBridge::Factory::GetInstance() {
  return base::Singleton<CitizenNotesAndroidBridge::Factory>::get();
}

// static
CitizenNotesAndroidBridge* CitizenNotesAndroidBridge::Factory::GetForProfile(
    Profile* profile) {
  return static_cast<CitizenNotesAndroidBridge*>(
      GetInstance()->GetServiceForBrowserContext(profile->GetOriginalProfile(),
                                                 true));
}

CitizenNotesAndroidBridge::Factory::Factory()
    : ProfileKeyedServiceFactory(
          "CitizenNotesAndroidBridge",
...
KeyedService* CitizenNotesAndroidBridge::Factory::BuildServiceInstanceFor(
    content::BrowserContext* context) const {
  Profile* profile = Profile::FromBrowserContext(context);

  return new CitizenNotesAndroidBridge(profile);
}

void CitizenNotesAndroidBridge::Shutdown() {
  // Needed for Chrome_CitizenNotesADBThread to shut down gracefully in tests.
  device_manager_.reset();
}

scoped_refptr<content::CitizenNotesAgentHost>
CitizenNotesAndroidBridge::GetBrowserAgentHost(
    scoped_refptr<RemoteBrowser> browser) {
  auto it = device_map_.find(browser->serial());
  if (it == device_map_.end())
    return nullptr;

  return CitizenNotesDeviceDiscovery::CreateBrowserAgentHost(it->second, browser);
}

void CitizenNotesAndroidBridge::SendJsonRequest(const std::string& browser_id_str,
...
void CitizenNotesAndroidBridge::OpenRemotePage(scoped_refptr<RemoteBrowser> browser,
...
CitizenNotesAndroidBridge::CitizenNotesAndroidBridge(Profile* profile)
    : profile_(profile),
      device_manager_(CNAndroidDeviceManager::Create()),
      port_forwarding_controller_(new CNPortForwardingController(profile)) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  pref_change_registrar_.Init(profile_->GetPrefs());
  pref_change_registrar_.Add(
      prefs::kCitizenNotesDiscoverUsbDevicesEnabled,
      base::BindRepeating(&CitizenNotesAndroidBridge::CreateDeviceProviders,
...
void CitizenNotesAndroidBridge::AddDeviceListListener(
...
void CitizenNotesAndroidBridge::RemoveDeviceListListener(
    DeviceListListener* listener) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void CitizenNotesAndroidBridge::AddDeviceCountListener(
...
void CitizenNotesAndroidBridge::RemoveDeviceCountListener(
    DeviceCountListener* listener) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void CitizenNotesAndroidBridge::AddPortForwardingListener(
...
void CitizenNotesAndroidBridge::RemovePortForwardingListener(
    PortForwardingListener* listener) {
...
CitizenNotesAndroidBridge::~CitizenNotesAndroidBridge() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void CitizenNotesAndroidBridge::StartDeviceCountPolling() {
  device_count_callback_.Reset(base::BindRepeating(
      &CitizenNotesAndroidBridge::ReceivedDeviceCount, AsWeakPtr()));
  RequestDeviceCount(device_count_callback_.callback());
}

void CitizenNotesAndroidBridge::StopDeviceCountPolling() {
  device_count_callback_.Cancel();
}

void CitizenNotesAndroidBridge::RequestDeviceCount(
...
void CitizenNotesAndroidBridge::ReceivedDeviceCount(int count) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  ...
  task_scheduler_.Run(base::BindOnce(&CitizenNotesAndroidBridge::RequestDeviceCount,
                                     AsWeakPtr(),
...
static scoped_refptr<CNTCPDeviceProvider> CreateCNTCPDeviceProvider(
  ...
  if (targets.empty())
    return nullptr;
  return new CNTCPDeviceProvider(targets);
}

void CitizenNotesAndroidBridge::CreateDeviceProviders() {
  CNAndroidDeviceManager::DeviceProviders device_providers;
  PrefService* service = profile_->GetPrefs();
  const base::Value::List* targets =
      service->GetBoolean(prefs::kCitizenNotesDiscoverTCPTargetsEnabled)
          ? std::addressof(service->GetList(prefs::kCitizenNotesTCPDiscoveryConfig))
          : nullptr;
  scoped_refptr<CNTCPDeviceProvider> provider = CreateCNTCPDeviceProvider(targets);
...
#if BUILDFLAG(ENABLE_SERVICE_DISCOVERY)
  device_providers.push_back(new CNCastDeviceProvider());
#endif

  device_providers.push_back(new CNAdbDeviceProvider());

  const PrefService::Preference* pref =
      service->FindPreference(prefs::kCitizenNotesDiscoverUsbDevicesEnabled);
  const base::Value* pref_value = pref->GetValue();

  if (pref_value->is_bool() && pref_value->GetBool()) {
    device_providers.push_back(new CNUsbDeviceProvider(profile_));
  }
...
void CitizenNotesAndroidBridge::set_tcp_provider_callback_for_test(
    TCPProviderCallback callback) {
  tcp_provider_callback_ = std::move(callback);
  CreateDeviceProviders();
}

void CitizenNotesAndroidBridge::set_usb_device_manager_for_test(
