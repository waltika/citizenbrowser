...
bool BrowserCompare(const CNAndroidDeviceManager::BrowserInfo& a,
                    const CNAndroidDeviceManager::BrowserInfo& b) {
  size_t count = std::size(kBrowserDescriptors);
  for (size_t i = 0; i < count; i++) {
...
gfx::Size CNParseScreenSize(base::StringPiece str) {
...
CNAndroidDeviceManager::BrowserInfo::Type
GetBrowserType(const std::string& socket) {
  if (base::StartsWith(socket, kChromeDefaultSocket,
                       base::CompareCase::SENSITIVE)) {
    return CNAndroidDeviceManager::BrowserInfo::kTypeChrome;
  }

  if (base::StartsWith(socket, kWebViewSocketPrefix,
                       base::CompareCase::SENSITIVE)) {
    return CNAndroidDeviceManager::BrowserInfo::kTypeWebView;
  }

  return CNAndroidDeviceManager::BrowserInfo::kTypeOther;
}

void ReceivedResponse(CNAndroidDeviceManager::DeviceInfoCallback callback,
                      int result,
  ...
  for (const auto& pair : socket_to_pid) {
    ...
    browser_info.display_name =
        CNAndroidDeviceManager::GetBrowserName(socket, package);
...
// static
std::string CNAndroidDeviceManager::GetBrowserName(const std::string& socket,
  ...
  if (GetBrowserType(socket) ==
      CNAndroidDeviceManager::BrowserInfo::kTypeWebView)
...
// static
void CNAndroidDeviceManager::QueryDeviceInfo(RunCommandCallback command_callback,
