...
const char kCitizenNotesAdbBridgeThreadName[] = "Chrome_CitizenNotesADBThread";
...
net::NetworkTrafficAnnotationTag kCNAndroidDeviceManagerTrafficAnnotation =
...
static void PostDeviceInfoCallback(
    ...
    CNAndroidDeviceManager::DeviceInfoCallback callback,
...
static void PostCommandCallback(
    scoped_refptr<base::SingleThreadTaskRunner> response_task_runner,
    CNAndroidDeviceManager::CommandCallback callback,
    int result,
...
static void PostHttpUpgradeCallback(
    scoped_refptr<base::SingleThreadTaskRunner> response_task_runner,
    CNAndroidDeviceManager::HttpUpgradeCallback callback,
    int result,
...
class HttpRequest {
 public:
  typedef CNAndroidDeviceManager::CommandCallback CommandCallback;
  typedef CNAndroidDeviceManager::HttpUpgradeCallback HttpUpgradeCallback;
  ...
  void DoSendRequest(int result) {
    while (result != net::ERR_IO_PENDING) {
      ...
      result = socket_->Write(
          ...
          kCNAndroidDeviceManagerTrafficAnnotation);
    }
  }
...
class CNDevicesRequest : public base::RefCountedThreadSafe<CNDevicesRequest> {
 public:
  using DeviceInfo = CNAndroidDeviceManager::DeviceInfo;
  using DeviceProvider = CNAndroidDeviceManager::DeviceProvider;
  using DeviceProviders = CNAndroidDeviceManager::DeviceProviders;
  using DeviceDescriptors = CNAndroidDeviceManager::DeviceDescriptors;
  ...
  static void Start(
    ...
    scoped_refptr<CNDevicesRequest> request =
        base::WrapRefCounted(new CNDevicesRequest(std::move(callback)));
    for (const auto& provider : providers) {
      device_task_runner->PostTask(
          FROM_HERE,
          base::BindOnce(&DeviceProvider::QueryDevices, provider,
                         base::BindOnce(&CNDevicesRequest::ProcessSerials,
                                        request, provider)));
    }
 ...
 private:
  explicit CNDevicesRequest(DescriptorsCallback callback)
      : response_task_runner_(
            base::SingleThreadTaskRunner::GetCurrentDefault()),
        callback_(std::move(callback)),
        descriptors_(new DeviceDescriptors()) {}

  friend class base::RefCountedThreadSafe<CNDevicesRequest>;
  ~CNDevicesRequest() {
...
CNAndroidDeviceManager::BrowserInfo::BrowserInfo()
    : type(kTypeOther) {
}

CNAndroidDeviceManager::BrowserInfo::BrowserInfo(const BrowserInfo& other) =
    default;

CNAndroidDeviceManager::BrowserInfo& CNAndroidDeviceManager::BrowserInfo::operator=(
    const BrowserInfo& other) = default;

CNAndroidDeviceManager::DeviceInfo::DeviceInfo()
    : model(kModelOffline), connected(false) {
}

CNAndroidDeviceManager::DeviceInfo::DeviceInfo(const DeviceInfo& other) = default;

CNAndroidDeviceManager::DeviceInfo::~DeviceInfo() {
}

CNAndroidDeviceManager::DeviceDescriptor::DeviceDescriptor() {
}
...
void CNAndroidDeviceManager::DeviceProvider::SendJsonRequest(
...
void CNAndroidDeviceManager::DeviceProvider::HttpUpgrade(
    const std::string& serial,
    const std::string& socket_name,
    const std::string& path,
    const std::string& extensions,
...
void CNAndroidDeviceManager::DeviceProvider::ReleaseDevice(
    const std::string& serial) {
}

CNAndroidDeviceManager::DeviceProvider::DeviceProvider() {
}

CNAndroidDeviceManager::DeviceProvider::~DeviceProvider() {
}

void CNAndroidDeviceManager::Device::QueryDeviceInfo(
...
void CNAndroidDeviceManager::Device::OpenSocket(const std::string& socket_name,
...
void CNAndroidDeviceManager::Device::SendJsonRequest(
    const std::string& socket_name,
    const std::string& request,
    CommandCallback callback) {
  task_runner_->PostTask(
...
void CNAndroidDeviceManager::Device::HttpUpgrade(const std::string& socket_name,
                                               const std::string& path,
                                               const std::string& extensions,
                                               HttpUpgradeCallback callback) {
  task_runner_->PostTask(
...
CNAndroidDeviceManager::Device::Device(
    scoped_refptr<base::SingleThreadTaskRunner> device_task_runner,
...
// static
CNAndroidDeviceManager::HandlerThread*
CNAndroidDeviceManager::HandlerThread::GetInstance() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  static base::NoDestructor<CNAndroidDeviceManager::HandlerThread> s_instance;
  return s_instance.get();
}

CNAndroidDeviceManager::HandlerThread::HandlerThread() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  thread_ = new base::Thread(kCitizenNotesAdbBridgeThreadName);
...
CNAndroidDeviceManager::HandlerThread::message_loop() {
  return thread_ ? thread_->task_runner() : nullptr;
}

// static
void CNAndroidDeviceManager::HandlerThread::StopThread(
    base::Thread* thread) {
  delete thread;
}
...
// static
std::unique_ptr<CNAndroidDeviceManager> CNAndroidDeviceManager::Create() {
  return base::WrapUnique(new CNAndroidDeviceManager());
}

void CNAndroidDeviceManager::SetDeviceProviders(
    const DeviceProviders& providers) {
...
void CNAndroidDeviceManager::QueryDevices(DevicesCallback callback) {
  CNDevicesRequest::Start(
      handler_thread_->message_loop(), providers_,
      base::BindOnce(&CNAndroidDeviceManager::UpdateDevices,
                     weak_factory_.GetWeakPtr(), std::move(callback)));
}

void CNAndroidDeviceManager::CountDevices(
    base::OnceCallback<void(int)> callback) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  handler_thread_->message_loop()->PostTask(
      FROM_HERE,
      base::BindOnce(&CNUsbDeviceManagerHelper::CountDevices,
                     base::BindOnce(&OnCountDevices, std::move(callback))));
}

void CNAndroidDeviceManager::set_usb_device_manager_for_test(
    mojo::PendingRemote<device::mojom::UsbDeviceManager> fake_usb_manager) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  handler_thread_->message_loop()->PostTask(
      FROM_HERE,
      base::BindOnce(&CNUsbDeviceManagerHelper::SetUsbManagerForTesting,
                     std::move(fake_usb_manager)));
}

CNAndroidDeviceManager::CNAndroidDeviceManager()
...
void CNAndroidDeviceManager::UpdateDevices(
