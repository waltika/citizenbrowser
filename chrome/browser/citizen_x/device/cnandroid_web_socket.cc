...
class CNAndroidDeviceManager::AndroidWebSocket::WebSocketImpl {
 public:
  WebSocketImpl(
...
CNAndroidDeviceManager::AndroidWebSocket::AndroidWebSocket(
...
CNAndroidDeviceManager::AndroidWebSocket::~AndroidWebSocket() = default;

void CNAndroidDeviceManager::AndroidWebSocket::SendFrame(
...
void CNAndroidDeviceManager::AndroidWebSocket::Connected(
    int result,
    const std::string& extensions,
    const std::string& body_head,
    std::unique_ptr<net::StreamSocket> socket) {
...
void CNAndroidDeviceManager::AndroidWebSocket::OnFrameRead(
    const std::string& message) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  delegate_->OnFrameRead(message);
}

void CNAndroidDeviceManager::AndroidWebSocket::OnSocketClosed() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  delegate_->OnSocketClosed();
}

CNAndroidDeviceManager::AndroidWebSocket*
CNAndroidDeviceManager::Device::CreateWebSocket(
    const std::string& socket_name,
    const std::string& path,
