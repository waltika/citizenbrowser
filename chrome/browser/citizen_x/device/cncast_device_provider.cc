...
// ServiceDiscoveryDeviceLister (on the UI thread) to CNCastDeviceProvider (on the
// CitizenNotes ADB thread). Cancellable callbacks are necessary since
// CNCastDeviceProvider and ServiceDiscoveryDeviceLister are destroyed on
// different threads in undefined order.
//
// TODO(crbug.com/963216): Consolidate DNS-SD implementations for Cast.
class CNCastDeviceProvider::DeviceListerDelegate
    : public ServiceDiscoveryDeviceLister::Delegate,
      public base::SupportsWeakPtr<DeviceListerDelegate> {
 public:
  DeviceListerDelegate(base::WeakPtr<CNCastDeviceProvider> provider,
  ...
  void OnDeviceChanged(const std::string& service_type,
    ...
    runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&CNCastDeviceProvider::OnDeviceChanged, provider_,
  ...
  void OnDeviceRemoved(const std::string& service_type,
    ...
    runner_->PostTask(FROM_HERE,
                      base::BindOnce(&CNCastDeviceProvider::OnDeviceRemoved,
                                     provider_, service_type, service_name));
  }

  void OnDeviceCacheFlushed(const std::string& service_type) override {
    runner_->PostTask(FROM_HERE,
                      base::BindOnce(&CNCastDeviceProvider::OnDeviceCacheFlushed,
                                     provider_, service_type));
  }

 private:
  // The device provider to notify of device changes.
  base::WeakPtr<CNCastDeviceProvider> provider_;
...
CNCastDeviceProvider::CNCastDeviceProvider() {}

CNCastDeviceProvider::~CNCastDeviceProvider() {}

void CNCastDeviceProvider::QueryDevices(SerialsCallback callback) {
  if (!lister_delegate_) {
  ...
  tcp_provider_ = new CNTCPDeviceProvider(targets);
  tcp_provider_->QueryDevices(std::move(callback));
}

void CNCastDeviceProvider::QueryDeviceInfo(const std::string& serial,
...
void CNCastDeviceProvider::OpenSocket(const std::string& serial,
                                    const std::string& socket_name,
                                    SocketCallback callback) {
  tcp_provider_->OpenSocket(serial, socket_name, std::move(callback));
}

void CNCastDeviceProvider::OnDeviceChanged(
...
void CNCastDeviceProvider::OnDeviceRemoved(const std::string& service_type,
...
void CNCastDeviceProvider::OnDeviceCacheFlushed(const std::string& service_type) {
