...
net::NetworkTrafficAnnotationTag kCNPortForwardingControllerTrafficAnnotation =
...
using CNResolveHostCallback = base::OnceCallback<void(net::AddressList)>;

// This class is created and runs on BrowserThread::UI thread.
class CNPortForwardingHostResolver : public network::ResolveHostClientBase {
 public:
  CNPortForwardingHostResolver(Profile* profile,
                             const std::string& host,
                             int port,
                             CNResolveHostCallback resolve_host_callback)
    ...
    receiver_.set_disconnect_handler(base::BindOnce(
        &CNPortForwardingHostResolver::OnComplete, base::Unretained(this),
        net::ERR_NAME_NOT_RESOLVED, net::ResolveErrorInfo(net::ERR_FAILED),
        /*resolved_addresses=*/absl::nullopt,
        /*endpoint_results_with_metadata=*/absl::nullopt));
  }

  CNPortForwardingHostResolver(const CNPortForwardingHostResolver&) = delete;
  CNPortForwardingHostResolver& operator=(const CNPortForwardingHostResolver&) =
      delete;

 private:
  ~CNPortForwardingHostResolver() override {
    DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  }
  ...
  CNResolveHostCallback resolve_host_callback_;
};
...
static void ResolveHost(Profile* profile,
                        ...
                        int port,
                        CNResolveHostCallback resolve_host_callback) {
  new CNPortForwardingHostResolver(profile, host, port,
...
class CNSocketTunnel {
 public:
  static void StartTunnel(Profile* profile,
    ...
    if (result == net::OK)
      new CNSocketTunnel(profile, std::move(socket), host, port);
  }

  CNSocketTunnel(const CNSocketTunnel&) = delete;
  CNSocketTunnel& operator=(const CNSocketTunnel&) = delete;

  ~CNSocketTunnel() { DCHECK_CALLED_ON_VALID_THREAD(thread_checker_); }

 private:
  CNSocketTunnel(Profile* profile,
               std::unique_ptr<net::StreamSocket> socket,
               const std::string& host,
               int port)
    ...
    CNResolveHostCallback resolve_host_callback = base::BindOnce(
        &CNSocketTunnel::OnCNResolveHostComplete, base::Unretained(this));
    content::GetUIThreadTaskRunner({})->PostTask(
        FROM_HERE, base::BindOnce(&ResolveHost, profile, host, port,
                                  std::move(resolve_host_callback)));
  }

  void OnCNResolveHostComplete(net::AddressList resolved_addresses) {
    DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
    ...
    if (resolved_addresses.empty()) {
      ...
      adb_thread_runner_->PostTask(
          FROM_HERE,
          base::BindOnce(&CNSocketTunnel::OnResolved, base::Unretained(this),
                         resolved_addresses));
    }
  }
  ...
  void OnResolved(net::AddressList resolved_addresses) {
    ...
    int result = host_socket_->Connect(
        base::BindOnce(&CNSocketTunnel::OnConnected, base::Unretained(this)));
  ...
  void Pump(net::StreamSocket* from, net::StreamSocket* to) {
    ...
    int result =
        from->Read(buffer.get(), kBufferSize,
                   base::BindOnce(&CNSocketTunnel::OnRead, base::Unretained(this),
                                  from, to, buffer));
    if (result != net::ERR_IO_PENDING)
  ...
  void OnRead(net::StreamSocket* from,
    ...
    result =
        to->Write(drainable.get(), total,
                  base::BindOnce(&CNSocketTunnel::OnWritten,
                                 base::Unretained(this), drainable, from, to),
                  kCNPortForwardingControllerTrafficAnnotation);
    if (result != net::ERR_IO_PENDING)
...
class CNPortForwardingController::Connection
 ...
 private:
  ...
  // CitizenNotesAndroidBridge::AndroidWebSocket::Delegate implementation:
  void OnSocketOpened() override;
  void OnFrameRead(const std::string& message) override;
  void OnSocketClosed() override;

  RAW_PTR_EXCLUSION Profile* profile_;
  RAW_PTR_EXCLUSION CNPortForwardingController::Registry* registry_;
  scoped_refptr<CNAndroidDeviceManager::Device> device_;
  scoped_refptr<CitizenNotesAndroidBridge::RemoteBrowser> browser_;
...
CNPortForwardingController::Connection::Connection(
    Profile* profile,
    Registry* registry,
    scoped_refptr<CNAndroidDeviceManager::Device> device,
    scoped_refptr<CitizenNotesAndroidBridge::RemoteBrowser> browser,
...
void CNPortForwardingController::Connection::UpdateForwardingMap(
...
void CNPortForwardingController::Connection::SerializeChanges(
    const std::string& method,
    const ForwardingMap& old_map,
    const ForwardingMap& new_map) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void CNPortForwardingController::Connection::SendCommand(
...
bool CNPortForwardingController::Connection::ProcessResponse(
...
void CNPortForwardingController::Connection::ProcessBindResponse(
    int port, PortStatus status) {
  port_status_[port] = status;
}

void CNPortForwardingController::Connection::ProcessUnbindResponse(
    int port, PortStatus status) {
  auto it = port_status_.find(port);
...
const CNPortForwardingController::PortStatusMap&
CNPortForwardingController::Connection::GetPortStatusMap() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  return port_status_;
}

void CNPortForwardingController::Connection::OnSocketOpened() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  connected_ = true;
  SerializeChanges(kBindMethod, ForwardingMap(), forwarding_map_);
}

void CNPortForwardingController::Connection::OnSocketClosed() {
  delete this;
}

void CNPortForwardingController::Connection::OnFrameRead(
    const std::string& message) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  ...
  device_->OpenSocket(*connection_id,
                      base::BindOnce(&CNSocketTunnel::StartTunnel, profile_,
                                     destination_host, destination_port));
}

CNPortForwardingController::CNPortForwardingController(Profile* profile)
...
CNPortForwardingController::~CNPortForwardingController() {}

CNPortForwardingController::ForwardingStatus
CNPortForwardingController::DeviceListChanged(
  ...
  for (const auto& pair : complete_devices) {
    scoped_refptr<CNAndroidDeviceManager::Device> device(pair.first);
    scoped_refptr<CitizenNotesAndroidBridge::RemoteDevice> remote_device(
        pair.second);
...
void CNPortForwardingController::CloseAllConnections() {
...
void CNPortForwardingController::UpdateConnections() {
