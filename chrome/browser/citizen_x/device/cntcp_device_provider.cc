...
static void CNRunSocketCallback(CNAndroidDeviceManager::SocketCallback callback,
                              std::unique_ptr<net::StreamSocket> socket,
                              int result) {
  std::move(callback).Run(result, std::move(socket));
}

class CNResolveHostAndOpenSocket final : public network::ResolveHostClientBase {
 public:
  CNResolveHostAndOpenSocket(const net::HostPortPair& address,
                           CNAndroidDeviceManager::SocketCallback callback)
    ...
    receiver_.set_disconnect_handler(base::BindOnce(
        &CNResolveHostAndOpenSocket::OnComplete, base::Unretained(this),
 ...
 private:
  ...
  void OnComplete(int result,
    ...
    if (result != net::OK) {
      CNRunSocketCallback(std::move(callback_), nullptr,
    ...
    auto split_callback = base::SplitOnceCallback(base::BindOnce(
        &CNRunSocketCallback, std::move(callback_), std::move(socket)));
  ...
  CNAndroidDeviceManager::SocketCallback callback_;
  mojo::Receiver<network::mojom::ResolveHostClient> receiver_{this};
};

}  // namespace

scoped_refptr<CNTCPDeviceProvider> CNTCPDeviceProvider::CreateForLocalhost(
    uint16_t port) {
  CNTCPDeviceProvider::HostPortSet targets;
  targets.insert(net::HostPortPair("127.0.0.1", port));
  return new CNTCPDeviceProvider(targets);
}

CNTCPDeviceProvider::CNTCPDeviceProvider(const HostPortSet& targets)
    : targets_(targets) {
}

void CNTCPDeviceProvider::QueryDevices(SerialsCallback callback) {
...
void CNTCPDeviceProvider::QueryDeviceInfo(const std::string& serial,
...
void CNTCPDeviceProvider::OpenSocket(const std::string& serial,
  ...
  new CNResolveHostAndOpenSocket(net::HostPortPair(serial, port),
                               std::move(callback));
}

void CNTCPDeviceProvider::ReleaseDevice(const std::string& serial) {
  if (!release_callback_.is_null())
    std::move(release_callback_).Run();
}

void CNTCPDeviceProvider::set_release_callback_for_test(
    base::OnceClosure callback) {
  release_callback_ = std::move(callback);
}

CNTCPDeviceProvider::~CNTCPDeviceProvider() {
}
