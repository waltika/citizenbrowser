...
std::unique_ptr<CNAndroidInterfaceInfo> FindAndroidInterface(
  ...
  for (const auto& interface : active_config->interfaces) {
    for (const auto& alternate : interface->alternates) {
      if (alternate->alternate_setting == 0 &&
        ...
        return std::make_unique<CNAndroidInterfaceInfo>(
...
void GetCNAndroidDeviceInfoList(
    CNAndroidDeviceInfoListCallback callback,
    std::vector<device::mojom::UsbDeviceInfoPtr> usb_devices) {
  std::vector<CNAndroidDeviceInfo> result;
  for (auto& device_info : usb_devices) {
    ...
    result.push_back(CNAndroidDeviceInfo(
        device_info->guid,
...
CNAndroidDeviceInfo::CNAndroidDeviceInfo(const CNAndroidDeviceInfo& other) = default;

// static
CNUsbDeviceManagerHelper* CNUsbDeviceManagerHelper::GetInstance() {
  static base::NoDestructor<CNUsbDeviceManagerHelper> s_instance;
  return s_instance.get();
}

// static
void CNUsbDeviceManagerHelper::CountDevices(
    base::OnceCallback<void(int)> callback) {
  GetInstance()->CountDevicesInternal(std::move(callback));
}

// static
void CNUsbDeviceManagerHelper::SetUsbManagerForTesting(
...
CNUsbDeviceManagerHelper::CNUsbDeviceManagerHelper() {}

CNUsbDeviceManagerHelper::~CNUsbDeviceManagerHelper() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
}

void CNUsbDeviceManagerHelper::GetAndroidDevices(
    CNAndroidDeviceInfoListCallback callback) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  ...
  device_manager_->GetDevices(
      ...
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(
          base::BindOnce(&GetCNAndroidDeviceInfoList, std::move(callback)),
          std::vector<device::mojom::UsbDeviceInfoPtr>()));
}

void CNUsbDeviceManagerHelper::GetDevice(
...
void CNUsbDeviceManagerHelper::EnsureUsbDeviceManagerConnection() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  ...
  if (testing_device_manager_) {
    ...
    device_manager_.set_disconnect_handler(
        base::BindOnce(&CNUsbDeviceManagerHelper::OnDeviceManagerConnectionError,
...
void CNUsbDeviceManagerHelper::CountDevicesInternal(
    base::OnceCallback<void(int)> callback) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  EnsureUsbDeviceManagerConnection();
  ...
  device_manager_->GetDevices(
      /*options=*/nullptr,
      mojo::WrapCallbackWithDefaultInvokeIfNotRun(
          base::BindOnce(&GetCNAndroidDeviceInfoList, std::move(countCb)),
          std::vector<device::mojom::UsbDeviceInfoPtr>()));
}

void CNUsbDeviceManagerHelper::SetUsbManagerForTestingInternal(
    mojo::PendingRemote<device::mojom::UsbDeviceManager> fake_usb_manager) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void CNUsbDeviceManagerHelper::OnDeviceManagerConnectionError() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
