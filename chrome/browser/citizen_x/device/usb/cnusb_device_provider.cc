...
void OnOpenSocket(CNUsbDeviceProvider::SocketCallback callback,
...
void OpenedForCommand(CNUsbDeviceProvider::CommandCallback callback,
...
void CNUsbDeviceProvider::QueryDeviceInfo(const std::string& serial,
                                        DeviceInfoCallback callback) {
  auto it = device_map_.find(serial);
  if (it == device_map_.end() || !it->second->is_connected()) {
    CNAndroidDeviceManager::DeviceInfo offline_info;
    std::move(callback).Run(offline_info);
    return;
  }
  CNAndroidDeviceManager::QueryDeviceInfo(base::BindOnce(&RunCommand, it->second),
                                        std::move(callback));
}

void CNUsbDeviceProvider::OpenSocket(const std::string& serial,
...
void CNUsbDeviceProvider::ReleaseDevice(const std::string& serial) {
  device_map_.erase(serial);
}

CNUsbDeviceProvider::~CNUsbDeviceProvider() {
}
