...
CNAutofillHandler::CNAutofillHandler(protocol::UberDispatcher* dispatcher,
...
CNAutofillHandler::~CNAutofillHandler() = default;

void CNAutofillHandler::Trigger(
    int field_id,
  ...
  host->GetUniqueFormControlId(
      field_id,
      base::BindOnce(&CNAutofillHandler::FinishTrigger,
                     weak_ptr_factory_.GetWeakPtr(), std::move(frame_id),
                     std::move(card), std::move(callback)));
}

void CNAutofillHandler::FinishTrigger(
    Maybe<String> frame_id,
    std::unique_ptr<protocol::Autofill::CreditCard> card,
    std::unique_ptr<TriggerCallback> callback,
    uint64_t field_id) {
...
void CNAutofillHandler::OnFillOrPreviewDataModelForm(
...
Response CNAutofillHandler::Enable() {
  if (enabled_) {
...
Response CNAutofillHandler::Disable() {
  enabled_ = false;
