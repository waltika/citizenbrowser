...
class CNCastHandler::MediaRoutesObserver
...
class CNCastHandler::IssuesObserver : public media_router::IssuesObserver {
 public:
  IssuesObserver(
...
CNCastHandler::CNCastHandler(content::WebContents* web_contents,
                         protocol::UberDispatcher* dispatcher)
    : CNCastHandler(web_contents) {
  frontend_ = std::make_unique<protocol::Cast::Frontend>(dispatcher->channel());
  protocol::Cast::Dispatcher::wire(dispatcher, this);
}

CNCastHandler::~CNCastHandler() = default;

Response CNCastHandler::SetSinkToUse(const std::string& in_sink_name) {
  ...
  media_router::PresentationServiceDelegateImpl::GetOrCreateForWebContents(
      ...
      ->set_start_presentation_cb(
          base::BindRepeating(&CNCastHandler::StartPresentation,
                              weak_factory_.GetWeakPtr(), in_sink_name));
  return Response::Success();
}

void CNCastHandler::StartDesktopMirroring(
  ...
  router_->CreateRoute(
      ...
      base::BindOnce(&CNCastHandler::OnDesktopMirroringStarted,
                     weak_factory_.GetWeakPtr(), std::move(callback)),
      media_router::GetRouteRequestTimeout(
          media_router::MediaCastMode::DESKTOP_MIRROR));
}

void CNCastHandler::StartTabMirroring(
    const std::string& in_sink_name,
  ...
  router_->CreateRoute(
      ...
      base::BindOnce(&CNCastHandler::OnTabMirroringStarted,
                     weak_factory_.GetWeakPtr(), std::move(callback)),
      media_router::GetRouteRequestTimeout(
          media_router::MediaCastMode::TAB_MIRROR));
}

Response CNCastHandler::StopCasting(const std::string& in_sink_name) {
  Response init_response = EnsureInitialized();
  if (!init_response.IsSuccess())
    return init_response;
  const media_router::MediaSink::Id& sink_id = GetSinkIdByName(in_sink_name);
...
Response CNCastHandler::Enable(protocol::Maybe<std::string> in_presentation_url) {
  Response init_response = EnsureInitialized();
  if (!init_response.IsSuccess())
    return init_response;
...
Response CNCastHandler::Disable() {
...
void CNCastHandler::OnSinksUpdated(
...
Response CNCastHandler::EnsureInitialized() {
  if (!router_)
  ...
  routes_observer_ = std::make_unique<MediaRoutesObserver>(
      router_, base::BindRepeating(&CNCastHandler::SendSinkUpdate,
                                   base::Unretained(this)));
  issues_observer_ = std::make_unique<IssuesObserver>(
      router_,
      base::BindRepeating(&CNCastHandler::OnIssue, base::Unretained(this)));
  return Response::Success();
}

void CNCastHandler::StartPresentation(
  ...
  router_->CreateRoute(
      ...
      base::BindOnce(&CNCastHandler::OnPresentationStarted,
                     weak_factory_.GetWeakPtr(), std::move(context)),
      media_router::GetRouteRequestTimeout(
          media_router::MediaCastMode::PRESENTATION));
}

media_router::MediaSink::Id CNCastHandler::GetSinkIdByName(
...
MediaRoute::Id CNCastHandler::GetRouteIdForSink(
...
void CNCastHandler::StartObservingForSinks(
...
void CNCastHandler::SendSinkUpdate() {
  if (!frontend_)
    return;
...
void CNCastHandler::OnDesktopMirroringStarted(
    std::unique_ptr<StartDesktopMirroringCallback> callback,
...
void CNCastHandler::OnTabMirroringStarted(
    std::unique_ptr<StartTabMirroringCallback> callback,
    media_router::mojom::RoutePresentationConnectionPtr connection,
    const media_router::RouteRequestResult& result) {
  if (result.result_code() == media_router::mojom::RouteRequestResultCode::OK) {
...
void CNCastHandler::OnPresentationStarted(
    std::unique_ptr<media_router::StartPresentationContext> context,
    media_router::mojom::RoutePresentationConnectionPtr connection,
    const media_router::RouteRequestResult& result) {
  if (result.result_code() == media_router::mojom::RouteRequestResultCode::OK)
...
void CNCastHandler::OnIssue(const std::string& issue) {
  if (frontend_)
