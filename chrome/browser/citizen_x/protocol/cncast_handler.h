...
#ifndef CHROME_BROWSER_CITIZENNOTES_PROTOCOL_CAST_HANDLER_H_
#define CHROME_BROWSER_CITIZENNOTES_PROTOCOL_CAST_HANDLER_H_
...
#include "chrome/browser/citizen_x/protocol/cast.h"
...
class CNCastHandler : public protocol::Cast::Backend,
                    public media_router::MediaSinkWithCastModesObserver {
 public:
  CNCastHandler(content::WebContents* web_contents,
              protocol::UberDispatcher* dispatcher);

  CNCastHandler(const CNCastHandler&) = delete;
  CNCastHandler& operator=(const CNCastHandler&) = delete;

  ~CNCastHandler() override;
 ...
 private:
  friend class CNCastHandlerTest;

  class MediaRoutesObserver;
  class IssuesObserver;

  // Constructor that does not wire the handler to a dispatcher. Used in tests.
  explicit CNCastHandler(content::WebContents* web_contents);
  ...
  RAW_PTR_EXCLUSION media_router::MediaRouter* router_;
  ...
  base::WeakPtrFactory<CNCastHandler> weak_factory_{this};
};

#endif  // CHROME_BROWSER_CITIZENNOTES_PROTOCOL_CAST_HANDLER_H_
