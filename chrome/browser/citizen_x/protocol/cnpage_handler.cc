...
CNPageHandler::CNPageHandler(scoped_refptr<content::CitizenNotesAgentHost> agent_host,
...
void CNPageHandler::ToggleAdBlocking(bool enabled) {
  if (!web_contents_)
    return;

  // Create the CitizennotesInteractionTracker lazily (note that this call is a
  // no-op if the object was already created).
  subresource_filter::CitizennotesInteractionTracker::CreateForWebContents(
      web_contents_.get());

  subresource_filter::CitizennotesInteractionTracker::FromWebContents(
      web_contents_.get())
      ->ToggleForceActivation(enabled);
}

protocol::Response CNPageHandler::Enable() {
  enabled_ = true;
...
protocol::Response CNPageHandler::Disable() {
  enabled_ = false;
...
protocol::Response CNPageHandler::SetAdBlockingEnabled(bool enabled) {
  if (!enabled_)
...
protocol::Response CNPageHandler::SetSPCTransactionMode(
...
protocol::Response CNPageHandler::SetRPHRegistrationMode(
    const protocol::String& mode) {
  if (!web_contents_) {
    return protocol::Response::ServerError("No web contents to host a dialog.");
  }
...
void CNPageHandler::GetInstallabilityErrors(
  ...
  manager->GetAllErrors(base::BindOnce(&CNPageHandler::GotInstallabilityErrors,
                                       std::move(callback)));
}

// static
void CNPageHandler::GotInstallabilityErrors(
    std::unique_ptr<GetInstallabilityErrorsCallback> callback,
...
void CNPageHandler::GetManifestIcons(
  ...
  manager->GetPrimaryIcon(
      base::BindOnce(&CNPageHandler::GotManifestIcons, std::move(callback)));
}

void CNPageHandler::GotManifestIcons(
    std::unique_ptr<GetManifestIconsCallback> callback,
...
void CNPageHandler::PrintToPDF(protocol::Maybe<bool> landscape,
  ...
  if (auto* print_manager = headless::HeadlessPrintManager::FromWebContents(
    ...
    print_manager->PrintToPdf(
        ...
        std::move(absl::get<printing::mojom::PrintPagesParamsPtr>(
            print_pages_params)),
        base::BindOnce(&CNPageHandler::OnPDFCreated,
...
void CNPageHandler::GetAppId(std::unique_ptr<GetAppIdCallback> callback) {
  ...
  manager->GetData(params, base::BindOnce(&CNPageHandler::OnDidGetManifest,
                                          weak_ptr_factory_.GetWeakPtr(),
                                          std::move(callback)));
}

void CNPageHandler::OnDidGetManifest(std::unique_ptr<GetAppIdCallback> callback,
...
#if BUILDFLAG(ENABLE_PRINTING)
void CNPageHandler::OnPDFCreated(bool return_as_stream,
                               std::unique_ptr<PrintToPDFCallback> callback,
