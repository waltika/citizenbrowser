...
CNSecurityHandler::CNSecurityHandler(content::WebContents* web_contents,
...
protocol::Response CNSecurityHandler::Enable() {
  if (enabled_)
...
protocol::Response CNSecurityHandler::Disable() {
  enabled_ = false;
  // Do not mark the command as handled. Let it fall through instead, so that
  // the handler in content gets a chance to process the command.
  return protocol::Response::FallThrough();
...
void CNSecurityHandler::DidChangeVisibleSecurityState() {
  if (!enabled_)
    return;
