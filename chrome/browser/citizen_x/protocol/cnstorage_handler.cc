...
CNStorageHandler::CNStorageHandler(content::WebContents* web_contents,
                               protocol::UberDispatcher* dispatcher)
    : web_contents_(web_contents->GetWeakPtr()) {
  protocol::Storage::Dispatcher::wire(dispatcher, this);
}

CNStorageHandler::~CNStorageHandler() = default;

void CNStorageHandler::RunBounceTrackingMitigations(
  ...
  dips_service->DeleteEligibleSitesImmediately(
      base::BindOnce(&CNStorageHandler::GotDeletedSites, std::move(callback)));
}

/* static */
void CNStorageHandler::GotDeletedSites(
    std::unique_ptr<RunBounceTrackingMitigationsCallback> callback,
