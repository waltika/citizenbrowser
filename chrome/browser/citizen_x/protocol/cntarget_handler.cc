...
CNTargetHandler::CNTargetHandler(protocol::UberDispatcher* dispatcher,
                             bool is_trusted)
    : is_trusted_(is_trusted) {
  protocol::Target::Dispatcher::wire(dispatcher, this);
}

CNTargetHandler::~CNTargetHandler() {
  ChromeCitizenNotesManagerDelegate* delegate =
      ChromeCitizenNotesManagerDelegate::GetInstance();
  if (delegate)
    delegate->UpdateDeviceDiscovery();
}

protocol::Response CNTargetHandler::SetRemoteLocations(
...
protocol::Response CNTargetHandler::CreateTarget(
  ...
  if (browser_context_id.has_value()) {
    ...
    profile =
        CitizenNotesBrowserContextManager::GetInstance().GetProfileById(profile_id);
    if (!profile) {
