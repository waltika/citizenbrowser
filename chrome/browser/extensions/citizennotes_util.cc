...
#include "chrome/browser/extensions/citizennotes_util.h"
...
void InspectExtensionHost(
  ...
  if (context_info != nullptr)
    CitizenNotesWindow::OpenCitizenNotesWindow(context_info->web_contents, opened_by);
}
...
void InspectServiceWorkerBackground(const Extension* extension,
  ...
  content::CitizenNotesAgentHost::List targets =
      content::CitizenNotesAgentHost::GetOrCreateAll();
  for (const scoped_refptr<content::CitizenNotesAgentHost>& host : targets) {
    if (host->GetType() == content::CitizenNotesAgentHost::kTypeServiceWorker &&
        host->GetURL() ==
            extension->GetResourceURL(
                BackgroundInfo::GetBackgroundServiceWorkerScript(extension)) &&
        host->GetBrowserContext() == profile) {
      CitizenNotesWindow::OpenCitizenNotesWindow(host, profile, opened_by);
      break;
    }
  }
}
