// Copyright 2021 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef CHROME_BROWSER_UI_VIEWS_CITIZEN_X_CITIZEN_X_BUBBLE_VIEW_IMPL_H_
#define CHROME_BROWSER_UI_VIEWS_CITIZEN_X_CITIZEN_X_BUBBLE_VIEW_IMPL_H_

#include "base/memory/raw_ptr.h"
#include "base/memory/weak_ptr.h"
#include "chrome/browser/share/share_attempt.h"
#include "chrome/browser/ui/citizen_x/citizen_x_bubble_view.h"
#include "chrome/browser/ui/views/location_bar/location_bar_bubble_delegate_view.h"
#include "ui/base/metadata/metadata_header_macros.h"

namespace citizen_x {

class CitizenXBubbleController;
class CitizenXBubbleActionButton;
struct CitizenXAction;

// View component of the Sharing Hub bubble that allows users to share/save the
// current page. The sharing hub bubble also optionally contains a preview of
// the content being shared.
class CitizenXBubbleViewImpl : public CitizenXBubbleView,
                                 public LocationBarBubbleDelegateView {
 public:
  METADATA_HEADER(CitizenXBubbleViewImpl);
  // Bubble will be anchored to |anchor_view|.
  CitizenXBubbleViewImpl(views::View* anchor_view,
                         share::ShareAttempt attempt,
                         CitizenXBubbleController* controller);

  CitizenXBubbleViewImpl(const CitizenXBubbleViewImpl&) = delete;
  CitizenXBubbleViewImpl& operator=(const CitizenXBubbleViewImpl&) = delete;

  ~CitizenXBubbleViewImpl() override;

  // CitizenXBubbleView:
  void Hide() override;

  // LocationBarBubbleDelegateView:
  void OnThemeChanged() override;

  // Public for testing.
  void OnActionSelected(CitizenXBubbleActionButton* button);

 private:
  // views::BubbleDialogDelegateView:
  void Init() override;

  // Creates the scroll view.
  void CreateScrollView();

  // Populates the scroll view containing sharing actions.
  void PopulateScrollView(
      const std::vector<CitizenXAction>& first_party_actions);

  // Resizes and potentially moves the bubble to fit the content's preferred
  // size.
  void MaybeSizeToContents();

  // Callback when the window is about to close, either from external causes
  // (parent window closed) or because the web contents requested it.
  void OnWindowClosing();

  // A raw pointer is *not* safe here; the controller can be torn down before
  // the bubble during the window close path, since the bubble will be closed
  // asynchronously during browser window teardown but the controller will be
  // destroyed synchronously.
  base::WeakPtr<CitizenXBubbleController> controller_;

  // ScrollView containing the list of share/save actions.
  raw_ptr<views::ScrollView> scroll_view_ = nullptr;
                                     
  // The share attempt this bubble was opened for.
  const share::ShareAttempt attempt_;

};

}  // namespace sharing_hub

#endif  // CHROME_BROWSER_UI_VIEWS_CITIZEN_X_CITIZEN_X_BUBBLE_VIEW_IMPL_H_
