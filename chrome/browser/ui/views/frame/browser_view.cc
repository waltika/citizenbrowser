...
BrowserView::BrowserView(std::unique_ptr<Browser> browser)
  ...
  contents_container->SetLayoutManager(std::make_unique<ContentsLayoutManager>(
      devtools_web_view_, citizennotes_web_view_, contents_web_view_, watermark_view));
...
BrowserView::CitizenNotesDockedPlacement BrowserView::GetCitizenNotesDockedPlacement(
    const gfx::Rect& contents_webview_bounds,
    const gfx::Rect& local_webview_container_bounds) {
  // If contents_webview has the same bounds as webview_container, it either
  // means that devtools are not open or devtools are open in a separate
