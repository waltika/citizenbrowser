...
void CitizenNotesDataSource::StartDataRequest(
    const GURL& url,
...
std::string CitizenNotesDataSource::GetMimeType(const GURL& url) {
  return GetMimeTypeForUrl(url);
}

bool CitizenNotesDataSource::ShouldAddContentSecurityPolicy() {
  return false;
}

bool CitizenNotesDataSource::ShouldDenyXFrameOptions() {
  return false;
}

bool CitizenNotesDataSource::ShouldServeMimeTypeAsContentTypeHeader() {
  return true;
}

void CitizenNotesDataSource::StartBundledDataRequest(
...
void CitizenNotesDataSource::StartRemoteDataRequest(
    const GURL& url,
    content::URLDataSource::GotDataCallback callback) {
  CHECK(url.is_valid());
...
void CitizenNotesDataSource::StartCustomDataRequest(
    const GURL& url,
    content::URLDataSource::GotDataCallback callback) {
  if (!url.is_valid()) {
    std::move(callback).Run(CreateNotFoundResponse());
...
void CitizenNotesDataSource::StartNetworkRequest(
    const GURL& url,
  ...
  request_iter->loader->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
      ...
      base::BindOnce(&CitizenNotesDataSource::OnLoadComplete,
...
void CitizenNotesDataSource::OnLoadComplete(
...
CitizenNotesDataSource::PendingRequest::PendingRequest() = default;

CitizenNotesDataSource::PendingRequest::PendingRequest(PendingRequest&& other) =
    default;

CitizenNotesDataSource::PendingRequest::~PendingRequest() {
  if (callback)
    std::move(callback).Run(CreateNotFoundResponse());
}
