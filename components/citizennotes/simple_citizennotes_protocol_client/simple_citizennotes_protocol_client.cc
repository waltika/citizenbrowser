...
#include "components/citizennotes/simple_citizennotes_protocol_client/simple_citizennotes_protocol_client.h"
...
namespace {
// Use --vmodule=simple_citizennotes_protocol_client=2 switch to log protocol
// messages.
...
SimpleCitizenNotesProtocolClient::SimpleCitizenNotesProtocolClient() = default;
SimpleCitizenNotesProtocolClient::SimpleCitizenNotesProtocolClient(
...
void SimpleCitizenNotesProtocolClient::AttachClient(
...
void SimpleCitizenNotesProtocolClient::DetachClient() {
  if (agent_host_) {
...
void SimpleCitizenNotesProtocolClient::AttachToBrowser() {
  AttachClient(CitizenNotesAgentHost::CreateForBrowser(
      /*tethering_task_runner=*/nullptr,
      CitizenNotesAgentHost::CreateServerSocketCallback()));
}

void SimpleCitizenNotesProtocolClient::AttachToWebContents(
    content::WebContents* web_contents) {
  AttachClient(CitizenNotesAgentHost::GetOrCreateFor(web_contents));
}

std::string SimpleCitizenNotesProtocolClient::GetTargetId() {
  DCHECK(agent_host_);
  return agent_host_->GetId();
}

std::unique_ptr<SimpleCitizenNotesProtocolClient>
SimpleCitizenNotesProtocolClient::CreateSession(const std::string& session_id) {
  auto client = std::make_unique<SimpleCitizenNotesProtocolClient>(session_id);
  client->parent_client_ = this;
  sessions_[session_id] = client.get();
  return client;
}

void SimpleCitizenNotesProtocolClient::DispatchProtocolMessage(
  ...
  if (const std::string* session_id = message.FindString("sessionId")) {
    ...
    if (it != sessions_.cend()) {
      content::GetUIThreadTaskRunner({})->PostTask(
          FROM_HERE,
          base::BindOnce(
              &SimpleCitizenNotesProtocolClient::DispatchProtocolMessageTask,
  ...
  content::GetUIThreadTaskRunner({})->PostTask(
      FROM_HERE,
      base::BindOnce(&SimpleCitizenNotesProtocolClient::DispatchProtocolMessageTask,
                     GetWeakPtr(), std::move(message)));
}

void SimpleCitizenNotesProtocolClient::AgentHostClosed(
    content::CitizenNotesAgentHost* agent_host) {
...
void SimpleCitizenNotesProtocolClient::DispatchProtocolMessageTask(
...
void SimpleCitizenNotesProtocolClient::SendProtocolMessage(
    base::Value::Dict message) {
...
void SimpleCitizenNotesProtocolClient::SendCommand(
...
void SimpleCitizenNotesProtocolClient::SendCommand(const std::string& method,
                                               base::Value::Dict params) {
...
void SimpleCitizenNotesProtocolClient::AddEventHandler(
    const std::string& event_name,
    EventCallback event_callback) {
  event_handler_map_[event_name].push_back(std::move(event_callback));
}

void SimpleCitizenNotesProtocolClient::RemoveEventHandler(
    const std::string& event_name,
...
bool SimpleCitizenNotesProtocolClient::HasEventHandler(
    const std::string& event_name,
    const EventCallback& event_callback) {
  auto it = event_handler_map_.find(event_name);
  if (it == event_handler_map_.cend())
...
base::WeakPtr<SimpleCitizenNotesProtocolClient>
SimpleCitizenNotesProtocolClient::GetWeakPtr() {
