...
class UICitizenNotesServerTest : public testing::Test {
 public:
  UICitizenNotesServerTest() {
    base::CommandLine::ForCurrentProcess()->AppendSwitch(
        switches::kEnableUiCitizenNotes);
  }
  ~UICitizenNotesServerTest() override = default;
  ...
  void SetUp() override {
    ...
    server_ =
        UiCitizenNotesServer::CreateForViews(network_context_remote_.get(), 0);
    server_->SetOnSocketConnectedForTesting(run_loop.QuitClosure());
    run_loop.Run();
  }

 protected:
  UiCitizenNotesServer* server() { return server_.get(); }
 ...
 private:
  std::unique_ptr<UiCitizenNotesServer> server_;
};

// Tests whether the server for Views is properly created so we can connect to
// it.
TEST_F(UICitizenNotesServerTest, ConnectionToViewsServer) {
...
// client ID.
TEST_F(UICitizenNotesServerTest, OutOfBoundsClientTest) {
  auto client =
      std::make_unique<UiCitizenNotesClient>("UiCitizenNotesClient", server());
  server()->AttachClient(std::move(client));
  // Cast for public `OnWebSocketRequest`.
  network::server::HttpServer::Delegate* citizennotes_server =
      static_cast<network::server::HttpServer::Delegate*>(server());
  network::server::HttpServerRequestInfo request;
  request.path = "/1";
  citizennotes_server->OnWebSocketRequest(0, request);
  request.path = "/42";
  citizennotes_server->OnWebSocketRequest(0, request);
  request.path = "/-5000";
  citizennotes_server->OnWebSocketRequest(0, request);
