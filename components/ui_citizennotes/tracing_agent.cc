...
using ui_citizennotes::protocol::Response;
...
class TracingAgent::CitizenNotesTraceEndpointProxy
    : public base::RefCountedThreadSafe<CitizenNotesTraceEndpointProxy> {
 public:
  explicit CitizenNotesTraceEndpointProxy(base::WeakPtr<TracingAgent> tracing_agent)
 ...
 protected:
  friend class base::RefCountedThreadSafe<CitizenNotesTraceEndpointProxy>;

 private:
  ~CitizenNotesTraceEndpointProxy() = default;
...
class TracingAgent::PerfettoTracingSession
 ...
 public:
  ...
  void DisableTracing(
      ...
      const scoped_refptr<CitizenNotesTraceEndpointProxy>& endpoint) {
...
void TracingAgent::start(
  ...
  // Since we want minimum changes to the citizennotes frontend, enable the
  // tracing categories for ui_citizennotes here.
  std::string ui_citizennotes_categories =
      "disabled-by-default-citizennotes.timeline,disabled-by-default-citizennotes."
      "timeline.frame,views,latency,toplevel,"
      "benchmark,cc,viz,input,latency,gpu,rail,viz,ui";
  trace_config_ = base::trace_event::TraceConfig(ui_citizennotes_categories,
                                                 options.value_or(""));
...
Response TracingAgent::end() {
  ...
  scoped_refptr<CitizenNotesTraceEndpointProxy> endpoint;
...
void TracingAgent::EditTraceDataForFrontend() {
  ...
  // which is the logic flow that browser citizennotes uses for the performance
  // panel. Without this trace event, citizennotes will use the generic trace, which
  // does not handle fps logic or use the color scheme defined by the
  // category-event map. The processes that are of interest in ui_citizennotes needs
  ...
  // 'BeginFrame'. There is only 1 layer tree in ui_citizennotes, so the layertreeid
  // for all frames in ui_citizennotes is 1. This is used to get the frames, which
