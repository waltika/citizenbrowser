...
#include "content/browser/citizennotes/citizennotes_background_services_context_impl.h"
...
#include "content/browser/citizennotes/citizennotes_background_services.pb.h"
...
void DidGetLoggedBackgroundServiceEvents(
    base::OnceClosure quit_closure,
    std::vector<citizennotes::proto::BackgroundServiceEvent>* out_feature_states,
    std::vector<citizennotes::proto::BackgroundServiceEvent> feature_states) {
...
class CitizenNotesBackgroundServicesContextTest
    : public ::testing::Test,
      CitizenNotesBackgroundServicesContextImpl::EventObserver {
 public:
  CitizenNotesBackgroundServicesContextTest()
      : task_environment_(BrowserTaskEnvironment::IO_MAINLOOP),
        embedded_worker_test_helper_(base::FilePath() /* in memory */) {}

  CitizenNotesBackgroundServicesContextTest(
      const CitizenNotesBackgroundServicesContextTest&) = delete;
  CitizenNotesBackgroundServicesContextTest& operator=(
      const CitizenNotesBackgroundServicesContextTest&) = delete;

  ~CitizenNotesBackgroundServicesContextTest() override = default;
 ...
 protected:
  MOCK_METHOD1(OnEventReceived,
               void(const citizennotes::proto::BackgroundServiceEvent& event));
  ...
  void SimulateBrowserRestart() {
    ...
    // Create |context_|.
    context_ = std::make_unique<CitizenNotesBackgroundServicesContextImpl>(
  ...
  void SimulateOneWeekPassing() {
    ...
    context_->expiration_times_
        [citizennotes::proto::BackgroundService::BACKGROUND_FETCH] = one_week_ago;
  }

  bool IsRecording() {
    return context_->IsRecording(
        citizennotes::proto::BackgroundService::BACKGROUND_FETCH);
  }
  ...
  void LogTestBackgroundServiceEvent(const std::string& log_message) {
    context_->LogBackgroundServiceEvent(
        ...
        CitizenNotesBackgroundService::kBackgroundFetch, kEventName, kInstanceId,
        {{"key", log_message}});
  }
  ...
  void StartRecording() {
    EXPECT_CALL(
        *this, OnRecordingStateChanged(
                   true, citizennotes::proto::BackgroundService::BACKGROUND_FETCH));
    ...
    EXPECT_CALL(
        *this,
        OnRecordingStateChanged(
            false, citizennotes::proto::BackgroundService::BACKGROUND_FETCH));
 ...
 private:
  ...
  std::unique_ptr<CitizenNotesBackgroundServicesContextImpl> context_;
...
#endif
TEST_F(CitizenNotesBackgroundServicesContextTest,
...
TEST_F(CitizenNotesBackgroundServicesContextTest, GetLoggedEvents) {
  StartRecording();
...
TEST_F(CitizenNotesBackgroundServicesContextTest, StopRecording) {
  StartRecording();
  // Initially there are no entries.
  EXPECT_TRUE(GetLoggedBackgroundServiceEvents().empty());
...
TEST_F(CitizenNotesBackgroundServicesContextTest, DelegateExpirationTimes) {
...
TEST_F(CitizenNotesBackgroundServicesContextTest, RecordingExpiration) {
  // Initially expiration time is null.
  EXPECT_FALSE(IsRecording());
...
TEST_F(CitizenNotesBackgroundServicesContextTest, ClearLoggedEvents) {
  StartRecording();
...
TEST_F(CitizenNotesBackgroundServicesContextTest, EventObserverCalled) {
  {
    EXPECT_CALL(*this, OnEventReceived(_)).Times(0);
    LogTestBackgroundServiceEvent("f1");
    task_environment_.RunUntilIdle();
