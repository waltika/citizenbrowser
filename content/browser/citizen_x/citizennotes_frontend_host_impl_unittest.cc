...
#include "content/browser/citizennotes/citizennotes_frontend_host_impl.h"
...
class CitizenNotesFrontendHostImplTest : public RenderViewHostTestHarness {
 public:
  void SetUp() override {
    ...
    citizennotes_frontend_host_impl_ = CitizenNotesFrontendHostImpl::CreateForTesting(
  ...
  void TearDown() override {
    ...
    processor_.reset();
    citizennotes_frontend_host_impl_.reset();
  ...
  void CallOnDidAddMessageToConsole(
    ...
    citizennotes_frontend_host_impl_->OnDidAddMessageToConsole(
        source_frame, log_level, message, line_no, source_id, stack_trace);
  }

 protected:
  scoped_refptr<SiteInstance> site_instance_;
  std::unique_ptr<CitizenNotesFrontendHostImpl> citizennotes_frontend_host_impl_;
  ...
  static constexpr char kSourceURL8[] = "citizennotes://here.is.error/bad.js";
  static constexpr char16_t kSourceURL16[] = u"citizennotes://here.is.error/bad.js";
  static constexpr char kStackTrace8[] =
      "at badFunction (citizennotes://page/my.js:20:30)\n"
      "at poorCaller (citizennotes://page/my.js:50:10)\n";
  static constexpr char16_t kStackTrace16[] =
      u"at badFunction (citizennotes://page/my.js:20:30)\n"
      u"at poorCaller (citizennotes://page/my.js:50:10)\n";
};

class CitizenNotesFrontendHostImplJsErrorReportingEnabledTest
    : public CitizenNotesFrontendHostImplTest {
 public:
  void SetUp() override {
    CitizenNotesFrontendHostImplTest::SetUp();
    scoped_feature_list_.InitAndEnableFeature(
        features::kEnableCitizenNotesJsErrorReporting);
  }
};

constexpr char CitizenNotesFrontendHostImplTest::kMessage8[];
constexpr char16_t CitizenNotesFrontendHostImplTest::kMessage16[];
constexpr char CitizenNotesFrontendHostImplTest::kSourceURL8[];
constexpr char16_t CitizenNotesFrontendHostImplTest::kSourceURL16[];
constexpr char CitizenNotesFrontendHostImplTest::kStackTrace8[];
constexpr char16_t CitizenNotesFrontendHostImplTest::kStackTrace16[];

TEST_F(CitizenNotesFrontendHostImplTest, ErrorNotReportedWhenFeatureIsNotEnabled) {
...
TEST_F(CitizenNotesFrontendHostImplJsErrorReportingEnabledTest, Sent) {
  CallOnDidAddMessageToConsole(main_rfh(),
                               blink::mojom::ConsoleMessageLevel::kError,
                               kMessage16, 5, kSourceURL16, kStackTrace16);
  task_environment()->RunUntilIdle();
  ...
  EXPECT_EQ(processor_->last_error_report().source_system,
            JavaScriptErrorReport::SourceSystem::kCitizenNotesObserver);
  EXPECT_THAT(processor_->last_error_report().stack_trace,
              Optional(Eq(kStackTrace8)));
  // CitizenNotes should use default product & version.
...
TEST_F(CitizenNotesFrontendHostImplJsErrorReportingEnabledTest,
       NotSentForNonErrors) {
  CallOnDidAddMessageToConsole(main_rfh(),
