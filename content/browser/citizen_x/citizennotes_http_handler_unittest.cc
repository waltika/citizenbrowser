...
#include "content/browser/citizennotes/citizennotes_http_handler.h"
...
#include "content/browser/citizennotes/citizennotes_agent_host_impl.h"
...
#include "content/public/test/mock_citizennotes_agent_host.h"
...
class DummyServerSocketFactory : public CitizenNotesSocketFactory {
 public:
...
class MockCitizenNotesManagerDelegate : public CitizenNotesManagerDelegate {
 public:
  static MockCitizenNotesManagerDelegate* last_instance;
  MockCitizenNotesManagerDelegate() { last_instance = this; }
  MOCK_METHOD(scoped_refptr<CitizenNotesAgentHost>,
              CreateNewTarget,
              (const GURL& url, TargetType target_type),
              (override));
  MOCK_METHOD(CitizenNotesAgentHost::List,
              RemoteDebuggingTargets,
              (TargetType target_type),
              (override));
};

MockCitizenNotesManagerDelegate* MockCitizenNotesManagerDelegate::last_instance =
    nullptr;

class MockCitizenNotesAgentHostWithType : public MockCitizenNotesAgentHost {
 public:
  explicit MockCitizenNotesAgentHostWithType(const std::string& type)
      : type_(type) {}

  std::string GetType() override { return type_; }

 private:
  ~MockCitizenNotesAgentHostWithType() override = default;
...
class BrowserClient : public ContentBrowserClient {
 public:
  ...
  CreateCitizenNotesManagerDelegate() override {
    return std::make_unique<MockCitizenNotesManagerDelegate>();
  }
};

}

class CitizenNotesHttpHandlerTest : public testing::Test {
 public:
  CitizenNotesHttpHandlerTest()
...
TEST_F(CitizenNotesHttpHandlerTest, TestStartStop) {
  base::RunLoop run_loop, run_loop_2;
  auto factory = std::make_unique<DummyServerSocketFactory>(
      run_loop.QuitClosure(), run_loop_2.QuitClosure());
  CitizenNotesAgentHost::StartRemoteDebuggingServer(
      std::move(factory), base::FilePath(), base::FilePath());
  // Our dummy socket factory will post a quit message once the server will
  // become ready.
  run_loop.Run();
  CitizenNotesAgentHost::StopRemoteDebuggingServer();
  // Make sure the handler actually stops.
  run_loop_2.Run();
}

TEST_F(CitizenNotesHttpHandlerTest, TestServerSocketFailed) {
  base::RunLoop run_loop, run_loop_2;
...
TEST_F(CitizenNotesHttpHandlerTest, TestCitizenNotesActivePort) {
  base::RunLoop run_loop, run_loop_2;
  ...
  // Now make sure the CitizenNotesActivePort was written into the
  // temporary directory and its contents are as expected.
  base::FilePath active_port_file =
      temp_dir.GetPath().Append(kCitizenNotesActivePortFileName);
...
TEST_F(CitizenNotesHttpHandlerTest, MutatingActionsiRequireSafeVerb) {
  base::RunLoop run_loop, run_loop_2;
  ...
  EXPECT_CALL(
      *MockCitizenNotesManagerDelegate::last_instance,
      CreateNewTarget(GURL("about:blank"), CitizenNotesManagerDelegate::kFrame))
      .WillOnce(Return(base::MakeRefCounted<MockCitizenNotesAgentHost>()));
...
TEST_F(CitizenNotesHttpHandlerTest, TestJsonNew) {
  base::RunLoop run_loop, run_loop_2;
  auto* factory = new TCPServerSocketFactory(run_loop.QuitClosure(),
                                             run_loop_2.QuitClosure());
  CitizenNotesAgentHost::StartRemoteDebuggingServer(
  ...
  EXPECT_CALL(*MockCitizenNotesManagerDelegate::last_instance,
  ...
  EXPECT_CALL(*MockCitizenNotesManagerDelegate::last_instance,
              CreateNewTarget(GURL("http://example.com"),
                              CitizenNotesManagerDelegate::kTab));
  url = GURL(base::StringPrintf(
...
TEST_F(CitizenNotesHttpHandlerTest, TestJsonList) {
  base::RunLoop run_loop, run_loop_2;
  auto* factory = new TCPServerSocketFactory(run_loop.QuitClosure(),
                                             run_loop_2.QuitClosure());
  CitizenNotesAgentHost::StartRemoteDebuggingServer(
  ...
  EXPECT_CALL(*MockCitizenNotesManagerDelegate::last_instance,
              RemoteDebuggingTargets(CitizenNotesManagerDelegate::kFrame));
  ...
  EXPECT_CALL(*MockCitizenNotesManagerDelegate::last_instance,
              RemoteDebuggingTargets(CitizenNotesManagerDelegate::kFrame))
      .WillOnce(Return(std::vector<scoped_refptr<CitizenNotesAgentHost>>{
          base::MakeRefCounted<MockCitizenNotesAgentHostWithType>("service_worker"),
          base::MakeRefCounted<MockCitizenNotesAgentHostWithType>("tab")}));
  ...
  EXPECT_CALL(*MockCitizenNotesManagerDelegate::last_instance,
              RemoteDebuggingTargets(CitizenNotesManagerDelegate::kTab))
      .WillOnce(Return(std::vector<scoped_refptr<CitizenNotesAgentHost>>{
          base::MakeRefCounted<MockCitizenNotesAgentHostWithType>("service_worker"),
          base::MakeRefCounted<MockCitizenNotesAgentHostWithType>("tab")}));
...
class CitizenNotesWebSocketHandlerTest : public CitizenNotesHttpHandlerTest {
 public:
  CitizenNotesWebSocketHandlerTest() = default;
...
TEST_F(CitizenNotesWebSocketHandlerTest,
...
TEST_F(CitizenNotesWebSocketHandlerTest, TestAllowsCLIOverrideAllowsOriginsStar) {
...
TEST_F(CitizenNotesWebSocketHandlerTest, TestAllowsCLIOverrideAllowsOrigins) {
  base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(
