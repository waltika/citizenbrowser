...
void DispatchToAgents(WebContents* web_contents,
                      void (Handler::*method)(MethodArgs...),
                      Args&&... args) {
  auto agent_host = CitizenNotesAgentHost::GetForTab(web_contents);
  if (agent_host) {
    DispatchToAgents(static_cast<CitizenNotesAgentHostImpl*>(agent_host.get()),
                     method, std::forward<Args>(args)...);
  }
  if (content::CitizenNotesAgentHost::HasFor(web_contents)) {
    DispatchToAgents(
        static_cast<CitizenNotesAgentHostImpl*>(
...
void OnResetNavigationRequest(NavigationRequest* navigation_request) {
  ...
  for (FrameTreeNode* node = navigation_request->frame_tree_node(); node;
    ...
    DispatchToAgents(node, &protocol::CNPageHandler::NavigationReset,
                     navigation_request);
  }
}
...
void OnFetchKeepAliveRequestComplete(
  ...
  DispatchToAgents(frame_tree_node, &protocol::CNNetworkHandler::LoadingComplete,
...
void BackForwardCacheNotUsed(
  ...
  DCHECK(nav_request);
  FrameTreeNode* ftn = nav_request->frame_tree_node();
  DispatchToAgents(ftn, &protocol::CNPageHandler::BackForwardCacheNotUsed,
                   nav_request, result, tree_result);
}

void WillSwapFrameTreeNode(FrameTreeNode& old_node, FrameTreeNode& new_node) {
  auto* host = static_cast<RenderFrameCitizenNotesAgentHost*>(
      RenderFrameCitizenNotesAgentHost::GetFor(&old_node));
  if (!host || host->HasSessionsWithoutTabTargetSupport()) {
    return;
  }
  // The new node may have a previous host associated, disconnect it first.
  scoped_refptr<RenderFrameCitizenNotesAgentHost> previous_host =
      static_cast<RenderFrameCitizenNotesAgentHost*>(
          RenderFrameCitizenNotesAgentHost::GetFor(&new_node));
...
void OnFrameTreeNodeDestroyed(FrameTreeNode& frame_tree_node) {
  ...
  if (RenderFrameCitizenNotesAgentHost::GetFor(&frame_tree_node) !=
...
bool IsPrerenderAllowed(FrameTree& frame_tree) {
  ...
  auto* render_frame_agent_host = static_cast<RenderFrameCitizenNotesAgentHost*>(
      RenderFrameCitizenNotesAgentHost::GetFor(ftn));
  ...
  DispatchToAgents(ftn, &protocol::CNPageHandler::IsPrerenderingAllowed,
                   is_allowed);
  return is_allowed;
}
...
void WillInitiatePrerender(FrameTree& frame_tree) {
  ...
  if (auto* host = WebContentsCitizenNotesAgentHost::GetFor(wc)) {
...
void OnNavigationRequestFailed(
  ...
  DispatchToAgents(ftn, &protocol::CNNetworkHandler::LoadingComplete, id,
                   protocol::Network::ResourceTypeEnum::Document, status);
}

bool ShouldBypassCSP(const NavigationRequest& nav_request) {
  CitizenNotesAgentHostImpl* agent_host =
      RenderFrameCitizenNotesAgentHost::GetFor(nav_request.frame_tree_node());
  if (!agent_host) {
    return false;
  }

  for (auto* page : protocol::CNPageHandler::ForAgentHost(agent_host)) {
...
void ApplyNetworkOverridesForDownload(
  ...
  if (ftn) {
    DispatchToAgents(
        ftn, &protocol::CNEmulationHandler::ApplyNetworkOverridesForDownload,
        parameters);
  }
}
...
void WillBeginDownload(download::DownloadCreateInfo* info,
  ...
  DispatchToAgents(ftn, &protocol::CNBrowserHandler::DownloadWillBegin, ftn,
                   item);
  DispatchToAgents(ftn, &protocol::CNPageHandler::DownloadWillBegin, ftn, item);

  for (auto* agent_host : BrowserCitizenNotesAgentHost::Instances()) {
    for (auto* browser_handler :
         protocol::CNBrowserHandler::ForAgentHost(agent_host)) {
...
void ThrottleForServiceWorkerAgentHost(
    ...
    CitizenNotesAgentHostImpl* requesting_agent_host,
    scoped_refptr<CitizenNotesThrottleHandle> throttle_handle) {
  for (auto* target_handler :
       protocol::CNTargetHandler::ForAgentHost(requesting_agent_host)) {
...
std::vector<std::unique_ptr<NavigationThrottle>> CreateNavigationThrottles(
  ...
  if (parent) {
    if (auto* agent_host = RenderFrameCitizenNotesAgentHost::GetFor(parent)) {
      agent_host->auto_attacher()->AppendNavigationThrottles(navigation_handle,
                                                             &result);
    }
  } else {
    for (CitizenNotesAgentHostImpl* host : BrowserCitizenNotesAgentHost::Instances()) {
...
void ThrottleServiceWorkerMainScriptFetch(
  ...
  ServiceWorkerCitizenNotesAgentHost* agent_host =
      ServiceWorkerCitizenNotesManager::GetInstance()
          ->GetCitizenNotesAgentHostForNewInstallingWorker(wrapper, version_id);
  DCHECK(agent_host);
...
void ThrottleWorkerMainScriptFetch(
  ...
  DispatchToAgents(ftn, &protocol::CNTargetHandler::AddWorkerThrottle, agent_host,
...
bool ShouldWaitForDebuggerInWindowOpen() {
  for (auto* browser_agent_host : BrowserCitizenNotesAgentHost::Instances()) {
    for (auto* target_handler :
         protocol::CNTargetHandler::ForAgentHost(browser_agent_host)) {
...
// overrides. It behaves the same as RenderFrameCitizenNotesAgentHost::GetFor for
// all FrameTreeNodes except those that are prerendering. For prerendering
// FrameTreeNodes, it returns the CitizenNotesAgentHost of the primary main frame,
...
CitizenNotesAgentHostImpl* GetCitizenNotesAgentHostForNetworkOverrides(
    FrameTreeNode* frame_tree_node) {
...
void ApplyAuctionNetworkRequestOverrides(
  ...
  for (auto* network : protocol::CNNetworkHandler::ForAgentHost(agent_host)) {
    if (!network->enabled()) {
      continue;
    }
...
void ApplyNetworkRequestOverrides(
  ...
  *citizennotes_user_agent_overridden = false;
  *citizennotes_accept_language_overridden = false;
  bool disable_cache = false;
  CitizenNotesAgentHostImpl* agent_host =
      GetCitizenNotesAgentHostForNetworkOverrides(frame_tree_node);
  if (!agent_host) {
  ...
  for (auto* network : protocol::CNNetworkHandler::ForAgentHost(agent_host)) {
    ...
    *report_raw_headers = true;
    network->ApplyOverrides(&headers, &begin_params->skip_service_worker,
                            &disable_cache, citizennotes_accepted_stream_types);
  }
  ...
  for (auto* emulation : protocol::CNEmulationHandler::ForAgentHost(agent_host)) {
    ...
    emulation->ApplyOverrides(&headers, &ua_overridden,
                              &accept_language_overridden);
    *citizennotes_user_agent_overridden |= ua_overridden;
    *citizennotes_accept_language_overridden |= accept_language_overridden;
  }
...
void OnPrefetchBodyDataReceived(FrameTreeNode* frame_tree_node,
  ...
  DispatchToAgents(frame_tree_node, &protocol::CNNetworkHandler::BodyDataReceived,
...
void OnAuctionWorkletNetworkResponseReceived(
  ...
  network::mojom::URLResponseHeadCitizenNotesInfoPtr head_info =
      network::ExtractCitizenNotesInfo(headers);
  DispatchToAgents(frame_tree_node_id,
                   &protocol::CNNetworkHandler::ResponseReceived, request_id,
...
void OnNavigationRequestWillBeSent(
  ...
  // the security checks are properly applied even if no CitizenNotes session is
  ...
  for (RenderFrameHostImpl* rfh =
    ...
    // Only check frames that qualify as CitizenNotes targets, i.e. (local)? roots.
    if (!RenderFrameCitizenNotesAgentHost::ShouldCreateCitizenNotesForHost(rfh)) {
      continue;
    }
    auto* agent_host = static_cast<RenderFrameCitizenNotesAgentHost*>(
  ...
  // don't report this request to CitizenNotes.
...
bool NotifyCertificateError(CitizenNotesAgentHost* host,
                            int cert_error,
                            const GURL& request_url,
                            const CertErrorCallback& callback) {
  CitizenNotesAgentHostImpl* host_impl = static_cast<CitizenNotesAgentHostImpl*>(host);
  for (auto* security_handler :
       protocol::CNSecurityHandler::ForAgentHost(host_impl)) {
...
bool HandleCertificateError(WebContents* web_contents,
  ...
  scoped_refptr<CitizenNotesAgentHost> agent_host =
      CitizenNotesAgentHost::GetOrCreateFor(web_contents).get();
...
void FencedFrameCreated(
  ...
  auto* agent_host = static_cast<RenderFrameCitizenNotesAgentHost*>(
      RenderFrameCitizenNotesAgentHost::GetFor(
...
void WillStartDragging(FrameTreeNode* main_frame_tree_node,
  ...
  DispatchToAgents(main_frame_tree_node, &protocol::CNInputHandler::StartDragging,
...
void OnWebTransportHandshakeFailed(
  ...
  auto entry =
      protocol::Log::LogEntry::Create()
          ...
          .Build();
  DispatchToAgents(ftn, &protocol::CNLogHandler::EntryAdded, entry.get());
}
...
void OnServiceWorkerMainScriptFetchingFailed(
  ...
  if (response_head) {
    DCHECK(agent_host);
    network::mojom::URLResponseHeadCitizenNotesInfoPtr head_info =
        network::ExtractCitizenNotesInfo(*response_head);
    auto worker_token = agent_host->citizennotes_worker_token().ToString();
    for (auto* network_handler :
         protocol::CNNetworkHandler::ForAgentHost(agent_host)) {
    ...
    for (auto* network_handler :
      ...
      network_handler->LoadingComplete(
          agent_host->citizennotes_worker_token().ToString(),
          protocol::Network::ResourceTypeEnum::Other, status);
    }
  }
}
...
void MaybeAssignResourceRequestId(FrameTreeNode* ftn,
                                  const std::string& id,
                                  network::ResourceRequest& request) {
  if (auto* host = RenderFrameCitizenNotesAgentHost::GetFor(ftn)) {
  ...
  ServiceWorkerCitizenNotesAgentHost* agent_host =
      ServiceWorkerCitizenNotesManager::GetInstance()
          ->GetCitizenNotesAgentHostForNewInstallingWorker(context_wrapper,
                                                       version_id);
  DCHECK(agent_host);
  const std::string request_id = agent_host->citizennotes_worker_token().ToString();
...
void OnWorkerMainScriptLoadingFinished(
    ...
    const network::URLLoaderCompletionStatus& status) {
  DCHECK(ftn);
  DispatchToAgents(ftn, &protocol::CNNetworkHandler::LoadingComplete,
...
void OnWorkerMainScriptRequestWillBeSent(
  ...
  auto* owner_host = RenderFrameCitizenNotesAgentHost::GetFor(ftn);
  if (!owner_host) {
    return;
  }
  MaybeAssignResourceRequestId(owner_host, worker_token.ToString(), request);
  DispatchToAgents(
      ftn, &protocol::CNNetworkHandler::RequestSent, worker_token.ToString(),
      /*loader_id=*/"", request.headers, *request_info,
...
void LogWorkletMessage(RenderFrameHostImpl& frame_host,
  ...
  // citizennotes logging path from the renderer.
...
void ApplyNetworkContextParamsOverrides(
  ...
  for (auto* agent_host : BrowserCitizenNotesAgentHost::Instances()) {
    for (auto* target_handler :
         protocol::CNTargetHandler::ForAgentHost(agent_host)) {
...
void UpdateDeviceRequestPrompt(RenderFrameHost* render_frame_host,
  ...
  DispatchToAgents(ftn,
                   &protocol::CNDeviceAccessHandler::UpdateDeviceRequestPrompt,
                   prompt_info);
}
...
void CleanUpDeviceRequestPrompt(RenderFrameHost* render_frame_host,
  ...
  DispatchToAgents(ftn,
                   &protocol::CNDeviceAccessHandler::CleanUpDeviceRequestPrompt,
                   prompt_info);
}
...
void WillSendFedCmRequest(RenderFrameHost& render_frame_host,
  ...
  DispatchToAgents(ftn, &protocol::CNFedCmHandler::WillSendRequest, intercept,
                   disable_delay);
}
...
void WillShowFedCmDialog(RenderFrameHost& render_frame_host, bool* intercept) {
  ...
  DispatchToAgents(ftn, &protocol::CNFedCmHandler::WillShowDialog, intercept);
}
...
void DidShowFedCmDialog(RenderFrameHost& render_frame_host) {
  ...
  DispatchToAgents(ftn, &protocol::CNFedCmHandler::DidShowDialog);
}
...
void DidCloseFedCmDialog(RenderFrameHost& render_frame_host) {
  ...
  DispatchToAgents(ftn, &protocol::CNFedCmHandler::DidCloseDialog);
}
...
void OnFencedFrameReportRequestSent(int initiator_frame_tree_node_id,
  ...
  DispatchToAgents(initiator_frame_tree_node_id,
                   &protocol::CNNetworkHandler::RequestSent,
...
void OnFencedFrameReportResponseReceived(
  ...
  network::mojom::URLResponseHeadCitizenNotesInfoPtr response_info =
      network::mojom::URLResponseHeadCitizenNotesInfo::New();
  response_info->headers = headers;

  DispatchToAgents(initiator_frame_tree_node_id,
                   &protocol::CNNetworkHandler::ResponseReceived,
                   /*request_id=*/citizennotes_request_id,
                   /*loader_id=*/citizennotes_request_id, final_url,
                   protocol::Network::ResourceTypeEnum::Other, *response_info,
                   /*frame_id=*/protocol::Maybe<std::string>());

  DispatchToAgents(initiator_frame_tree_node_id,
                   &protocol::CNNetworkHandler::LoadingComplete,
                   /*request_id=*/citizennotes_request_id,
                   protocol::Network::Initiator::TypeEnum::Other,
...
void DidChangeFrameLoadingState(FrameTreeNode& ftn) {
  DispatchToAgents(&ftn, &protocol::CNPageHandler::DidChangeFrameLoadingState,
                   ftn);
}
