...
#include "content/browser/citizennotes/citizennotes_instrumentation.h"
#include "content/browser/citizennotes/protocol/audits.h"
#include "content/browser/citizennotes/protocol/citizennotes_protocol_test_support.h"
#include "content/browser/citizennotes/render_frame_citizennotes_agent_host.h"
...
class CitizenNotesIssueStorageBrowserTest : public CitizenNotesProtocolTest {
 public:
...
void ReportDummyIssue(RenderFrameHostImpl* rfh) {
  ...
  auto inspector_issue =
      protocol::Audits::InspectorIssue::Create()
          ...
          .Build();
  citizennotes_instrumentation::ReportBrowserInitiatedIssue(rfh,
                                                        inspector_issue.get());
}

}  // namespace

IN_PROC_BROWSER_TEST_F(CitizenNotesIssueStorageBrowserTest,
                       CitizenNotesReceivesBrowserIssues) {
...
IN_PROC_BROWSER_TEST_F(CitizenNotesIssueStorageBrowserTest,
                       CitizenNotesReceivesBrowserIssuesWhileAttached) {
  EXPECT_TRUE(NavigateToURL(shell(), GURL("about:blank")));
  Attach();
  SendCommandSync("Audits.enable");
  // Report an empty SameSite cookie issue.
...
IN_PROC_BROWSER_TEST_F(CitizenNotesIssueStorageBrowserTest,
  ...
  GURL test_url =
      embedded_test_server()->GetURL("/citizennotes/page-with-oopif.html");
  ...
  // 4) Open CitizenNotes and enable Audits domain.
  Attach();
  SendCommandSync("Audits.enable");
...
IN_PROC_BROWSER_TEST_F(CitizenNotesIssueStorageBrowserTest,
  ...
  // 3) Navigate to /citizennotes/navigation.html
  ASSERT_TRUE(embedded_test_server()->Start());
  GURL test_url = embedded_test_server()->GetURL("/citizennotes/navigation.html");
  EXPECT_TRUE(NavigateToURL(shell(), test_url));
...
class CitizenNotesIssueStorageWithBackForwardCacheBrowserTest
    : public CitizenNotesIssueStorageBrowserTest {
 public:
...
IN_PROC_BROWSER_TEST_F(CitizenNotesIssueStorageWithBackForwardCacheBrowserTest,
  ...
  // 5) Open CitizenNotes and enable Audits domain.
  Attach();
  SendCommandSync("Audits.enable");
...
class CitizenNotesIssueStorageWithPrerenderBrowserTest
    : public CitizenNotesIssueStorageBrowserTest {
 public:
  CitizenNotesIssueStorageWithPrerenderBrowserTest()
      : prerender_test_helper_(base::BindRepeating(
            &CitizenNotesIssueStorageWithPrerenderBrowserTest::GetWebContents,
            base::Unretained(this))) {}

  void SetUp() override {
    prerender_test_helper().RegisterServerRequestMonitor(
        embedded_test_server());
    CitizenNotesIssueStorageBrowserTest::SetUp();
  }
...
IN_PROC_BROWSER_TEST_F(CitizenNotesIssueStorageWithPrerenderBrowserTest,
...
class CitizenNotesIssueStorageFencedFrameTest
    : public CitizenNotesIssueStorageBrowserTest {
 public:
...
IN_PROC_BROWSER_TEST_F(CitizenNotesIssueStorageFencedFrameTest,
