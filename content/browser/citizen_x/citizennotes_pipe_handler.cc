...
const size_t kReceiveBufferSizeForCitizenNotes = 100 * 1024 * 1024;  // 100Mb
...
class PipeReaderBase : public PipeIOBase {
 public:
  PipeReaderBase(base::WeakPtr<CitizenNotesPipeHandler> citizennotes_handler,
                 int read_fd)
      : PipeIOBase("CitizenNotesPipeHandlerReadThread"),
        citizennotes_handler_(std::move(citizennotes_handler)),
        read_fd_(read_fd) {
#if BUILDFLAG(IS_WIN)
...
// Concurrently discard the pipe handles to successfully join threads.
      ...
      if (had_error) {
        if (!shutting_down_.IsSet()) {
          ...
          GetUIThreadTaskRunner({})->PostTask(
              FROM_HERE, base::BindOnce(&CitizenNotesPipeHandler::OnDisconnect,
                                        citizennotes_handler_));
        }
        return 0;
      }
  ...
  void HandleMessage(std::vector<uint8_t> message) {
    GetUIThreadTaskRunner({})->PostTask(
        FROM_HERE, base::BindOnce(&CitizenNotesPipeHandler::HandleMessage,
                                  citizennotes_handler_, std::move(message)));
  }
 ...
 private:
  void ReadLoop() {
    ...
    GetUIThreadTaskRunner({})->PostTask(
        FROM_HERE,
        base::BindOnce(&CitizenNotesPipeHandler::Shutdown, citizennotes_handler_));
  }

  base::WeakPtr<CitizenNotesPipeHandler> citizennotes_handler_;
  int read_fd_;
#if BUILDFLAG(IS_WIN)
  HANDLE read_handle_;
#endif
...
class PipeWriterBase : public PipeIOBase {
 public:
  explicit PipeWriterBase(int write_fd)
      : PipeIOBase("CitizenNotesPipeHandlerWriteThread"), write_fd_(write_fd) {
#if BUILDFLAG(IS_WIN)
    ScopedInvalidParameterHandlerOverride invalid_parameter_handler_override;
...
class PipeReaderASCIIZ : public PipeReaderBase {
 public:
  PipeReaderASCIIZ(base::WeakPtr<CitizenNotesPipeHandler> citizennotes_handler,
                   int read_fd)
      : PipeReaderBase(std::move(citizennotes_handler), read_fd) {
    read_buffer_ = new net::HttpConnection::ReadIOBuffer();
    read_buffer_->set_max_buffer_size(kReceiveBufferSizeForCitizenNotes);
  }
...
class PipeReaderCBOR : public PipeReaderBase {
 public:
  PipeReaderCBOR(base::WeakPtr<CitizenNotesPipeHandler> citizennotes_handler,
                 int read_fd)
      : PipeReaderBase(std::move(citizennotes_handler), read_fd) {}
...
// CitizenNotesPipeHandler ---------------------------------------------------

CitizenNotesPipeHandler::CitizenNotesPipeHandler(int read_fd,
                                         int write_fd,
                                         base::OnceClosure on_disconnect)
    : on_disconnect_(std::move(on_disconnect)),
      read_fd_(read_fd),
      write_fd_(write_fd) {
  browser_target_ = CitizenNotesAgentHost::CreateForBrowser(
  ...
  mode_ = str_mode == "cbor" ? CitizenNotesPipeHandler::ProtocolMode::kCBOR
                             : CitizenNotesPipeHandler::ProtocolMode::kASCIIZ;
...
void CitizenNotesPipeHandler::Shutdown() {
  if (shutting_down_)
    return;
  shutting_down_ = true;
...
CitizenNotesPipeHandler::~CitizenNotesPipeHandler() {
  Shutdown();
}

void CitizenNotesPipeHandler::HandleMessage(std::vector<uint8_t> message) {
  if (browser_target_)
    browser_target_->DispatchProtocolMessage(this, message);
}

void CitizenNotesPipeHandler::OnDisconnect() {
  if (on_disconnect_)
    std::move(on_disconnect_).Run();
}

void CitizenNotesPipeHandler::DispatchProtocolMessage(
...
void CitizenNotesPipeHandler::AgentHostClosed(CitizenNotesAgentHost* agent_host) {}

bool CitizenNotesPipeHandler::UsesBinaryProtocol() {
  return mode_ == ProtocolMode::kCBOR;
}

bool CitizenNotesPipeHandler::AllowUnsafeOperations() {
  return true;
}

std::string CitizenNotesPipeHandler::GetTypeForMetrics() {
