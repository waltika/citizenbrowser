...
scoped_refptr<base::SequencedTaskRunner> cnimpl_task_runner() {
...
scoped_refptr<CitizenNotesStreamFile> CitizenNotesStreamFile::Create(
    CitizenNotesIOContext* context,
    bool binary) {
  return new CitizenNotesStreamFile(context, binary);
}

CitizenNotesStreamFile::CitizenNotesStreamFile(CitizenNotesIOContext* context, bool binary)
    : CitizenNotesIOContext::Stream(cnimpl_task_runner()),
      handle_(Register(context)),
      binary_(binary),
      task_runner_(cnimpl_task_runner()) {}

CitizenNotesStreamFile::~CitizenNotesStreamFile() {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
}

bool CitizenNotesStreamFile::InitOnFileSequenceIfNeeded() {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
  if (had_errors_)
    return false;
  if (file_.IsValid())
...
void CitizenNotesStreamFile::Read(off_t position,
                              size_t max_size,
                              ReadCallback callback) {
  task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&CitizenNotesStreamFile::ReadOnFileSequence, this,
                                position, max_size, std::move(callback)));
}

void CitizenNotesStreamFile::Append(std::unique_ptr<std::string> data) {
  task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&CitizenNotesStreamFile::AppendOnFileSequence, this,
                                std::move(data)));
}

void CitizenNotesStreamFile::ReadOnFileSequence(off_t position,
                                            size_t max_size,
                                            ReadCallback callback) {
...
CitizenNotesIOContext::Stream::Status CitizenNotesStreamFile::InnerReadOnFileSequence(
    off_t position,
    size_t max_size,
    std::string& buffer) {
  DCHECK(task_runner_->RunsTasksInCurrentSequence());
...
void CitizenNotesStreamFile::AppendOnFileSequence(
