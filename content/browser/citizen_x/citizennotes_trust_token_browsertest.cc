...
#include "content/browser/citizennotes/protocol/network.h"
...
class CitizenNotesTrustTokenBrowsertest : public CitizenNotesProtocolTest,
                                      public TrustTokenBrowsertest {
 public:
  void SetUpOnMainThread() override {
    TrustTokenBrowsertest::SetUpOnMainThread();
    CitizenNotesProtocolTest::SetUpOnMainThread();
  }

  void TearDownOnMainThread() override {
    TrustTokenBrowsertest::TearDownOnMainThread();
    CitizenNotesProtocolTest::TearDownOnMainThread();
  }
...
IN_PROC_BROWSER_TEST_F(CitizenNotesTrustTokenBrowsertest,
  ...
  // 2) Open CitizenNotes and enable Network domain.
  Attach();
  SendCommandSync("Network.enable");

  // Make sure there are no existing CitizenNotes events in the queue.
...
IN_PROC_BROWSER_TEST_F(CitizenNotesTrustTokenBrowsertest, FetchEndToEnd) {
  ProvideRequestHandlerKeyCommitmentsToNetworkService({"a.test"});
...
IN_PROC_BROWSER_TEST_F(CitizenNotesTrustTokenBrowsertest, IframeEndToEnd) {
  ProvideRequestHandlerKeyCommitmentsToNetworkService({"a.test"});
...
// When the server rejects issuance, CitizenNotes gets a failed notification.
IN_PROC_BROWSER_TEST_F(CitizenNotesTrustTokenBrowsertest,
...
IN_PROC_BROWSER_TEST_F(CitizenNotesTrustTokenBrowsertest, GetTrustTokens) {
  ProvideRequestHandlerKeyCommitmentsToNetworkService({"a.test"});
...
IN_PROC_BROWSER_TEST_F(CitizenNotesTrustTokenBrowsertest, ClearTrustTokens) {
  ProvideRequestHandlerKeyCommitmentsToNetworkService({"a.test"});
