...
#include "content/browser/citizennotes/citizennotes_video_consumer.h"
...
// CitizenNotesVideoConsumer::ChangeTarget(viz::FrameSinkId) is called
...
// CitizenNotesVideoConsumer via the OnFrameCapturedCallback.
class MockCitizenNotesVideoFrameReceiver {
 public:
  MOCK_METHOD1(OnFrameFromVideoConsumerMock,
               void(scoped_refptr<media::VideoFrame> frame));

  MockCitizenNotesVideoFrameReceiver() {}
  ...
  std::unique_ptr<CitizenNotesVideoConsumer> CreateCitizenNotesVideoConsumer() {
    return std::make_unique<CitizenNotesVideoConsumer>(base::BindRepeating(
        &MockCitizenNotesVideoFrameReceiver::OnFrameFromVideoConsumer,
        weak_factory_.GetWeakPtr()));
  }

 private:
  std::vector<scoped_refptr<media::VideoFrame>> frames_;
  base::WeakPtrFactory<MockCitizenNotesVideoFrameReceiver> weak_factory_{this};
};

class CitizenNotesVideoConsumerTest : public testing::Test {
 public:
  CitizenNotesVideoConsumerTest() {}

  void SetUp() override {
    consumer_ = receiver_.CreateCitizenNotesVideoConsumer();
  ...
  static gfx::Size GetVideoConsumerDefaultMinFrameSize() {
    return CitizenNotesVideoConsumer::kDefaultMinFrameSize;
  }

  static gfx::Size GetVideoConsumerDefaultMaxFrameSize() {
    return CitizenNotesVideoConsumer::kDefaultMaxFrameSize;
  }
 ...
 protected:
  ...
  MockCitizenNotesVideoFrameReceiver receiver_;
  std::unique_ptr<CitizenNotesVideoConsumer> consumer_;
 ...
 private:
  std::unique_ptr<viz::ClientFrameSinkVideoCapturer> CreateMockCapturer() {
    return std::make_unique<viz::ClientFrameSinkVideoCapturer>(
        base::BindRepeating(
            [](base::WeakPtr<CitizenNotesVideoConsumerTest> self,
  ...
  base::WeakPtrFactory<CitizenNotesVideoConsumerTest> weak_factory_{this};
};
...
TEST_F(CitizenNotesVideoConsumerTest, CallbacksAreCalledWhenBufferValid) {
...
TEST_F(CitizenNotesVideoConsumerTest, CallbackIsNotCalledWhenBufferIsTooSmall) {
...
TEST_F(CitizenNotesVideoConsumerTest, StartCaptureCallsSetFunctions) {
...
// Tests that calling 'Set' functions in CitizenNotesVideoConsumer before
...
TEST_F(CitizenNotesVideoConsumerTest, CapturerIsPassedCachedValues) {
  // These values are chosen so that they are valid, and different from
  // the default values in CitizenNotesVideoConsumer.
...
// Tests that CitizenNotesVideoConsumer::IsValidMinAndMaxFrameSize adheres to the
// limits set by media::limits::kMaxDimension
TEST_F(CitizenNotesVideoConsumerTest, IsValidMinAndMaxFrameSize) {
