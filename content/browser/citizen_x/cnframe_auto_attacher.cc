...
using ScopeAgentsMap =
    std::map<GURL, std::unique_ptr<ServiceWorkerCitizenNotesAgentHost::List>>;

void GetMatchingHostsByScopeMap(
    const ServiceWorkerCitizenNotesAgentHost::List& agent_hosts,
  ...
  for (const auto& host : agent_hosts) {
    ...
    if (it == scope_agents_map->end()) {
      std::unique_ptr<ServiceWorkerCitizenNotesAgentHost::List> new_list(
          new ServiceWorkerCitizenNotesAgentHost::List());
...
void AddEligibleHosts(const ServiceWorkerCitizenNotesAgentHost::List& list,
                      ServiceWorkerCitizenNotesAgentHost::Map* result) {
...
ServiceWorkerCitizenNotesAgentHost::Map GetMatchingServiceWorkers(
...
CNFrameAutoAttacher::CNFrameAutoAttacher(CitizenNotesRendererChannel* renderer_channel)
    : CNRendererAutoAttacherBase(renderer_channel) {}

CNFrameAutoAttacher::~CNFrameAutoAttacher() = default;

void CNFrameAutoAttacher::SetRenderFrameHost(
...
void CNFrameAutoAttacher::DidFinishNavigation(
  ...
  // TargetHandler::ResponseThrottle because CitizenNotes wasn't attached when the
  ...
  scoped_refptr<RenderFrameCitizenNotesAgentHost> agent_host =
...
void CNFrameAutoAttacher::UpdatePages() {
  if (!auto_attach())
    return;
    ...
    render_frame_host_->ForEachRenderFrameHostWithAction(
        [root = render_frame_host_, &new_hosts](RenderFrameHostImpl* rfh) {
          ...
          if (frame_tree_node->IsMainFrame() &&
            ...
            scoped_refptr<CitizenNotesAgentHost> new_host =
                RenderFrameCitizenNotesAgentHost::GetOrCreateFor(frame_tree_node);
  ...
  DispatchSetAttachedTargetsOfType(new_hosts, CitizenNotesAgentHost::kTypePage);
}

void CNFrameAutoAttacher::AutoAttachToPage(FrameTree* frame_tree,
                                         bool wait_for_debugger_on_start) {
  if (!auto_attach())
    return;
  scoped_refptr<CitizenNotesAgentHost> agent_host =
      RenderFrameCitizenNotesAgentHost::GetOrCreateFor(frame_tree->root());
  DispatchAutoAttach(agent_host.get(), wait_for_debugger_on_start);
}

void CNFrameAutoAttacher::UpdateAutoAttach(base::OnceClosure callback) {
  if (auto_attach()) {
    UpdateFrames();
    UpdatePages();
    ...
    if (render_frame_host_ && !observing_shared_storage_worklets_) {
      ...
      SharedStorageWorkletCitizenNotesManager::GetInstance()->AddObserver(this);
    }
  } else {
    if (observing_service_workers_) {
    ...
    if (observing_shared_storage_worklets_) {
      SharedStorageWorkletCitizenNotesManager::GetInstance()->RemoveObserver(this);
      observing_shared_storage_worklets_ = false;
    }
  }
  CNRendererAutoAttacherBase::UpdateAutoAttach(std::move(callback));
}

void CNFrameAutoAttacher::WorkerCreated(ServiceWorkerCitizenNotesAgentHost* host,
...
void CNFrameAutoAttacher::WorkerDestroyed(ServiceWorkerCitizenNotesAgentHost* host) {
  ReattachServiceWorkers();
}

void CNFrameAutoAttacher::AuctionWorkletCreated(DebuggableAuctionWorklet* worklet,
                                              bool& should_pause_on_start) {
  if (!render_frame_host_)
    return;
  if (!AuctionWorkletCitizenNotesAgentHost::IsRelevantTo(render_frame_host_,
                                                     worklet)) {
    return;
  }
  should_pause_on_start = wait_for_debugger_on_start();
  DispatchAutoAttach(AuctionWorkletCitizenNotesAgentHostManager::GetInstance()
                         .GetOrCreateFor(worklet)
                         .get(),
                     should_pause_on_start);
}

void CNFrameAutoAttacher::SharedStorageWorkletCreated(
...
void CNFrameAutoAttacher::SharedStorageWorkletDestroyed(
    SharedStorageWorkletCitizenNotesAgentHost* host) {
  if (!render_frame_host_) {
    return;
  }
...
void CNFrameAutoAttacher::ReattachServiceWorkers() {
...
void CNFrameAutoAttacher::UpdateFrames() {
  DCHECK(auto_attach());

  Hosts new_hosts;
  CitizenNotesAgentHost::List new_auction_worklet_hosts;
  CitizenNotesAgentHost::List new_shared_storage_worklet_hosts;
  if (render_frame_host_) {
    render_frame_host_->ForEachRenderFrameHostWithAction(
        [root = render_frame_host_, &new_hosts](RenderFrameHostImpl* rfh) {
          ...
          if (should_create) {
            scoped_refptr<CitizenNotesAgentHost> new_host =
                RenderFrameCitizenNotesAgentHost::GetOrCreateFor(node);
            new_hosts.insert(new_host);
          }

          // Note: We don't search through children of local roots as they will
          // be handled by a CNFrameAutoAttacher that is created for the local
          // root.
          return RenderFrameHost::FrameIterationAction::kSkipChildren;
        });

    AuctionWorkletCitizenNotesAgentHostManager::GetInstance().GetAllForFrame(
        render_frame_host_, &new_auction_worklet_hosts);

    SharedStorageWorkletCitizenNotesManager::GetInstance()->GetAllForFrame(
        render_frame_host_, &new_shared_storage_worklet_hosts);
  }

  DispatchSetAttachedTargetsOfType(new_hosts, CitizenNotesAgentHost::kTypeFrame);
  DispatchSetAttachedTargetsOfType(
      CNTargetAutoAttacher::Hosts(new_auction_worklet_hosts.begin(),
                                new_auction_worklet_hosts.end()),
      CitizenNotesAgentHost::kTypeAuctionWorklet);
  DispatchSetAttachedTargetsOfType(
      CNTargetAutoAttacher::Hosts(new_shared_storage_worklet_hosts.begin(),
                                new_shared_storage_worklet_hosts.end()),
      CitizenNotesAgentHost::kTypeSharedStorageWorklet);
}
