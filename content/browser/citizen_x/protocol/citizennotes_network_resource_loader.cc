...
CitizenNotesNetworkResourceLoader::CitizenNotesNetworkResourceLoader(
...
CitizenNotesNetworkResourceLoader::~CitizenNotesNetworkResourceLoader() = default;

// We can trust the |origin| parameter here, as it is the last committed origin
// of a RenderFrameHost identified by a CitizenNotes frame token. Note that there
// is a potential race condition when CitizenNotes sends a request while the frame
// already navigates away. This is difficult to fix before the
// RenderDocumentHost refactoring is done.
// static
std::unique_ptr<CitizenNotesNetworkResourceLoader>
CitizenNotesNetworkResourceLoader::Create(
    mojo::Remote<network::mojom::URLLoaderFactory> url_loader_factory,
    GURL gurl,
  ...
  net::NetworkTrafficAnnotationTag traffic_annotation =
      net::DefineNetworkTrafficAnnotation("citizennotes_cdp_network_resource", R"(
        semantics {
  ...
  return base::WrapUnique(new CitizenNotesNetworkResourceLoader(
      std::move(resource_request), traffic_annotation,
      std::move(url_loader_factory), std::move(completion_callback)));
}

void CitizenNotesNetworkResourceLoader::OnRetry(base::OnceClosure start_retry) {
  NOTREACHED();
}

void CitizenNotesNetworkResourceLoader::DownloadAsStream() {
  content_.erase();
...
void CitizenNotesNetworkResourceLoader::OnDataReceived(base::StringPiece chunk,
                                                   base::OnceClosure resume) {
  content_.append(chunk);
  std::move(resume).Run();
}

void CitizenNotesNetworkResourceLoader::OnComplete(bool success) {
