 ...
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    test_source_map_path =
        test_source_map_path.AppendASCII("citizennotes").AppendASCII("source.map");
  ...
  static void CheckSuccess(DevtoolsNetworkResourceLoaderTest* test,
                           ...
                           protocol::CitizenNotesNetworkResourceLoader* loader,
  ...
  std::unique_ptr<protocol::CitizenNotesNetworkResourceLoader> CreateLoader(
      GURL url,
      protocol::CitizenNotesNetworkResourceLoader::Caching caching,
      protocol::CitizenNotesNetworkResourceLoader::CompletionCallback callback) {
    return protocol::CitizenNotesNetworkResourceLoader::Create(
        ...
        protocol::CitizenNotesNetworkResourceLoader::Credentials::kInclude,
        std::move(callback));
  }
...
IN_PROC_BROWSER_TEST_F(DevtoolsNetworkResourceLoaderTest, BasicDownload) {
  ...
  const GURL source_map_url(
      embedded_test_server()->GetURL("a.com", "/citizen_x/source.map"));
  auto loader =
      CreateLoader(source_map_url,
                   protocol::CitizenNotesNetworkResourceLoader::Caching::kDefault,
...
IN_PROC_BROWSER_TEST_F(DevtoolsNetworkResourceLoaderTest,
  ...
  base::RunLoop run_loop;
  const GURL source_map_url(
      embedded_test_server()->GetURL("b.com", "/citizen_x/source.map"));
...
IN_PROC_BROWSER_TEST_F(DevtoolsNetworkResourceLoaderTest,
  ...
  auto loader = CreateLoader(
      source_map_url, protocol::CitizenNotesNetworkResourceLoader::Caching::kBypass,
      base::BindOnce(CheckSuccess, this, &run_loop));
  run_loop.Run();
