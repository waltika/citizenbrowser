...
CNBrowserHandler::CNBrowserHandler(bool allow_set_download_behavior)
    : CitizenNotesDomainHandler(Browser::Metainfo::domainName),
      download_events_enabled_(false),
      allow_set_download_behavior_(allow_set_download_behavior) {}

CNBrowserHandler::~CNBrowserHandler() = default;

Response CNBrowserHandler::Disable() {
  ...
  for (auto& browser_context_id : contexts_with_overridden_downloads_) {
    ...
    if (browser_context) {
      auto* delegate =
          CitizenNotesDownloadManagerDelegate::GetInstance(browser_context);
      if (delegate) {
        delegate->set_download_behavior(
            CitizenNotesDownloadManagerDelegate::DownloadBehavior::DEFAULT);
      }
    }
  }
...
void CNBrowserHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<Browser::Frontend>(dispatcher->channel());
  Browser::Dispatcher::wire(dispatcher, this);
}

Response CNBrowserHandler::GetVersion(std::string* protocol_version,
                                    std::string* product,
                                    std::string* revision,
                                    std::string* user_agent,
                                    std::string* js_version) {
  *protocol_version = CitizenNotesAgentHost::GetProtocolVersion();
...
// static
Response CNBrowserHandler::FindBrowserContext(
...
// static
std::vector<CNBrowserHandler*> CNBrowserHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNBrowserHandler>(Browser::Metainfo::domainName);
}

Response CNBrowserHandler::SetPermission(
...
Response CNBrowserHandler::GrantPermissions(
...
Response CNBrowserHandler::ResetPermissions(
    Maybe<std::string> browser_context_id) {
  BrowserContext* browser_context = nullptr;
  Response response = FindBrowserContext(browser_context_id, &browser_context);
  if (!response.IsSuccess())
...
Response CNBrowserHandler::SetDownloadBehavior(
...
Response CNBrowserHandler::DoSetDownloadBehavior(
    const std::string& behavior,
    BrowserContext* browser_context,
    Maybe<std::string> download_path) {
  ...
  CitizenNotesManagerDelegate* manager_delegate =
      CitizenNotesManager::GetInstance()->delegate();
  if (!manager_delegate) {
    return Response::ServerError(
        "Browser context management is not supported.");
  ...
  auto* delegate =
      CitizenNotesDownloadManagerDelegate::GetOrCreateInstance(browser_context);
  if (behavior == Browser::SetDownloadBehavior::BehaviorEnum::Allow) {
    delegate->set_download_behavior(
        CitizenNotesDownloadManagerDelegate::DownloadBehavior::ALLOW);
    delegate->set_download_path(download_path.value());
  } else if (behavior ==
             Browser::SetDownloadBehavior::BehaviorEnum::AllowAndName) {
    delegate->set_download_behavior(
        CitizenNotesDownloadManagerDelegate::DownloadBehavior::ALLOW_AND_NAME);
    delegate->set_download_path(download_path.value());
  } else if (behavior == Browser::SetDownloadBehavior::BehaviorEnum::Deny) {
    delegate->set_download_behavior(
        CitizenNotesDownloadManagerDelegate::DownloadBehavior::DENY);
  } else {
    delegate->set_download_behavior(
        CitizenNotesDownloadManagerDelegate::DownloadBehavior::DEFAULT);
  }
...
Response CNBrowserHandler::CancelDownload(const std::string& guid,
                                        Maybe<std::string> browser_context_id) {
  BrowserContext* browser_context = nullptr;
  Response response = FindBrowserContext(browser_context_id, &browser_context);
  if (!response.IsSuccess())
...
Response CNBrowserHandler::GetHistograms(
...
Response CNBrowserHandler::GetHistogram(
...
Response CNBrowserHandler::GetBrowserCommandLine(
...
Response CNBrowserHandler::Crash() {
  CHECK(false);
  return Response::Success();
}

Response CNBrowserHandler::CrashGpuProcess() {
...
void CNBrowserHandler::OnDownloadUpdated(download::DownloadItem* item) {
  std::string state;
...
void CNBrowserHandler::OnDownloadDestroyed(download::DownloadItem* item) {
  pending_downloads_.erase(item);
}

void CNBrowserHandler::DownloadWillBegin(FrameTreeNode* ftn,
...
void CNBrowserHandler::SetDownloadEventsEnabled(bool enabled) {
  if (!enabled) {
...
std::unique_ptr<Browser::Histogram> CNBrowserHandler::GetHistogramData(
