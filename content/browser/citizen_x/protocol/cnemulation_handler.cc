...
CNEmulationHandler::CNEmulationHandler()
    : CitizenNotesDomainHandler(Emulation::Metainfo::domainName),
      touch_emulation_enabled_(false),
      device_emulation_enabled_(false),
      focus_emulation_enabled_(false),
      host_(nullptr) {}

CNEmulationHandler::~CNEmulationHandler() = default;

// static
std::vector<CNEmulationHandler*> CNEmulationHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNEmulationHandler>(
      Emulation::Metainfo::domainName);
}

void CNEmulationHandler::SetRenderer(int process_host_id,
...
void CNEmulationHandler::Wire(UberDispatcher* dispatcher) {
  Emulation::Dispatcher::wire(dispatcher, this);
}

Response CNEmulationHandler::Disable() {
  if (touch_emulation_enabled_) {
    touch_emulation_enabled_ = false;
    UpdateTouchEventEmulationState();
  }
...
void CNEmulationHandler::GetOverriddenSensorInformation(
...
/*  it->second->GetVirtualSensorInformation(base::BindOnce(
      ...
      std::move(callback)));*/ //TODO: Clean
}

void CNEmulationHandler::SetSensorOverrideEnabled(
    bool enabled,
    const Emulation::SensorType& type,
...
/*  if (enabled) {
    ...
    auto virtual_sensor =
        WebContentsSensorProviderProxy::GetOrCreate(GetWebContents())
            ->CreateVirtualSensorForCitizenNotes(
  ...
  callback->sendSuccess();*/ //TODO: Clean
}

void CNEmulationHandler::SetSensorOverrideReadings(
    const Emulation::SensorType& type,
...
/*  it->second->UpdateVirtualSensor(
      ...
      base::BindOnce(
          ...
          std::move(callback)));*/ // TODO: Change
}

Response CNEmulationHandler::SetIdleOverride(bool is_user_active,
...
Response CNEmulationHandler::ClearIdleOverride() {
  if (!host_)
    return Response::InternalError();
...
Response CNEmulationHandler::SetGeolocationOverride(Maybe<double> latitude,
...
Response CNEmulationHandler::ClearGeolocationOverride() {
  if (!host_)
    return Response::InternalError();
...
Response CNEmulationHandler::SetEmitTouchEventsForMouse(
    bool enabled,
...
Response CNEmulationHandler::CanEmulate(bool* result) {
#if BUILDFLAG(IS_ANDROID)
  *result = false;
#else
  *result = true;
  if (host_) {
    if (GetWebContents()->GetVisibleURL().SchemeIs(kChromeCitizenNotesScheme) ||
...
Response CNEmulationHandler::SetDeviceMetricsOverride(
    int width,
    int height,
...
Response CNEmulationHandler::ClearDeviceMetricsOverride() {
  if (!host_)
    return Response::ServerError("Can't find the associated web contents");
  if (host_->GetParentOrOuterDocument())
    return Response::ServerError(kCommandIsOnlyAvailableAtTopTarget);
...
Response CNEmulationHandler::SetVisibleSize(int width, int height) {
  if (width < 0 || height < 0)
...
Response CNEmulationHandler::SetUserAgentOverride(
...
Response CNEmulationHandler::SetFocusEmulationEnabled(bool enabled) {
...
Response CNEmulationHandler::SetEmulatedMedia(
...
blink::DeviceEmulationParams CNEmulationHandler::GetDeviceEmulationParams() {
  return device_emulation_params_;
}

void CNEmulationHandler::SetDeviceEmulationParams(
...
void CNEmulationHandler::UpdateDeviceEmulationState() {
  if (!host_)
    return;
  ...
  // this is tricky since we'd have to track the CitizenNotes message id with the
  // WidgetMsg and acknowledgment, as well as plump the acknowledgment back to
  // the CNEmulationHandler somehow. Mojo callbacks should make this much simpler.
...
void CNEmulationHandler::UpdateDeviceEmulationStateForHost(
...
void CNEmulationHandler::ApplyOverrides(net::HttpRequestHeaders* headers,
...
bool CNEmulationHandler::ApplyUserAgentMetadataOverrides(
...
void CNEmulationHandler::ApplyNetworkOverridesForDownload(
