...
CNFedCmHandler::CNFedCmHandler()
: CitizenNotesDomainHandler(FedCm::Metainfo::domainName) {}

CNFedCmHandler::~CNFedCmHandler() = default;

// static
std::vector<CNFedCmHandler*> CNFedCmHandler::ForAgentHost(
                                                          CitizenNotesAgentHostImpl* host) {
    return host->HandlersByName<CNFedCmHandler>(FedCm::Metainfo::domainName);
}

void CNFedCmHandler::SetRenderer(int process_host_id,
                                 RenderFrameHostImpl* frame_host) {
    frame_host_ = frame_host;
}

void CNFedCmHandler::Wire(UberDispatcher* dispatcher) {
    frontend_ = std::make_unique<FedCm::Frontend>(dispatcher->channel());
    FedCm::Dispatcher::wire(dispatcher, this);
}

DispatchResponse CNFedCmHandler::Enable(Maybe<bool> in_disableRejectionDelay) {
    ...
    // This could happen if CNFedCmHandler::Enable was called to enable/disable the
    // rejection delay.
...
DispatchResponse CNFedCmHandler::Disable() {
    enabled_ = false;
    return DispatchResponse::Success();
}

void CNFedCmHandler::DidShowDialog() {
    DCHECK(frontend_);
    if (!enabled_) {
        return;
    }
...
void CNFedCmHandler::DidCloseDialog() {
    CHECK(frontend_);
    if (!enabled_) {
        return;
    }
...
DispatchResponse CNFedCmHandler::SelectAccount(const String& in_dialogId,
                                               int in_accountIndex) {
...
DispatchResponse CNFedCmHandler::OpenUrl(
...
DispatchResponse CNFedCmHandler::ClickDialogButton(
                                                   const String& in_dialogId,
...
DispatchResponse CNFedCmHandler::DismissDialog(const String& in_dialogId,
...
DispatchResponse CNFedCmHandler::ResetCooldown() {
...
url::Origin CNFedCmHandler::GetEmbeddingOrigin() {
    CHECK(frame_host_);
    CHECK(frame_host_->GetMainFrame());
    return frame_host_->GetMainFrame()->GetLastCommittedOrigin();
}

FederatedAuthRequestPageData* CNFedCmHandler::GetPageData() {
    if (!frame_host_) {
        return nullptr;
    }
...
FederatedAuthRequestImpl* CNFedCmHandler::GetFederatedAuthRequest() {
...
const std::vector<IdentityProviderData>* CNFedCmHandler::GetIdentityProviderData(
...
CNFedCmHandler::GetApiPermissionContext() {
    if (!frame_host_) {
        return nullptr;
    }
