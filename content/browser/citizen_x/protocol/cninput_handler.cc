...
#include "content/browser/citizen_x/protocol/cnnative_input_event_builder.h"
...
class CNInputHandler::InputInjector
    : public RenderWidgetHost::InputEventObserver {
 public:
  InputInjector(CNInputHandler* owner, RenderWidgetHostImpl* widget_host)
  ...
  void InjectKeyboardEvent(const NativeWebKeyboardEvent& keyboard_event,
    ...
    base::WeakPtr<CNInputHandler::InputInjector> weak_this =
 ...
 private:
  ...
  RAW_PTR_EXCLUSION CNInputHandler* const owner_;
  ...
  base::WeakPtrFactory<CNInputHandler::InputInjector> weak_ptr_factory_{this};
};

struct CNInputHandler::DragController::DragState {
  DropData data;
...
struct CNInputHandler::DragController::InitialState {
  std::unique_ptr<blink::WebMouseEvent> event;
  base::WeakPtr<RenderWidgetHostImpl> host;
};

CNInputHandler::DragController::DragController(CNInputHandler& handler)
    : handler_(handler) {}

CNInputHandler::DragController::~DragController() = default;

bool CNInputHandler::DragController::HandleMouseEvent(
...
void CNInputHandler::DragController::EnsureDraggingEntered(
    RenderWidgetHostImpl& host,
    const blink::WebMouseEvent& event) {
...
void CNInputHandler::DragController::StartDragging(
  ...
  UpdateDragging(*initial_state_->host,
                 ...
                 // CNInputHandler::DragController::HandleMouseEvent.
                 std::make_unique<blink::WebMouseEvent>(*initial_state_->event),
                 nullptr);
}

void CNInputHandler::DragController::CancelDragging(base::OnceClosure callback) {
...
void CNInputHandler::DragController::UpdateDragging(
    RenderWidgetHostImpl& host,
    std::unique_ptr<blink::WebMouseEvent> event,
  ...
  drag_state_->host->DragTargetDragOver(
      ...
      base::BindOnce(&CNInputHandler::DragController::DragUpdated,
                     weak_factory_.GetWeakPtr(), std::move(event),
                     std::move(callback)));
}

void CNInputHandler::DragController::DragUpdated(
    std::unique_ptr<blink::WebMouseEvent> event,
    std::unique_ptr<FailSafe<DispatchMouseEventCallback>> callback,
...
void CNInputHandler::DragController::EndDragging(
    RenderWidgetHostImpl* host_hint,
    std::unique_ptr<blink::WebMouseEvent> event,
    std::unique_ptr<FailSafe<DispatchMouseEventCallback>> callback) {
  if (drag_state_->updating > 0) {
    auto update_callback = base::BindOnce(
        &CNInputHandler::DragController::EndDragging, weak_factory_.GetWeakPtr(),
  ...
  handler_.web_contents_->GetInputEventRouter()
      ->GetRenderWidgetHostAtPointAsynchronously(
          ...
          base::BindOnce(&CNInputHandler::DragController::
                             EndDraggingWithRenderWidgetHostAtPoint,
                         weak_factory_.GetWeakPtr(), std::move(event),
                         std::move(callback)));
}

void CNInputHandler::DragController::EndDraggingWithRenderWidgetHostAtPoint(
    std::unique_ptr<blink::WebMouseEvent> event,
    std::unique_ptr<FailSafe<DispatchMouseEventCallback>> callback,
...
CNInputHandler::CNInputHandler(bool allow_file_access,
                           bool allow_sending_input_to_browser)
    : CitizenNotesDomainHandler(Input::Metainfo::domainName),
      drag_controller_(*this),
      allow_file_access_(allow_file_access),
      allow_sending_input_to_browser_(allow_sending_input_to_browser) {}

CNInputHandler::~CNInputHandler() = default;

// static
std::vector<CNInputHandler*> CNInputHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNInputHandler>(Input::Metainfo::domainName);
}

void CNInputHandler::SetRenderer(int process_host_id,
...
void CNInputHandler::DispatchKeyEvent(
    const std::string& type,
    Maybe<int> modifiers,
    Maybe<double> timestamp,
  ...
  if (event.native_key_code && allow_sending_input_to_browser_)
    event.os_event = CNNativeInputEventBuilder::CreateEvent(event);
  else
...
void CNInputHandler::InsertText(const std::string& text,
...
void CNInputHandler::ImeSetComposition(
  ...
  // Currently no CitizenNotes target for Prerender.
...
void CNInputHandler::DispatchMouseEvent(
    const std::string& event_type,
    double x,
    double y,
    Maybe<int> modifiers,
...
void CNInputHandler::HandleMouseEvent(
    std::unique_ptr<blink::WebMouseEvent> event,
    std::unique_ptr<DispatchMouseEventCallback> callback) {
  bool is_wheel_event =
      event->GetType() == blink::WebInputEvent::Type::kMouseWheel;
  ...
  auto findWidgetAndDispatchEvent = base::BindOnce(
      [](base::WeakPtr<CNInputHandler> self,
        ...
        widget_host->delegate()
            ...
            ->GetRenderWidgetHostAtPointAsynchronously(
                ...
                base::BindOnce(&CNInputHandler::OnWidgetForDispatchMouseEvent,
...
void CNInputHandler::DispatchDragEvent(
    const std::string& event_type,
    double x,
    double y,
  ...
  widget_host->delegate()
      ...
      ->GetRenderWidgetHostAtPointAsynchronously(
          ...
          base::BindOnce(&CNInputHandler::OnWidgetForDispatchDragEvent,
                         weak_factory_.GetWeakPtr(), event_type, x, y,
                         std::move(data), std::move(modifiers),
                         std::move(callback)));
}

void CNInputHandler::OnWidgetForDispatchDragEvent(
    const std::string& event_type,
    double x,
    double y,
    std::unique_ptr<Input::DragData> data,
...
float CNInputHandler::ScaleFactor() {
...
void CNInputHandler::StartDragging(const content::DropData& drop_data,
...
void CNInputHandler::DragEnded() {
  drag_controller_.drag_state_ = nullptr;
  drag_controller_.initial_state_ = nullptr;
}

Response CNInputHandler::SetInterceptDrags(bool enabled) {
  intercept_drags_ = enabled;
  return Response::Success();
}

void CNInputHandler::OnWidgetForDispatchMouseEvent(
    std::unique_ptr<DispatchMouseEventCallback> callback,
    std::unique_ptr<blink::WebMouseEvent> event,
    base::WeakPtr<RenderWidgetHostViewBase> target,
...
void CNInputHandler::DispatchTouchEvent(
    const std::string& event_type,
    std::unique_ptr<Array<Input::TouchPoint>> touch_points,
...
void CNInputHandler::CancelDragging(
...
void CNInputHandler::DispatchWebTouchEvent(
    const std::string& event_type,
    std::unique_ptr<Array<Input::TouchPoint>> touch_points,
    protocol::Maybe<int> modifiers,
    protocol::Maybe<double> timestamp,
      ...
      [](base::WeakPtr<CNInputHandler> self,
        ...
        widget_host->delegate()
            ...
            ->GetRenderWidgetHostAtPointAsynchronously(
                ...
                base::BindOnce(&CNInputHandler::OnWidgetForDispatchWebTouchEvent,
...
void CNInputHandler::OnWidgetForDispatchWebTouchEvent(
    std::unique_ptr<DispatchTouchEventCallback> callback,
    std::vector<blink::WebTouchEvent> events,
    base::WeakPtr<RenderWidgetHostViewBase> target,
...
void CNInputHandler::DispatchSyntheticPointerActionTouch(
    const std::string& event_type,
    std::unique_ptr<Array<Input::TouchPoint>> touch_points,
    protocol::Maybe<int> modifiers,
    protocol::Maybe<double> timestamp,
...
SyntheticPointerActionParams CNInputHandler::PrepareSyntheticPointerActionParams(
    SyntheticPointerActionParams::PointerActionType pointer_action_type,
    int id,
    double x,
    double y,
...
Response CNInputHandler::EmulateTouchFromMouseEvent(const std::string& type,
                                                  int x,
                                                  int y,
...
Response CNInputHandler::SetIgnoreInputEvents(bool ignore) {
...
void CNInputHandler::SynthesizePinchGesture(
    double x,
    double y,
    double scale_factor,
...
void CNInputHandler::SynthesizeScrollGesture(
    double x,
    double y,
...
void CNInputHandler::SynthesizeRepeatingScroll(
    SyntheticSmoothScrollGestureParams gesture_params,
    int repeat_count,
...
void CNInputHandler::OnScrollFinished(
  ...
  if (repeat_count > 0) {
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE,
        base::BindOnce(&CNInputHandler::SynthesizeRepeatingScroll,
...
void CNInputHandler::SynthesizeTapGesture(
    double x,
    double y,
    Maybe<int> duration,
    Maybe<int> tap_count,
...
void CNInputHandler::ClearInputState() {
...
bool CNInputHandler::PointIsWithinContents(gfx::PointF point) const {
  gfx::Rect bounds = host_->GetView()->GetViewBounds();
  bounds -= bounds.OffsetFromOrigin();  // Translate the bounds to (0,0).
  return bounds.Contains(point.x(), point.y());
}

CNInputHandler::InputInjector* CNInputHandler::EnsureInjector(
    RenderWidgetHostImpl* widget_host) {
...
RenderWidgetHostViewBase* CNInputHandler::GetRootView() {
  if (!host_)
    return nullptr;
