...
#ifndef CONTENT_BROWSER_CITIZENNOTES_PROTOCOL_INPUT_HANDLER_H_
#define CONTENT_BROWSER_CITIZENNOTES_PROTOCOL_INPUT_HANDLER_H_
...
#include "content/browser/citizen_x/protocol/input.h"
...
class CNInputHandler : public CitizenNotesDomainHandler, public Input::Backend {
 public:
  CNInputHandler(bool allow_file_access, bool allow_sending_input_to_browser);

  CNInputHandler(const CNInputHandler&) = delete;
  CNInputHandler& operator=(const CNInputHandler&) = delete;

  ~CNInputHandler() override;

  static std::vector<CNInputHandler*> ForAgentHost(CitizenNotesAgentHostImpl* host);

  void Wire(UberDispatcher* dispatcher) override;
  void SetRenderer(int process_host_id,
                   RenderFrameHostImpl* frame_host) override;

  // StartDragging is used to inform CDP's CNInputHandler to start dragging. This
  ...
  void StartDragging(const DropData& drop_data,
                     ...
                     bool* intercepted);
  // DragEnded is used to inform CDP's CNInputHandler when a drag has ended. This
 ...
 private:
  ...
  class DragController {
   public:
    ...
    explicit DragController(CNInputHandler& handler);
    ~DragController();
   ...
   private:
    ...
    friend void CNInputHandler::StartDragging(
        const DropData& drop_data,
        const blink::mojom::DragData& drag_data,
        blink::DragOperationsMask drag_operations_mask,
        bool* intercepted);
    friend void CNInputHandler::DragEnded();
  ...
  RAW_PTR_EXCLUSION WebContentsImpl* web_contents_ = nullptr;
  ...
  base::WeakPtrFactory<CNInputHandler> weak_factory_{this};
};

}  // namespace protocol
}  // namespace content

#endif  // CONTENT_BROWSER_CITIZENNOTES_PROTOCOL_INPUT_HANDLER_H_
