...
CNMemoryHandler::CNMemoryHandler()
    : CitizenNotesDomainHandler(Memory::Metainfo::domainName),
      process_host_id_(ChildProcessHost::kInvalidUniqueID) {}

CNMemoryHandler::~CNMemoryHandler() = default;

void CNMemoryHandler::Wire(UberDispatcher* dispatcher) {
  Memory::Dispatcher::wire(dispatcher, this);
}

void CNMemoryHandler::SetRenderer(int process_host_id,
                                RenderFrameHostImpl* frame_host) {
  process_host_id_ = process_host_id;
}

Response CNMemoryHandler::GetBrowserSamplingProfile(
...
Response CNMemoryHandler::SetPressureNotificationsSuppressed(
    bool suppressed) {
  base::MemoryPressureListener::SetNotificationsSuppressed(suppressed);
  return Response::Success();
}

Response CNMemoryHandler::SimulatePressureNotification(
...
void CNMemoryHandler::PrepareForLeakDetection(
  ...
  leak_detector_.set_disconnect_handler(base::BindOnce(
      &CNMemoryHandler::OnLeakDetectorIsGone, base::Unretained(this)));
  leak_detector_->PerformLeakDetection(base::BindOnce(
      &CNMemoryHandler::OnLeakDetectionComplete, weak_factory_.GetWeakPtr()));
}

void CNMemoryHandler::OnLeakDetectionComplete(
...
void CNMemoryHandler::OnLeakDetectorIsGone() {
