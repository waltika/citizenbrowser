...
#include "content/browser/citizen_x/protocol/citizennotes_network_resource_loader.h"
#include "content/browser/citizen_x/protocol/cnhandler_helpers.h"
#include "content/browser/citizen_x/protocol/network.h"
#include "content/browser/citizen_x/protocol/page.h"
...
std::unique_ptr<Network::Cookie> BuildCookie(
  ...
  std::unique_ptr<Network::Cookie> citizennotes_cookie =
  ...
  if (maybe_same_site) {
    citizennotes_cookie->SetSameSite(*maybe_same_site);
  }
  ...
  if (partition_key) {
    ...
    if (partition_key->IsSerializeable()) {
      ...
      DCHECK(serialized);
      citizennotes_cookie->SetPartitionKey(serialized_partition_key);
    } else {
      citizennotes_cookie->SetPartitionKeyOpaque(partition_key->site().opaque());
...
MakeCookieFromProtocolValues(const std::string& name,
  ...
  // TODO(crbug.com/1225444) Add Partitioned to CitizenNotes cookie structures.
...
absl::optional<CitizenNotesURLLoaderInterceptor::InterceptionStage>
ToInterceptorStage(
...
String GetProtocol(const GURL& url,
                   const network::mojom::URLResponseHeadCitizenNotesInfo& info) {
...
class BackgroundSyncRestorer {
 ...
 private:
  void SetServiceWorkerOfflineStatus(bool offline) {
    ...
    scoped_refptr<CitizenNotesAgentHost> host =
        CitizenNotesAgentHost::GetForId(host_id_);
    if (!host || !storage_partition_ ||
        host->GetType() != CitizenNotesAgentHost::kTypeServiceWorker) {
      return;
    }
    scoped_refptr<ServiceWorkerCitizenNotesAgentHost> service_worker_host =
        static_cast<ServiceWorkerCitizenNotesAgentHost*>(host.get());
...
CNNetworkHandler::CNNetworkHandler(
    ...
    : CitizenNotesDomainHandler(Network::Metainfo::domainName),
      host_id_(host_id),
...
CNNetworkHandler::~CNNetworkHandler() = default;

// static
std::unique_ptr<Array<Network::Cookie>> CNNetworkHandler::BuildCookieArray(
...
// static
net::Error CNNetworkHandler::NetErrorFromString(const std::string& error,
                                              bool* ok) {
  *ok = true;
...
// static
String CNNetworkHandler::NetErrorToString(int net_error) {
  switch (net_error) {
...
// static
bool CNNetworkHandler::AddInterceptedResourceType(
...
// static
const char* CNNetworkHandler::ResourceTypeToString(
...
// static
std::vector<CNNetworkHandler*> CNNetworkHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNNetworkHandler>(Network::Metainfo::domainName);
}

void CNNetworkHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<Network::Frontend>(dispatcher->channel());
  Network::Dispatcher::wire(dispatcher, this);
}

void CNNetworkHandler::SetRenderer(int render_process_host_id,
...
Response CNNetworkHandler::Enable(Maybe<int> max_total_size,
...
Response CNNetworkHandler::Disable() {
  enabled_ = false;
...
CNNetworkHandler::BuildProtocolReport(const net::ReportingReport& report) {
  if (!host_) {
    return nullptr;
  }
...
void CNNetworkHandler::OnReportAdded(const net::ReportingReport& report) {
...
void CNNetworkHandler::OnReportUpdated(const net::ReportingReport& report) {
  auto protocol_report = BuildProtocolReport(report);
  if (protocol_report) {
...
CNNetworkHandler::BuildProtocolEndpoint(const net::ReportingEndpoint& endpoint) {
...
void CNNetworkHandler::OnEndpointsUpdatedForOrigin(
...
Response CNNetworkHandler::EnableReportingApi(const bool enable) {
...
Response CNNetworkHandler::SetCacheDisabled(bool cache_disabled) {
  cache_disabled_ = cache_disabled;
  return Response::FallThrough();
}

Response CNNetworkHandler::SetAcceptedEncodings(
...
Response CNNetworkHandler::ClearAcceptedEncodingsOverride() {
  accepted_stream_types_ = absl::nullopt;
  return Response::FallThrough();
}

class CitizennotesClearCacheObserver
    : public content::BrowsingDataRemover::Observer {
 public:
  explicit CitizennotesClearCacheObserver(
      content::BrowsingDataRemover* remover,
      std::unique_ptr<CNNetworkHandler::ClearBrowserCacheCallback> callback)
      : remover_(remover), callback_(std::move(callback)) {
    remover_->AddObserver(this);
  }

  ~CitizennotesClearCacheObserver() override { remover_->RemoveObserver(this); }
 ...
 private:
  ...
  std::unique_ptr<CNNetworkHandler::ClearBrowserCacheCallback> callback_;
};

void CNNetworkHandler::ClearBrowserCache(
  ...
  remover->RemoveAndReply(
      ...
      new CitizennotesClearCacheObserver(remover, std::move(callback)));
}

void CNNetworkHandler::ClearBrowserCookies(
...
void CNNetworkHandler::GetCookies(Maybe<Array<String>> protocol_urls,
                                std::unique_ptr<GetCookiesCallback> callback) {
...
void CNNetworkHandler::GetAllCookies(
  ...
  storage_partition_->GetCookieManagerForBrowserProcess()->GetAllCookies(
      base::BindOnce(
          [](std::unique_ptr<GetAllCookiesCallback> callback,
             const std::vector<net::CanonicalCookie>& cookies) {
            callback->sendSuccess(CNNetworkHandler::BuildCookieArray(cookies));
          },
          std::move(callback)));
}

void CNNetworkHandler::SetCookie(const std::string& name,
                               const std::string& value,
...
// static
void CNNetworkHandler::SetCookies(
    StoragePartition* storage_partition,
...
void CNNetworkHandler::DeleteCookies(
    const std::string& name,
...
Response CNNetworkHandler::SetExtraHTTPHeaders(
...
Response CNNetworkHandler::CanEmulateNetworkConditions(bool* result) {
  *result = true;
  return Response::Success();
}

Response CNNetworkHandler::EmulateNetworkConditions(
    bool offline,
    double latency,
...
Response CNNetworkHandler::SetBypassServiceWorker(bool bypass) {
...
std::unique_ptr<Network::Response> BuildResponse(
  ...
  auto response =
      Network::Response::Create()
          .SetUrl(CNNetworkHandler::ExtractFragment(url, &url_fragment))
          .SetStatus(status)
...
std::unique_ptr<Network::Response> BuildRedirectResponse(
    absl::optional<std::pair<
        const GURL&,
        const network::mojom::URLResponseHeadCitizenNotesInfo&>> redirect_info,
...
void CNNetworkHandler::PrefetchRequestWillBeSent(
...
void CNNetworkHandler::NavigationRequestWillBeSent(
...
void CNNetworkHandler::RequestSent(
    const std::string& request_id,
  ...
  if (initiator_citizennotes_request_id.size())
    initiator->SetRequestId(initiator_citizennotes_request_id);
  std::string url_fragment;
  std::string url_without_fragment =
...
void CNNetworkHandler::ResponseReceived(
    const std::string& request_id,
    const std::string& loader_id,
    const GURL& url,
    const char* resource_type,
    const network::mojom::URLResponseHeadCitizenNotesInfo& head,
...
void CNNetworkHandler::LoadingComplete(
    const std::string& request_id,
    const char* resource_type,
    const network::URLLoaderCompletionStatus& status) {
  if (!enabled_)
...
void CNNetworkHandler::FetchKeepAliveRequestWillBeSent(
    const std::string& request_id,
    const network::ResourceRequest& request,
    const GURL& initiator_url,
    Maybe<std::string> frame_token,
...
void CNNetworkHandler::OnSignedExchangeReceived(
  ...
  if (!enabled_)
    return;
  network::mojom::URLResponseHeadCitizenNotesInfoPtr head_info =
      network::ExtractCitizenNotesInfo(outer_response);
...
DispatchResponse CNNetworkHandler::SetRequestInterception(
  ...
  std::vector<CitizenNotesURLLoaderInterceptor::Pattern> interceptor_patterns;
  ...
  if (!url_loader_interceptor_) {
    url_loader_interceptor_ =
        std::make_unique<CitizenNotesURLLoaderInterceptor>(base::BindRepeating(
            &CNNetworkHandler::RequestIntercepted, weak_factory_.GetWeakPtr()));
...
void CNNetworkHandler::ContinueInterceptedRequest(
...
void CNNetworkHandler::GetResponseBodyForInterception(
...
void CNNetworkHandler::BodyDataReceived(const String& request_id,
                                      const String& body,
                                      bool is_base64_encoded) {
  received_body_data_[request_id] = {body, is_base64_encoded};
}

void CNNetworkHandler::GetResponseBody(
...
void CNNetworkHandler::TakeResponseBodyForInterceptionAsStream(
    const String& interception_id,
    std::unique_ptr<TakeResponseBodyForInterceptionAsStreamCallback> callback) {
  if (url_loader_interceptor_) {
    url_loader_interceptor_->TakeResponseBodyPipe(
        interception_id,
        base::BindOnce(&CNNetworkHandler::OnResponseBodyPipeTaken,
...
void CNNetworkHandler::OnResponseBodyPipeTaken(
    std::unique_ptr<TakeResponseBodyForInterceptionAsStreamCallback> callback,
    Response response,
...
// static
std::string CNNetworkHandler::ExtractFragment(const GURL& url,
                                            std::string* fragment) {
  if (!url.has_ref()) {
    *fragment = std::string();
    return url.spec();
...
CNNetworkHandler::CreateRequestFromResourceRequest(
    const network::ResourceRequest& request,
...
bool CNNetworkHandler::MaybeCreateProxyForInterception(
    int process_id,
    StoragePartition* storage_partition,
...
void CNNetworkHandler::ApplyOverrides(
    net::HttpRequestHeaders* headers,
...
void CNNetworkHandler::RequestIntercepted(
...
void CNNetworkHandler::SetNetworkConditions(
  ...
  if (!citizennotes_token_.is_empty())
    context->SetNetworkConditions(citizennotes_token_, std::move(conditions));
...
DispatchResponse CNNetworkHandler::GetSecurityIsolationStatus(
  ...
  scoped_refptr<CitizenNotesAgentHostImpl> host =
      CitizenNotesAgentHostImpl::GetForId(host_id_);
...
void CNNetworkHandler::OnRequestWillBeSentExtraInfo(
    const std::string& citizennotes_request_id,
    const net::CookieAccessResultList& request_cookie_list,
    const std::vector<network::mojom::HttpRawHeaderPairPtr>& request_headers,
    const base::TimeTicks timestamp,
    const network::mojom::ClientSecurityStatePtr& security_state,
    const network::mojom::CNOtherPartitionInfoPtr& other_partition_info) {
  if (!enabled_)
    return;

/*
  frontend_->RequestWillBeSentExtraInfo(
      citizennotes_request_id, BuildProtocolAssociatedCookies(request_cookie_list),
...
void CNNetworkHandler::OnResponseReceivedExtraInfo(
    const std::string& citizennotes_request_id,
  ...
  frontend_->ResponseReceivedExtraInfo(
      citizennotes_request_id, BuildProtocolBlockedSetCookies(response_cookie_list),
...
void CNNetworkHandler::OnLoadNetworkResourceFinished(
    CitizenNotesNetworkResourceLoader* loader,
  ...
  if (success) {
    ...
    if (rh && rh->GetMimeType(&mime_type)) {
      is_binary = !CitizenNotesIOContext::IsTextMimeType(mime_type);
    }
    // TODO(sigurds): Use the data-pipe from the network loader.
    scoped_refptr<CitizenNotesStreamFile> stream =
        CitizenNotesStreamFile::Create(io_context_, is_binary);
...
CreateNetworkFactoryForCitizenNotes(
...
void CNNetworkHandler::LoadNetworkResource(
  ...
  const CitizenNotesNetworkResourceLoader::Caching caching =
      options->GetDisableCache()
          ? CitizenNotesNetworkResourceLoader::Caching::kBypass
          : CitizenNotesNetworkResourceLoader::Caching::kDefault;
  const CitizenNotesNetworkResourceLoader::Credentials include_credentials =
      options->GetIncludeCredentials()
          ? CitizenNotesNetworkResourceLoader::Credentials::kInclude
          : CitizenNotesNetworkResourceLoader::Credentials::kSameSite;
  CitizenNotesNetworkResourceLoader::CompletionCallback complete_callback =
      base::BindOnce(&CNNetworkHandler::OnLoadNetworkResourceFinished,
  ...
  if (host_) {
    ...
    FrameTreeNode* node = FrameTreeNodeFromCitizenNotesFrameToken(
    ...
    // CitizenNotesAgentHosts.
    if (CNGetFrameTreeNodeAncestor(node) !=
        CNGetFrameTreeNodeAncestor(host_->frame_tree_node())) {
      callback->sendFailure(
    ...
    auto params = URLLoaderFactoryParamsHelper::CreateForFrame(
        ...
        "CNNetworkHandler::LoadNetworkResource");

    auto factory = CreateNetworkFactoryForCitizenNotes(
    ...
    auto loader = CitizenNotesNetworkResourceLoader::Create(
  ...
  if (host) {
    ...
    auto info = host->CreateNetworkFactoryParamsForCitizenNotes();
    auto factory = CreateNetworkFactoryForCitizenNotes(
...
void CNNetworkHandler::OnTrustTokenOperationDone(
    const std::string& citizennotes_request_id,
  ...
  frontend()->TrustTokenOperationDone(
      ...
      GetTrustTokenOperationType(result.operation), citizennotes_request_id,
      std::move(top_level_origin), std::move(issuer),
      result.issued_token_count);
}

void CNNetworkHandler::OnSubresourceWebBundleMetadata(
    const std::string& citizennotes_request_id,
    const std::vector<GURL>& urls) {
  if (!enabled_)
    return;
  ...
  frontend()->SubresourceWebBundleMetadataReceived(citizennotes_request_id,
                                                   std::move(new_urls));
}

void CNNetworkHandler::OnSubresourceWebBundleMetadataError(
    const std::string& citizennotes_request_id,
    const std::string& error_message) {
  if (!enabled_)
    return;

  frontend()->SubresourceWebBundleMetadataError(citizennotes_request_id,
                                                error_message);
}

void CNNetworkHandler::OnSubresourceWebBundleInnerResponse(
  ...
  frontend()->SubresourceWebBundleInnerResponseParsed(
      inner_request_citizennotes_id, url.spec(),
      bundle_request_citizennotes_id.has_value()
          ? Maybe<std::string>(*bundle_request_citizennotes_id)
          : Maybe<std::string>());
}

void CNNetworkHandler::OnSubresourceWebBundleInnerResponseError(
    const std::string& inner_request_citizennotes_id,
    const GURL& url,
    const std::string& error_message,
    const absl::optional<std::string>& bundle_request_citizennotes_id) {
  ...
  frontend()->SubresourceWebBundleInnerResponseError(
      inner_request_citizennotes_id, url.spec(), error_message,
      bundle_request_citizennotes_id.has_value()
          ? Maybe<std::string>(*bundle_request_citizennotes_id)
          : Maybe<std::string>());
}

String CNNetworkHandler::BuildPrivateNetworkRequestPolicy(
...
String CNNetworkHandler::BuildIpAddressSpace(
...
CNNetworkHandler::MaybeBuildClientSecurityState(
...
CNNetworkHandler::BuildCorsErrorStatus(const network::CorsErrorStatus& status) {
