...
#ifndef CONTENT_BROWSER_CITIZENNOTES_PROTOCOL_NETWORK_HANDLER_H_
...
class CitizenNotesNetworkResourceLoader;

class CNNetworkHandler : public CitizenNotesDomainHandler,
#if BUILDFLAG(ENABLE_REPORTING)
                       public network::mojom::ReportingApiObserver,
#endif  // BUILDFLAG(ENABLE_REPORTING)
                       public Network::Backend {
 public:
  CNNetworkHandler(const std::string& host_id,
  ...
  CNNetworkHandler(const CNNetworkHandler&) = delete;
  CNNetworkHandler& operator=(const CNNetworkHandler&) = delete;

  ~CNNetworkHandler() override;

  static std::vector<CNNetworkHandler*> ForAgentHost(CitizenNotesAgentHostImpl* host);
  ...
  // associated with the CNNetworkHandler itself (which is the token of the local
  // root frame).
 ...
 private:
  void OnLoadNetworkResourceFinished(CitizenNotesNetworkResourceLoader* loader,
                                     const net::HttpResponseHeaders* rh,
                                     bool success,
                                     int net_error,
                                     std::string content);
  void RequestIntercepted(std::unique_ptr<CNInterceptedRequestInfo> request_info);
  ...
  std::unique_ptr<CitizenNotesURLLoaderInterceptor> url_loader_interceptor_;
  ...
  std::map<std::unique_ptr<CitizenNotesNetworkResourceLoader>,
...
#endif  // CONTENT_BROWSER_CITIZENNOTES_PROTOCOL_NETWORK_HANDLER_H_
