...
CNPageHandler::CNPageHandler(
    CNEmulationHandler* emulation_handler,
    CNBrowserHandler* browser_handler,
    bool allow_unsafe_operations,
    bool is_trusted,
    absl::optional<url::Origin> navigation_initiator_origin,
    bool may_read_local_files)
    : CitizenNotesDomainHandler(Page::Metainfo::domainName),
...
#endif
  video_consumer_ = std::make_unique<CitizenNotesVideoConsumer>(base::BindRepeating(
      &CNPageHandler::OnFrameFromVideoConsumer, weak_factory_.GetWeakPtr()));
  video_consumer_->SetFormat(kScreencastPixelFormat);
  DCHECK(emulation_handler_);
}

CNPageHandler::~CNPageHandler() = default;

// static
std::vector<CNPageHandler*> CNPageHandler::EnabledForWebContents(
    WebContentsImpl* contents) {
  if (!CitizenNotesAgentHost::HasFor(contents))
    return std::vector<CNPageHandler*>();
  std::vector<CNPageHandler*> result;
  for (auto* handler :
       CNPageHandler::ForAgentHost(static_cast<CitizenNotesAgentHostImpl*>(
           CitizenNotesAgentHost::GetOrCreateFor(contents).get()))) {
    if (handler->enabled_)
...
// static
std::vector<CNPageHandler*> CNPageHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNPageHandler>(Page::Metainfo::domainName);
}

void CNPageHandler::SetRenderer(int process_host_id,
...
void CNPageHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<Page::Frontend>(dispatcher->channel());
  Page::Dispatcher::wire(dispatcher, this);
}

void CNPageHandler::RenderWidgetHostVisibilityChanged(
...
void CNPageHandler::RenderWidgetHostDestroyed(RenderWidgetHost* widget_host) {
  DCHECK(observation_.IsObservingSource(widget_host));
  observation_.Reset();
}

void CNPageHandler::DidAttachInterstitialPage() {
  if (!enabled_)
    return;
  frontend_->InterstitialShown();
}

void CNPageHandler::DidDetachInterstitialPage() {
  if (!enabled_)
    return;
  frontend_->InterstitialHidden();
}

void CNPageHandler::DidRunJavaScriptDialog(const GURL& url,
...
Response CNPageHandler::Enable() {
  enabled_ = true;
  return Response::FallThrough();
}

Response CNPageHandler::Disable() {
  enabled_ = false;
  bypass_csp_ = false;
...
void CNPageHandler::Reload(Maybe<bool> bypassCache,
...
void CNPageHandler::Navigate(const std::string& url,
  ...
  base::WeakPtr<CNPageHandler> weak_self = weak_factory_.GetWeakPtr();
...
void CNPageHandler::DownloadWillBegin(FrameTreeNode* ftn,
...
void CNPageHandler::OnFrameDetached(const base::UnguessableToken& frame_id) {
  if (!enabled_)
    return;
  frontend_->FrameDetached(frame_id.ToString(), "remove");
}

void CNPageHandler::DidChangeFrameLoadingState(const FrameTreeNode& ftn) {
  if (!enabled_) {
    return;
  }
...
void CNPageHandler::OnDownloadDestroyed(download::DownloadItem* item) {
  pending_downloads_.erase(item);
}

void CNPageHandler::OnDownloadUpdated(download::DownloadItem* item) {
  if (!enabled_)
    return;
  std::string state;
...
Response CNPageHandler::GetNavigationHistory(
    int* current_index,
...
Response CNPageHandler::NavigateToHistoryEntry(int entry_id) {
  Response response = AssureTopLevelActiveFrame();
  if (response.IsError())
    return response;
...
Response CNPageHandler::ResetNavigationHistory() {
  Response response = AssureTopLevelActiveFrame();
  if (response.IsError())
    return response;
...
void CNPageHandler::CaptureSnapshot(
  ...
  DCHECK(host_);
  CitizenNotesMHTMLHelper::Capture(
...
void CNPageHandler::CaptureFullPageScreenshot(
    Maybe<std::string> format,
    Maybe<int> quality,
...
void CNPageHandler::CaptureScreenshot(
    Maybe<std::string> format,
    Maybe<int> quality,
...
/*
  ...
  if (from_surface.value_or(true) && capture_beyond_viewport.value_or(false) &&
    ...
    main_frame->GetFullPageSize(base::BindOnce(
        &CNPageHandler::CaptureFullPageScreenshot, weak_factory_.GetWeakPtr(),
  ...
  if (!from_surface.value_or(true)) {
    ...
    widget_host->GetSnapshotFromBrowser(
        base::BindOnce(&CNPageHandler::ScreenshotCaptured,
...
Response CNPageHandler::StartScreencast(Maybe<std::string> format,
                                      Maybe<int> quality,
                                      Maybe<int> max_width,
...
Response CNPageHandler::StopScreencast() {
...
Response CNPageHandler::ScreencastFrameAck(int session_id) {
  if (session_id == session_id_)
    --frames_in_flight_;
  return Response::Success();
}

Response CNPageHandler::HandleJavaScriptDialog(bool accept,
...
Response CNPageHandler::BringToFront() {
  ResponseOrWebContents result = GetWebContentsForTopLevelActiveFrame();
  if (absl::holds_alternative<Response>(result))
    return absl::get<Response>(result);
...
Response CNPageHandler::SetDownloadBehavior(const std::string& behavior,
...
void CNPageHandler::GetAppManifest(
    std::unique_ptr<GetAppManifestCallback> callback) {
  if (!CanExecuteGlobalCommands(this, callback))
    return;
  ManifestManagerHost::GetOrCreateForPage(host_->GetPage())
      ->RequestManifestDebugInfo(base::BindOnce(&CNPageHandler::GotManifest,
                                                weak_factory_.GetWeakPtr(),
                                                std::move(callback)));
}

CNPageHandler::ResponseOrWebContents
CNPageHandler::GetWebContentsForTopLevelActiveFrame() {
  Response response = AssureTopLevelActiveFrame();
  if (response.IsError())
    return response;
...
void CNPageHandler::NotifyScreencastVisibility(bool visible) {
  frontend_->ScreencastVisibilityChanged(visible);
}

bool CNPageHandler::ShouldCaptureNextScreencastFrame() {
  return frames_in_flight_ <= kMaxScreencastFramesInFlight &&
         !(++frame_counter_ % capture_every_nth_frame_);
}

void CNPageHandler::OnFrameFromVideoConsumer(
  ...
  ScreencastFrameCaptured(std::move(page_metadata),
                          CitizenNotesVideoConsumer::GetSkBitmapFromFrame(frame));
}

void CNPageHandler::ScreencastFrameCaptured(
    std::unique_ptr<Page::ScreencastFrameMetadata> page_metadata,
    const SkBitmap& bitmap) {
  ...
  base::ThreadPool::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&CNPageHandler::ScreencastFrameEncoded,
                     weak_factory_.GetWeakPtr(), std::move(page_metadata)));
}

void CNPageHandler::ScreencastFrameEncoded(
    std::unique_ptr<Page::ScreencastFrameMetadata> page_metadata,
...
void CNPageHandler::ScreenshotCaptured(
    std::unique_ptr<CaptureScreenshotCallback> callback,
...
void CNPageHandler::GotManifest(std::unique_ptr<GetAppManifestCallback> callback,
...
Response CNPageHandler::StopLoading() {
  ResponseOrWebContents result = GetWebContentsForTopLevelActiveFrame();
...
Response CNPageHandler::SetWebLifecycleState(const std::string& state) {
...
void CNPageHandler::GetInstallabilityErrors(
...
void CNPageHandler::GetManifestIcons(
...
void CNPageHandler::GetAppId(std::unique_ptr<GetAppIdCallback> callback) {
  // TODO: Use InstallableManager once it moves into content/.
  // Until then, this code is only used to return no image data in the tests.
...
Response CNPageHandler::SetBypassCSP(bool enabled) {
  bypass_csp_ = enabled;
  return Response::FallThrough();
}

Page::BackForwardCacheNotRestoredReason CNNotRestoredReasonToProtocol(
...
Page::BackForwardCacheNotRestoredReason CNBlocklistedFeatureToProtocol(
...
CNBlockingDetailsToProtocol(const blink::mojom::BlockingDetailsPtr& details) {
...
CNDisableForRenderFrameHostReasonToProtocol(
...
Page::BackForwardCacheNotRestoredReasonType CNMapNotRestoredReasonToType(
    BackForwardCacheMetrics::NotRestoredReason reason) {
  using Reason = BackForwardCacheMetrics::NotRestoredReason;
  switch (reason) {
    case Reason::kNotPrimaryMainFrame:
...
Page::BackForwardCacheNotRestoredReasonType CNMapBlocklistedFeatureToType(
    WebSchedulerTrackedFeature feature) {
  switch (feature) {
    case WebSchedulerTrackedFeature::kWebRTC:
    case WebSchedulerTrackedFeature::kOutstandingNetworkRequestOthers:
...
CMMapDisableForRenderFrameHostReasonToType(
    BackForwardCache::DisabledReason reason) {
  return Page::BackForwardCacheNotRestoredReasonTypeEnum::SupportPending;
}
...
CNCreateNotRestoredExplanation(
    ...
    if (not_restored_reason ==
      ...
      for (blink::scheduler::WebSchedulerTrackedFeature feature :
        ...
        for (const auto& detail : details.at(feature)) {
          if (detail->line_number != 0 && detail->column_number != 0) {
            details_list->push_back(CNBlockingDetailsToProtocol(detail));
          } else {
        ...
        auto explanation =
            Page::BackForwardCacheNotRestoredExplanation::Create()
                .SetType(CNMapBlocklistedFeatureToType(feature))
                .SetReason(CNBlocklistedFeatureToProtocol(feature))
                .Build();
      ...
      for (const auto& [disabled_reason, _] : disabled_reasons) {
        auto reason =
            Page::BackForwardCacheNotRestoredExplanation::Create()
                .SetType(
                    CMMapDisableForRenderFrameHostReasonToType(disabled_reason))
                .SetReason(
                    CNDisableForRenderFrameHostReasonToProtocol(disabled_reason))
                .Build();
      ...
      reasons->emplace_back(
          Page::BackForwardCacheNotRestoredExplanation::Create()
              .SetType(CNMapNotRestoredReasonToType(not_restored_reason))
              .SetReason(CNNotRestoredReasonToProtocol(not_restored_reason))
              .Build());
    }
  }
  return reasons;
...
CNCNCreateNotRestoredExplanationTree(
  ...
  for (auto& child : tree_result.GetChildren()) {
    children_array->emplace_back(
        CNCNCreateNotRestoredExplanationTree(*(child.get())));
  }
...
Response CNPageHandler::AddCompilationCache(const std::string& url,
                                          const Binary& data) {
...
void CNPageHandler::IsPrerenderingAllowed(bool& is_allowed) {
  is_allowed &= is_prerendering_allowed_;
}

Response CNPageHandler::SetPrerenderingAllowed(bool is_allowed) {
  Response response = AssureTopLevelActiveFrame();
  if (response.IsError()) {
    return response;
  }
...
Response CNPageHandler::AssureTopLevelActiveFrame() {
  if (!host_)
    return Response::ServerError(kErrorNotAttached);
...
void CNPageHandler::BackForwardCacheNotUsed(
...
bool CNPageHandler::ShouldBypassCSP() {
