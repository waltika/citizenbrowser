...
CNPreloadHandler::CNPreloadHandler()
    : CitizenNotesDomainHandler(Preload::Metainfo::domainName) {}

CNPreloadHandler::~CNPreloadHandler() = default;

// static
std::vector<CNPreloadHandler*> CNPreloadHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNPreloadHandler>(Preload::Metainfo::domainName);
}
...
Response CNPreloadHandler::Enable() {
  enabled_ = true;
  SendInitialPreloadEnabledState();
  return Response::FallThrough();
}

Response CNPreloadHandler::Disable() {
  enabled_ = false;
  return Response::FallThrough();
}

void CNPreloadHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<Preload::Frontend>(dispatcher->channel());
  Preload::Dispatcher::wire(dispatcher, this);
}

void CNPreloadHandler::SetRenderer(int process_host_id,
                                 RenderFrameHostImpl* frame_host) {
  host_ = frame_host;
}

void CNPreloadHandler::SendInitialPreloadEnabledState() {
  if (!host_) {
    return;
  }
