...
// static
std::vector<CNSecurityHandler*> CNSecurityHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNSecurityHandler>(Security::Metainfo::domainName);
}

CNSecurityHandler::CNSecurityHandler()
    : CitizenNotesDomainHandler(Security::Metainfo::domainName),
      enabled_(false),
      host_(nullptr) {
}

CNSecurityHandler::~CNSecurityHandler() = default;

void CNSecurityHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<Security::Frontend>(dispatcher->channel());
  Security::Dispatcher::wire(dispatcher, this);
}

void CNSecurityHandler::AttachToRenderFrameHost() {
  DCHECK(host_);
...
void CNSecurityHandler::SetRenderer(int process_host_id,
...
void CNSecurityHandler::DidFinishNavigation(NavigationHandle* navigation_handle) {
...
void CNSecurityHandler::FlushPendingCertificateErrorNotifications() {
...
bool CNSecurityHandler::NotifyCertificateError(int cert_error,
...
Response CNSecurityHandler::Enable() {
  if (host_) {
...
Response CNSecurityHandler::Disable() {
  enabled_ = false;
...
Response CNSecurityHandler::HandleCertificateError(int event_id,
                                                 const String& action) {
...
Response CNSecurityHandler::SetOverrideCertificateErrors(bool override) {
  if (override) {
    if (!enabled_)
...
Response CNSecurityHandler::SetIgnoreCertificateErrors(bool ignore) {
  if (ignore) {
    if (cert_error_override_mode_ == CertErrorOverrideMode::kHandleEvents)
      return Response::ServerError(
...
Response CNSecurityHandler::AssureTopLevelActiveFrame() {
  DCHECK(host_);
