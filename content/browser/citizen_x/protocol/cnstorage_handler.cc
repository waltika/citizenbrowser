...
#include "content/browser/citizen_x/protocol/storage.h"
...
// informs the CNStorageHandler on the UI thread for origins of interest.
// Created and used exclusively on the UI thread.
class CNStorageHandler::CacheStorageObserver
    : storage::mojom::CacheStorageObserver {
 public:
  CacheStorageObserver(
      base::WeakPtr<CNStorageHandler> owner_storage_handler,
 ...
 private:
  ...
  base::WeakPtr<CNStorageHandler> owner_;
...
// informs the CNStorageHandler on the UI thread for storage_keys of interest.
// Created on the UI thread but predominantly used and deleted on the IDB
// thread.
class CNStorageHandler::IndexedDBObserver
...
// informs the CNStorageHandler on the UI thread for origins of interest.
// Created and used exclusively on the UI thread.
class CNStorageHandler::SharedStorageObserver
    : content::SharedStorageWorkletHostManager::SharedStorageObserverInterface {
 public:
  explicit SharedStorageObserver(CNStorageHandler* owner_storage_handler)
 ...
 private:
  raw_ptr<CNStorageHandler> const owner_;
...
class CNStorageHandler::QuotaManagerObserver
    : storage::mojom::QuotaManagerObserver {
 public:
  QuotaManagerObserver(base::WeakPtr<CNStorageHandler> owner_storage_handler,
...
CNStorageHandler::CNStorageHandler(bool client_is_trusted)
    : CitizenNotesDomainHandler(Storage::Metainfo::domainName),
      client_is_trusted_(client_is_trusted) {}

CNStorageHandler::~CNStorageHandler() {
  DCHECK(!cache_storage_observer_);
  DCHECK(!indexed_db_observer_);
}

// static
std::vector<CNStorageHandler*> CNStorageHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNStorageHandler>(Storage::Metainfo::domainName);
}

void CNStorageHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<Storage::Frontend>(dispatcher->channel());
  Storage::Dispatcher::wire(dispatcher, this);
}

void CNStorageHandler::SetRenderer(int process_host_id,
...
Response CNStorageHandler::Disable() {
...
void CNStorageHandler::GetCookies(Maybe<std::string> browser_context_id,
  ...
  Response response = CNStorageHandler::FindStoragePartition(browser_context_id,
                                                           &storage_partition);
...
void CNStorageHandler::SetCookies(
...
void CNStorageHandler::ClearCookies(
    Maybe<std::string> browser_context_id,
...
Response CNStorageHandler::GetStorageKeyForFrame(
    const std::string& frame_id,
    std::string* serialized_storage_key) {
  if (!frame_host_) {
    return Response::InvalidParams("Frame host not found");
  }
  FrameTreeNode* node = protocol::FrameTreeNodeFromCitizenNotesFrameToken(
...
void CNStorageHandler::ClearDataForOrigin(
...
void CNStorageHandler::ClearDataForStorageKey(
...
void CNStorageHandler::GetUsageAndQuota(
...
void CNStorageHandler::OverrideQuotaForOrigin(
    const String& origin_string,
...
Response CNStorageHandler::TrackCacheStorageForOrigin(
...
Response CNStorageHandler::TrackCacheStorageForStorageKey(
    const std::string& storage_key) {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
Response CNStorageHandler::UntrackCacheStorageForOrigin(
    const std::string& origin_string) {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
Response CNStorageHandler::UntrackCacheStorageForStorageKey(
    const std::string& storage_key) {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
Response CNStorageHandler::TrackIndexedDBForOrigin(
    const std::string& origin_string) {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
Response CNStorageHandler::TrackIndexedDBForStorageKey(
    const std::string& storage_key) {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
Response CNStorageHandler::UntrackIndexedDBForOrigin(
    const std::string& origin_string) {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
Response CNStorageHandler::UntrackIndexedDBForStorageKey(
    const std::string& storage_key) {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
CNStorageHandler::CacheStorageObserver*
CNStorageHandler::GetCacheStorageObserver() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
CNStorageHandler::IndexedDBObserver* CNStorageHandler::GetIndexedDBObserver() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
CNStorageHandler::GetSharedStorageWorkletHostManager() {
...
CNStorageHandler::GetSharedStorageManager() {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
storage::QuotaManagerProxy* CNStorageHandler::GetQuotaManagerProxy() {
  DCHECK(storage_partition_);
...
void CNStorageHandler::NotifyCacheStorageListChanged(
    const storage::BucketLocator& bucket_locator) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void CNStorageHandler::NotifyCacheStorageContentChanged(
    const storage::BucketLocator& bucket_locator,
...
void CNStorageHandler::NotifyIndexedDBListChanged(
...
void CNStorageHandler::NotifyIndexedDBContentChanged(
    storage::BucketLocator bucket_locator,
    const std::u16string& database_name,
...
Response CNStorageHandler::FindStoragePartition(
    const Maybe<std::string>& browser_context_id,
    StoragePartition** storage_partition) {
  BrowserContext* browser_context = nullptr;
  Response response =
      CNBrowserHandler::FindBrowserContext(browser_context_id, &browser_context);
  if (!response.IsSuccess()) {
    return response;
  }
...
void SendTrustTokens(
    std::unique_ptr<CNStorageHandler::GetTrustTokensCallback> callback,
...
void CNStorageHandler::GetTrustTokens(
...
void SendClearTrustTokensStatus(
    std::unique_ptr<CNStorageHandler::ClearTrustTokensCallback> callback,
...
void CNStorageHandler::ClearTrustTokens(
...
void CNStorageHandler::OnInterestGroupAccessed(
...
void SendGetInterestGroup(
    std::unique_ptr<CNStorageHandler::GetInterestGroupDetailsCallback> callback,
...
void CNStorageHandler::GetInterestGroupDetails(
...
Response CNStorageHandler::SetInterestGroupTracking(bool enable) {
  if (!storage_partition_) {
    return Response::InternalError();
  }
...
Response CNStorageHandler::SetInterestGroupAuctionTracking(bool enable) {
...
void SendSharedStorageMetadata(
    std::unique_ptr<CNStorageHandler::GetSharedStorageMetadataCallback> callback,
...
void CNStorageHandler::GetSharedStorageMetadata(
    const std::string& owner_origin_string,
...
void RetrieveSharedStorageEntries(
    std::unique_ptr<CNStorageHandler::GetSharedStorageEntriesCallback> callback,
...
void CNStorageHandler::GetSharedStorageEntries(
    const std::string& owner_origin_string,
...
void CNStorageHandler::SetSharedStorageEntry(
    const std::string& owner_origin_string,
...
void CNStorageHandler::DeleteSharedStorageEntry(
    const std::string& owner_origin_string,
    const std::string& key,
...
void CNStorageHandler::ClearSharedStorageEntries(
    const std::string& owner_origin_string,
...
Response CNStorageHandler::SetSharedStorageTracking(bool enable) {
  if (enable) {
...
void CNStorageHandler::ResetSharedStorageBudget(
    const std::string& owner_origin_string,
...
void CNStorageHandler::NotifySharedStorageAccessed(
    const base::Time& access_time,
...
DispatchResponse CNStorageHandler::SetStorageBucketTracking(
  ...
  if (enable) {
    ...
    if (!quota_manager_observer_) {
      quota_manager_observer_ =
          std::make_unique<CNStorageHandler::QuotaManagerObserver>(
...
DispatchResponse CNStorageHandler::DeleteStorageBucket(
...
void CNStorageHandler::NotifyCreateOrUpdateBucket(
    const storage::BucketInfo& bucket_info) {
  frontend_->StorageBucketCreatedOrUpdated(BuildBucketInfo(bucket_info));
}

void CNStorageHandler::NotifyDeleteBucket(
    const storage::BucketLocator& bucket_locator) {
  frontend_->StorageBucketDeleted(
      base::NumberToString(bucket_locator.id.value()));
}

AttributionManager* CNStorageHandler::GetAttributionManager() {
  if (!storage_partition_) {
    return nullptr;
  }
  return static_cast<StoragePartitionImpl*>(storage_partition_)
...
void CNStorageHandler::SetAttributionReportingLocalTestingMode(
    bool enabled,
...
void CNStorageHandler::ResetAttributionReporting() {
...
void CNStorageHandler::OnSourceHandled(
...
void CNStorageHandler::OnTriggerHandled(const AttributionTrigger& trigger,
                                      std::optional<uint64_t> cleared_debug_key,
...
Response CNStorageHandler::SetAttributionReportingTracking(bool enable) {
  if (enable) {
    auto* manager = GetAttributionManager();
    if (!manager) {
...
void CNStorageHandler::NotifyInterestGroupAuctionEventOccurred(
...
void CNStorageHandler::NotifyInterestGroupAuctionNetworkRequestCreated(
  ...
  frontend_->InterestGroupAuctionNetworkRequestCreated(
      ...
      std::make_unique<std::vector<std::string>>(citizennotes_auction_ids));
}
