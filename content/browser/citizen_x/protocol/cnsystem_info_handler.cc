...
class AuxGPUInfoEnumerator : public gpu::GPUInfo::Enumerator {
 ...
 private:
  ...
  void AddBinary(const char* name,
    ...
    // TODO(penghuang): send vulkan info to CitizenNotes.
  }
...
class CNSystemInfoHandlerGpuObserver : public content::GpuDataManagerObserver {
 public:
  explicit CNSystemInfoHandlerGpuObserver(
      std::unique_ptr<GetInfoCallback> callback)
      : callback_(std::move(callback)) {
    GetUIThreadTaskRunner({})->PostDelayedTask(
        FROM_HERE,
        base::BindOnce(&CNSystemInfoHandlerGpuObserver::ObserverWatchdogCallback,
 ...
 private:
  ...
  base::WeakPtrFactory<CNSystemInfoHandlerGpuObserver> weak_factory_{this};
};

CNSystemInfoHandler::CNSystemInfoHandler(bool is_browser_session)
    : CitizenNotesDomainHandler(SystemInfo::Metainfo::domainName),
      is_browser_session_(is_browser_session) {}

CNSystemInfoHandler::~CNSystemInfoHandler() = default;

void CNSystemInfoHandler::Wire(UberDispatcher* dispatcher) {
  SystemInfo::Dispatcher::wire(dispatcher, this);
}

void CNSystemInfoHandler::GetInfo(std::unique_ptr<GetInfoCallback> callback) {
  ...
  new CNSystemInfoHandlerGpuObserver(std::move(callback));
}
...
void CNSystemInfoHandler::GetProcessInfo(
...
Response CNSystemInfoHandler::GetFeatureState(const String& in_featureState,
                                            bool* featureEnabled) {
