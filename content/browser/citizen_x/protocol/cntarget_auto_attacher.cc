...
CNTargetAutoAttacher::CNTargetAutoAttacher() = default;

CNTargetAutoAttacher::~CNTargetAutoAttacher() {
  for (auto& client : clients_)
    client.AutoAttacherDestroyed(this);
}

bool CNTargetAutoAttacher::auto_attach() const {
  return !clients_.empty();
}

bool CNTargetAutoAttacher::wait_for_debugger_on_start() const {
  return !clients_requesting_wait_for_debugger_.empty();
}

scoped_refptr<RenderFrameCitizenNotesAgentHost>
CNTargetAutoAttacher::HandleNavigation(NavigationRequest* navigation_request,
  ...
  // (e.g. download). Skip possibly detaching the old agent host so the CitizenNotes
  // message logged via the old RFH can be seen.
  if (!new_host)
    return nullptr;

  scoped_refptr<RenderFrameCitizenNotesAgentHost> agent_host =
      RenderFrameCitizenNotesAgentHost::FindForDangling(frame_tree_node);
  ...
  if (needs_host_attached) {
    if (!agent_host) {
      agent_host = RenderFrameCitizenNotesAgentHost::
...
void CNTargetAutoAttacher::UpdateAutoAttach(base::OnceClosure callback) {
  std::move(callback).Run();
}

void CNTargetAutoAttacher::AddClient(Client* client,
                                   bool wait_for_debugger_on_start,
...
void CNTargetAutoAttacher::UpdateWaitForDebuggerOnStart(
    Client* client,
    bool wait_for_debugger_on_start,
    base::OnceClosure callback) {
...
void CNTargetAutoAttacher::RemoveClient(Client* client) {
...
void CNTargetAutoAttacher::AppendNavigationThrottles(
...
void CNTargetAutoAttacher::DispatchAutoAttach(CitizenNotesAgentHost* host,
...
void CNTargetAutoAttacher::DispatchAutoDetach(CitizenNotesAgentHost* host) {
  for (auto& client : clients_)
    client.AutoDetach(this, host);
}

void CNTargetAutoAttacher::DispatchSetAttachedTargetsOfType(
...
void CNTargetAutoAttacher::DispatchTargetInfoChanged(CitizenNotesAgentHost* host) {
  for (auto& client : clients_)
    client.TargetInfoChanged(host);
}

CNRendererAutoAttacherBase::CNRendererAutoAttacherBase(
    CitizenNotesRendererChannel* renderer_channel)
    : renderer_channel_(renderer_channel) {}

CNRendererAutoAttacherBase::~CNRendererAutoAttacherBase() = default;

void CNRendererAutoAttacherBase::UpdateAutoAttach(base::OnceClosure callback) {
  CitizenNotesRendererChannel::ChildTargetCreatedCallback report_worker_callback;
  if (auto_attach()) {
    report_worker_callback = base::BindRepeating(
        &CNRendererAutoAttacherBase::ChildWorkerCreated, base::Unretained(this));
  }
...
void CNRendererAutoAttacherBase::ChildWorkerCreated(
