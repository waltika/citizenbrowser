...
using CreateServerSocketCallback = CNTetheringHandler::CreateServerSocketCallback;
...
// CNTetheringHandler::TetheringImpl -------------------------------------------

class CNTetheringHandler::TetheringImpl {
 public:
  TetheringImpl(base::WeakPtr<CNTetheringHandler> handler,
 ...
 private:
  base::WeakPtr<CNTetheringHandler> handler_;
  CreateServerSocketCallback socket_callback_;
  std::map<uint16_t, std::unique_ptr<BoundSocket>> bound_sockets_;
};

CNTetheringHandler::TetheringImpl::TetheringImpl(
    base::WeakPtr<CNTetheringHandler> handler,
    CreateServerSocketCallback socket_callback)
    : handler_(handler), socket_callback_(std::move(socket_callback)) {}

CNTetheringHandler::TetheringImpl::~TetheringImpl() = default;

void CNTetheringHandler::TetheringImpl::Bind(
  ...
  BoundSocket::AcceptedCallback accepted_callback = base::BindRepeating(
      &CNTetheringHandler::TetheringImpl::Accepted, base::Unretained(this));
...
void CNTetheringHandler::TetheringImpl::Unbind(
...
void CNTetheringHandler::TetheringImpl::Accepted(uint16_t port,
                                               const std::string& name) {
  GetUIThreadTaskRunner({})->PostTask(
      FROM_HERE,
      base::BindOnce(&CNTetheringHandler::Accepted, handler_, port, name));
}
...
// static
CNTetheringHandler::TetheringImpl* CNTetheringHandler::impl_ = nullptr;

CNTetheringHandler::CNTetheringHandler(
    CreateServerSocketCallback socket_callback,
    scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : CitizenNotesDomainHandler(Tethering::Metainfo::domainName),
...
void CNTetheringHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<Tethering::Frontend>(dispatcher->channel());
  Tethering::Dispatcher::wire(dispatcher, this);
}

void CNTetheringHandler::Accepted(uint16_t port, const std::string& name) {
  frontend_->Accepted(port, name);
}

bool CNTetheringHandler::Activate() {
  if (is_active_)
    return true;
  if (impl_)
    return false;
...
void CNTetheringHandler::Unbind(
