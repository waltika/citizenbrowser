...
#include "content/browser/citizen_x/citizennotes_video_consumer.h"
...
class CitizenNotesTraceEndpointProxy : public TracingController::TraceDataEndpoint {
 public:
  explicit CitizenNotesTraceEndpointProxy(base::WeakPtr<CNTracingHandler> handler)
      : tracing_handler_(handler) {}

  void ReceiveTraceChunk(std::unique_ptr<std::string> chunk) override {
    if (CNTracingHandler* h = tracing_handler_.get())
 ...
 private:
  ~CitizenNotesTraceEndpointProxy() override = default;

  base::WeakPtr<CNTracingHandler> tracing_handler_;
};

class CitizenNotesStreamEndpoint : public TracingController::TraceDataEndpoint {
 public:
  explicit CitizenNotesStreamEndpoint(
      base::WeakPtr<CNTracingHandler> handler,
      const scoped_refptr<CitizenNotesStreamFile>& stream)
  ...
  void ReceiveTraceChunk(std::unique_ptr<std::string> chunk) override {
    if (!BrowserThread::CurrentlyOn(BrowserThread::UI)) {
      GetUIThreadTaskRunner({})->PostTask(
          FROM_HERE, base::BindOnce(&CitizenNotesStreamEndpoint::ReceiveTraceChunk,
                                    this, std::move(chunk)));
      return;
    }
  ...
  void ReceivedTraceFinalContents() override {
    if (!BrowserThread::CurrentlyOn(BrowserThread::UI)) {
      GetUIThreadTaskRunner({})->PostTask(
          FROM_HERE,
          base::BindOnce(&CitizenNotesStreamEndpoint::ReceivedTraceFinalContents,
                         this));
      return;
    }
    if (CNTracingHandler* h = tracing_handler_.get())
 ...
 private:
  ~CitizenNotesStreamEndpoint() override = default;

  scoped_refptr<CitizenNotesStreamFile> stream_;
  base::WeakPtr<CNTracingHandler> tracing_handler_;
};
...
class CNTracingHandler::PerfettoTracingSession {
 public:
...
CNTracingHandler::CNTracingHandler(CitizenNotesAgentHostImpl* host,
                               CitizenNotesIOContext* io_context,
                               CitizenNotesSession* root_session)
    : CitizenNotesDomainHandler(Tracing::Metainfo::domainName),
  ...
  video_consumer_ = std::make_unique<CitizenNotesVideoConsumer>(base::BindRepeating(
      &CNTracingHandler::OnFrameFromVideoConsumer, base::Unretained(this)));
}

CNTracingHandler::~CNTracingHandler() = default;

// static
std::vector<CNTracingHandler*> CNTracingHandler::ForAgentHost(
    CitizenNotesAgentHostImpl* host) {
  return host->HandlersByName<CNTracingHandler>(Tracing::Metainfo::domainName);
}

void CNTracingHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<Tracing::Frontend>(dispatcher->channel());
  Tracing::Dispatcher::wire(dispatcher, this);
}

Response CNTracingHandler::Disable() {
  if (session_)
    StopTracing(nullptr);
...
void CNTracingHandler::OnTraceDataCollected(
...
void CNTracingHandler::OnTraceComplete() {
...
std::string CNTracingHandler::UpdateTraceDataBuffer(
...
void CNTracingHandler::OnTraceToStreamComplete(const std::string& stream_handle) {
  bool data_loss = session_->HasDataLossOccurred();
  process_set_monitor_.reset();
  session_.reset();
...
void CNTracingHandler::Start(Maybe<std::string> categories,
  ...
  if (perfetto_config.has_value()) {
    ...
    if (config.has_value()) {
      ...
      browser_config =
          GetTraceConfigFromCitizenNotesConfig(base::Value(std::move(dict)));
  ...
  if (session_for_process_filter_) {
    process_set_monitor_ = CNTracingProcessSetMonitor::Start(
        *session_for_process_filter_,
        base::BindRepeating(&CNTracingHandler::AddProcessToFilter,
                            base::Unretained(this)));
  ...
  session_->EnableTracing(
      trace_config_,
      base::BindOnce(&CNTracingHandler::OnRecordingEnabled,
                     weak_factory_.GetWeakPtr(), std::move(callback)));
}

perfetto::TraceConfig CNTracingHandler::CreatePerfettoConfiguration(
...
void CNTracingHandler::AddProcessToFilter(base::ProcessId pid) {
...
void CNTracingHandler::AttemptAdoptStartupSession(
    bool return_as_stream,
    bool gzip_compression,
    bool proto_format,
...
Response CNTracingHandler::End() {
  ...
  if (return_as_stream_) {
    endpoint = new CitizenNotesStreamEndpoint(
        weak_factory_.GetWeakPtr(),
        CitizenNotesStreamFile::Create(
    ...
    trace_data_buffer_state_ = CNTracingHandler::TraceDataBufferState();
    endpoint = new CitizenNotesTraceEndpointProxy(weak_factory_.GetWeakPtr());
  }
...
void CNTracingHandler::GetCategories(
    std::unique_ptr<GetCategoriesCallback> callback) {
  // TODO(eseckler): Support this via the perfetto service too.
  TracingController::GetInstance()->GetCategories(
      base::BindOnce(&CNTracingHandler::OnCategoriesReceived,
                     weak_factory_.GetWeakPtr(), std::move(callback)));
}

void CNTracingHandler::OnRecordingEnabled(std::unique_ptr<StartCallback> callback,
...
void CNTracingHandler::OnCategoriesReceived(
    std::unique_ptr<GetCategoriesCallback> callback,
...
void CNTracingHandler::RequestMemoryDump(
  ...
  auto on_memory_dump_finished =
      base::BindOnce(&CNTracingHandler::OnMemoryDumpFinished,
                     weak_factory_.GetWeakPtr(), std::move(callback));
...
void CNTracingHandler::OnMemoryDumpFinished(
    std::unique_ptr<RequestMemoryDumpCallback> callback,
    bool success,
    uint64_t dump_id) {
...
void CNTracingHandler::OnFrameFromVideoConsumer(
  ...
  if (number_of_screenshots_from_video_consumer_ >=
      CitizenNotesTraceableScreenshot::kMaximumNumberOfScreenshots) {
    video_consumer_->StopCapture();
  }
}

Response CNTracingHandler::RecordClockSyncMarker(const std::string& sync_id) {
  if (!IsTracing())
    return Response::ServerError("Tracing is not started");
...
void CNTracingHandler::SetupTimer(double usage_reporting_interval) {
  ...
  buffer_usage_poll_timer_->Start(
      FROM_HERE, interval,
      base::BindRepeating(&CNTracingHandler::UpdateBufferUsage,
                          weak_factory_.GetWeakPtr()));
}

void CNTracingHandler::UpdateBufferUsage() {
  session_->GetBufferUsage(base::BindOnce(&CNTracingHandler::OnBufferUsage,
                                          weak_factory_.GetWeakPtr()));
}

void CNTracingHandler::StopTracing(
...
bool CNTracingHandler::IsTracing() const {
  return TracingController::GetInstance()->IsTracing() || g_any_agent_tracing;
}

void CNTracingHandler::EmitFrameTree() {
  auto data = std::make_unique<base::trace_event::TracedValue>();
  if (WebContents* wc = host_ ? host_->GetWebContents() : nullptr) {
    auto* frame_host =
        static_cast<RenderFrameHostImpl*>(wc->GetPrimaryMainFrame());
...
void CNTracingHandler::FrameDeleted(int frame_tree_node_id) {
  if (!did_initiate_recording_)
    return;
...
// static
bool CNTracingHandler::IsStartupTracingActive() {
