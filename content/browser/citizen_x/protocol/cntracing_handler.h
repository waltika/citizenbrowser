...
#ifndef CONTENT_BROWSER_CITIZENNOTES_PROTOCOL_TRACING_HANDLER_H_
#define CONTENT_BROWSER_CITIZENNOTES_PROTOCOL_TRACING_HANDLER_H_
...
#include "content/browser/citizen_x/protocol/tracing.h"
...
class CitizenNotesVideoConsumer;
class CitizenNotesIOContext;
class CitizenNotesSession;
class NavigationRequest;
class CNTracingProcessSetMonitor;

namespace protocol {

class CNTracingHandler : public CitizenNotesDomainHandler, public Tracing::Backend {
 public:
  CONTENT_EXPORT CNTracingHandler(CitizenNotesAgentHostImpl* host,
                                CitizenNotesIOContext* io_context,
                                CitizenNotesSession* root_session);

  CNTracingHandler(const CNTracingHandler&) = delete;
  CNTracingHandler& operator=(const CNTracingHandler&) = delete;

  CONTENT_EXPORT ~CNTracingHandler() override;

  static std::vector<CNTracingHandler*> ForAgentHost(CitizenNotesAgentHostImpl* host);
 ...
 private:
  friend class CNTracingHandlerTest;
  ...
  const raw_ptr<CitizenNotesIOContext> io_context_;
  const raw_ptr<CitizenNotesAgentHostImpl> host_;  // Only null in unit tests.

  // Session is for use in process filter and is null in browser.
  const raw_ptr<CitizenNotesSession> session_for_process_filter_;
  ...
  std::unique_ptr<CitizenNotesVideoConsumer> video_consumer_;
  int number_of_screenshots_from_video_consumer_ = 0;
  perfetto::TraceConfig trace_config_;
  std::unique_ptr<PerfettoTracingSession> session_;
  std::unique_ptr<CNTracingProcessSetMonitor> process_set_monitor_;
  base::WeakPtrFactory<CNTracingHandler> weak_factory_{this};

  FRIEND_TEST_ALL_PREFIXES(CNTracingHandlerTest,
                           GetTraceConfigFromCitizenNotesConfig);
};

}  // namespace protocol
}  // namespace content

#endif  // CONTENT_BROWSER_CITIZENNOTES_PROTOCOL_TRACING_HANDLER_H_
