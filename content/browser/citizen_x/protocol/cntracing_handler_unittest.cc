...
#include "content/browser/citizen_x/protocol/tracing_handler.h"
...
const char kCustomTraceConfigStringCitizenNotesStyle[] =
    "{"
...
class CNTracingHandlerTest : public testing::Test {
 public:
  void SetUp() override {
    tracing_handler_ =
        std::make_unique<CNTracingHandler>(nullptr, nullptr, nullptr);
  }
 ...
 private:
  std::unique_ptr<CNTracingHandler> tracing_handler_;
};

TEST_F(CNTracingHandlerTest, GetTraceConfigFromCitizenNotesConfig) {
  base::Value devtools_config =
      base::JSONReader::Read(kCustomTraceConfigStringCitizenNotesStyle).value();

  base::trace_event::TraceConfig trace_config =
      CNTracingHandler::GetTraceConfigFromCitizenNotesConfig(devtools_config);

  EXPECT_STREQ(kCustomTraceConfigString, trace_config.ToString().c_str());
}

TEST_F(CNTracingHandlerTest, SimpleGetValidTraceFragment) {
  // No prefix is valid.
...
TEST_F(CNTracingHandlerTest, GetValidTraceFragmentBreakBeforeComma) {
  EXPECT_EQ("{pid: 1}", GetValidTraceFragment("{pid: 1}"));
  // The comma should be ignored.
  EXPECT_EQ("{pid: 2}", GetValidTraceFragment(",{pid: 2}"));
}

TEST_F(CNTracingHandlerTest, ComplexGetValidTraceFragment) {
