...
#include "content/browser/citizen_x/protocol/web_authn.h"
...
static constexpr char kCitizenNotesNotAttached[] =
    "The CitizenNotes session is not attached to a frame";
...
CNWebAuthnHandler::CNWebAuthnHandler()
    : CitizenNotesDomainHandler(WebAuthn::Metainfo::domainName) {}

CNWebAuthnHandler::~CNWebAuthnHandler() = default;

void CNWebAuthnHandler::SetRenderer(int process_host_id,
...
void CNWebAuthnHandler::Wire(UberDispatcher* dispatcher) {
  frontend_ = std::make_unique<WebAuthn::Frontend>(dispatcher->channel());
  WebAuthn::Dispatcher::wire(dispatcher, this);
}

Response CNWebAuthnHandler::Enable(Maybe<bool> enable_ui) {
  if (!frame_host_)
    return Response::ServerError(kCitizenNotesNotAttached);
...
Response CNWebAuthnHandler::Disable() {
  if (frame_host_) {
...
Response CNWebAuthnHandler::AddVirtualAuthenticator(
...
Response CNWebAuthnHandler::RemoveVirtualAuthenticator(
...
Response CNWebAuthnHandler::SetResponseOverrideBits(
    const String& authenticator_id,
...
void CNWebAuthnHandler::AddCredential(
    const String& authenticator_id,
...
void CNWebAuthnHandler::GetCredential(
    const String& authenticator_id,
...
void CNWebAuthnHandler::GetCredentials(
    const String& authenticator_id,
...
Response CNWebAuthnHandler::RemoveCredential(const String& authenticator_id,
                                           const Binary& credential_id) {
  VirtualAuthenticator* authenticator;
  Response response = FindAuthenticator(authenticator_id, &authenticator);
  if (!response.IsSuccess())
...
Response CNWebAuthnHandler::ClearCredentials(const String& authenticator_id) {
  VirtualAuthenticator* authenticator;
  Response response = FindAuthenticator(authenticator_id, &authenticator);
  if (!response.IsSuccess())
    return response;
...
Response CNWebAuthnHandler::SetUserVerified(const String& authenticator_id,
...
Response CNWebAuthnHandler::SetAutomaticPresenceSimulation(
    const String& authenticator_id,
    bool enabled) {
  VirtualAuthenticator* authenticator;
  Response response = FindAuthenticator(authenticator_id, &authenticator);
...
Response CNWebAuthnHandler::FindAuthenticator(
    const String& id,
...
void CNWebAuthnHandler::OnCredentialCreated(
    VirtualAuthenticator* authenticator,
...
void CNWebAuthnHandler::OnAssertion(
    VirtualAuthenticator* authenticator,
    const device::VirtualFidoDevice::Credential& credential) {
...
void CNWebAuthnHandler::OnAuthenticatorWillBeDestroyed(
    VirtualAuthenticator* authenticator) {
