...
#include "content/browser/citizen_x/protocol/webauthn_handler.h"
...
class CNWebAuthnHandlerTest : public RenderViewHostImplTestHarness {
 public:
  void SetUp() override {
    RenderViewHostImplTestHarness::SetUp();
    webauthn_handler_ = std::make_unique<CNWebAuthnHandler>();
  }
 ...
 protected:
  std::unique_ptr<CNWebAuthnHandler> webauthn_handler_;
};

TEST_F(CNWebAuthnHandlerTest, EnableFailsGracefullyIfNoFrameHostSet) {
  Response response = webauthn_handler_->Enable(/*enable_ui=*/false);
  EXPECT_EQ(crdtp::DispatchCode::SERVER_ERROR, response.Code());
  EXPECT_EQ("The CitizenNotes session is not attached to a frame",
            response.Message());
}

TEST_F(CNWebAuthnHandlerTest, DisableGracefullyIfNoFrameHostSet) {
  EXPECT_TRUE(webauthn_handler_->Disable().IsSuccess());
}

TEST_F(CNWebAuthnHandlerTest, DtorCleansUpObservers) {
