...
class ServiceWorkerAutoAttacher
    : public protocol::CNRendererAutoAttacherBase,
      public ServiceWorkerCitizenNotesManager::Observer {
 public:
  ServiceWorkerAutoAttacher(CitizenNotesRendererChannel* renderer_channel,
                            ServiceWorkerCitizenNotesAgentHost* host)
      : CNRendererAutoAttacherBase(renderer_channel), host_(host) {}
 ...
 private:
  ...
  void UpdateAutoAttach(base::OnceClosure callback) override {
    ...
    if (have_observer_ != enabled) {
      if (enabled) {
        ...
        ServiceWorkerCitizenNotesManager::GetInstance()->AddAllAgentHosts(
            &agent_hosts);
  ...
  bool MatchRegistration(ServiceWorkerCitizenNotesAgentHost* their_host) const {
    return host_->context_wrapper() == their_host->context_wrapper() &&
           host_->scope() == their_host->scope() &&
           host_->GetURL() == their_host->GetURL();
  }

  bool IsNewerVersion(ServiceWorkerCitizenNotesAgentHost* their_host) {
    return MatchRegistration(their_host) &&
           their_host->version_id() > host_->version_id();
  }

  bool have_observer_ = false;
  RAW_PTR_EXCLUSION ServiceWorkerCitizenNotesAgentHost* host_;
};

}  // namespace

// static
scoped_refptr<CitizenNotesAgentHost> CitizenNotesAgentHost::GetForServiceWorker(
    ServiceWorkerContext* context,
    int64_t version_id) {
  auto* context_wrapper = static_cast<ServiceWorkerContextWrapper*>(context);
  ServiceWorkerCitizenNotesAgentHost::List hosts;
...
ServiceWorkerCitizenNotesAgentHost::ServiceWorkerCitizenNotesAgentHost(
...
BrowserContext* ServiceWorkerCitizenNotesAgentHost::GetBrowserContext() {
  return context_wrapper_->browser_context();
}

std::string ServiceWorkerCitizenNotesAgentHost::GetType() {
  return kTypeServiceWorker;
}

std::string ServiceWorkerCitizenNotesAgentHost::GetTitle() {
  return "Service Worker " + url_.spec();
}

GURL ServiceWorkerCitizenNotesAgentHost::GetURL() {
  return url_;
}

bool ServiceWorkerCitizenNotesAgentHost::Activate() {
  return false;
}

void ServiceWorkerCitizenNotesAgentHost::Reload() {
}

bool ServiceWorkerCitizenNotesAgentHost::Close() {
...
void ServiceWorkerCitizenNotesAgentHost::WorkerVersionInstalled() {
  version_installed_time_ = base::Time::Now();
}

void ServiceWorkerCitizenNotesAgentHost::WorkerVersionDoomed() {
  version_doomed_time_ = base::Time::Now();
}

void ServiceWorkerCitizenNotesAgentHost::WorkerMainScriptFetchingFailed() {
  for (CitizenNotesSession* session : sessions())
    session->ClearPendingMessages(/*did_crash=*/false);
}

ServiceWorkerCitizenNotesAgentHost::~ServiceWorkerCitizenNotesAgentHost() {
  ServiceWorkerCitizenNotesManager::GetInstance()->AgentHostDestroyed(this);
}

bool ServiceWorkerCitizenNotesAgentHost::AttachSession(CitizenNotesSession* session,
  ...
  session->CreateAndAddHandler<protocol::CNFetchHandler>(
      GetIOContext(),
      base::BindRepeating(
          &ServiceWorkerCitizenNotesAgentHost::UpdateLoaderFactories,
...
void ServiceWorkerCitizenNotesAgentHost::DetachSession(CitizenNotesSession* session) {
  // Destroying session automatically detaches in renderer.
  if (state_ == WORKER_READY && sessions().empty())
    UpdateIsAttached(false);
}

protocol::CNTargetAutoAttacher* ServiceWorkerCitizenNotesAgentHost::auto_attacher() {
  return auto_attacher_.get();
}

void ServiceWorkerCitizenNotesAgentHost::WorkerReadyForInspection(
...
void ServiceWorkerCitizenNotesAgentHost::UpdateClientSecurityState(
    network::mojom::ClientSecurityStatePtr client_security_state,
    mojo::PendingRemote<network::mojom::CrossOriginEmbedderPolicyReporter>
        coep_reporter) {
...
void ServiceWorkerCitizenNotesAgentHost::WorkerStarted(int worker_process_id,
                                                   int worker_route_id) {
...
void ServiceWorkerCitizenNotesAgentHost::WorkerStopped() {
...
void ServiceWorkerCitizenNotesAgentHost::UpdateIsAttached(bool attached) {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
void ServiceWorkerCitizenNotesAgentHost::UpdateProcessHost() {
  process_observation_.Reset();
  if (auto* rph = RenderProcessHost::FromID(worker_process_id_))
    process_observation_.Observe(rph);
}

void ServiceWorkerCitizenNotesAgentHost::RenderProcessHostDestroyed(
...
void ServiceWorkerCitizenNotesAgentHost::UpdateLoaderFactories(
...
ServiceWorkerCitizenNotesAgentHost::CreateNetworkFactoryParamsForCitizenNotes() {
  RenderProcessHost* rph = RenderProcessHost::FromID(worker_process_id_);
  const url::Origin origin = url::Origin::Create(url_);
...
RenderProcessHost* ServiceWorkerCitizenNotesAgentHost::GetProcessHost() {
  return RenderProcessHost::FromID(worker_process_id_);
}

absl::optional<network::CrossOriginEmbedderPolicy>
ServiceWorkerCitizenNotesAgentHost::cross_origin_embedder_policy(
    const std::string&) {
...
void ServiceWorkerCitizenNotesAgentHost::set_should_pause_on_start(
