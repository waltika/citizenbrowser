...
#ifndef CONTENT_PUBLIC_BROWSER_CITIZENNOTES_AGENT_HOST_H_
#define CONTENT_PUBLIC_BROWSER_CITIZENNOTES_AGENT_HOST_H_
...
// Describes interface for managing citizennotes agents from browser process.
class CONTENT_EXPORT CitizenNotesAgentHost
    : public base::RefCounted<CitizenNotesAgentHost> {
 public:
  ...
  // File descriptor used by CitizenNotes remote debugging pipe handler
  // to read and write protocol messages.
  static constexpr int kReadFD = 3;
  static constexpr int kWriteFD = 4;

  // Latest CitizenNotes protocol version supported.
  static std::string GetProtocolVersion();

  // Returns whether particular version of CitizenNotes protocol is supported.
  ...
  static scoped_refptr<CitizenNotesAgentHost> GetForId(const std::string& id);

  // Returns CitizenNotesAgentHost that can be used for inspecting |web_contents|.
  // A new CitizenNotesAgentHost will be created if it does not exist.
  static scoped_refptr<CitizenNotesAgentHost> GetOrCreateFor(
      WebContents* web_contents);

  // Similar to the above, but returns a CitizenNotesAgentHost representing 'tab'
  ...
  static scoped_refptr<CitizenNotesAgentHost> GetForTab(WebContents* web_contents);
  static scoped_refptr<CitizenNotesAgentHost> GetOrCreateForTab(
      WebContents* web_contents);

  // Returns true iff an instance of CitizenNotesAgentHost for the |web_contents|
  // exists. This is equivalent to if a CitizenNotesAgentHost has ever been
  // created for the |web_contents|.
  static bool HasFor(WebContents* web_contents);

  // Return an instance of CitizenNotesAgentHost associated with the specified
  // service worker version, if such instance exists.
  static scoped_refptr<CitizenNotesAgentHost> GetForServiceWorker(
      ServiceWorkerContext* context,
      int64_t version_id);

  // Creates CitizenNotesAgentHost that communicates to the target by means of
  // provided |delegate|. |delegate| ownership is passed to the created agent
  // host.
  static scoped_refptr<CitizenNotesAgentHost> Forward(
      const std::string& id,
      std::unique_ptr<CitizenNotesExternalAgentProxyDelegate> delegate);

  // Creates CitizenNotesAgentHost that communicates to the target using mojom, and
  // gets details from |delegate|. |delegate| ownership is passed to the created
  // agent host.
  static scoped_refptr<CitizenNotesAgentHost> CreateForMojomDelegate(
      const std::string& id,
  ...
  // Creates CitizenNotesAgentHost for the browser, which works with browser-wide
  // debugging protocol.
  static scoped_refptr<CitizenNotesAgentHost> CreateForBrowser(
      scoped_refptr<base::SingleThreadTaskRunner> tethering_task_runner,
      const CreateServerSocketCallback& socket_callback);

  // Creates CitizenNotesAgentHost for discovery, which supports part of the
  // protocol to discover other agent hosts.
  static scoped_refptr<CitizenNotesAgentHost> CreateForDiscovery();

  static bool IsDebuggerAttached(WebContents* web_contents);

  using List = std::vector<scoped_refptr<CitizenNotesAgentHost>>;

  // Returns all CitizenNotesAgentHosts without forcing their creation.
  static List GetAll();

  // Returns all non-browser target CitizenNotesAgentHosts content is aware of.
  ...
  static void RemoveObserver(CitizenNotesAgentHostObserver*);

  // Create a CitizenNotes IO Stream from data.
  // Returns a CitizenNotes IO Stream handle that can be used to read and close the
  // stream.
  ...
  virtual bool AttachClient(CitizenNotesAgentHostClient* client) = 0;

  // Same as the above, but does not acquire the WakeLock.
  virtual bool AttachClientWithoutWakeLock(CitizenNotesAgentHostClient* client) = 0;

  // Already attached client detaches from this agent host to stop debugging it.
  // Returns true iff detach succeeded.
  virtual bool DetachClient(CitizenNotesAgentHostClient* client) = 0;

  // Returns true if there is a client attached.
  virtual bool IsAttached() = 0;

  // Sends |message| from |client| to the agent.
  virtual void DispatchProtocolMessage(CitizenNotesAgentHostClient* client,
  ...
  // Returns the CitizenNotes token of this window's opener, or empty string if no
  // opener.
 ...
 protected:
  friend class base::RefCounted<CitizenNotesAgentHost>;
  virtual ~CitizenNotesAgentHost() {}
};

}  // namespace content

#endif  // CONTENT_PUBLIC_BROWSER_CITIZENNOTES_AGENT_HOST_H_
