...
#ifndef CONTENT_PUBLIC_BROWSER_CITIZENNOTES_MANAGER_DELEGATE_H_
#define CONTENT_PUBLIC_BROWSER_CITIZENNOTES_MANAGER_DELEGATE_H_
...
class CONTENT_EXPORT CitizenNotesManagerDelegate {
 public:
  // Opens the inspector for |agent_host|.
  virtual void Inspect(CitizenNotesAgentHost* agent_host);

  // Returns CitizenNotesAgentHost type to use for given |web_contents| target.
  virtual std::string GetTargetType(WebContents* web_contents);

  // Returns CitizenNotesAgentHost title to use for given |web_contents| target.
  ...
  virtual CitizenNotesAgentHost::List RemoteDebuggingTargets(TargetType target_type);

  // Creates new inspectable target given the |url|.
  virtual scoped_refptr<CitizenNotesAgentHost> CreateNewTarget(
      const GURL& url,
  ...
  virtual void ClientAttached(CitizenNotesAgentHostClientChannel* channel);
  virtual void ClientDetached(CitizenNotesAgentHostClientChannel* channel);

  // Call callback if command was not handled.
  using NotHandledCallback =
      base::OnceCallback<void(base::span<const uint8_t>)>;
  virtual void HandleCommand(CitizenNotesAgentHostClientChannel* channel,
  ...
  // clearance from the citizennotes owners.
  virtual bool IsBrowserTargetDiscoverable();

  virtual ~CitizenNotesManagerDelegate();
};

}  // namespace content

#endif  // CONTENT_PUBLIC_BROWSER_CITIZENNOTES_MANAGER_DELEGATE_H_
