...
OSCitizenNotesAgent::OSCitizenNotesAgent(
...
void OSCitizenNotesAgent::Connect(
...
OSCitizenNotesAgent::ConnectSession(OSCitizenNotesSession* session,
...
void OSCitizenNotesAgent::MaybeTriggerInstrumentationBreakpoint(
    const std::string& name) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  for (OSCitizenNotesSession* session : sessions_) {
    session->MaybeTriggerInstrumentationBreakpoint(name);
  }
}

void OSCitizenNotesAgent::AttachCitizenNotesSession(
  ...
  auto session_destroyed_callback = base::BindOnce(
      &OSCitizenNotesAgent::SessionDestroyed, weak_ptr_factory_.GetWeakPtr());

  auto session = std::make_unique<OSCitizenNotesSession>(
...
void OSCitizenNotesAgent::InspectElement(const ::gfx::Point& point) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  NOTIMPLEMENTED();  // Should not be used with this.
}

void OSCitizenNotesAgent::ReportChildTargets(bool report,
...
void OSCitizenNotesAgent::GetUniqueFormControlId(
    int32_t nodeId,
    GetUniqueFormControlIdCallback callback) {
  NOTIMPLEMENTED();  // Should not be used with this.
}

void OSCitizenNotesAgent::runMessageLoopOnPause(int context_group_id) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  // NO-OP --- TODO(b/290815208).
}

void OSCitizenNotesAgent::quitMessageLoopOnPause() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  debug_command_queue_->QuitPauseForDebugger();
}
void OSCitizenNotesAgent::runIfWaitingForDebugger(int context_group_id) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  // NO-OP --- TODO(b/290815208).
}

void OSCitizenNotesAgent::SessionDestroyed(OSCitizenNotesSession* session) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  // Pointer cleanup.
