...
#include "services/accessibility/features/citizennotes/os_citizennotes_session.h"
...
#include "services/accessibility/features/citizennotes/debug_command_queue.h"
...
// OSCitizenNotesSession::IOSession, which handles the pipe passed to the
// `io_session` parameter of CitizenNotesAgent::AttachCitizenNotesSession(), runs on a
...
class OSCitizenNotesSession::IOSession : public blink::mojom::CitizenNotesSession {
 public:
  ...
  // CitizenNotesSession implementation.
 ...
 private:
  ...
  static void ConnectReceiver(
      ...
      mojo::PendingReceiver<blink::mojom::CitizenNotesSession>
          io_session_receiver) {
...
OSCitizenNotesSession::OSCitizenNotesSession(
    V8Environment& v8_env,
    OSCitizenNotesAgent& agent,
    const scoped_refptr<DebugCommandQueue> debug_command_queue,
  ...
  IOSession::Create(
      ...
      debug_command_queue,
      base::BindRepeating(&OSCitizenNotesSession::DispatchProtocolCommandFromIO,
...
base::OnceClosure OSCitizenNotesSession::MakeAbortPauseCallback() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  ...
  return base::BindOnce(&OSCitizenNotesSession::AbortDebuggerPause,
                        weak_ptr_factory_.GetWeakPtr());
}

void OSCitizenNotesSession::MaybeTriggerInstrumentationBreakpoint(
    const std::string& name) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  // TODO(290815208)
}

void OSCitizenNotesSession::DispatchProtocolCommandFromIO(
    int32_t call_id,
    const std::string& method,
    std::vector<uint8_t> message) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
...
void OSCitizenNotesSession::DispatchProtocolCommand(
    int32_t call_id,
    const std::string& method,
...
void OSCitizenNotesSession::sendResponse(
    int call_id,
...
void OSCitizenNotesSession::sendNotification(
    std::unique_ptr<v8_inspector::StringBuffer> message) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  SendNotificationImpl(::GetStringBytes(message.get()->string()));
}

void OSCitizenNotesSession::flushProtocolNotifications() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  NOTIMPLEMENTED();
}

void OSCitizenNotesSession::SendProtocolResponse(
    int call_id,
...
void OSCitizenNotesSession::SendProtocolNotification(
    std::unique_ptr<crdtp::Serializable> message) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  SendNotificationImpl(message->Serialize());
}

void OSCitizenNotesSession::FallThrough(int call_id,
...
void OSCitizenNotesSession::AbortDebuggerPause() {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
...
void OSCitizenNotesSession::SendProtocolResponseImpl(int call_id,
                                                 std::vector<uint8_t> message) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
...
void OSCitizenNotesSession::SendNotificationImpl(std::vector<uint8_t> message) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(v8_sequence_checker_);
  crdtp::span<uint8_t> msg_span(message.data(), message.size());
...
// documented as requirements in CitizenNotesAgent.AttachCitizenNotesSession mojo
// method documentation, and then encapsulate it inside a mojo
// CitizenNotesMessage.
//
// This is pretty much a copy-paste job from
// third_party/blink/renderer/core/inspector/citizennotes_session.cc.
// The primary difference is that namespacing of CitizenNotesMessage.
blink::mojom::CitizenNotesMessagePtr OSCitizenNotesSession::FinalizeMessage(
  ...
  auto mojo_msg = blink::mojom::CitizenNotesMessage::New();
