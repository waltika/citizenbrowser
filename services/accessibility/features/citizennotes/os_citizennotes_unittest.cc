...
// Class that pretends to be a citizennotes session host so we can check our
// evaluations.
class FakeCitizenNotesSessionHost : public blink::mojom::CitizenNotesSessionHost {
 public:
  explicit FakeCitizenNotesSessionHost(
      base::RepeatingCallback<void(int)> expectation_complete_cb)
      : expectation_complete_cb_(std::move(expectation_complete_cb)) {}
  ~FakeCitizenNotesSessionHost() override {
  ...
  void DispatchProtocolResponse(
    ...
    // See
    // https://chromecitizennotes.github.io/citizennotes-protocol/tot/Runtime/#type-RemoteObject
  ...
  void DispatchProtocolNotification(
      blink::mojom::CitizenNotesMessagePtr message,
      blink::mojom::CitizenNotesSessionStatePtr updates) override {}
  mojo::AssociatedReceiver<blink::mojom::CitizenNotesSessionHost> receiver{this};
...
// the associated mojo pipes that citizennotes uses.
class FakeAccessibilityService : public mojom::AccessibilityService {
 public:
  ...
  void ConnectCitizenNotesAgent(
    ...
    v8_manager_->ConnectCitizenNotesAgent(std::move(agent));
...
// Unit test for setting up and interacting with ATP citizennotes.
class OSCitizenNotesTest : public testing::Test {
 public:
  OSCitizenNotesTest()
      : fake_session_host_(
            base::BindRepeating(&OSCitizenNotesTest::ExpectationComplete,
                                base::Unretained(this))) {}
  OSCitizenNotesTest(const OSCitizenNotesTest&) = delete;
  OSCitizenNotesTest& operator=(const OSCitizenNotesTest&) = delete;
  ~OSCitizenNotesTest() override = default;
  ...
  void ConnectCitizenNotesAgent() {
    // Connect the agent.
    mojo::PendingAssociatedRemote<blink::mojom::CitizenNotesAgent> agent_remote_p;
    service_remote_->ConnectCitizenNotesAgent(
        agent_remote_p.InitWithNewEndpointAndPassReceiver(),
        ax::mojom::AssistiveTechnologyType::kChromeVox);
    // Wait for connect citizennotes agent to be called.
  ...
  void AttachSession() {
    // Attach the session.
    agent_remote->AttachCitizenNotesSession(
  ...
  void DisconnectCitizenNotesAgent() { fake_service_->v8_manager_.reset(); }
 ...
 protected:
  ...
  // Session Host
  FakeCitizenNotesSessionHost fake_session_host_;
  // Session Remote
  mojo::AssociatedRemote<blink::mojom::CitizenNotesSession> session_remote_;
  // IO Session Remote
  mojo::Remote<blink::mojom::CitizenNotesSession> io_session_remote_;
  // Agent Remote
  mojo::AssociatedRemote<blink::mojom::CitizenNotesAgent> agent_remote;
  // Session State
  blink::mojom::CitizenNotesSessionStatePtr reattach_session_state_;
...
TEST_F(OSCitizenNotesTest, ConnectAndEvalJS) {
  // Set expectations.
