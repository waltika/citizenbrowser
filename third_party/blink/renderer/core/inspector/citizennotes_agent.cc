...
using StatePtr = mojo::StructPtr<blink::mojom::blink::CitizenNotesSessionState>;
template <>
...
CitizenNotesAgent* CitizenNotesAgentFromContext(ExecutionContext* execution_context) {
  if (!execution_context)
    return nullptr;
  if (auto* scope = DynamicTo<WorkerGlobalScope>(execution_context)) {
    return scope->GetThread()
        ->GetCNWorkerInspectorController()
        ->GetCitizenNotesAgent();
  }
  if (auto* window = DynamicTo<LocalDOMWindow>(execution_context)) {
    ...
    if (!web_frame)
      return nullptr;
    return web_frame->CitizenNotesAgentImpl()->GetCitizenNotesAgent();
  }
  return nullptr;
}
...
// Used by the CitizenNotesAgent class to bind the passed |receiver| on the IO
// thread. Lives on the IO thread and posts to |inspector_task_runner| to do
// actual work. This class is used when CitizenNotesAgent runs on a worker so we
// don't block its execution.
class CitizenNotesAgent::IOAgent : public mojom::blink::CitizenNotesAgent {
 public:
  IOAgent(scoped_refptr<base::SingleThreadTaskRunner> io_task_runner,
          scoped_refptr<InspectorTaskRunner> inspector_task_runner,
          CrossThreadWeakHandle<::blink::CitizenNotesAgent> agent,
          mojo::PendingReceiver<mojom::blink::CitizenNotesAgent> receiver)
    ...
    // is necessary so that AttachCitizenNotesSession can be called on a worker
  ...
  void AttachCitizenNotesSession(
      mojo::PendingAssociatedRemote<mojom::blink::CitizenNotesSessionHost> host,
    ...
    inspector_task_runner_->AppendTask(CrossThreadBindOnce(
        &::blink::CitizenNotesAgent::AttachCitizenNotesSessionImpl,
  ...
  void ReportChildTargets(bool report,
    ...
    bool did_append_task =
        inspector_task_runner_->AppendTask(CrossThreadBindOnce(
            &blink::CitizenNotesAgent::ReportChildTargetsPostCallbackToIO,
 ...
 private:
  ...
  CrossThreadWeakHandle<::blink::CitizenNotesAgent> agent_;
  mojo::Receiver<mojom::blink::CitizenNotesAgent> receiver_{this};
};

CitizenNotesAgent::CitizenNotesAgent(
    Client* client,
...
CitizenNotesAgent::~CitizenNotesAgent() = default;

void CitizenNotesAgent::Trace(Visitor* visitor) const {
...
void CitizenNotesAgent::Dispose() {
  HeapHashSet<Member<CitizenNotesSession>> copy(sessions_);
  for (auto& session : copy)
    session->Detach();
  CleanupConnection();
}

void CitizenNotesAgent::BindReceiverForWorker(
    mojo::PendingRemote<mojom::blink::CitizenNotesAgentHost> host_remote,
    mojo::PendingReceiver<mojom::blink::CitizenNotesAgent> receiver,
  ...
  host_remote_.set_disconnect_handler(WTF::BindOnce(
      &CitizenNotesAgent::CleanupConnection, WrapWeakPersistent(this)));

  io_agent_ = new IOAgent(io_task_runner_, inspector_task_runner_,
                          MakeCrossThreadWeakHandle(this), std::move(receiver));
}

void CitizenNotesAgent::BindReceiver(
    mojo::PendingAssociatedRemote<mojom::blink::CitizenNotesAgentHost> host_remote,
...
void CitizenNotesAgent::AttachCitizenNotesSessionImpl(
    mojo::PendingAssociatedRemote<mojom::blink::CitizenNotesSessionHost> host,
    mojo::PendingAssociatedReceiver<mojom::blink::CitizenNotesSession>
        session_receiver,
    mojo::PendingReceiver<mojom::blink::CitizenNotesSession> io_session_receiver,
    mojom::blink::CitizenNotesSessionStatePtr reattach_session_state,
    bool client_expects_binary_responses,
    bool client_is_trusted,
    const WTF::String& session_id,
...
void CitizenNotesAgent::AttachCitizenNotesSession(
    mojo::PendingAssociatedRemote<mojom::blink::CitizenNotesSessionHost> host,
    mojo::PendingAssociatedReceiver<mojom::blink::CitizenNotesSession>
        session_receiver,
    mojo::PendingReceiver<mojom::blink::CitizenNotesSession> io_session_receiver,
...
void CitizenNotesAgent::InspectElementImpl(const gfx::Point& point) {
  client_->InspectElement(point);
}

void CitizenNotesAgent::InspectElement(const gfx::Point& point) {
  if (associated_receiver_.is_bound()) {
    client_->InspectElement(point);
  } else {
    // InspectElement on a worker doesn't make sense.
...
void CitizenNotesAgent::FlushProtocolNotifications() {
  for (auto& session : sessions_)
    session->FlushProtocolNotifications();
}

void CitizenNotesAgent::DebuggerPaused() {
...
void CitizenNotesAgent::DebuggerResumed() {
  CHECK(!host_remote_.is_bound());
  if (associated_host_remote_.is_bound()) {
...
void CitizenNotesAgent::ReportChildTargetsPostCallbackToIO(
    bool report,
    bool wait_for_debugger,
...
void CitizenNotesAgent::GetUniqueFormControlId(
    int nodeId,
...
// static
std::unique_ptr<WorkerCitizenNotesParams> CitizenNotesAgent::WorkerThreadCreated(
  ExecutionContext* parent_context,
  WorkerThread* worker_thread,
  const KURL& url,
  const String& global_scope_name,
  const absl::optional<const blink::DedicatedWorkerToken>& token) {
  auto result = std::make_unique<WorkerCitizenNotesParams>();
  base::UnguessableToken citizennotes_worker_token =
        token.has_value() ? token.value().value()
                          : base::UnguessableToken::Create();
  result->citizennotes_worker_token = citizennotes_worker_token;
  CitizenNotesAgent* agent = CitizenNotesAgentFromContext(parent_context);
  if (!agent)
    return result;

  mojom::blink::CitizenNotesExecutionContextType context_type =
      token.has_value()
          ? mojom::blink::CitizenNotesExecutionContextType::kDedicatedWorker
          : mojom::blink::CitizenNotesExecutionContextType::kWorklet;
  ...
  data->citizennotes_worker_token = result->citizennotes_worker_token;
...
// static
void CitizenNotesAgent::WorkerThreadTerminated(ExecutionContext* parent_context,
                                           WorkerThread* worker_thread) {
  if (CitizenNotesAgent* agent = CitizenNotesAgentFromContext(parent_context))
    agent->unreported_child_worker_threads_.erase(worker_thread);
}

void CitizenNotesAgent::ReportChildTarget(std::unique_ptr<WorkerData> data) {
  if (host_remote_.is_bound()) {
    host_remote_->ChildTargetCreated(
        ...
        data->citizennotes_worker_token, data->waiting_for_debugger,
        data->context_type);
...
void CitizenNotesAgent::CleanupConnection() {
  if (io_agent_) {
