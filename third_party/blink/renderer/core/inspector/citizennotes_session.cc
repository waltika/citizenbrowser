...
class CitizenNotesSession::IOSession : public mojom::blink::CitizenNotesSession {
 public:
  IOSession(scoped_refptr<base::SingleThreadTaskRunner> io_task_runner,
            scoped_refptr<InspectorTaskRunner> inspector_task_runner,
            CrossThreadWeakHandle<::blink::CitizenNotesSession> session,
            mojo::PendingReceiver<mojom::blink::CitizenNotesSession> receiver)
  ...
  void BindInterface(
    ...
    // We set the disconnect handler for the IO session to detach the citizennotes
    ...
    receiver_.set_disconnect_handler(WTF::BindOnce(
        [](scoped_refptr<InspectorTaskRunner> inspector_task_runner,
          ...
          inspector_task_runner->AppendTask(CrossThreadBindOnce(
              &::blink::CitizenNotesSession::DetachFromV8,
  ...
  // mojom::blink::CitizenNotesSession implementation.
  void DispatchProtocolCommand(int call_id,
    ...
    if (ShouldInterruptForMethod(method)) {
      inspector_task_runner_->AppendTask(CrossThreadBindOnce(
          &::blink::CitizenNotesSession::DispatchProtocolCommandImpl,
 ...
 private:
  ...
  CrossThreadWeakHandle<::blink::CitizenNotesSession> session_;
  mojo::Receiver<mojom::blink::CitizenNotesSession> receiver_{this};
};

CitizenNotesSession::CitizenNotesSession(
    CitizenNotesAgent* agent,
    ...
    mojo::PendingAssociatedReceiver<mojom::blink::CitizenNotesSession>
        main_receiver,
    mojo::PendingReceiver<mojom::blink::CitizenNotesSession> io_receiver,
    ...
    : agent_(agent),
      ...
      session_state_(nullptr), //TODO: Check
  ...
  host_remote_.set_disconnect_handler(
      WTF::BindOnce(&CitizenNotesSession::Detach, WrapWeakPersistent(this)));
  ...
  //agent_->probe_sink_->AddCitizenNotesSession(this); TODO: Check
  if (restore) {
...
void CitizenNotesSession::ConnectToV8(v8_inspector::V8Inspector* inspector,
...
bool CitizenNotesSession::IsDetached() {
  return !io_session_;
}

void CitizenNotesSession::Append(InspectorAgent* agent) {
  agents_.push_back(agent);
  agent->Init(agent_->probe_sink_.Get(), inspector_backend_dispatcher_.get(),
              &session_state_);
}

void CitizenNotesSession::Detach() {
  ...
  io_session_ = nullptr;
  // agent_->probe_sink_->RemoveCitizenNotesSession(this); TODO: Check
...
void CitizenNotesSession::DetachFromV8() {
  if (v8_session_) {
    v8_session_->stop();
  }
}

void CitizenNotesSession::DispatchProtocolCommand(
    int call_id,
    const String& method,
...
void CitizenNotesSession::DispatchProtocolCommandImpl(
    int call_id,
    const String& method,
  ...
  if (v8_inspector::V8InspectorSession::canDispatchMethod(
    ...
    // This message has already been checked by content::CitizenNotesSession.
...
void CitizenNotesSession::DidStartProvisionalLoad(LocalFrame* frame) {
...
void CitizenNotesSession::DidFailProvisionalLoad(LocalFrame* frame) {
  if (v8_session_ && agent_->inspected_frames_->Root() == frame)
    v8_session_->setSkipAllPauses(false);
}

void CitizenNotesSession::DidCommitLoad(LocalFrame* frame, DocumentLoader*) {
  for (wtf_size_t i = 0; i < agents_.size(); i++)
...
void CitizenNotesSession::PaintTiming(Document* document,
                                  const char* name,
                                  double timestamp) {
  if (v8_session_ &&
...
void CitizenNotesSession::DomContentLoadedEventFired(LocalFrame* local_frame) {
...
void CitizenNotesSession::sendResponse(
    int call_id,
...
void CitizenNotesSession::sendNotification(
...
void CitizenNotesSession::Trace(Visitor* visitor) const {
...
blink::mojom::blink::CitizenNotesMessagePtr CitizenNotesSession::FinalizeMessage(
  ...
  auto mojo_msg = mojom::blink::CitizenNotesMessage::New();
