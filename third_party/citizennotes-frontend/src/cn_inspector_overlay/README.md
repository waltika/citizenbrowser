...
Inspector Overlay provides JS/CSS modules which are responsible for rendering the overlay content when you inspect an element in CitizenNotes.
...
or images to be bundled into JS files. Unlike CitizenNotes themselves, inspector overlay does not have an ability to
...
CSS and that approach should not be used in CitizenNotes itself because importing CSS this way is not standard and CitizenNotes
...
## Importing modules from CitizenNotes

It's possible to re-use modules from CitizenNotes for the overlay implementation.
...
Overlay modules can be unit tested like other parts of CitizenNotes. For an example, see `test/unittests/front_end/inspector_overlay/common_test.ts`.
