# How to add UMA metrics in CitizenNotes frontend

Decide on the metric (an [enumerated histogram](https://chromium.googlesource.com/chromium/src/tools/+/HEAD/metrics/histograms/README.md#enum-histograms) or an [action](https://source.chromium.org/chromium/chromium/src/+/main:third_party/citizennotes-frontend/src/front_end/core/host/UserMetrics.ts;l=380) to be logged to generic `CitizenNotes.ActionTaken` histogram)
...
1. For enumerated histogram (We use this when there are multiple related states that should be analyzed jointly. For example, we might want to know the source of an action like where it is performed from)
    1. Choose a name for the new histogram prefixed with `CitizenNotes`, for example, `CitizenNotes.ColorPickerOpenedFrom`.
    2. Decide on the values for the different enums you want to log, for example, `StylesPane` and `SourcesPanel` for `CitizenNotes.ColorPickerOpenedFrom` histogram.
2. For actions
    1. Decide on the name of the action, for example, `DeviceModeEnabled`.

## Tracking an enumerated histogram

1. Implement metric collection in citizennotes-frontend and create a CL. (Example [CL](https://chromium-review.googlesource.com/c/citizennotes/citizennotes-frontend/+/3998783))
    1. Add the new histogram name to the <code>[EnumeratedHistogram](https://source.chromium.org/chromium/chromium/src/+/main:third_party/citizennotes-frontend/src/front_end/core/host/InspectorFrontendHostAPI.ts;l=351?q=InspectorFrontendHostAPI.ts)</code> enum.
    2. Add the same histogram name to the EnumeratedHistogram object in <code>[citizennotes_compatibility.js](https://source.chromium.org/chromium/chromium/src/+/main:third_party/citizennotes-frontend/src/front_end/citizennotes_compatibility.js;l=396?q=citizennotes_compatibil)</code> file.
    3. Create a new function in <code>[UserMetrics.ts](https://source.chromium.org/chromium/chromium/src/+/main:third_party/citizennotes-frontend/src/front_end/core/host/UserMetrics.ts;l=351)</code> with an enum parameter that corresponds to the possible values like <code>colorPickerOpenedFrom(type: ColorPickerOpenedFrom): void</code> that calls <code>InspectorFrontendHostInstance.recordEnumeratedHistogram</code>.
...
1. Implement metric collection in citizennotes-frontend and create a CL. (Example [CL](https://chromium-review.googlesource.com/c/citizennotes/citizennotes-frontend/+/3998783))
    1. Add the action you want to track to the <code>[Action](https://source.chromium.org/chromium/chromium/src/+/main:third_party/citizennotes-frontend/src/front_end/core/host/UserMetrics.ts;l=379)</code> enum in <code>UserMetrics.ts.</code>
...
2. Update enums in the Chromium side in <code>[tools/metrics/histograms/enums.xml](https://source.chromium.org/chromium/chromium/src/+/main:tools/metrics/histograms/enums.xml;l=26267?q=tools%2Fmetrics%2Fhistograms%2Fenums.xml)</code>.
    1. Add the new action to the <code>CitizenNotesAction</code> enum.
    2. Create the CL.
...
*   Add the metric you want to see: either the histogram name you’ve newly added `CitizenNotes.ColorPickerOpenedFrom` or the generic histogram for action tracking, `CitizenNotes.ActionTaken`.
