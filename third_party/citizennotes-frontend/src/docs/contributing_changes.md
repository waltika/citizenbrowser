# Contributing changes to Chromium CitizenNotes

See [Get the Code](get_the_code.md) for details on how to checkout the code, and [Chrome CitizenNotes Design Review Guidelines](design_guidelines.md) for
...
Usual [steps](https://chromium.googlesource.com/chromium/src/+/main/docs/contributing.md#creating-a-change) for creating a change work out of the box, when executed in the CitizenNotes frontend repository.
...
_Merge request/approval is handled by Chromium Release Managers. CitizenNotes follows [Chromium's merge criteria](https://chromium.googlesource.com/chromium/src.git/+/refs/heads/main/docs/process/merge_request.md#merge-criteria-phases). In exceptional cases please get in touch with hablich@chromium.org._
...
1. Backmerges are done to the `chromium/xxxx` (e.g. `chromium/3979`) branch on the CitizenNotes frontend repo.
...
1. Open the to-be-merged commit in Gerrit
   ([example](https://chromium-review.googlesource.com/c/citizennotes/citizennotes-frontend/+/1928912)).
...
1. The cherry-pick CL is created
   ([example](https://chromium-review.googlesource.com/c/citizennotes/citizennotes-frontend/+/1928913)).
...
You can then resolve any conflicts, run tests, build CitizenNotes, etc, locally to verify everything is working. Then run `git cl upload` to upload the CL and get a review as normal.
