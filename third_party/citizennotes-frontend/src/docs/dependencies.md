...
To sync dependencies from Chromium to CitizenNotes frontend, use `scripts/deps/roll_deps.py && npm run generate-protocol-resources`.
Note that this may:
...
- Introduce breaking changes to the citizennotes protocol, causing compilation failures. Unfortunately these need to be handled manually as there are some changes (e.g. removing an enum value) that cannot fail gracefully.


The following scripts run as AutoRollers, but can be manually invoked if desired:

- To roll the `HEAD` commit of CitizenNotes frontend into Chromium, use `scripts/deps/roll_to_chromium.py`.
- To update CitizenNotes frontend's DEPS, use `roll-dep`.

## Third-party Guidelines

When you want to integrate or use third-party content in CitizenNotes, there are a
couple of different ways to do so. Most of the time, we have to make a
distinction between "third-party code we use as part of CitizenNotes implementation"
and "third-party code we use to build CitizenNotes itself, but is not included in
the product".

### Third-party code included in CitizenNotes bundle

All third-party content that you want to ship as part of the CitizenNotes bundle
must be included in `front_end/third_party`. The typical way to update these
packages is to download the relevant packages from [npm]. Since CitizenNotes does
...
Since CitizenNotes ships as part of the Chrome binary, bundle size
...
For all third-party packages that are used either as part of the CitizenNotes build
...
> output to be purged, as they are part of all CitizenNotes modules. Whenever you are
...
[GN][] (Chromium/CitizenNotes build system) or [clang-format][] (multi-language
formatter).
...
CitizenNotes-specific bots. To avoid excessive network bandwidth usage, by default
...
infrastructure teams and are used to build CitizenNotes in `DEPS`. For packages that
are CitizenNotes-specific, prefer adding them to `scripts/deps/manage_node_deps.py`
instead.
