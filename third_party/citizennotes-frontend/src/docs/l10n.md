...
4. The following commands would add the new strings to `en-US.json`:
  ...
  - `node third_party/i18n/collect-strings.js` under the CitizenNotes src folder
...
Access localized strings in the CitizenNotes frontend using the following localization calls.
...
### i18nLazyString
The `i18nString` function returns the translated string, with placeholders resolved. To do this, it needs access to the translated strings for the user's locale, which are not available until after CitizenNotes has finished starting up.
...
**Brandings:**
Lighthouse, GitHub, CitizenNotes, Chrome Data Saver, Safari, BlackBerry Z30, Kindle Fire HDX, Pixel 2, Microsoft Lumia 550
**Code snippets:**
