...
extensions provide CitizenNotes with capabilities that serve a similar purpose as
...
Language extensions are regular Chrome CitizenNotes extensions that register one or more language plugins which implement
...
association is fixed, meaning that CitizenNotes will make all subsequent "debugging" queries to that plugin.

In the following, we describe the Chrome extension APIs offered by CitizenNotes for language extensions. For a precise
...
## Registration
Plugins are registered by calling `chrome.citizennotes.languageServices.registerLanguageExtensionPlugin`, and unregistered
by calling `chrome.citizennotes.languageServices.unregisterLanguageExtensionPlugin`. Along with the plugin instance itself
...
When a new script or "raw module" is loaded that is matched to a plugin, CitizenNotes will call the plugin's `addRawModule`
...
CitizenNotes will present to the user (as an object with a single `missingSymbolFiles` property containing an array of
URLs).

When a script is unloaded, CitizenNotes will call `removeRawModule` passing the raw module identifier.
...
In order to identify which lines of a source file it can possibly set breakpoints in, CitizenNotes will call the
...
As was already alluded to above, language extensions provide information on inlined function calls. This allows CitizenNotes
...
When paused, CitizenNotes will populate the Scope View with all variables in the current scope using the
...
optionally `icon` which contains a data URL encoding an icon for CitizenNotes to show for the scope entry. The scope type is
...
In order to display the value of variables or the result of an expression, CitizenNotes will call the `evaluate` function,
...
to the wasm linear memory). The CitizenNotes extension API offers four functions for this: `getWasmLinearMemory`,
