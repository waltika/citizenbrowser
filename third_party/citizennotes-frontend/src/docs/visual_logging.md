...
CitizenNotes. The current UMA logging is unreliable and inconsistent. This can lead
incorrect conclusions about how users are interacting with the product.

We want to be able to understand how users are interacting with CitizenNotes so that
...
We log impressions and interactions for a subtree of the actual CitizenNotes
...
Some CitizenNotes UI is not represented in DOM, such as tracks in the Performance
...
structure look like. To do that, call `setVeDebuggingEnabled(true)` in CitizenNotes
on CitizenNotes. This will add red outline to each visual element and will show the
