# Chrome CitizenNotes C/C++ Debugging Extension

This extension enables debugging capabilities in CitizenNotes for C++ programs compiled to WebAssembly.

## Build the extension

**Note**: Currently, the extension supports only Linux builds.

Some of the extension's dependencies are not checked out by default together with `citizennotes-frontend`.
   ...
   solutions = [
    {
      ...
      "url"         : "https://chromium.googlesource.com/citizennotes/citizennotes-frontend",
      "deps_file"   : "DEPS",
      "managed"     : True,
      "custom_deps" : {
      },
...
- `CitizenNotes_CXX_Debugging.stage1` that contains some native binaries required for the second build stage.
- `CitizenNotes_CXX_Debugging.stage2` that contains the built extension.
...
You can load the extension to Chrome directly from the CitizenNotes repository root with the following command:

```bash
third_party/chrome/chrome-linux/chrome --load-extension=$PWD/out/CitizenNotes_CXX_Debugging.stage2/src
```

## Test front-end

The extension contains TypeScript and general front-end components, which are tested via
`karma`-based tests located in the `tests/` folder. They follow the `foo_test.ts` naming convention used in CitizenNotes front-end.
