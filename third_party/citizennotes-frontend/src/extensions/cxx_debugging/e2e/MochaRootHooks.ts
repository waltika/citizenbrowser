...
const EXTENSION_DIR = path.join(__dirname, '..', '..', '..', 'CitizenNotes_CXX_Debugging.stage2', 'gen');
const CITIZENNOTES_DIR = path.join(__dirname, '..', '..', '..', 'citizennotes-frontend', 'gen');
...
async function beforeAll() {
  ...
  const browser = await puppeteer.launch({
    ...
    defaultViewport,
    args: [
      // The fingerprint should match citizennotes-frontend's
      ...
      `--custom-citizennotes-frontend=${new URL(`${CITIZENNOTES_DIR}/front_end`, 'file://').href}`,
    ],
  });
  ...
  const citizennotesTarget = await conn.waitForTarget(target => target.url().startsWith('citizennotes://'));
  const frontend = await citizennotesTarget?.page();
  if (!frontend) {
...
async function beforeEach() {
  ...
  await click('[aria-label="Customize and control CitizenNotes"]');
