...
def get_artifact_dir(project, *paths):
    dirs = {
        'citizennotes-frontend': 'citizennotes-frontend',
        'test_suite': 'extensions/cxx_debugging/e2e',
        'cxx_debugging': 'CitizenNotes_CXX_Debugging.stage2'
    }
...
class Compile(RunnerCommand):
    ...
    def __call__(self, options):
        ...
        self.build_citizennotes(options.build_root, options.verbose)
    ...
    def build_citizennotes(self, build_root, verbose):
        ...
        citizennotes_build_root = repo_path(build_root,
                                        get_artifact_dir('citizennotes-frontend'))
        make_dir(citizennotes_build_root)

        run_process(gn,
                    'gen',
                    citizennotes_build_root,
                    cwd=repo_path('//'),
                    verbose=verbose)
        ninja(build_root, 'citizennotes-frontend', verbose)
...
class Init(RunnerCommand):
    ...
    def __call__(self, options):
        ...
        create_symlink(
            ...
            repo_path(options.build_root,
                      get_artifact_dir('citizennotes-frontend'), 'gen',
...
class Inspect(Init):
    ...
    def __call__(self, options):
        ...
        ninja(options.build_root, 'citizennotes-frontend', options.verbose)
        ...
        run_process(
            chrome_binary,
            f'--auto-open-citizennotes-for-tabs',
            f'--load-extension={repo_path(options.build_root, get_artifact_dir("cxx_debugging"), "src")}',
            f'--custom-citizennotes-frontend=file://{repo_path(options.build_root, get_artifact_dir("citizennotes-frontend"), "gen", "front_end")}',
