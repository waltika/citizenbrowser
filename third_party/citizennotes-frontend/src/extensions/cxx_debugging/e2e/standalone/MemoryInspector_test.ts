...
describe('LinearMemoryInspector', () => {
  it('can show variables', async () => {
    ...
    await installEventListener(frontend, 'CitizenNotes.DebuggerPaused');
    ...
    await target.reload();
    await waitForFunction(async () => ((await getPendingEvents(frontend, 'CitizenNotes.DebuggerPaused')) || []).length > 0);
