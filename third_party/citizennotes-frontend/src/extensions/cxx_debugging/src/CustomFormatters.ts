...
export class CXXValue implements Value, LazyObject {
  ...
  async asRemoteObject(): Promise<Chrome.CitizenNotes.RemoteObject> {
    if (this.type.hasValue && this.type.arraySize === 0) {
      const formatter = CustomFormatters.get(this.type);
      if (!formatter) {
        const type = 'undefined' as Chrome.CitizenNotes.RemoteObjectType;
    ...
    const type = (this.type.arraySize > 0 ? 'array' : 'object') as Chrome.CitizenNotes.RemoteObjectType;
    const {objectId} = this;
    return {
      type,
...
export interface LazyObject {
  ...
  asRemoteObject(): Promise<Chrome.CitizenNotes.RemoteObject>;
}
...
export function primitiveObject<T>(
  ...
  if (['number', 'string', 'boolean', 'bigint', 'undefined'].includes(typeof value)) {
    ...
    return new PrimitiveLazyObject(
        typeof value as Chrome.CitizenNotes.RemoteObjectType, value, description, linearMemoryAddress, type?.size);
  }
  return null;
}
...
function lazyObjectFromAny(
  ...
  if (typeof value === 'object') {
    if (value === null) {
      return new PrimitiveLazyObject(
          'null' as Chrome.CitizenNotes.RemoteObjectType, value, description, linearMemoryAddress);
    }
...
export class PrimitiveLazyObject<T> implements LazyObject {
  readonly type: Chrome.CitizenNotes.RemoteObjectType;
  readonly value: T;
  readonly description: string;
  private readonly linearMemoryAddress?: number;
  private readonly linearMemorySize?: number;
  constructor(
      type: Chrome.CitizenNotes.RemoteObjectType, value: T, description?: string, linearMemoryAddress?: number,
...
export class LocalLazyObject implements LazyObject {
  ...
  async asRemoteObject(): Promise<Chrome.CitizenNotes.RemoteObject> {
    const type = (Array.isArray(this.value) ? 'array' : 'object') as Chrome.CitizenNotes.RemoteObjectType;
