...
export type RawModule = Chrome.CitizenNotes.RawModule&{dwp?: ArrayBuffer};
...
export class DWARFLanguageExtensionPlugin implements Chrome.CitizenNotes.LanguageExtensionPlugin {
  ...
  async listVariablesInScope(rawLocation: Chrome.CitizenNotes.RawLocation): Promise<Chrome.CitizenNotes.Variable[]> {
    const {flush, manage} = createEmbindPool();
    const moduleInfo = await this.getModuleInfo(rawLocation.rawModuleId);
    try {
  ...
  async getInlinedFunctionRanges(rawLocation: Chrome.CitizenNotes.RawLocation):
      Promise<Chrome.CitizenNotes.RawLocationRange[]> {
    const {flush, manage} = createEmbindPool();
    const moduleInfo = await this.getModuleInfo(rawLocation.rawModuleId);
    try {
  ...
  async getInlinedCalleesRanges(rawLocation: Chrome.CitizenNotes.RawLocation): Promise<Chrome.CitizenNotes.RawLocationRange[]> {
    const {flush, manage} = createEmbindPool();
    const moduleInfo = await this.getModuleInfo(rawLocation.rawModuleId);
    try {
      const rawLocations = manage(
  ...
  async getValueInfo(expression: string, context: Chrome.CitizenNotes.RawLocation, stopId: unknown): Promise<{
  ...
  async evaluate(expression: string, context: SymbolsBackend.RawLocation, stopId: unknown):
    ...
    if (!cxxObject) {
      return {
        type: 'undefined' as Chrome.CitizenNotes.RemoteObjectType,
        hasChildren: false,
  ...
  async getProperties(objectId: Chrome.CitizenNotes.RemoteObjectId): Promise<Chrome.CitizenNotes.PropertyDescriptor[]> {
  ...
  async releaseObject(objectId: Chrome.CitizenNotes.RemoteObjectId): Promise<void> {
