...
class LLDBEvalDebugger implements LLDBEvalTests.Debugger {
  ...
  constructor(dbg: Debugger, plugin: Chrome.CitizenNotes.LanguageExtensionPlugin) {
    this.#debugger = dbg;
    this.#plugin = plugin;
  }
  ...
  private async stringify(result: Chrome.CitizenNotes.RemoteObject): Promise<string> {
