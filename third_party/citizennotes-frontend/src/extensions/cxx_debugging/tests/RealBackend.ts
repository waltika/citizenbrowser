...
import type Protocol from 'citizennotes-protocol';
// eslint-disable-next-line rulesdir/es_modules_import
...
type PauseLocation = {
  rawLocation: Chrome.CitizenNotes.RawLocation,
...
export class Debugger {
  ...
  private constructor(targetId: string) {
    const url = `ws://localhost:9222/citizennotes/page/${targetId}`;
  ...
  async setBreakpointByRawLocation(scriptId: string, rawLocationRange: Chrome.CitizenNotes.RawLocationRange):
  ...
  async setBreakpointsOnSourceLines(
      sourceLines: Array<string|RegExp>, sourceFileURL: URL, plugin: Chrome.CitizenNotes.LanguageExtensionPlugin,
  ...
  async setBreakpointOnSourceLine(
      sourceLine: string|RegExp, sourceFileURL: URL, plugin: Chrome.CitizenNotes.LanguageExtensionPlugin,
  ...
  async setBreakpoint(
      sourceLineNumber: number, sourceFileURL: URL, plugin: Chrome.CitizenNotes.LanguageExtensionPlugin,
      rawModuleId: string): Promise<BreakLocation> {
...
async function slideLine(
    plugin: Chrome.CitizenNotes.LanguageExtensionPlugin, rawModuleId: string, sourceUrl: string,
