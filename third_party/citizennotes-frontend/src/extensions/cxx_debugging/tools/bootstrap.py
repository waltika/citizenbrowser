...
def citizennotes_dir(source_dir):
    return os.path.dirname(os.path.dirname(source_dir))


def node_path(source_dir):
    return os.path.join(
        citizennotes_dir(source_dir), 'third_party', 'node', *{
...
def stage1(sysroot_dir, source_dir, OPTIONS):
    ...
    binary_dir = os.path.abspath(
        os.path.join(OPTIONS.build_dir, 'CitizenNotes_CXX_Debugging.stage1'))
...
def stage2(source_dir, stage1_dir, OPTIONS):
    ...
    emcc = os.path.join(citizennotes_dir(source_dir), 'third_party',
                        'emscripten-releases', 'install', 'emscripten', 'emcc')

    binary_dir = os.path.abspath(
        os.path.join(OPTIONS.build_dir, 'CitizenNotes_CXX_Debugging.stage2'))
    if not os.path.exists(binary_dir):
        os.makedirs(binary_dir)
