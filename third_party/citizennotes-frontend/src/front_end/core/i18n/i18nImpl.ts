...
import {CitizenNotesLocale} from './CitizenNotesLocale.js';
import {
...
// All the locales that are part of the CitizenNotes bundle and should not be fetched
// remotely.
 ...
 */
export function lookupClosestSupportedCitizenNotesLocale(locale: string): string {
  return i18nInstance.lookupClosestSupportedLocale(locale);
}

/**
 * Returns a list of all supported CitizenNotes locales, including pseudo locales.
 */
export function getAllSupportedCitizenNotesLocales(): string[] {
  return [...i18nInstance.supportedLocales];
}

/**
 * Returns the Url from which a locale can be fetched. This depends on the
 * specific locale, as some are bundled with CitizenNotes while others
...
function getLocaleFetchUrl(locale: Intl.UnicodeBCP47LocaleIdentifier, location: string): string {
  ...
  if (remoteBase && remoteBase.version && !BUNDLED_LOCALES.has(locale)) {
    return REMOTE_FETCH_PATTERN.replace('@HOST@', 'citizennotes://citizennotes')
...
export function getLocalizedString(
  ...
  return registeredStrings.getLocalizedStringSetFor(CitizenNotesLocale.instance().locale).getLocalizedString(id, values) as
      Platform.UIString.LocalizedString;
}
...
export function getFormatLocalizedString(
  ...
  const formatter =
      registeredStrings.getLocalizedStringSetFor(CitizenNotesLocale.instance().locale).getMessageFormatterFor(stringId);
 ...
 * where the former locale representation is written in the currently enabled CitizenNotes
...
export function getLocalizedLanguageRegion(
    ...
    citizennotesLocale: CitizenNotesLocale): Platform.UIString.LocalizedString {
  ...
  const citizennotesLoc = new Intl.Locale(citizennotesLocale.locale);
  const targetLanguage = localLanguage === citizennotesLoc.language ? 'en' : localBaseName;
  const languageInCurrentLocale = new Intl.DisplayNames([citizennotesLocale.locale], {type: 'language'}).of(localLanguage);
  ...
  if (locale.region) {
    const regionInCurrentLocale =
        new Intl.DisplayNames([citizennotesLocale.locale], {type: 'region', style: 'short'}).of(locale.region);
