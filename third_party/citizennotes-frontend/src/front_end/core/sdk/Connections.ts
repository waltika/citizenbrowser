...
export class WebSocketConnection implements ProtocolClient.InspectorBackend.Connection {
  ...
  #connected: boolean;
  #messages: string[];
  constructor(url: Platform.CitizenNotesPath.UrlString, onWebSocketDisconnect: () => void) {
...
export class StubConnection implements ProtocolClient.InspectorBackend.Connection {
  ...
  private respondWithError(message: string): void {
    ...
    const error = {
      ...
      code: ProtocolClient.InspectorBackend.CitizenNotesStubErrorCode,
      data: messageObject,
    };
    if (this.onMessage) {
...
function createMainConnection(websocketConnectionLost: () => void): ProtocolClient.InspectorBackend.Connection {
  ...
  if (wsParam || wssParam) {
    const ws = (wsParam ? `ws://${wsParam}` : `wss://${wssParam}`) as Platform.CitizenNotesPath.UrlString;
