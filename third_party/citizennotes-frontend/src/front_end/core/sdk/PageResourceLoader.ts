...
export type PageResourceLoadInitiator = {
  ...
  initiatorUrl: Platform.CitizenNotesPath.UrlString|null,
}|{
  target: Target,
...
/**
 * The page resource loader is a bottleneck for all CitizenNotes-initiated resource loads. For each such load, it keeps a
...
export class PageResourceLoader extends Common.ObjectWrapper.ObjectWrapper<EventTypes> {
  ...
  static makeKey(url: Platform.CitizenNotesPath.UrlString, initiator: PageResourceLoadInitiator): string {
  ...
  async loadResource(url: Platform.CitizenNotesPath.UrlString, initiator: PageResourceLoadInitiator): Promise<{
    content: string,
  }> {
  ...
  private async dispatchLoad(url: Platform.CitizenNotesPath.UrlString, initiator: PageResourceLoadInitiator): Promise<{
    success: boolean,
    content: string,
    errorDescription: Host.ResourceLoader.LoadErrorDescription,
  }> {
    ...
    const eligibleForLoadFromTarget = getLoadThroughTargetSetting().get() && parsedURL && parsedURL.scheme !== 'file' &&
        parsedURL.scheme !== 'data' && parsedURL.scheme !== 'citizennotes';
  ...
  private async loadFromTarget(
      target: Target, frameId: Protocol.Page.FrameId|null, url: Platform.CitizenNotesPath.UrlString): Promise<{
    success: boolean,
    content: string,
    errorDescription: {
      statusCode: number,
