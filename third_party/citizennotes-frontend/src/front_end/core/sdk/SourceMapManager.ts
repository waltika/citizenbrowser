...
export class SourceMapManager<T extends FrameAssociated> extends Common.ObjectWrapper.ObjectWrapper<EventTypes<T>> {
  ...
  private static getBaseUrl(target: Target|null): Platform.CitizenNotesPath.UrlString {
    while (target && target.type() !== Type.Frame) {
      target = target.parentTarget();
    }
    return target?.inspectedURL() ?? Platform.CitizenNotesPath.EmptyUrlString;
  }

  static resolveRelativeSourceURL(target: Target|null, url: Platform.CitizenNotesPath.UrlString):
  ...
  attachSourceMap(
      client: T, relativeSourceURL: Platform.CitizenNotesPath.UrlString, relativeSourceMapURL: string|undefined): void {
...
async function loadSourceMap(
    url: Platform.CitizenNotesPath.UrlString, initiator: PageResourceLoadInitiator): Promise<SourceMapV3> {
  try {
...
type ClientData = {
  relativeSourceURL: Platform.CitizenNotesPath.UrlString,
