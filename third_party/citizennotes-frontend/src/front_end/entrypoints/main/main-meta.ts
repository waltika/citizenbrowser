...
const UIStrings = {
  ...
  /**
   *@description Name of the default set of CitizenNotes keyboard shortcuts
   */
  citizennotesDefault: 'CitizenNotes (Default)',
  /**
   * @description Title of the language setting that allows users to switch the locale
   * in which CitizenNotes is presented.
   */
  language: 'Language:',
  /**
   * @description Users can choose this option when picking the language in which
   * CitizenNotes is presented. Choosing this option means that the CitizenNotes language matches
   ...
   * of syncing CitizenNotes settings via Chrome Sync.
   */
...
// `inspector_main` belongs to the `citizennotes_app`).
...
UI.ActionRegistration.registerActionExtension({
  ...
  category: UI.ActionRegistration.ActionCategory.GLOBAL,
  title: i18nLazyString(UIStrings.reloadCitizennotes),
  async loadActionDelegate() {
...
Common.Settings.registerSettingExtension({
  ...
  options: [
    ...
    ...i18n.i18n.getAllSupportedCitizenNotesLocales().sort().map(locale => createOptionForLocale(locale)),
  ],
  reloadRequired: true,
});
...
Common.Settings.registerSettingExtension({
  ...
  settingType: Common.Settings.SettingType.ENUM,
  defaultValue: 'citizenNotesDefault',
  options: [
    {
      value: 'citizenNotesDefault',
      title: i18nLazyString(UIStrings.citizennotesDefault),
      text: i18nLazyString(UIStrings.citizennotesDefault),
    },
    {
      value: 'vsCode',
...
function createLazyLocalizedLocaleSettingText(localeString: string): () => Common.UIString.LocalizedString {
  return (): Common.UIString.LocalizedString =>
             i18n.i18n.getLocalizedLanguageRegion(localeString, i18n.CitizenNotesLocale.CitizenNotesLocale.instance());
}
...
Common.Settings.registerSettingExtension({
  ...
  // This name must be kept in sync with CitizenNotesSettings::kSyncCitizenNotesPreferencesFrontendName.
