...
/**
 * This monkey patches console functions in CitizenNotes context so the console
 ...
 */
export function _printCitizenNotesConsole() {
  if (isDebugTest()) {
    return;
  }
  console.log = (...args) => {
...
export function textContentWithoutStyles(node) {
  ...
  while (currentNode.traverseNextNode(node)) {
    currentNode = currentNode.traverseNextNode(
        node, currentNode.tagName === 'CITIZENNOTES-CSS-LENGTH' || currentNode.tagName === 'CITIZENNOTES-ICON');
 ...
 * (i.e. http/tests/citizennotes/resources/inspected-page.html).
 * @param {string} html
 * @return {!Promise<*>}
 */
...
export const formatters = {
  ...
  formatAsURL(value) {
    ...
    const lastIndex = value.lastIndexOf('citizennotes/');
    if (lastIndex < 0) {
      return value;
    }
