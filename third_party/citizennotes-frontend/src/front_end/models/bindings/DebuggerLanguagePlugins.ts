...
const UIStrings = {
   ...
   *@example {C/C++ CitizenNotes Support (DWARF)} PH1
...
function getRawLocation(callFrame: SDK.DebuggerModel.CallFrame): Chrome.CitizenNotes.RawLocation {
...
class SourceScopeRemoteObject extends SDK.RemoteObject.RemoteObjectImpl {
  variables: Chrome.CitizenNotes.Variable[];
...
export class ExtensionRemoteObject extends SDK.RemoteObject.RemoteObject {
  private readonly extensionObject: Chrome.CitizenNotes.RemoteObject;
  private readonly plugin: DebuggerLanguagePlugin;
  readonly callFrame: SDK.DebuggerModel.CallFrame;
  constructor(
      callFrame: SDK.DebuggerModel.CallFrame, extensionObject: Chrome.CitizenNotes.RemoteObject,
      plugin: DebuggerLanguagePlugin) {
    super();
...
export class DebuggerLanguagePluginManager implements
  ...
  readonly #rawModuleHandles: Map<string, {
    ...
    addRawModulePromise: Promise<Array<Platform.CitizenNotesPath.UrlString>|{missingSymbolFiles: string[]}>,
  }>;
  ...
  uiSourceCodeForURL(debuggerModel: SDK.DebuggerModel.DebuggerModel, url: Platform.CitizenNotesPath.UrlString):
    ...
    try {
      ...
      for (const sourceLocation of sourceLocations) {
        const uiSourceCode = this.uiSourceCodeForURL(
            script.debuggerModel, sourceLocation.sourceFileURL as Platform.CitizenNotesPath.UrlString);
        if (!uiSourceCode) {
          continue;
        }
  ...
  setDebugInfoURL(script: SDK.Script.Script, externalURL: Platform.CitizenNotesPath.UrlString): void {
    if (this.hasPluginForScript(script)) {
      return;
    }
    ...
    for (const plugin of this.#plugins) {
      ...
      if (!rawModuleHandle) {
        const sourceFileURLsPromise =
            (async(): Promise<Platform.CitizenNotesPath.UrlString[]|{missingSymbolFiles: string[]}> => {
              ...
              try {
                ...
                const sourceFileURLs = addModuleResult as Platform.CitizenNotesPath.UrlString[];
  ...
  getSourcesForScript(script: SDK.Script.Script):
      Promise<Array<Platform.CitizenNotesPath.UrlString>|{missingSymbolFiles: string[]}|undefined> {
    const rawModuleId = rawModuleIdForScript(script);
    const rawModuleHandle = this.#rawModuleHandles.get(rawModuleId);
    if (rawModuleHandle) {
  ...
  async getFunctionInfo(script: SDK.Script.Script, location: SDK.DebuggerModel.Location):
      Promise<{frames: Array<Chrome.CitizenNotes.FunctionInfo>}|{missingSymbolFiles: string[]}|null> {
    const {rawModuleId, plugin} = await this.rawModuleIdAndPluginForScript(script);
    if (!plugin) {
      return null;
    }

    const rawLocation: Chrome.CitizenNotes.RawLocation = {
      rawModuleId,
...
class ModelData {
  ...
  addSourceFiles(script: SDK.Script.Script, urls: Platform.CitizenNotesPath.UrlString[]): void {
...
export interface DebuggerLanguagePlugin extends Chrome.CitizenNotes.LanguageExtensionPlugin {
  name: string;
