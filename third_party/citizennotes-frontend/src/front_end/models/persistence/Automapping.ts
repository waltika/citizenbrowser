...
class FilePathIndex {
  ...
  addPath(path: Platform.CitizenNotesPath.UrlString): void {
    const reversePathParts = path.split('/').reverse();
    this.#reversedIndex.add(reversePathParts);
  }

  removePath(path: Platform.CitizenNotesPath.UrlString): void {
    const reversePathParts = path.split('/').reverse();
    this.#reversedIndex.remove(reversePathParts);
  }

  similarFiles(networkPath: Platform.CitizenNotesPath.EncodedPathString): Platform.CitizenNotesPath.UrlString[] {
    ...
    return this.#reversedIndex.words(longestCommonPrefix)
               .map(reversePathParts => reversePathParts.reverse().join('/')) as Platform.CitizenNotesPath.UrlString[];
  }
}
...
class FolderIndex {
  ...
  addFolder(path: Platform.CitizenNotesPath.UrlString): boolean {
  ...
  removeFolder(path: Platform.CitizenNotesPath.UrlString): boolean {
    const pathParts = this.#removeTrailingSlash(path).split('/');
    const pathForCount = pathParts.join('/');
    const count = this.#folderCount.get(pathForCount) ?? 0;
    if (!count) {
  ...
  closestParentFolder(path: Platform.CitizenNotesPath.UrlString): Platform.CitizenNotesPath.UrlString {
    const pathParts = path.split('/');
    const commonPrefix = this.#index.longestPrefix(pathParts, /* fullWordOnly */ true);
    return commonPrefix.join('/') as Platform.CitizenNotesPath.UrlString;
  }

  #removeTrailingSlash(path: Platform.CitizenNotesPath.UrlString): Platform.CitizenNotesPath.UrlString {
    if (path.endsWith('/')) {
...
class FileSystemUISourceCodes {
  private readonly sourceCodes: Map<Platform.CitizenNotesPath.UrlString, Workspace.UISourceCode.UISourceCode>;

  constructor() {
    this.sourceCodes = new Map();
  }

  private getPlatformCanonicalFileUrl(path: Platform.CitizenNotesPath.UrlString): Platform.CitizenNotesPath.UrlString {
  ...
  get(fileUrl: Platform.CitizenNotesPath.UrlString): Workspace.UISourceCode.UISourceCode|undefined {
    fileUrl = this.getPlatformCanonicalFileUrl(fileUrl);
    return this.sourceCodes.get(fileUrl);
  }

  delete(fileUrl: Platform.CitizenNotesPath.UrlString): void {
    fileUrl = this.getPlatformCanonicalFileUrl(fileUrl);
