...
export class NetworkPersistenceManager extends Common.ObjectWrapper.ObjectWrapper<EventTypes> implements
  ...
  encodedPathFromUrl(url: Platform.CitizenNotesPath.UrlString, ignoreInactive?: boolean):
  ...
  rawPathFromUrl(url: Platform.CitizenNotesPath.UrlString, ignoreInactive?: boolean): Platform.CitizenNotesPath.RawPathString {
    if ((!this.activeInternal && !ignoreInactive) || !this.projectInternal) {
      return Platform.CitizenNotesPath.EmptyRawPathString;
    }
    ...
    const projectPath =
        FileSystemWorkspaceBinding.fileSystemPath(this.projectInternal.id() as Platform.CitizenNotesPath.UrlString);
    ...
    return Common.ParsedURL.ParsedURL.join(encodedPathParts as Platform.CitizenNotesPath.RawPathString[], '/');
  }

  static encodeEncodedPathToLocalPathParts(encodedPath: Platform.CitizenNotesPath.EncodedPathString): string[] {
    const encodedParts = [];
  ...
  static #fileNamePartsFromEncodedPath(encodedPath: Platform.CitizenNotesPath.EncodedPathString): string[] {
    encodedPath = Common.ParsedURL.ParsedURL.urlWithoutHash(encodedPath) as Platform.CitizenNotesPath.EncodedPathString;
  ...
  fileUrlFromNetworkUrl(url: Platform.CitizenNotesPath.UrlString, ignoreInactive?: boolean):
      Platform.CitizenNotesPath.UrlString {
    if (!this.projectInternal) {
      return Platform.CitizenNotesPath.EmptyUrlString;
    }
  ...
  getHeadersUISourceCodeFromUrl(url: Platform.CitizenNotesPath.UrlString): Workspace.UISourceCode.UISourceCode|null {
  ...
  async getOrCreateHeadersUISourceCodeFromUrl(url: Platform.CitizenNotesPath.UrlString):
  ...
  private fileCreatedForTest(_path: Platform.CitizenNotesPath.EncodedPathString, _fileName: string): void {
  }
  ...
  // because the fix here has no effect on non-CitizenNotes CDP clients.
  ...
  private async networkUISourceCodeAdded(uiSourceCode: Workspace.UISourceCode.UISourceCode): Promise<void> {
    ...
    const url = Common.ParsedURL.ParsedURL.urlWithoutHash(uiSourceCode.url()) as Platform.CitizenNotesPath.UrlString;
  ...
  #doubleDecodeEncodedPathString(relativePath: Platform.CitizenNotesPath.EncodedPathString):
      {singlyDecodedPath: Platform.CitizenNotesPath.EncodedPathString, decodedPath: Platform.CitizenNotesPath.RawPathString} {
    ...
    const singlyDecodedPath = this.decodeLocalPathToUrlPath(relativePath) as Platform.CitizenNotesPath.EncodedPathString;
    const decodedPath = this.decodeLocalPathToUrlPath(singlyDecodedPath) as Platform.CitizenNotesPath.RawPathString;
  ...
  async generateHeaderPatterns(uiSourceCode: Workspace.UISourceCode.UISourceCode): Promise<{
    ...
    path: Platform.CitizenNotesPath.EncodedPathString,
  ...
  #generateHeaderPatternsForHttpUrl(
      decodedPath: Platform.CitizenNotesPath.RawPathString, headerOverrides: HeaderOverride[]): {
    headerPatterns: Set<string>,
    overridesWithRegex: HeaderOverrideWithRegex[],
  } {
  ...
  #generateHeaderPatternsForLongUrl(
      decodedPath: Platform.CitizenNotesPath.RawPathString, headerOverrides: HeaderOverride[],
      relativePathPart: Platform.CitizenNotesPath.EncodedPathString): {
    headerPatterns: Set<string>,
    overridesWithRegex: HeaderOverrideWithRegex[],
  } {
    const headerPatterns = new Set<string>();
  ...
  private async onProjectAdded(project: Workspace.Workspace.Project): Promise<void> {
    ...
    const fileSystemPath = FileSystemWorkspaceBinding.fileSystemPath(project.id() as Platform.CitizenNotesPath.UrlString);
    if (!fileSystemPath) {
      return;
    }
    if (this.projectInternal) {
  ...
  #maybeMergeHeadersForPathSegment(
      path: Platform.CitizenNotesPath.EncodedPathString, requestUrl: Platform.CitizenNotesPath.UrlString,
  ...
  handleHeaderInterception(interceptedRequest: SDK.NetworkManager.InterceptedRequest): Protocol.Fetch.HeaderEntry[] {
    ...
    const urlSegments =
        this.rawPathFromUrl(interceptedRequest.request.url as Platform.CitizenNotesPath.UrlString).split('/') as
        Platform.CitizenNotesPath.EncodedPathString[];
    ...
    let path = Platform.CitizenNotesPath.EmptyEncodedPathString;
    result = this.#maybeMergeHeadersForPathSegment(
        path, interceptedRequest.request.url as Platform.CitizenNotesPath.UrlString, result);
  ...
  private async interceptionHandler(interceptedRequest: SDK.NetworkManager.InterceptedRequest): Promise<void> {
    ...
    const path = this.fileUrlFromNetworkUrl(interceptedRequest.request.url as Platform.CitizenNotesPath.UrlString);
