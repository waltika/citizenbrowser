...
export class PersistenceImpl extends Common.ObjectWrapper.ObjectWrapper<EventTypes> {
  ...
  filePathHasBindings(filePath: Platform.CitizenNotesPath.UrlString): boolean {
...
class FilePathPrefixesBindingCounts {
  ...
  private getPlatformCanonicalFilePath(path: Platform.CitizenNotesPath.UrlString): Platform.CitizenNotesPath.UrlString {
    return Host.Platform.isWin() ? Common.ParsedURL.ParsedURL.toLowerCase(path) : path;
  }

  add(filePath: Platform.CitizenNotesPath.UrlString): void {
  ...
  remove(filePath: Platform.CitizenNotesPath.UrlString): void {
    filePath = this.getPlatformCanonicalFilePath(filePath);
    let relative = '';
    for (const token of filePath.split('/')) {
      relative += token + '/';
  ...
  hasBindingPrefix(filePath: Platform.CitizenNotesPath.UrlString): boolean {
    filePath = this.getPlatformCanonicalFilePath(filePath);
