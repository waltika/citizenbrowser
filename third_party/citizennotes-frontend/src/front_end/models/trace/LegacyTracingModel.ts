...
export class TracingModel {
  ...
  readonly #citizenNotesMetadataEventsInternal: Event[];
  ...
  constructor(title?: string) {
    ...
    this.#citizenNotesMetadataEventsInternal = [];
    this.#asyncEvents = [];
  ...
  static isTopLevelEvent(event: CompatibleTraceEvent): boolean {
    return eventHasCategory(event, CitizenNotesTimelineEventCategory) && event.name === 'RunTask' ||
        eventHasCategory(event, LegacyTopLevelEventCategory) ||
        eventHasCategory(event, CitizenNotesMetadataEventCategory) &&
  ...
  static browserMainThread(tracingModel: TracingModel): Thread|null {
    ...
    const tracingStartedInBrowser =
        tracingModel.citizenNotesMetadataEvents().filter(e => e.name === 'TracingStartedInBrowser');
  ...
  citizenNotesMetadataEvents(): Event[] {
    return this.#citizenNotesMetadataEventsInternal;
  }
  ...
  private addEvent(payload: EventPayload): void {
    ...
    if (event.hasCategory(CitizenNotesMetadataEventCategory)) {
      this.#citizenNotesMetadataEventsInternal.push(event);
    }
...
export const CitizenNotesMetadataEventCategory = 'disabled-by-default-citizennotes.timeline';
export const CitizenNotesTimelineEventCategory = 'disabled-by-default-citizennotes.timeline';
