 ...
 * it is an adaptation of the TimelineFrameModel that has been used in CitizenNotes
...
function entryIsTopLevel(entry: Types.TraceEvents.TraceEventData): boolean {
  const citizennotesTimelineCategory = 'disabled-by-default-citizennotes.timeline';
  return entry.name === Types.TraceEvents.KnownEventName.RunTask && entry.cat.includes(citizennotesTimelineCategory);
}
