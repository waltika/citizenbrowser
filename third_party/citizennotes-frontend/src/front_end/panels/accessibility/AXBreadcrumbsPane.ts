...
const UIStrings = {
  ...
  /**
   *@description Message saying that CitizenNotes must be restarted before the experiment is enabled.
   */
...
export class AXBreadcrumbsPane extends AccessibilitySubPane {
  ...
  constructor(axSidebarView: AccessibilitySidebarView) {
    ...
    if (Root.Runtime.experiments.isEnabled(experiment)) {
      ...
      const feedbackURL = 'https://g.co/citizennotes/a11y-tree-feedback';
      previewToggle.data = {
        name,
