...
export class AppManifestView extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.VBox>(UI.Widget.VBox)
  ...
  constructor(
    ...
    this.emptyView.appendLink('https://web.dev/add-manifest/' as Platform.CitizenNotesPath.UrlString);
  ...
  private async renderManifest(
      url: Platform.CitizenNotesPath.UrlString, data: string|null, errors: Protocol.Page.AppManifestError[],
    ...
    if (hasNewNoteUrl) {
      const completeURL = (Common.ParsedURL.ParsedURL.completeURL(url, newNoteUrl) as Platform.CitizenNotesPath.UrlString);
      const link = Components.Linkifier.Linkifier.linkifyURL(
    ...
    for (const shortcut of shortcuts) {
      ...
      const shortcutUrl = Common.ParsedURL.ParsedURL.completeURL(url, shortcut.url) as Platform.CitizenNotesPath.UrlString;
      const link = Components.Linkifier.Linkifier.linkifyURL(
  ...
  private async loadImage(url: Platform.CitizenNotesPath.UrlString): Promise<{
  ...
  private async appendImageResourceToSection(
      ...
      baseUrl: Platform.CitizenNotesPath.UrlString, imageResource: any, section: UI.ReportView.Section,
  ...
  private async appendWindowControlsToSection(
      overlayModel: SDK.OverlayModel.OverlayModel, url: Platform.CitizenNotesPath.UrlString,
