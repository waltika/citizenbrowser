...
const UIStrings = {
   ...
   */
  testPushMessageFromCitizennotes: 'Test push message from CitizenNotes.',
  /**
...
export class ServiceWorkersView extends UI.Widget.VBox implements
  ...
  constructor() {
    ...
    const seeOthers =
        UI.Fragment
            .html`<a class="citizennotes-link" role="link" tabindex="0" href="chrome://serviceworker-internals" target="_blank" style="display: inline; cursor: pointer;">${
                i18nString(UIStrings.seeAllRegistrations)}</a>`;
    self.onInvokeElement(seeOthers, event => {
      const rootTarget = SDK.TargetManager.TargetManager.instance().rootTarget();
      rootTarget &&
          void rootTarget.targetAgent().invoke_createTarget({url: 'chrome://serviceworker-internals?citizennotes'});
      event.consume(true);
    });
...
export class Section {
  ...
  constructor(
    ...
    this.pushNotificationDataSetting = Common.Settings.Settings.instance().createLocalSetting(
        'pushData', i18nString(UIStrings.testPushMessageFromCitizennotes));
    this.syncTagNameSetting =
        Common.Settings.Settings.instance().createLocalSetting('syncTagName', 'test-tag-from-citizennotes');
    this.periodicSyncTagNameSetting =
        Common.Settings.Settings.instance().createLocalSetting('periodicSyncTagName', 'test-tag-from-citizennotes');
  ...
  private updateSourceField(version: SDK.ServiceWorkerManager.ServiceWorkerVersion): void {
    ...
    if (this.registration.errors.length) {
      ...
      errorsLabel.classList.add('citizennotes-link', 'link');
  ...
  private createLink(parent: Element, title: string, listener: () => void, className?: string, useCapture?: boolean):
    ...
    button.classList.add('link', 'citizennotes-link');
