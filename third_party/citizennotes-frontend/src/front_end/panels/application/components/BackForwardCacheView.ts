...
export class BackForwardCacheView extends LegacyWrapper.LegacyWrapper.WrappableComponent {
  static readonly litTagName = LitHtml.literal`citizennotes-resources-back-forward-cache-view`;
  ...
  async #navigateAwayAndBack(): Promise<void> {
    ...
    void resourceTreeModel.navigate('chrome://terms' as Platform.CitizenNotesPath.UrlString);
  }
  ...
  #renderMainFrameInformation(): LitHtml.TemplateResult {
    ...
    // Prevent running BFCache test on the CitizenNotes window itself via CitizenNotes on CitizenNotes
    const isTestingForbidden = Common.ParsedURL.schemeIs(frame.url, 'citizennotes:');
    // clang-format off
    return LitHtml.html`
    ...
    rows.push(...details.map(
        detail => LitHtml.html`${
            linkifier.linkifyScriptLocation(
                null, null, detail.url as Platform.CitizenNotesPath.UrlString, detail.lineNumber, {
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-resources-back-forward-cache-view', BackForwardCacheView);

declare global {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  interface HTMLElementTagNameMap {
    'citizennotes-resources-back-forward-cache-view': BackForwardCacheView;
  }
}
