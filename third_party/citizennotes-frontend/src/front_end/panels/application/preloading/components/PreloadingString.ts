...
const UIStrings = {
  ...
  /**
   *  Description text for PrerenderFinalStatus::kPrerenderingDisabledByCitizenNotes.
   */
  prerenderFinalStatusPrerenderingDisabledByCitizenNotes:
      'The prerender was not performed because CitizenNotes has been used to disable prerendering.',
  /**
...
export function prefetchFailureReason({prefetchStatus}: SDK.PreloadingModel.PrefetchAttempt): string|null {
  ...
  switch (prefetchStatus) {
    ...
    case Protocol.Preload.PrefetchStatus.PrefetchResponseUsed:
      return null;
    // Holdback related status is expected to be overridden when CitizenNotes is
    // opened.
...
export function prerenderFailureReason(attempt: SDK.PreloadingModel.PrerenderAttempt): string|null {
  ...
  switch (attempt.prerenderStatus) {
    ...
    case Protocol.Preload.PrerenderFinalStatus.TriggerDestroyed:
      // After https://crrev.com/c/4515841, this won't occur if CitizenNotes is opened.
    ...
    case Protocol.Preload.PrerenderFinalStatus.ActivationNavigationDestroyedBeforeSuccess:
      // Should not occur in CitizenNotes because tab is alive?
      return i18n.i18n.lockedString('Internal error');
    ...
    case Protocol.Preload.PrerenderFinalStatus.PrerenderingDisabledByCitizenNotes:
      return i18nString(UIStrings.prerenderFinalStatusPrerenderingDisabledByCitizenNotes);
...
export function ruleSetLocationShort(
    ruleSet: Protocol.Preload.RuleSet, pageURL: Platform.CitizenNotesPath.UrlString): string {
  const url = ruleSet.url === undefined ? pageURL : ruleSet.url as Platform.CitizenNotesPath.UrlString;
