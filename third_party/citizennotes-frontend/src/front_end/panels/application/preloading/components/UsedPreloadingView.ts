...
export class UsedPreloadingView extends LegacyWrapper.LegacyWrapper.WrappableComponent<UI.Widget.VBox> {
  static readonly litTagName = LitHtml.literal`citizennotes-resources-used-preloading-view`;

  readonly #shadow = this.attachShadow({mode: 'open'});
  #data: UsedPreloadingViewData = {
    pageURL: '' as Platform.CitizenNotesPath.UrlString,
    previousAttempts: [],
    currentAttempts: [],
  };
  ...
  #maybeMismatchedHTTPHeadersSections(): LitHtml.LitTemplate {
    ...
    if (attempt.key.url !== this.#data.pageURL) {
      ...
      // TODO(crbug.com/1456673): remove this check once CitizenNotes support embedder-triggered prerender or prerender
      ...
      <${ReportView.ReportView.ReportSection.litTagName}>
        <div>
          ...
          <div class="reveal-links">
            <button class="link citizennotes-link" @click=${revealRuleSetView}>
              ${i18nString(UIStrings.viewAllRules)}
            </button>
           ・
            <button class="link citizennotes-link" @click=${revealAttemptViewWithFilter}>
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-resources-used-preloading-view', UsedPreloadingView);

declare global {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  interface HTMLElementTagNameMap {
    'citizennotes-resources-used-preloading-view': UsedPreloadingView;
  }
}
