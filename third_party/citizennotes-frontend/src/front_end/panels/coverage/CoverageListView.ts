...
export class CoverageListView extends UI.Widget.VBox {
  ...
  updateSourceNodes(
      sourcesURLCoverageInfo: Map<Platform.CitizenNotesPath.UrlString, SourceURLCoverageInfo>, maxSize: number,
      node: GridNode): void {
...
function getPercentageFormatter(): Intl.NumberFormat {
  if (!percentageFormatter) {
    percentageFormatter = new Intl.NumberFormat(i18n.CitizenNotesLocale.CitizenNotesLocale.instance().locale, {
      style: 'percent',
...
export class GridNode extends DataGrid.SortableDataGrid.SortableDataGridNode<GridNode> {
  ...
  private url: Platform.CitizenNotesPath.UrlString;
