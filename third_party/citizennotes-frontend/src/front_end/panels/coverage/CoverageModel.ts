...
export class CoverageModel extends SDK.SDKModel.SDKModel<EventTypes> {
  ...
  private coverageByURL: Map<Platform.CitizenNotesPath.UrlString, URLCoverageInfo>;
  ...
  getCoverageForUrl(url: Platform.CitizenNotesPath.UrlString): URLCoverageInfo|null {
  ...
  private calculateSizeForSources(sourceMap: SDK.SourceMap.SourceMap, text: TextUtils.Text.Text, contentLength: number):
      [
        Map<Platform.CitizenNotesPath.UrlString, number>,
        SourceSegment[],
      ] {
    ...
    const sourceSizeMap = new Map<Platform.CitizenNotesPath.UrlString, number>();
    ...
    sourceSegments.push({end: totalSegmentSize, sourceUrl: '' as Platform.CitizenNotesPath.UrlString});
    ...
    for (let i = 0; i < mappings.length; i++) {
      ...
      if (curEntry.sourceURL !== lastEntry.sourceURL) {
        if (text) {
          ...
          sourceSegments.push(
              {end: endOffsetForLastEntry, sourceUrl: lastEntry.sourceURL || '' as Platform.CitizenNotesPath.UrlString});
        } else {
          sourceSegments.push(
              {end: totalSegmentSize, sourceUrl: lastEntry.sourceURL || '' as Platform.CitizenNotesPath.UrlString});
        }
      }
      lastEntry = curEntry;
      ...
      if (i === mappings.length - 1) {
        sourceSegments.push(
            {end: contentLength, sourceUrl: curEntry.sourceURL || '' as Platform.CitizenNotesPath.UrlString});
      }
    }
  ...
  private addCoverageForSource(
      url: Platform.CitizenNotesPath.UrlString, size: number, type: CoverageType,
...
export class URLCoverageInfo extends Common.ObjectWrapper.ObjectWrapper<URLCoverageInfo.EventTypes> {
  private readonly urlInternal: Platform.CitizenNotesPath.UrlString;
  ...
  sourcesURLCoverageInfo: Map<Platform.CitizenNotesPath.UrlString, SourceURLCoverageInfo> = new Map();
  sourceSegments: SourceSegment[]|undefined;

  constructor(url: Platform.CitizenNotesPath.UrlString) {
    super();
...
export class SourceURLCoverageInfo extends URLCoverageInfo {
  ...
  constructor(sourceUrl: Platform.CitizenNotesPath.UrlString, generatedUrlCoverage: URLCoverageInfo) {
    super(sourceUrl);
...
export class CoverageInfo {
  ...
  sourceUsedSizeMap: Map<Platform.CitizenNotesPath.UrlString, number> = new Map();
  sourceDeltaMap: Map<Platform.CitizenNotesPath.UrlString, number> = new Map();
  sourceUsedRangeMap = new Map<Platform.CitizenNotesPath.UrlString, RangeOffset[]>();
