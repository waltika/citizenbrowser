...
export class ImagePreviewPopover {
    ...
    return {
      ...
      show: async(popover: UI.GlassPane.GlassPane): Promise<boolean> => {
        ...
        const preview = await Components.ImagePreview.ImagePreview.build(
            node.domModel().target(), href as Platform.CitizenNotesPath.UrlString, true,
  ...
  static setImageUrl(element: Element, url: Platform.CitizenNotesPath.UrlString): Element {
    elementToURLMap.set(element, url);
    return element;
  }

  static getImageURL(element: Element): Platform.CitizenNotesPath.UrlString|undefined {
    return elementToURLMap.get(element);
  }
}

const elementToURLMap = new WeakMap<Element, Platform.CitizenNotesPath.UrlString>();
