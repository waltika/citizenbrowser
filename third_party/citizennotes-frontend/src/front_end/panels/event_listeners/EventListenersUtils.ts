...
export async function frameworkEventListeners(object: SDK.RemoteObject.RemoteObject):
  ...
  function frameworkEventListenersImpl(this: Object): {eventListeners: Array<EventListenerObjectInInspectedPage>} {
    ...
    try {
      ...
      if (self.citizennotesFrameworkEventListeners && isArrayLike(self.citizennotesFrameworkEventListeners)) {
        // @ts-ignore Here because of layout tests.
        fetchers = fetchers.concat(self.citizennotesFrameworkEventListeners);
      }
    } catch (e) {
      errorLines.push('citizennotesFrameworkEventListeners call produced error: ' + toString(e));
    }
