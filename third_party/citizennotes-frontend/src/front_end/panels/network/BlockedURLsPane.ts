...
export class BlockedURLsPane extends UI.Widget.VBox implements
  ...
  addPattern(): void {
    ...
    this.list.addNewItem(0, {url: Platform.CitizenNotesPath.EmptyUrlString, enabled: true});
  }
  ...
  commitEdit(
      ...
      isNew: boolean): void {
    const url = editor.control('url').value as Platform.CitizenNotesPath.UrlString;
    const patterns = this.manager.blockedPatterns();
    if (isNew) {
