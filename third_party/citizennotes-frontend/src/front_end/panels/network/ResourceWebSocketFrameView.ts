...
export class ResourceWebSocketFrameNode extends DataGrid.SortableDataGrid.SortableDataGridNode<unknown> {
  private readonly url: Platform.CitizenNotesPath.UrlString;
  ...
  constructor(url: Platform.CitizenNotesPath.UrlString, frame: SDK.NetworkRequest.WebSocketFrame) {
    ...
    if (!this.binaryViewInternal) {
      if (this.dataTextInternal.length > 0) {
        this.binaryViewInternal = new BinaryResourceView(
            this.dataTextInternal, Platform.CitizenNotesPath.EmptyUrlString, Common.ResourceType.resourceTypes.WebSocket);
      }
    }
