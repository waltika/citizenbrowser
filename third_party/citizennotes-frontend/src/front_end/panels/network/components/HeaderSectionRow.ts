...
export const compareHeaders = (first: string|null|undefined, second: string|null|undefined): boolean => {
  ...
  // Tab characters are invalid in headers (and CitizenNotes shows a warning for
...
export class HeaderSectionRow extends HTMLElement {
  static readonly litTagName = LitHtml.literal`citizennotes-header-section-row`;
  ...
  override focus(): void {
    requestAnimationFrame(() => {
      const editableName = this.#shadow.querySelector<HTMLElement>('.header-name citizennotes-editable-span');
      editableName?.focus();
    });
  }
  ...
  #maybeRenderBlockedDetailsLink(blockedDetails?: BlockedDetailsDescriptor): LitHtml.LitTemplate {
    if (blockedDetails?.reveal) {
      ...
      // clang-format off
      return html`
        <div class="citizennotes-link" @click=${blockedDetails.reveal}>
  ...
  #onRemoveOverrideClick(): void {
    ...
    const headerValueElement = this.#shadow.querySelector('.header-value citizennotes-editable-span') as EditableSpan;
  ...
  #onKeyDown(event: Event): void {
    ...
    if (keyboardEvent.key === 'Escape') {
      event.consume();
      if (target.matches('.header-name citizennotes-editable-span')) {
        target.value = this.#header?.name || '';
        this.#onHeaderNameEdit(event);
      } else if (target.matches('.header-value citizennotes-editable-span')) {
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-header-section-row', HeaderSectionRow);

declare global {
  interface HTMLElementTagNameMap {
    'citizennotes-header-section-row': HeaderSectionRow;
    'citizennotes-editable-span': EditableSpan;
  }
