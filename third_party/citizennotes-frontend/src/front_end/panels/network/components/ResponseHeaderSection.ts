...
export class ResponseHeaderSection extends HTMLElement {
  static readonly litTagName = LitHtml.literal`citizennotes-response-header-section`;
  ...
  set data(data: ResponseHeaderSectionData) {
    ...
    // the Chromium backend. CitizenNotes therefore uses previously stored values.
  ...
  #fileNameFromUrl(url: Platform.CitizenNotesPath.UrlString): Platform.CitizenNotesPath.RawPathString {
    const rawPath =
  ...
  #removeEntryFromOverrides(
      rawFileName: Platform.CitizenNotesPath.RawPathString, headerName: Platform.StringUtilities.LowerCaseString,
  ...
  #onAddHeaderClick(): void {
    ...
    const rows = this.#shadow.querySelectorAll<HeaderSectionRow>('citizennotes-header-section-row');
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-response-header-section', ResponseHeaderSection);

declare global {
  interface HTMLElementTagNameMap {
    'citizennotes-response-header-section': ResponseHeaderSection;
  }
}
