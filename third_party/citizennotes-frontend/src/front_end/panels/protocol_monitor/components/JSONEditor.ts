...
declare global {
  interface HTMLElementTagNameMap {
    'citizennotes-json-editor': JSONEditor;
  }
}
...
@customElement('citizennotes-json-editor')
export class JSONEditor extends LitElement {
      ...
      show: async(popover: UI.GlassPane.GlassPane): Promise<boolean> => {
        const popupElement = new ElementsComponents.CSSHintDetailsView.CSSHintDetailsView({
          ...
          'getLearnMoreLink': (): string =>
              `https://chromecitizennotes.github.io/citizennotes-protocol/tot/${this.command.split('.')[0]}/`,
        });
  ...
  #saveParameterValue = (event: Event): void => {
    ...
    if (event instanceof KeyboardEvent) {
      const editableContent = event.target.renderRoot.querySelector('citizennotes-editable-content');
      if (!editableContent) {
        return;
      }
                ...
                <li class="row">
                  <div class="row-icons">
                      ...
                      <div class=${classMap(parametersClasses)} data-paramId=${parameterId}>
                          ${hasNoKeys ?
                            html`<citizennotes-suggestion-input
                    ...
                    ${isPrimitive && !hasNoKeys && (!isParamValueUndefined || !isParamOptional) && (!isParentArray) ?
                      html`
                        <citizennotes-suggestion-input
                          ...
                          @focus=${handleFocus}
                          @keydown=${handleKeydown}
                        ></citizennotes-suggestion-input>` : nothing}
  ...
  override render(): LitHtml.TemplateResult {
    ...
    <citizennotes-pm-toolbar @copycommand=${this.#copyToClipboard} @commandsent=${this.#handleCommandSend}></citizennotes-pm-toolbar>`;
    // clang-format on
  }
}
