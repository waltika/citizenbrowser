...
Recorder is a panel in CitizenNotes that allows recording and replaying user actions.
...
- `models` folder contains the "business logic" part of Recorder wrapping CitizenNotes SDK modules and Puppeteer to provide record/replay/import/export functionality. The name `models` is historical and, probably, a better name could be found: record/replay used to be implemented as a single SDK Model and was placed in the `models` folder in CitizenNotes before ending up here.
...
3. Playback: covers the scenaraio when a user has a recording and hits the Replay button:
   ...
   2. The instance suspends of the CitizenNotes interactions with the targets (i.e., `SDK.TargetManager.TargetManager.instance().suspendAllTargets();`) and creates a `PuppeteerConnection`.
   ...
   3. The `PuppeteerConnection` is sending and receiving messages on top of the original CitizenNotes connection (which could be a web socket or a binding connection).
