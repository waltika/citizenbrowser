...
const GET_EXTENSIONS_URL = 'https://goo.gle/recorder-extension-list' as Platform.CitizenNotesPath.UrlString;

declare global {
  interface HTMLElementTagNameMap {
    'citizennotes-recorder-controller': RecorderController;
  }
...
@customElement('citizennotes-recorder-controller')
export class RecorderController extends LitElement {
  ...
  async #onRecordingStarted(event: Components.CreateRecordingView.RecordingStartedEvent): Promise<void> {
    ...
    this.currentRecordingSession.addEventListener(
        Models.RecordingSession.Events.RecordingUpdated, async ({data}: {data: Models.Schema.UserFlow}) => {
          ...
          const recordingView = this.shadowRoot?.querySelector('citizennotes-recording-view');
  ...
  async #handleAddAssertionEvent(): Promise<void> {
    ...
    this.renderRoot.querySelector('citizennotes-recording-view')
        ?.shadowRoot?.querySelector('.section:last-child citizennotes-step-view:last-of-type')
    ...
    switch (actionId) {
      ...
      case Actions.RecorderActions.StartRecording:
        if (this.currentPage !== Pages.CreateRecordingPage && !this.isRecording) {
          ...
          const view = this.renderRoot.querySelector('citizennotes-create-recording-view');
          if (view) {
      ...
      case Actions.RecorderActions.ToggleCodeView: {
        const view = this.renderRoot.querySelector('citizennotes-recording-view');
        if (view) {
          view.showCodeToggle();
        }
        return;
