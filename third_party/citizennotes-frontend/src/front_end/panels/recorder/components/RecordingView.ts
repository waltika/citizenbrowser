...
declare global {
  interface HTMLElementTagNameMap {
    'citizennotes-recording-view': RecordingView;
  }
}
...
export class RecordingView extends HTMLElement {
  static readonly litTagName = LitHtml.literal`citizennotes-recording-view`;
  ...
  #onSelectMenuLabelClick = (event: Event): void => {
    ...
    if (target.matches('.wrapping-label')) {
      target.querySelector('citizennotes-select-menu')?.click();
    }
  };
  ...
  #highlightCodeForStep = (step: Models.Schema.Step, scroll = false): void => {
    ...
    const editor = this.#shadow.querySelector('citizennotes-text-editor') as | TextEditor.TextEditor.TextEditor | undefined;
    if (!editor) {
      return;
    }
            ...
            <div class="section">
              ...
              <div class="content">
                <div class="steps">
                  ...
                  ${!this.#recordingTogglingInProgress && this.#isRecording && i === this.#sections.length - 1 ? LitHtml.html`<citizennotes-button
                  ...
                  >${i18nString(UIStrings.addAssertion)}</citizennotes-button>` : undefined}
                  ${
