...
export class SelectorPicker implements SDK.TargetManager.Observer {
  ...
  async #injectApplicationScript(target: SDK.Target.Target): Promise<void> {
    ...
    const script = `${injectedScript};CitizenNotesRecorder.startSelectorPicker({getAccessibleName, getAccessibleRole}, ${
    ...
    const [{identifier}] = await Promise.all([
      target.pageAgent().invoke_addScriptToEvaluateOnNewDocument({
        source: script,
        worldName: Util.CITIZENNOTES_RECORDER_WORLD_NAME,
        includeCommandLineAPI: true,
      }),
      Models.SDKUtils.evaluateInAllFrames(Util.CITIZENNOTES_RECORDER_WORLD_NAME, target, script),
    ]);
  ...
  async #injectCleanupScript(target: SDK.Target.Target): Promise<void> {
    ...
    const script = 'CitizenNotesRecorder.stopSelectorPicker()';
    await Models.SDKUtils.evaluateInAllFrames(Util.CITIZENNOTES_RECORDER_WORLD_NAME, target, script);
  }
  ...
  async #addBindings(target: SDK.Target.Target): Promise<void> {
    ...
    await model.addBinding({
      name: BINDING_NAME,
      executionContextName: Util.CITIZENNOTES_RECORDER_WORLD_NAME,
    });
  }
