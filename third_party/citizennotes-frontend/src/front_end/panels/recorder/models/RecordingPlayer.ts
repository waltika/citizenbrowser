...
function isPageTarget(target: Protocol.Target.TargetInfo): boolean {
  // Treat CitizenNotes targets as page targets too.
  return (
      Common.ParsedURL.schemeIs(target.url as Platform.CitizenNotesPath.UrlString, 'citizennotes:') || target.type === 'page' ||
  ...
  async play(): Promise<void> {
    ...
    class ExtensionWithBreak extends PuppeteerReplay.PuppeteerRunnerExtension {
      ...
      override async runStep(
        ...
        // When replaying on a CitizenNotes target we skip setViewport and navigate steps
        // because navigation and viewport changes are not supported there.
        if (Common.ParsedURL.schemeIs(page?.url() as Platform.CitizenNotesPath.UrlString, 'citizennotes:') &&
