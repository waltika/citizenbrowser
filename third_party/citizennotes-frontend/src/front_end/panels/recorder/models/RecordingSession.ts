...
export class RecordingSession extends Common.ObjectWrapper.ObjectWrapper<EventTypes> {
  ...
  async #addBindings(target: SDK.Target.Target): Promise<void> {
    ...
    await Promise.all(
        Object.values(RecorderBinding)
            .map(name => runtimeModel.addBinding({name, executionContextName: Util.CITIZENNOTES_RECORDER_WORLD_NAME})));
  }
  ...
  async #injectApplicationScript(target: SDK.Target.Target): Promise<void> {
    ...
    const script = `
      ${injectedScript};CitizenNotesRecorder.startRecording({getAccessibleName, getAccessibleRole}, {
    ...
    const [{identifier}] = await Promise.all([
      target.pageAgent().invoke_addScriptToEvaluateOnNewDocument(
          {source: script, worldName: Util.CITIZENNOTES_RECORDER_WORLD_NAME, includeCommandLineAPI: true}),
      evaluateInAllFrames(Util.CITIZENNOTES_RECORDER_WORLD_NAME, target, script),
    ]);
  ...
  async #injectCleanUpScript(target: SDK.Target.Target): Promise<void> {
    ...
    await evaluateInAllTargets(
        Util.CITIZENNOTES_RECORDER_WORLD_NAME, [...this.#targets.values()], 'CitizenNotesRecorder.stopRecording()');
  }
