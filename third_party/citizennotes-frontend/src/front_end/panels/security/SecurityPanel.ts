...
const UIStrings = {
  ...
  /**
   *@description Message to display in citizennotes security tab when the page you are on triggered a safety tip.
   */
  thisPageIsSuspicious: 'This page is suspicious',
  /**
   *@description Body of message to display in citizennotes security tab when you are viewing a page that triggered a safety tip.
   */
  chromeHasDeterminedThatThisSiteS: 'Chrome has determined that this site could be fake or fraudulent.',
  /**
   *@description Second part of the body of message to display in citizennotes security tab when you are viewing a page that triggered a safety tip.
   */
  ...
  ifYouBelieveThisIsShownInErrorSafety:
      'If you believe this is shown in error please visit https://g.co/chrome/lookalike-warnings.',
  /**
   *@description Title of the citizennotes security tab when the page you are on triggered a safety tip.
   */
...
export class SecurityPanel extends UI.Panel.PanelWithSidebar implements
  ...
  static createHighlightedUrl(url: Platform.CitizenNotesPath.UrlString, securityState: string): Element {
  ...
  showOrigin(origin: Platform.CitizenNotesPath.UrlString): void {
...
export class SecurityPanelSidebarTree extends UI.TreeOutline.TreeOutlineInShadow {
  ...
  addOrigin(origin: Platform.CitizenNotesPath.UrlString, securityState: Protocol.Security.SecurityState): void {
...
export class SecurityMainView extends UI.Widget.VBox {
  ...
  private addMixedContentExplanation(
    ...
    const requestsAnchor = element.createChild('div', 'security-mixed-content citizennotes-link') as HTMLElement;
...
export class SecurityOriginView extends UI.Widget.VBox {
  ...
  constructor(panel: SecurityPanel, origin: Platform.CitizenNotesPath.UrlString, originState: OriginState) {
    super();
    this.panel = panel;
    this.setMinimumSize(200, 100);
    ...
    if (originState.securityDetails) {
      ...
      // named like TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256. The CitizenNotes protocol
      ...
      // named group (e.g. X25519), which the CitizenNotes protocol exposes as
      // "keyExchangeGroup".
      //
...
export type Origin = Platform.CitizenNotesPath.UrlString;
