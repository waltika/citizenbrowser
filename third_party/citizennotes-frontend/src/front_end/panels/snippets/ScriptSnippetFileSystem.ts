...
function escapeSnippetName(name: Platform.CitizenNotesPath.RawPathString): Platform.CitizenNotesPath.EncodedPathString {
  return Common.ParsedURL.ParsedURL.rawPathToEncodedPathString(name);
}

function unescapeSnippetName(name: Platform.CitizenNotesPath.EncodedPathString): Platform.CitizenNotesPath.RawPathString {
...
export class SnippetFileSystem extends Persistence.PlatformFileSystem.PlatformFileSystem {
  ...
  constructor() {
    super('snippet://' as Platform.CitizenNotesPath.UrlString, 'snippets');
  ...
  override async createFile(_path: Platform.CitizenNotesPath.EncodedPathString, _name: Platform.CitizenNotesPath.RawPathString|null):
    ...
    const snippetName =
        i18nString(UIStrings.scriptSnippet, {PH1: nextId}) as string as Platform.CitizenNotesPath.RawPathString;
  ...
  override async deleteFile(path: Platform.CitizenNotesPath.EncodedPathString): Promise<boolean> {
  ...
  override async requestFileContent(path: Platform.CitizenNotesPath.EncodedPathString):
  ...
  override async setFileContent(path: Platform.CitizenNotesPath.EncodedPathString, content: string, _isBase64: boolean):
      Promise<boolean> {
    const name = unescapeSnippetName(Common.ParsedURL.ParsedURL.substring(path, 1));
    const snippets: Snippet[] = this.snippetsSetting.get();
    const snippet = snippets.find(snippet => snippet.name === name);
  ...
  override mimeFromPath(_path: Platform.CitizenNotesPath.UrlString): string {
...
export interface Snippet {
  name: Platform.CitizenNotesPath.RawPathString;
  content: string;
}
