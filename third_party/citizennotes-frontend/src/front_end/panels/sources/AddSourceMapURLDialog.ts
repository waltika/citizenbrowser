...
export class AddDebugInfoURLDialog extends UI.Widget.HBox {
  ...
  private readonly callback: (arg0: Platform.CitizenNotesPath.UrlString) => void;
  private constructor(
      label: Platform.UIString.LocalizedString, callback: (arg0: Platform.CitizenNotesPath.UrlString) => void) {
  ...
  static createAddSourceMapURLDialog(callback: (arg0: Platform.CitizenNotesPath.UrlString) => void): AddDebugInfoURLDialog {
    return new AddDebugInfoURLDialog(i18nString(UIStrings.sourceMapUrl), callback);
  }

  static createAddDWARFSymbolsURLDialog(callback: (arg0: Platform.CitizenNotesPath.UrlString) => void):
  ...
  private done(value: Platform.CitizenNotesPath.UrlString): void {
    this.dialog.hide();
    this.callback(value);
  }

  private apply(): void {
    this.done(this.input.value as Platform.CitizenNotesPath.UrlString);
  }
