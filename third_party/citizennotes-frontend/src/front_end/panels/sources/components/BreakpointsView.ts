...
const UIStrings = {
  /**
   *@description Label for a checkbox to toggle pausing on uncaught exceptions in the breakpoint sidebar of the Sources panel. When the checkbox is checked, CitizenNotes will pause if an uncaught exception is thrown at runtime.
   */
  pauseOnUncaughtExceptions: 'Pause on uncaught exceptions',
  /**
   *@description Label for a checkbox to toggling pausing on caught exceptions in the breakpoint sidebar of the Sources panel. When the checkbox is checked, CitizenNotes will pause if an exception is thrown, but caught (handled) at runtime.
   */
...
export class BreakpointsSidebarController implements UI.ContextFlavorListener.ContextFlavorListener {
  ...
  readonly #collapsedFilesSettings: Common.Settings.Setting<Platform.CitizenNotesPath.UrlString[]>;
  readonly #collapsedFiles: Set<Platform.CitizenNotesPath.UrlString>;
  ...
  expandedStateChanged(url: Platform.CitizenNotesPath.UrlString, expanded: boolean): void {
    if (expanded) {
...
export class BreakpointsView extends LegacyWrapper.LegacyWrapper.WrappableComponent {
  ...
  static readonly litTagName = LitHtml.literal`citizennotes-breakpoint-view`;
  ...
  #urlToDifferentiatingPath: Map<Platform.CitizenNotesPath.UrlString, string> = new Map();
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-breakpoint-view', BreakpointsView);

declare global {
  interface HTMLElementTagNameMap {
    'citizennotes-breakpoint-view': BreakpointsView;
  }
}
