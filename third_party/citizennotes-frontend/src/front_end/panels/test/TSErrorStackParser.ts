...
export interface TSParsedErrorFrame {
  line: string;
  link?: {
 ...
 */
export function testParseSourcePositionsFromErrorStack(
    runtimeModel: SDK.RuntimeModel.RuntimeModel, stack: string): TSParsedErrorFrame[]|null {
  ...
  for (const line of lines) {
    ...
    let url = testParseOrScriptMatch(debuggerModel, splitResult.url);
    if (!url && Common.ParsedURL.ParsedURL.isRelativeURL(splitResult.url)) {
      url = testParseOrScriptMatch(debuggerModel, Common.ParsedURL.ParsedURL.completeURL(baseURL, splitResult.url));
    }
    if (!url) {
      return null;
    }
...
function testParseOrScriptMatch(debuggerModel: SDK.DebuggerModel.DebuggerModel, url: Platform.CitizenNotesPath.UrlString|null):
    Platform.CitizenNotesPath.UrlString|null {
  if (!url) {
    return null;
  }
  ...
  if (debuggerModel.scriptsForSourceURL(fileUrl.href).length) {
    return fileUrl.href as Platform.CitizenNotesPath.UrlString;
  }
  return null;
}
 ...
 */
export function testAugmentErrorStackWithScriptIds(
  parsedFrames: TSParsedErrorFrame[], protocolStackTrace: Protocol.Runtime.StackTrace): void {
  ...
  for (const parsedFrame of parsedFrames) {
    const protocolFrame = protocolStackTrace.callFrames.find(frame => testFramesMatch(parsedFrame, frame));
...
function testFramesMatch(parsedFrame: TSParsedErrorFrame, protocolFrame: Protocol.Runtime.CallFrame): boolean {
  if (!parsedFrame.link) {
    return false;
  }
