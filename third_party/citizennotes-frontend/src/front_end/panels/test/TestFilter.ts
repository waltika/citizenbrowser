...
import {type TestGroupViewMessage, type TestViewMessage} from './TestViewMessage.js';
...
export class TestFilter {
  ...
  constructor(
    ...
    this.levelsMask = levelsMask || TestFilter.defaultLevelsFilterValue();
  }
  ...
  static defaultLevelsFilterValue(): LevelsMask {
    const result = TestFilter.allLevelsFilterValue();
  ...
  clone(): TestFilter {
    ...
    return new TestFilter(this.name, parsedFilters, this.executionContext, levelsMask);
  }
  ...
  shouldBeVisible(viewMessage: TestViewMessage): boolean {
    ...
    if (message.type === SDK.TestModel.FrontendMessageType.Command ||
        ...
        message.type === Protocol.Runtime.TestAPICalledEventType.EndGroup) {
      return true;
    }
    ...
    return this.applyFilter(viewMessage) || this.parentGroupHasMatch(viewMessage.testGroup());
  }

  // A message is visible if there is a match in any of the parent groups' titles.
  parentGroupHasMatch(viewMessage: TestGroupViewMessage|null): boolean {
    if (viewMessage === null) {
      return false;
    }
    return this.applyFilter(viewMessage) || this.parentGroupHasMatch(viewMessage.testGroup());
  ...
  applyFilter(viewMessage: TestViewMessage): boolean {
    const message = viewMessage.testMessage();
        ...
        switch (filter.key) {
          ...
          case FilterType.Source: {
            const sourceNameForMessage = message.source ?
                SDK.TestModel.MessageSourceDisplayName.get((message.source as SDK.TestModel.MessageSource)) :
                message.source;
