 ...
 * Test Standard (https://test.spec.whatwg.org/#formatter). Here we
        ...
        while (codes.length) {
          ...
          switch (code) {
            ...
            default: {
              ...
              if (color !== undefined) {
                currentStyle.set('color', `var(--test-color-${color})`);
              } else {
                const background = ANSI_COLORS[code - 40] ?? ANSI_BRIGHT_COLORS[code - 100];
                if (background !== undefined) {
                  currentStyle.set('background-color', `var(--test-color-${background})`);
                }
              }
              break;
            }
...
export const updateStyle = (currentStyle: Map<string, {value: string, priority: string}>, styleToAdd: string): void => {
  ...
  for (const property of buffer.style) {
    ...
    if (potentialUrls.some(
            potentialUrl => !Common.ParsedURL.schemeIs(potentialUrl as Platform.CitizenNotesPath.UrlString, 'data:'))) {
      continue;
    }
