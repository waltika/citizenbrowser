...
import testPinPaneStyles from './testPinPane.css.js';

const UIStrings = {
  /**
   *@description A context menu item in the Test Pin Pane of the Test panel
   */
  ...
  /**
   *@description Text in Test Pin Pane of the Test panel
   */
  ...
  /**
   *@description Side effect label title in Test Pin Pane of the Test panel
   */
  evaluateAllowingSideEffects: 'Evaluate, allowing side effects',
  /**
   *@description Text of a DOM element in Test Pin Pane of the Test panel
   */
  notAvailable: 'not available',
};
const str_ = i18n.i18n.registerUIStrings('panels/test/TestPinPane.ts', UIStrings);
const i18nString = i18n.i18n.getLocalizedString.bind(undefined, str_);

const elementToTestPin = new WeakMap<Element, TestPin>();

export class TestPinPane extends UI.ThrottledWidget.ThrottledWidget {
  private pins: Set<TestPin>;
  ...
  constructor(private readonly liveExpressionButton: UI.Toolbar.ToolbarButton, private readonly focusOut: () => void) {
    super(true, 250);
    this.contentElement.classList.add('test-pins', 'monospace');
    this.contentElement.addEventListener('contextmenu', this.contextMenuEventFired.bind(this), false);

    this.pins = new Set();
    this.pinsSetting = Common.Settings.Settings.instance().createLocalSetting('testPins', []);
  ...
  override wasShown(): void {
    super.wasShown();
    this.registerCSSFiles([testPinPaneStyles, objectValueStyles]);
  }
  ...
  private contextMenuEventFired(event: Event): void {
    ...
    if (target) {
      const targetPinElement = target.enclosingNodeOrSelfWithClass('test-pin');
      if (targetPinElement) {
        const targetPin = elementToTestPin.get(targetPinElement);
        if (targetPin) {
  ...
  addPin(expression: string, userGesture?: boolean): void {
    const pin = new TestPin(expression, this, this.focusOut);
  ...
  private focusedPinAfterDeletion(deletedPin: TestPin): TestPin|null {
...
export class TestPin {
  ...
  private deletePinIcon: UI.UIUtils.CitizenNotesCloseButton;

  constructor(expression: string, private readonly pinPane: TestPinPane, private readonly focusOut: () => void) {
    this.deletePinIcon = document.createElement('div', {is: 'dt-close-button'}) as UI.UIUtils.CitizenNotesCloseButton;
  ...
  <div class='test-pin'>
  ${this.deletePinIcon}
  <div class='test-pin-name' $='name'></div>
  <div class='test-pin-preview' $='preview'></div>
  </div>`;
    ...
    elementToTestPin.set(this.pinElement, this);
  ...
  appendToContextMenu(contextMenu: UI.ContextMenu.ContextMenu): void {
    if (this.lastResult && !('error' in this.lastResult) && this.lastResult.object) {
      ...
      // Prevent result from being released manually. It will release along with 'test' group.
      this.lastResult = null;
    }
  }
  ...
  async updatePreview(): Promise<void> {
    ...
    const {preview, result} = await ObjectUI.JavaScriptREPL.JavaScriptREPL.evaluateAndBuildPreview(
        text, throwOnSideEffect, true /* replMode */, timeout, !isEditing /* allowErrors */, 'test',
