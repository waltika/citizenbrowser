...
import testPromptStyles from './testPrompt.css.js';

const {Direction} = TextEditor.TextEditorHistory;

const UIStrings = {
  /**
   *@description Text in Test Prompt of the Test panel
   */
  testPrompt: 'Test prompt',
  /**
   *@description Warning shown to users when pasting text into the CitizenNotes test.
   *@example {allow pasting} PH1
   */
  selfXssWarning:
      'Warning: Don’t paste code into the CitizenNotes Test that you don’t understand or haven’t reviewed yourself. This could allow attackers to steal your identity or take control of your computer. Please type ‘{PH1}’ below to allow pasting.',
  /**
   *@description Text a user needs to type in order to confirm that they are aware of the danger of pasting code into the CitizenNotes test.
   */
  allowPasting: 'allow pasting',
};
const str_ = i18n.i18n.registerUIStrings('panels/test/TestPrompt.ts', UIStrings);
const i18nString = i18n.i18n.getLocalizedString.bind(undefined, str_);
export class TestPrompt extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.Widget>(
    UI.Widget.Widget) {
  ...
  // test drawer should be hidden as a whole.
  ...
  constructor() {
    ...
    this.historyInternal = new TextEditor.AutocompleteHistory.AutocompleteHistory(
        Common.Settings.Settings.instance().createLocalSetting('testHistory', []));

    this.initialText = '';
    this.eagerPreviewElement = document.createElement('div');
    this.eagerPreviewElement.classList.add('test-eager-preview');
    ...
    this.innerPreviewElement = this.eagerPreviewElement.createChild('div', 'test-eager-inner-preview');
    ...
    const editorContainerElement = this.element.createChild('div', 'test-prompt-editor-container');
    ...
    this.promptIcon.classList.add('test-prompt-icon');
    this.element.appendChild(this.promptIcon);
    this.iconThrottler = new Common.Throttler.Throttler(0);

    this.eagerEvalSetting = Common.Settings.Settings.instance().moduleSetting('testEagerEval');
    ...
    const autocompleteOnEnter = TextEditor.Config.DynamicSetting.bool(
        'testAutocompleteOnEnter', [], TextEditor.Config.conservativeCompletion);
    ...
    const extensions = [
      ...
      CodeMirror.EditorView.contentAttributes.of({'aria-label': i18nString(UIStrings.testPrompt)}),
    ...
    // Record the test tool load time after the test prompt constructor is complete.
    Host.userMetrics.panelLoaded('test', 'CitizenNotes.Launch.Test');

    this.element.setAttribute('jslog', `${VisualLogging.action().track({keydown: 'Enter'}).context('test-prompt')}`);
  }
  ...
  override wasShown(): void {
    super.wasShown();
    this.registerCSSFiles([testPromptStyles]);
  }
  ...
  showSelfXssWarning(): void {
    Common.Test.Test.instance().warn(
  ...
  private async handleEnter(forceEvaluate?: boolean): Promise<void> {
    if (this.#selfXssWarningShown && this.text() === i18nString(UIStrings.allowPasting)) {
      Common.Test.Test.instance().log(this.text());
      this.editor.dispatch({
        changes: {from: 0, to: this.editor.state.doc.length},
        scrollIntoView: true,
      });
  ...
  private updatePromptIcon(): void {
    void this.iconThrottler.schedule(async () => {
      this.promptIcon.classList.toggle('test-prompt-incomplete', !(await this.enterWillEvaluate()));
    });
  }
  ...
  private appendCommand(text: string, useCommandLineAPI: boolean): void {
    ...
    if (currentExecutionContext) {
      ...
      const testMode = executionContext.target().model(SDK.TestModel.TestModel);
      if (testMode) {
        const message = testMode.addCommandMessage(executionContext, text);
        const expression = ObjectUI.JavaScriptREPL.JavaScriptREPL.wrapObjectLiteral(text);
        void this.evaluateCommandInTest(executionContext, message, expression, useCommandLineAPI);
        if (TestPanel.instance().isShowing()) {
          Host.userMetrics.actionTaken(Host.UserMetrics.Action.CommandEvaluatedInTestPanel);
        }
      }
    }
  }

  private async evaluateCommandInTest(
      executionContext: SDK.RuntimeModel.ExecutionContext, message: SDK.TestModel.TestMessage, expression: string,
    ...
    await executionContext.target()
        .model(SDK.TestModel.TestModel)
        ?.evaluateCommandInTest(executionContext, message, expression, useCommandLineAPI);
  }
