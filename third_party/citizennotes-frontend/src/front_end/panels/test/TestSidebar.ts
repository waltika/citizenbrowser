...
import {type TestViewMessage} from './TestViewMessage.js';
import testSidebarStyles from './testSidebar.css.js';

const UIStrings = {
  /**
   * @description Filter name in Test Sidebar of the Test panel. This is shown when we fail to
   * parse a URL when trying to display test messages from each URL separately. This might be
   * because the test message does not come from any particular URL. This should be translated as
   * a term that indicates 'not one of the other URLs listed here'.
   */
  other: '<other>',
  /**
   *@description Text in Test Sidebar of the Test panel to show how many user messages exist.
   */
  dUserMessages: '{n, plural, =0 {No user messages} =1 {# user message} other {# user messages}}',
  /**
   *@description Text in Test Sidebar of the Test panel to show how many messages exist.
   */
  dMessages: '{n, plural, =0 {No messages} =1 {# message} other {# messages}}',
  /**
   *@description Text in Test Sidebar of the Test panel to show how many errors exist.
   */
  dErrors: '{n, plural, =0 {No errors} =1 {# error} other {# errors}}',
  /**
   *@description Text in Test Sidebar of the Test panel to show how many warnings exist.
   */
  dWarnings: '{n, plural, =0 {No warnings} =1 {# warning} other {# warnings}}',
  /**
   *@description Text in Test Sidebar of the Test panel to show how many info messages exist.
   */
  dInfo: '{n, plural, =0 {No info} =1 {# info} other {# info}}',
  /**
   *@description Text in Test Sidebar of the Test panel to show how many verbose messages exist.
   */
  dVerbose: '{n, plural, =0 {No verbose} =1 {# verbose} other {# verbose}}',
};
const str_ = i18n.i18n.registerUIStrings('panels/test/TestSidebar.ts', UIStrings);
const i18nString = i18n.i18n.getLocalizedString.bind(undefined, str_);

export class TestSidebar extends Common.ObjectWrapper.eventMixin<EventTypes, typeof UI.Widget.VBox>(UI.Widget.VBox) {
  ...
  constructor() {
    ...
    const selectedFilterSetting =
        ...
        // @ts-expect-error
        Common.Settings.Settings.instance().createSetting<string>('test.sidebarSelectedFilter', null);

    const testAPIParsedFilters = [{
      key: FilterType.Source,
      text: SDK.TestModel.FrontendMessageSource.TestAPI,
      negative: false,
      regex: undefined,
    }];
    this.appendGroup(
        GroupName.All, [], TestFilter.allLevelsFilterValue(), UI.Icon.Icon.create('list'), selectedFilterSetting);
    this.appendGroup(
        GroupName.TestAPI, testAPIParsedFilters, TestFilter.allLevelsFilterValue(),
        UI.Icon.Icon.create('profile'), selectedFilterSetting);
    this.appendGroup(
        GroupName.Error, [], TestFilter.singleLevelMask(Protocol.Log.LogEntryLevel.Error),
        UI.Icon.Icon.create('cross-circle'), selectedFilterSetting);
    this.appendGroup(
        GroupName.Warning, [], TestFilter.singleLevelMask(Protocol.Log.LogEntryLevel.Warning),
        UI.Icon.Icon.create('warning'), selectedFilterSetting);
    this.appendGroup(
        GroupName.Info, [], TestFilter.singleLevelMask(Protocol.Log.LogEntryLevel.Info), UI.Icon.Icon.create('info'),
        selectedFilterSetting);
    this.appendGroup(
        GroupName.Verbose, [], TestFilter.singleLevelMask(Protocol.Log.LogEntryLevel.Verbose),
  ...
  private appendGroup(
    ...
    const filter = new TestFilter(name, parsedFilters, null, levelsMask);
  ...
  onMessageAdded(viewMessage: TestViewMessage): void {
    for (const treeElement of this.treeElements) {
  ...
  shouldBeVisible(viewMessage: TestViewMessage): boolean {
    if (this.selectedTreeElement instanceof TestSidebarTreeElement) {
  ...
  override wasShown(): void {
    super.wasShown();
    this.tree.registerCSSFiles([testSidebarStyles]);
  }
}
...
class TestSidebarTreeElement extends UI.TreeOutline.TreeElement {
  protected filterInternal: TestFilter;

  constructor(title: string|Node, filter: TestFilter) {
    super(title);
...
export class URLGroupTreeElement extends TestSidebarTreeElement {
...
const stringForFilterSidebarItemMap = new Map<GroupName, string>([
  [GroupName.TestAPI, UIStrings.dUserMessages],
...
export class FilterTreeElement extends TestSidebarTreeElement {
  ...
  constructor(filter: TestFilter, icon: UI.Icon.Icon, selectedFilterSetting: Common.Settings.Setting<string>) {
    super(filter.name, filter);
  ...
  onMessageAdded(viewMessage: TestViewMessage): void {
    ...
    const shouldIncrementCounter = message.type !== SDK.TestModel.FrontendMessageType.Command &&
  ...
  private childElement(url?: Platform.CitizenNotesPath.UrlString): URLGroupTreeElement {
