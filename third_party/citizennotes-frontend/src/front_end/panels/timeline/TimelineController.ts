...
export class TimelineController implements TraceEngine.TracingManager.TracingManagerClient {
  ...
  /**
   * We always need to profile against the CitizenNotes root target, which is
   * the target that CitizenNotes is attached to.
   *
   * In most cases, this will be the tab that CitizenNotes is inspecting.
  ...
  async startRecording(options: RecordingOptions): Promise<Protocol.ProtocolResponseWithError> {
    ...
    // 'disabled-by-default-citizennotes.screenshot'
    //   └ default: on, option: captureFilmStrip
    // 'disabled-by-default-citizennotes.timeline.invalidationTracking'
    ...
    const categoriesArray = [
      ...
      'citizennotes.timeline',
      disabledByDefault('citizennotes.timeline'),
      disabledByDefault('citizennotes.timeline.frame'),
      disabledByDefault('citizennotes.timeline.stack'),
    ...
    if (Root.Runtime.experiments.isEnabled('timelineInvalidationTracking')) {
      categoriesArray.push(disabledByDefault('citizennotes.timeline.invalidationTracking'));
    }
    if (options.capturePictures) {
      categoriesArray.push(
          disabledByDefault('citizennotes.timeline.layers'), disabledByDefault('citizennotes.timeline.picture'),
          disabledByDefault('blink.graphics_context_annotations'));
    }
    if (options.captureFilmStrip) {
      categoriesArray.push(disabledByDefault('citizennotes.screenshot'));
    }
