...
export class AggregatedTimelineTreeView extends TimelineTreeView {
  ...
  private executionContextNamesByOrigin = new Map<Platform.CitizenNotesPath.UrlString, string>();
  ...
  private beautifyDomainName(this: AggregatedTimelineTreeView, name: string): string {
    if (AggregatedTimelineTreeView.isExtensionInternalURL(name as Platform.CitizenNotesPath.UrlString)) {
      name = i18nString(UIStrings.chromeExtensionsOverhead);
    } else if (AggregatedTimelineTreeView.isV8NativeURL(name as Platform.CitizenNotesPath.UrlString)) {
      name = i18nString(UIStrings.vRuntime);
    } else if (name.startsWith('chrome-extension')) {
      name = this.executionContextNamesByOrigin.get(name as Platform.CitizenNotesPath.UrlString) || name;
    }
    return name;
  }
  ...
  private static isExtensionInternalURL(url: Platform.CitizenNotesPath.UrlString): boolean {
    return url.startsWith(AggregatedTimelineTreeView.extensionInternalPrefix);
  }

  private static isV8NativeURL(url: Platform.CitizenNotesPath.UrlString): boolean {
