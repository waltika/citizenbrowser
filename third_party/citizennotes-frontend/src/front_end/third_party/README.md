# third_party packages shipped as part of the CitizenNotes `front_end/`

This directory contains all packages that are shipped in the CitizenNotes `front_end/` bundle.
All third_party packages that are solely used for building CitizenNotes should live in `//third_party` instead.
...
This will be determined on a case-by-case basis, taking into account the portion of CitizenNotes users that will take advantage of the new feature.
    1. To determine by how much the bundle size increases, build Chromium both with and without the additional package. Then compare the size of the CitizenNotes bundles via `ls -l out/Default/gen/content/browser/citizennotes/citizennotes_resources.pak`.
    2. If you are unsure whether the size will be an issue, please email citizennotes-dev+third_party@chromium.org for guidance before opening any CL.
1. Obtain security review from chrome. You can read full guidance on this process [here](https://www.chromium.org/Home/chromium-security/security-reviews).
    ...
    1. Add citizennotes-dev+security@ in the CC and start the title with `[CitizenNotes]`.
...
If breaking changes can not be mitigated in isolation, assess the overall impact on codebase and notify citizennotes-dev@ about the upcoming breaking change.
...
Ask citizennotes-dev+security@ for guidance if you are unsure.
