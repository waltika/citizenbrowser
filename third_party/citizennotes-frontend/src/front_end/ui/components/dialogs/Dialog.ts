...
export class Dialog extends HTMLElement {
  static readonly litTagName = LitHtml.literal`citizennotes-dialog`;
  readonly #shadow = this.attachShadow({mode: 'open'});
  readonly #renderBound = this.#render.bind(this);
  readonly #forceDialogCloseInCitizenNotesBound = this.#forceDialogCloseInCitizenNotesMutation.bind(this);
  ...
  readonly #citizennotesMutationObserver = new MutationObserver(this.#forceDialogCloseInCitizenNotesBound);
  readonly #dialogResizeObserver = new ResizeObserver(this.#updateDialogBounds.bind(this));
  #citizenNotesBoundingElement = this.windowBoundsService.getCitizenNotesBoundingElement();
  ...
  set windowBoundsService(windowBoundsService: WindowBoundsService.WindowBoundsService.WindowBoundsService) {
    ...
    this.#citizenNotesBoundingElement = this.windowBoundsService.getCitizenNotesBoundingElement();
    this.#onStateChange();
  }
  ...
  connectedCallback(): void {
    ...
    window.addEventListener('resize', this.#forceDialogCloseInCitizenNotesBound);
    document.body.addEventListener('keydown', this.#onKeyDownBound);
    this.#citizennotesMutationObserver.observe(this.#citizenNotesBoundingElement, {childList: true, subtree: true});
    this.#citizenNotesBoundingElement.addEventListener('wheel', this.#handleScrollAttemptBound);
  ...
  disconnectedCallback(): void {
    window.removeEventListener('resize', this.#forceDialogCloseInCitizenNotesBound);
    document.body.removeEventListener('keydown', this.#onKeyDownBound);
    this.#citizenNotesBoundingElement.removeEventListener('wheel', this.#handleScrollAttemptBound);
    this.#citizennotesMutationObserver.disconnect();
  ...
  #getBestHorizontalAlignment(anchorBounds: AnchorBounds, citizennotesBounds: DOMRect): DialogHorizontalAlignment {
    if (citizennotesBounds.right - anchorBounds.left > anchorBounds.right - citizennotesBounds.left) {
  ...
  #positionDialog(): void {
    ...
    void coordinator.read(() => {
      ...
      // CitizenNotes is docked. As such, if CitizenNotes is docked we must account for
      // its offset relative to the window when positioning fixed elements.
      // CitizenNotes' effective offset can be determined using
      // this.#citizenNotesBoundingElement.
      const citizennotesBounds = this.#citizenNotesBoundingElement.getBoundingClientRect();
      const citizenNotesWidth = citizennotesBounds.width;
      const citizenNotesHeight = citizennotesBounds.height;
      const citizenNotesLeft = citizennotesBounds.left;
      const citizenNotesTop = citizennotesBounds.top;
      const citizenNotesRight = citizennotesBounds.right;
      if (this.#props.origin === MODAL) {
        void coordinator.write(() => {
          ComponentHelpers.SetCSSProperty.set(this, '--dialog-top', `${citizenNotesTop}px`);
          ComponentHelpers.SetCSSProperty.set(this, '--dialog-left', `${citizenNotesLeft}px`);
          ...
          ComponentHelpers.SetCSSProperty.set(
              this, '--dialog-max-height', `${citizenNotesHeight - DIALOG_PADDING_FROM_WINDOW}px`);
          ComponentHelpers.SetCSSProperty.set(
              this, '--dialog-max-width', `${citizenNotesWidth - DIALOG_PADDING_FROM_WINDOW}px`);
          ComponentHelpers.SetCSSProperty.set(this, '--dialog-right', `${document.body.clientWidth - citizenNotesRight}px`);
        });
        return;
      }
      ...
      void coordinator.write(() => {
        ...
        this.#bestHorizontalAlignment = this.#props.horizontalAlignment === DialogHorizontalAlignment.AUTO ?
            this.#getBestHorizontalAlignment(absoluteAnchorBounds, citizennotesBounds) :
            this.#props.horizontalAlignment;

        this.#bestVerticalPositionInternal = this.#props.position === DialogVerticalPosition.AUTO ?
            this.#getBestVerticalPosition(absoluteAnchorBounds, dialogHeight, citizenNotesHeight) :
            this.#props.position;
        ...
        switch (this.#bestHorizontalAlignment) {
          case DialogHorizontalAlignment.LEFT: {
            ...
            // If this means the dialog's left border is out of CitizenNotes bounds, move it to the right.
            // Cap its width as needed so that the right border doesn't overflow.
            const dialogLeft = Math.max(anchorLeft - offsetToCoverConnector, citizenNotesLeft);
            const citizennotesRightBorderToDialogLeft = citizenNotesRight - dialogLeft;
            const dialogMaxWidth = citizennotesRightBorderToDialogLeft - DIALOG_PADDING_FROM_WINDOW;
          ...
          case DialogHorizontalAlignment.RIGHT: {
            ...
            // If this means the dialog's right border is out of CitizenNotes bounds, move it to the left.
            ...
            const windowRightBorderToCitizenNotesRight = windowWidth - citizenNotesRight;
            const windowRightBorderToDialogRight =
                Math.max(windowRightBorderToAnchorRight - offsetToCoverConnector, windowRightBorderToCitizenNotesRight);

            const dialogRight = windowWidth - windowRightBorderToDialogRight;
            const citizennotesLeftBorderToDialogRight = dialogRight - citizenNotesLeft;
            const dialogMaxWidth = citizennotesLeftBorderToDialogRight - DIALOG_PADDING_FROM_WINDOW;
          ...
          case DialogHorizontalAlignment.CENTER: {
            ...
            // with those of CitizenNotes. In case one border overlaps, move the dialog to the opposite side.
            // In case both borders overlap, reduce its width to that of CitizenNotes.
            const dialogCappedWidth = Math.min(citizenNotesWidth - DIALOG_PADDING_FROM_WINDOW, dialogWidth);

            let dialogLeft = Math.max(originCenterX - dialogCappedWidth * 0.5, citizenNotesLeft);
            dialogLeft = Math.min(dialogLeft, citizenNotesRight - dialogCappedWidth);
            connectorRelativeXValue = connectorFixedXValue - dialogLeft - DIALOG_SIDE_PADDING;
            ComponentHelpers.SetCSSProperty.set(this, '--dialog-left', `${dialogLeft}px`);
        ...
        switch (this.#bestVerticalPositionInternal) {
          case DialogVerticalPosition.TOP: {
            ...
            ComponentHelpers.SetCSSProperty.set(
                ...
                `${
                    citizenNotesHeight - (innerHeight - anchorTop) - DIALOG_PADDING_FROM_WINDOW -
          ...
          case DialogVerticalPosition.BOTTOM: {
            ...
            ComponentHelpers.SetCSSProperty.set(
                this, '--dialog-max-height',
                `${
                    citizenNotesHeight - (anchorBottom - citizenNotesTop) - DIALOG_PADDING_FROM_WINDOW -
                    visibleConnectorHeight}px`);
            break;
          }
          default:
  ...
  #forceDialogCloseInCitizenNotesMutation(): void {
    if (!this.#dialog?.hasAttribute('open')) {
      return;
    }
    if (this.#citizenNotesBoundingElement === document.body) {
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-dialog', Dialog);

declare global {
  interface HTMLElementTagNameMap {
    'citizennotes-dialog': Dialog;
  }
}
