...
```
return html`<citizennotes-foo .name=${name}></citizennotes-foo>`
```

Lit knows that the only dynamic part of that template is the `name` property, and as such on each re-render it only updates that part (and only if it has changed). It will not recreate `citizennotes-foo`.
...
When introducing lit-html into the CitizenNotes codebase we made the decision to prefer:
...
```
<citizennotes-foo .x=${x} .y=${y}></citizennotes-foo>
```
...
return html`<citizennotes-foo .onClick=${onClick}></citizennotes-foo>`
```

And within `citizennotes-foo`, we have:
...
In this example, we would update `citizennotes-foo` to dispatch an event when the user performs an action. We could then update our `#render()` method to:

```ts
return html`<citizennotes-foo @click=${this.#onClick}></citizennotes-foo>`
```

Lit will manage this event listener and ensure that it is not needlessly unbound and rebound. Now, we don't cause `citizennotes-foo` to re-render when really nothing is changing.
