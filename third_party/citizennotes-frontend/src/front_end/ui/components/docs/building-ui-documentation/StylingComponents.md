...
And then add `:css_files` as a dependency to the `citizennotes_entrypoint`:
...
Remember to use theme colors (`var(--sys-color-on-surface)`) when styling elements to ensure consistency across CitizenNotes.
