 ...
 * This makes sure that all links are accounted for and no bad links are introduced to citizennotes via markdown.
 */
export class MarkdownLink extends HTMLElement {
  static readonly litTagName = LitHtml.literal`citizennotes-markdown-link`;
  ...
  #render(): void {
    ...
    const output = LitHtml.html`
      <x-link class="citizennotes-link" href=${this.#linkUrl} jslog=${VisualLogging.link().track({click: true})}>${this.#linkText}</x-link>
    `;
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-markdown-link', MarkdownLink);

declare global {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  interface HTMLElementTagNameMap {
    'citizennotes-markdown-link': MarkdownLink;
  }
}
