...
declare global {
  interface HTMLElementTagNameMap {
    'citizennotes-text-editor': TextEditor;
  }
}

export class TextEditor extends HTMLElement {
  static readonly litTagName = LitHtml.literal`citizennotes-text-editor`;
  ...
  #citizennotesResizeObserver = new ResizeObserver(this.#resizeListener);
  ...
  disconnectedCallback(): void {
    if (this.#activeEditor) {
      ...
      this.#citizennotesResizeObserver.disconnect();
  ...
  #startObservingResize(): void {
    const citizennotesElement =
        WindowBoundsService.WindowBoundsService.WindowBoundsServiceImpl.instance().getCitizenNotesBoundingElement();
    if (citizennotesElement) {
      this.#citizennotesResizeObserver.observe(citizennotesElement);
    }
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-text-editor', TextEditor);
