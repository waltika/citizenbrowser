...
## Actions
Action extensions describe functionalities CitizenNotes can perform and that the user can trigger, for instance clearing the console or hiding an element in the elements tree.
...
- `platform` refers to the OS on which CitizenNotes is running and can be either `mac` or `windows,linux`.
...
UI.ActionRegistration.registerActionExtension({
  ...
  bindings: [
    {
      shortcut: 'Esc',
      keybindSets: ['citizenNotesDefault'],
    },
  ],
});
```
