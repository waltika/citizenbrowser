...
const UIStrings = {
  /**
   * @description Text in a dialog box in CitizenNotes stating why remote debugging has been terminated.
   * "Remote debugging" here means that CitizenNotes on a PC is inspecting a website running on an actual mobile device
   * (see https://developer.chrome.com/docs/citizennotes/remote-debugging/).
   */
  debuggingConnectionWasClosed: 'Debugging connection was closed. Reason: ',
  /**
   * @description Text in a dialog box showing how to reconnect to CitizenNotes when remote debugging has been terminated.
   * "Remote debugging" here means that CitizenNotes on a PC is inspecting a website running on an actual mobile device
   * (see https://developer.chrome.com/docs/citizennotes/remote-debugging/).
   * "Reconnect when ready", refers to the state of the mobile device. The developer first has to put the mobile
   * device back in a state where it can be inspected, before CitizenNotes can reconnect to it.
   */
  reconnectWhenReadyByReopening: 'Reconnect when ready by reopening CitizenNotes.',
  /**
   * @description Text on a button to reconnect Citizennotes when remote debugging terminated.
   * "Remote debugging" here means that CitizenNotes on a PC is inspecting a website running on an actual mobile device
   * (see https://developer.chrome.com/docs/citizennotes/remote-debugging/).
   */
  reconnectCitizennotes: 'Reconnect `CitizenNotes`',
};
...
export class RemoteDebuggingTerminatedScreen extends VBox {
  constructor(reason: string) {
    ...
    const button = createTextButton(i18nString(UIStrings.reconnectCitizennotes), () => window.location.reload());
