...
export class ShortcutRegistry {
  ...
  private readonly citizenNotesDefaultShortcutActions: Set<string>;
  ...
  constructor(actionRegistry: ActionRegistry) {
    ...
    this.citizenNotesDefaultShortcutActions = new Set();
  ...
  actionHasDefaultShortcut(actionId: string): boolean {
    return this.citizenNotesDefaultShortcutActions.has(actionId);
  }
  ...
  async handleKey(key: number, domKey: string, event?: KeyboardEvent, handlers?: {
    ...
    function isPossiblyInputKey(): boolean {
      ...
      // Undo/Redo will also cause input, so textual undo should take precedence over CitizenNotes undo when editing.
  ...
  private registerBindings(): void {
    ...
    this.citizenNotesDefaultShortcutActions.clear();
    const forwardedKeys: {
      keyCode: number,
      modifiers: number,
    }[] = [];
      ...
      for (let i = 0; bindings && i < bindings.length; ++i) {
        ...
        if (shortcutDescriptors.length > 0) {
          if (this.isDisabledDefault(shortcutDescriptors, actionId)) {
            this.citizenNotesDefaultShortcutActions.add(actionId);
            continue;
          }
          ...
          if (!keybindSets) {
            ...
            if (keybindSets.includes(KeybindSet.CITIZENNOTES_DEFAULT)) {
              this.citizenNotesDefaultShortcutActions.add(actionId);
            }
...
export const DefaultShortcutSetting = 'citizenNotesDefault';
