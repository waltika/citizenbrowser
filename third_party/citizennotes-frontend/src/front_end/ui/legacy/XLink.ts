...
export class XLink extends XElement {
  hrefInternal: Platform.CitizenNotesPath.UrlString|null;
  ...
  static create(url: string, linkText?: string, className?: string, preventClick?: boolean): HTMLElement {
    ...
    className = className || '';
    // clang-format off
    // TODO(dgozman): migrate css from 'citizennotes-link' to 'x-link'.
    const element = html `
  <x-link href='${url}' tabindex="0" class='${className} citizennotes-link' ${preventClick ? 'no-click' : ''}
  ...
  get href(): Platform.CitizenNotesPath.UrlString|null {
  ...
  override attributeChangedCallback(attr: string, oldValue: string|null, newValue: string|null): void {
    ...
    if (attr === 'href') {
      ...
      let href: Platform.CitizenNotesPath.UrlString|null = null;
      let url: URL|null = null;
      try {
        url = new URL(addReferrerToURLIfNecessary(newValue as Platform.CitizenNotesPath.UrlString));
        href = url.toString() as Platform.CitizenNotesPath.UrlString;
      } catch {
      }
