...
export class CSSAngle extends HTMLElement {
  static readonly litTagName = LitHtml.literal`citizennotes-css-angle`;
  ...
  popOver(): void {
    ...
    if (!this.swatchElement) {
      this.swatchElement = this.shadow.querySelector<HTMLElement>('citizennotes-css-angle-swatch');
    }
    ...
    if (miniIconBottom && miniIconLeft) {
      ...
      // pane's positions because CitizenNotes' root SplitWidget's
      // insertion-point-sidebar slot, where most of the CitizenNotes content lives,
...
ComponentHelpers.CustomElements.defineComponent('citizennotes-css-angle', CSSAngle);

declare global {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  interface HTMLElementTagNameMap {
    'citizennotes-css-angle': CSSAngle;
  }
}
