...
class FontPropertyInputs {
  ...
  private readonly selectedNode: SDK.DOMModel.DOMNode|null;
  private sliderInput: UI.UIUtils.CitizenNotesSlider;
  ...
  private createSliderInput(field: Element, _label: string): UI.UIUtils.CitizenNotesSlider {
    const min = this.initialRange.min;
    const max = this.initialRange.max;
    const step = this.initialRange.step;

    const slider = (UI.UIUtils.createSlider(min, max, -1) as UI.UIUtils.CitizenNotesSlider);
