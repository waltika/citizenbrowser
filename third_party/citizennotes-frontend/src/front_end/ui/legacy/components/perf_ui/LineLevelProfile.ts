...
export class Memory {
  ...
  appendHeapProfile(profile: Protocol.HeapProfiler.SamplingHeapProfile, target: SDK.Target.Target|null): void {
    ...
    function processNode(node: Protocol.HeapProfiler.SamplingHeapProfileNode): void {
      ...
      const script = Number(node.callFrame.scriptId) || node.callFrame.url as Platform.CitizenNotesPath.UrlString;
      if (!script) {
        return;
      }
...
export class Helper {
  ...
  private lineData!: Map<SDK.Target.Target|null, Map<Platform.CitizenNotesPath.UrlString|number, Map<number, number>>>;
  ...
  addLineData(
      target: SDK.Target.Target|null, scriptIdOrUrl: Platform.CitizenNotesPath.UrlString|number, line: number,
      data: number): void {
