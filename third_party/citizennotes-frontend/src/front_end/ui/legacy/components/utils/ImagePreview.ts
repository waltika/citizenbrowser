...
export interface PrecomputedFeatures {
  ...
  currentSrc?: Platform.CitizenNotesPath.UrlString;
}
...
export class ImagePreview {
  static async build(
      target: SDK.Target.Target, originalImageURL: Platform.CitizenNotesPath.UrlString, showDimensions: boolean, options: {
    ...
    // When opening CitizenNotes for the first time, base64 resource has no content.
  ...
  static async loadDimensionsForNode(node: SDK.DOMModel.DOMNode): Promise<PrecomputedFeatures|undefined> {
    ...
    function features(this: HTMLImageElement): PrecomputedFeatures {
      return {
        ...
        currentSrc: this.currentSrc as Platform.CitizenNotesPath.UrlString,
      };
    }
  }

  static defaultAltTextForImageURL(url: Platform.CitizenNotesPath.UrlString): string {
