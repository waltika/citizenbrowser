...
def main(argv):
    ...
    for expected_file in expected_json:
        if expected_file not in generated_json:
            print(
                ...
                " Either remove it from config/gni/citizennotes_grd_files.gni" +
                " or add the missing file to an action" +
                " (for example a citizennotes_module or citizennotes_entrypoint definition).\n"
            )
    ...
    for generated_file in generated_json:
        if generated_file not in expected_json:
            print(
                "File " + generated_file +
                " is not listed in config/gni/citizennotes_grd_files.gni." +
