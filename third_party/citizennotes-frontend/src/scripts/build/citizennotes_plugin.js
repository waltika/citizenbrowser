...
function citizennotesPlugin(source, importer) {
  if (!importer) {
    return null;
  }
  ...
  // our full path is `third_party/citizennotes-frontend/src/`, which thus *always*
...
function esbuildPlugin(outdir) {
  return args => {
    ...
    const res = citizennotesPlugin(args.path, args.importer);
    if (!res) {
      return null;
    }
