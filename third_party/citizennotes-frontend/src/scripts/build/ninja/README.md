# The CitizenNotes build system

CitizenNotes is built using the [GN build system](https://gn.googlesource.com/gn/) with some additional conventions and tools on top of it.

## Entrypoints and modules

To understand how CitizenNotes is structured it is important to understand how CitizenNotes defines **modules**.
...
Although this does cause references to be more verbose, it also aids in clarity as it is clear where a piece of code is coming from, and it is also important to enable build optimisations in CitizenNotes release builds (see below).
...
Within the `citizennotes_module` we list all the source files - a list of every TypeScript file that is in this directory - and ensure that any dependencies are listed in the `deps` section. Because we just added an import to `../../core/sdk/sdk.ts`, we need to update the `deps` entry accordingly:
...
When we depend on modules, we are depending on the entrypoint. In the build system, each entrypoint (defined using `citizennotes_entrypoint`) is named `bundle`. This is why when we depend on modules, we depend on `core/sdk:bundle`.

In a `BUILD.gn` file, any dependency that starts with a colon is a dependency to another target within the same `BUILD.gn` file. Notice here that the entrypoint depends on `:timeline`, which is the `citizennotes_module` target which lists all the other source files and dependencies that the module has.
...
The reason behind the naming of `citizennotes_entrypoint` and why every entrypoint is always named `bundle` is due to how we build CitizenNotes for production builds.
...
## Listing all files in CitizenNotes: GRD file generation

To make sure that files are loaded in Chromium, CitizenNotes generates a GRD file that includes all files that are allowed to be loaded by the backend.

We ship CitizenNotes in the Chromium binary as a compressed archive of CitizenNotes' build output (concretely a brotli-compressed GRIT file). We need to tell Chromium which files to include in the archive.

We could do this automatically by looking at the build rules and targets (`citizennotes_entrypoint`/`citizennotes_module`), but that runs the risk of unintentionally including files in the final archive and shipping too much. At the same time we might add new files to build rules incorrectly and not ship them at all by mistake.
...
All entrypoints are listed in `grd_files_release_sources` specified in `/config/gni/citizennotes_grd_files.gni`. If you create a new module and thus a new entrypoint, you must add it to this list.

> Rule: in both release and debug builds, entrypoints are always included in the GRD file.

### Module implementation files

All implementation files for components are listed in `grd_files_debug_sources` specified in `/config/gni/citizennotes_grd_files.gni`. If you create a new file within a module, you must add it to this list.
...
# Implementing CitizenNotes build system templates
...
The CitizenNotes build system contains several templates to integrate files in the build system.
...
Historically, CitizenNotes used the location-agnostic [`out/Default/resources/inspector`](https://groups.google.com/a/chromium.org/g/citizennotes-dev/c/BENZ8ZTW0Ls/m/W7BhAvOzBAAJ) folder, but we have since adopted the GN guidelines for mirroring our source folder.
...
Since GN has no notion of a "root" folder for CitizenNotes, computation of sub-folders can only be done based on heuristics, which will fail sooner rather than later.
