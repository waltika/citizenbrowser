...
const {citizennotesPlugin, esbuildPlugin} = require('../citizennotes_plugin.js');

describe('citizennotes_plugin can compute paths with', () => {
  it('same directory import', () => {
    assert.deepEqual(
        citizennotesPlugin('./AnotherFile.js', 'front_end/core/sdk/FirstFile.js'),
        {id: path.join('front_end', 'core', 'sdk', 'AnotherFile.js'), external: false});
  });

  it('different directory import', () => {
    assert.deepEqual(
        citizennotesPlugin('../common/common.js', 'front_end/core/sdk/FirstFile.js'),
        {id: path.join('front_end', 'core', 'common', 'common.js'), external: true});
  });

  it('node built-in modules', () => {
    assert.deepEqual(citizennotesPlugin('fs', 'scripts/some-script.js'), {id: 'fs', external: true});
  });

  it('importing generated files', () => {
    assert.strictEqual(citizennotesPlugin('../../generated/Protocol.js', 'front_end/core/sdk/FirstFile.js'), null);
  });

  it('importing lighthouse files', () => {
    assert.deepEqual(
        citizennotesPlugin('./front_end/third_party/lighthouse/lighthouse-dt-bundle.js', 'front_end/core/sdk/FirstFile.js'),
        {
          id: path.join(
