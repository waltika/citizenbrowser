...
/**
 * Determines if AST Node is a call to register a CitizennotesExperiment
 */
...
/**
 * Determines if AST Node is the CitizennotesExperiments Enum declaration
 */
function isExperimentEnumDeclaration(node) {
  return node.type === 'ExportNamedDeclaration' && node?.declaration?.id?.name === 'CitizennotesExperiments';
}
...
function compareExperimentLists(mainImplList, userMetricsList) {
  ...
  if (!mainImplList) {
    console.log(
        'Changes to Citizennotes Experiment registration have prevented this check from finding registered experiments.');
    console.log('Please update scripts/check_experiments.js to account for the new experiment registration.');
    errorFound = true;
  }
  if (!userMetricsList) {
    console.log(
        'Changes to Citizennotes Experiment UserMetrics enum have prevented this check from finding experiments registered for telemetry.');
  ...
  if (missingTelemetry.length) {
    console.log('Citizennotes Experiments have been added without corresponding histogram update!');
    console.log(missingTelemetry.join('\n'));
    console.log(
        'Please ensure that the CitizennotesExperiments enum in UserMetrics.ts is updated with the new experiment.');
    console.log(
        'Please ensure that a corresponding CL is openend against chromium.src/tools/metrics/histograms/enums.xml to update the CitizennotesExperiments enum');
    errorFound = true;
  }
  if (staleTelemetry.length) {
    console.log('Citizennotes Experiments that are no longer registered are still listed in the telemetry enum!');
    console.log(staleTelemetry.join('\n'));
    console.log(
        'Please ensure that the CitizennotesExperiments enum in UserMetrics.ts is updated to remove these stale experiments.');
    errorFound = true;
  }
  if (errorFound) {
    process.exit(1);
  }
  console.log('CitizenNotes Experiment Telemetry checker passed.');
}
