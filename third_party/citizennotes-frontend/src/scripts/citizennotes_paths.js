 ...
 * directories. It is the Node equivalent of citizennotes_paths.py.
 *
 * Note that not all paths implemented in citizennotes_paths.py are implemented
 ...
 * from chromium/src/third_party/citizennotes-frontend => citizennotes-frontend repo
 ...
 * the citizennotes-frontend repository/directory, we can use that file as the
 ...
 *
 * process.argv[1] =
 * /full/path/citizennotes-frontend/src/scripts/test/run_lint_check_css.js
 */
 ...
 * citizennotes-frontend repository.
 */
function isInChromiumDirectory() {
  ...
  const citizennotesPath = 'src/third_party/citizennotes-frontend';
  const isInChromium = normalizedPath.includes(citizennotesPath);
  const potentialChromiumDir = PATH_TO_EXECUTED_FILE.substring(0, normalizedPath.indexOf(citizennotesPath));
  const result = {isInChromium, chromiumDirectory: potentialChromiumDir};
  _lookUpCaches.set('chromium', result);
  return result;
}
/**
 * Returns the path to the root of the citizennotes-frontend repository.
 *
 * If we're in Chromium, this will be /path/to/chromium/src/third_party/citizennotes-frontend/src
 * If it's standalone, it will be /path/to/citizennotes-frontend
 */
function citizennotesRootPath() {
 ...
 * if we're in standalone, this is /path/to/citizennotes-frontend
 *
 * Note this is different to citizennotesRootPath(), which always returns the path
 * to the citizennotes-frontend source code.
 */
function rootPath() {
 ...
 * directory, not from the citizennotes-frontend third_party directory.
 */
function thirdPartyPath() {
  return path.join(rootPath(), 'third_party');
}

function citizenNotesThirdPartyPath() {
  const {isInChromium} = isInChromiumDirectory();
  if (isInChromium) {
    return path.join(rootPath(), 'third_party', 'citizennotes-frontend', 'src', 'third_party');
  }
...
/**
 * The path to the citizennotes-frontend node_modules folder.
 */
function nodeModulesPath() {
  return path.join(citizennotesRootPath(), 'node_modules');
}
...
function downloadedChromeBinaryPath() {
  ...
  return path.join(citizenNotesThirdPartyPath(), 'chrome', paths[os.platform()]);
}

module.exports = {
  thirdPartyPath,
  nodePath,
  citizennotesRootPath,
  nodeModulesPath,
  mochaExecutablePath,
