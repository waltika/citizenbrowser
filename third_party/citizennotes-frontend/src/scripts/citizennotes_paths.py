...
# In the external repository, standalone build, this is the citizennotes-frontend directory.
# In the external repository, integrated build, this is the src/chromium directory.
def root_path():
    SCRIPTS_PATH = path.dirname(path.abspath(__file__))
    ABS_CITIZENNOTES_PATH = path.dirname(SCRIPTS_PATH)
    PARENT_PATH = path.dirname(ABS_CITIZENNOTES_PATH)
    ...
    elif path.basename(PARENT_PATH) == 'citizennotes-frontend' or path.basename(
            PARENT_PATH) == 'citizennotes-frontend-internal':
        # External repository, integrated build
        return path.dirname(path.dirname(PARENT_PATH))
    else:
        # External repository, standalone build
        return ABS_CITIZENNOTES_PATH
...
def citizennotes_root_path():
    return path.dirname((path.dirname(path.abspath(__file__))))


def node_modules_path():
    return path.join(citizennotes_root_path(), 'node_modules')
...
def hosted_mode_script_path():
    return path.join(citizennotes_root_path(), 'scripts', 'hosted_mode', 'server.js')


def esbuild_path():
    return path.join(citizennotes_root_path(), 'third_party', 'esbuild', 'esbuild')
...
def downloaded_chrome_binary_path():
    return path.abspath(
        path.join(
            *{
                'Linux': (citizennotes_root_path(), 'third_party', 'chrome',
                          'chrome-linux', 'chrome'),
                'Darwin': (citizennotes_root_path(), 'third_party', 'chrome',
                           'chrome-mac', 'Google Chrome for Testing.app',
                           'Contents', 'MacOS', 'Google Chrome for Testing'),
                'Windows': (citizennotes_root_path(), 'third_party', 'chrome',
...
def package_lock_json_path():
    return path.join(citizennotes_root_path(), 'package-lock.json')


def package_json_path():
    return path.join(citizennotes_root_path(), 'package.json')


def browser_protocol_path():
    return path.join(third_party_path(), 'blink', 'public', 'citizennotes_protocol', 'browser_protocol.pdl')
