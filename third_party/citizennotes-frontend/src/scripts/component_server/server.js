 ...
 * citizennotesRoot/out/Target/gen/front_end/ui/components/docs, but if you need to add a
 * prefix you can pass this argument. Passing `foo` will redirect the server to
 * look in citizennotesRoot/out/Target/gen/foo/front_end/ui/components/docs.
 */
...
const citizennotesRootFolder = path.resolve(path.join(pathToBuiltOutTargetDirectory, 'gen'));
const componentDocsBaseFolder = path.join(citizennotesRootFolder, componentDocsBaseArg);

if (!fs.existsSync(citizennotesRootFolder)) {
  console.error(`ERROR: Generated front_end folder (${citizennotesRootFolder}) does not exist.`);
  console.log(
      'The components server works from the built Ninja output; you may need to run Ninja to update your built CitizenNotes.');
...
// All paths that are injected globally into real CitizenNotes, so we do the same
...
function createComponentIndexFile(componentPath, componentExamples) {
  ...
  <html>
    <head>
      ...
      <title>CitizenNotes component: ${componentName}</title>
      <style>
...
function createServerIndexFile(componentNames) {
  ...
  <html>
    <head>
      <meta charset="UTF-8" />
      <meta name="viewport" content="width=device-width" />
      <title>CitizenNotes components</title>
      ${linksToStyleSheets}
    </head>
    <body id="index-page">
      <h1>CitizenNotes components</h1>
...
async function requestHandler(request, response) {
  ...
  if (['/', '/index.html'].includes(filePath) && tracesMode === false) {
    ...
    if (!fullPath.startsWith(citizennotesRootFolder) && !fileIsInTestFolder) {
      console.error(`Path ${fullPath} is outside the CitizenNotes Frontend root dir.`);
      process.exit(1);
    }
    ...
    const isComputingCoverageRequest = request.headers['citizennotes-compute-coverage'] === '1';
...
function createTracesIndexFile(traceFilenames) {
  function pageFunction() {
    ...
    document.body.addEventListener('click', async e => {
      ...
      const citizennotesLoadingTraceUrl = `citizennotes://citizennotes/bundled/citizennotes_app.html?loadTimelineFromURL=${traceUrl}`;

      try {
        await navigator.clipboard.writeText(citizennotesLoadingTraceUrl);
  ...
  <html>
    ...
    <body id="traces-page">
      <h1>First</h1>
      <p><textarea cols=100>citizennotes://citizennotes/bundled/citizennotes_app.html</textarea>
...
async function handleTracesModeRequest(request, response, filePath) {
  ...
  if (filePath === '/') {
    ...
    // Traces need CORS to be loaded by citizennotes:// origin
