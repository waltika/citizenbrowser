...
# CitizenNotes was built with the expectation of an older version of CDP, that
...
# CDP definitions in such a way that it would break compilation of CitizenNotes.
...
# such, if the CDP definitions change and CitizenNotes would break on it, we
# currently have to manually patch these on CitizenNotes side. This currently
# happens when CitizenNotes engineers roll our deps, regenerate the types, run the
...
# If we were to take the PDL definition from Chromium and compile CitizenNotes with
...
# the CitizenNotes build. That's why we have to maintain a copy of CDP that we
# build CitizenNotes with and ensure that we remain up-to-date on the Chromium side,
...
PROTOCOL_LOCATION = path.join(ROOT_DIRECTORY, 'third_party', 'blink', 'public', 'citizennotes_protocol')
...
NODE_LOCATION = citizennotes_paths.node_path()
TSC_LOCATION = citizennotes_paths.typescript_compiler_path()
