...
def exec_command(cmd):
    try:
        ...
        cmd_proc_result = subprocess.check_call(cmd,
                                                cwd=citizennotes_paths.root_path(),
                                                env=new_env)
...
def ensure_licenses():
    cmd = [
        citizennotes_paths.node_path(),
        citizennotes_paths.license_checker_path(),
        '--onlyAllow',
        ('%s' % (';'.join(LICENSES)))
    ]
...
def strip_private_fields():
    ...
    packages = []
    for root, _, filenames in os.walk(citizennotes_paths.node_modules_path()):
...
def install_missing_deps():
    with open(citizennotes_paths.package_lock_json_path(), 'r+') as pkg_lock_file:
        try:
...
def append_package_json_entries():
    with open(citizennotes_paths.package_json_path(), 'r+') as pkg_file:
        try:
            pkg_data = load_json_file(pkg_file)
...
def addClangFormat():
    with open(path.join(citizennotes_paths.node_modules_path(), '.clang-format'), 'w+') as clang_format_file:
        try:
...
def addOwnersFile():
    with open(path.join(citizennotes_paths.node_modules_path(), 'OWNERS'),
              'w+') as owners_file:
        try:
...
def addChromiumReadme():
    with open(path.join(citizennotes_paths.node_modules_path(), 'README.chromium'),
              'w+') as readme_file:
        try:
...
def run_npm_command(npm_command_args=None):
    ...
    if not runs_analysis_command:
        ...
        # To minimize disk usage for Chrome CitizenNotes node_modules, always try to dedupe dependencies.
