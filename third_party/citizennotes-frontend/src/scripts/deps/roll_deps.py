...
# Files whose location within citizennotes-frontend matches the upstream location.
FILES = [
...
# Files whose location within citizennotes-frontend differs from the upstream location.
FILE_MAPPINGS = {
    # chromium_path => citizennotes_frontend_path
    'components/variations/proto/citizennotes/client_variations.js':
...
def parse_options(cli_args):
    ...
    parser.add_argument('citizennotes_dir',
                        help='path to citizennotes/citizennotes-frontend directory')
...
def copy_files(options):
    for from_path, to_path in FILE_MAPPINGS.items():
        ...
        shutil.copy(os.path.join(options.chromium_dir, from_path),
                    os.path.join(options.citizennotes_dir, to_path))
...
def generate_signatures(options):
    ...
    subprocess.check_call([
        node_path(options),
        os.path.join(options.citizennotes_dir, 'scripts', 'javascript_natives',
                     'index.js'), options.chromium_dir, options.citizennotes_dir
    ])
...
def generate_dom_pinned_properties(options):
    ...
    subprocess.check_call([
        node_path(options),
        os.path.join(options.citizennotes_dir, 'scripts', 'webidl-properties',
                     'index.js'), options.citizennotes_dir
    ])
