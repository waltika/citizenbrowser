...
CITIZENNOTES_PATH = os.path.dirname(
...
def main():
    ...
    path = CITIZENNOTES_PATH + f'/front_end/third_party/{output_dir}'
    ...
    # Update citizennotes_grd_files.gni
    update_gn_var(
        './config/gni/citizennotes_grd_files.gni', 'grd_files_debug_sources', [
            f'front_end/third_party/{output_dir}/' + name
            for name in members if name.startswith(f'package/{library_dir}/')
            and name not in excluded_sources and name.endswith('.js')
        ] + [
            name for name in read_gn_var('./config/gni/citizennotes_grd_files.gni',
    ...
    subprocess.check_call(['git', 'cl', 'format'], cwd=CITIZENNOTES_PATH)
