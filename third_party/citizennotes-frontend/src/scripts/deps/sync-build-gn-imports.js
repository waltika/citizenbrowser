 ...
 * template call and everything contained within: citizennotes_module("handlers")
...
function parseBuildGN(input) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // Matches any lines containing citizennotes_*("name") {
    ...
    // BUILD.gn.
    const isOpeningLine = /(citizennotes_\w+)\("(\w+)"\) \{/.exec(line);
    if (isOpeningLine) {
      // The first group is the template (citizennotes_X) and the second is the name.
...
function compareDeps({buildGN, sourceCode}) {
  ...
  // `citizennotes_module` that the entrypoint depends on. This is because of how
  // we structure our BUILD.gn files, the citizennotes_entrypoint never lists all
  // the dependencies, but instead depends on the `citizennotes_module`, which does
  // list all the dependencies.
  if (buildGNModule.template === 'citizennotes_entrypoint') {
    // We are going to use the dependencies from the relevant citizennotes_module
  ...
  if (!buildGNModule) {
    throw new Error(`Could not find citizennotes_module in BUILD.gn for the citizennotes_entrypoint of ${sourceCode.filePath}`);
  }
