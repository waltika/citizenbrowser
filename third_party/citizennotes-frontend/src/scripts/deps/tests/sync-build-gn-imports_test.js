...
describe('parsing a BUILD.gn file to find its modules', () => {
  it('can parse a BUILD.gn that has sources and deps on one line', () => {
    const input = `citizennotes_module("handlers") {
...
citizennotes_module("other") {
  sources = ["foo.ts"]
    ...
    assert.deepEqual(result, [
      ...
      {template: 'citizennotes_module', moduleName: 'other', sources: ['foo.ts'], deps: []}
    ]);
  });
...
citizennotes_module("other") {
  sources = []
    ...
    const result = parseBuildGN(input);
    assert.deepEqual(result, [
      {template: 'citizennotes_module', moduleName: 'handlers', sources: ['HandlerOne.ts', 'HandlerTwo.ts'], deps: []},
      {template: 'citizennotes_module', moduleName: 'other', sources: [], deps: []}
    ]);
  });
});
...
describe('comparing imports from BUILD.gn and a source file', () => {
  it('can match up imports', () => {
    const buildGNFileContents = `citizennotes_module("handlers") {
  sources = [
    "HandlerOne.ts",
    "HandlerTwo.ts",
  ]
...
import * as SDK from '../../../core/sdk/sdk.js';
  ...
  it('can correctly identify sub-modules and the DEPS for them', () => {
    const buildGNFileContents = `citizennotes_module("helpers") {
  sources = ["helpers.ts"]
}

citizennotes_module("handlers") {
  sources = [
    "HandlerOne.ts",
  ]
...
import {Utils} from './Utils.js';
  ...
  it('uses the DEPS of the citizennotes_module if we find a citizennotes_entrypoint', () => {
    const buildGNFileContents = `citizennotes_module("handlers") {
  sources = [
    "HandlerOne.ts",
    "HandlerTwo.ts",
...
export {HandlerOne, HandlerTwo, Services};
  `;
    ...
    // and depend on a citizennotes_module, which is where the real logic is.
    const result = compareDeps({buildGN, sourceCode});
    assert.deepEqual(result, {
      inBoth: ['services'],
      missingBuildGNSources: [],
