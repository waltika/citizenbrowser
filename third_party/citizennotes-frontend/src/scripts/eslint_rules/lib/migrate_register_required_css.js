...
function updateGRDFile(cssFilePath) {
  ...
  const contents = fs.readFileSync('config/gni/citizennotes_grd_files.gni', 'utf-8').split('\n').map(c => c.trim());
  ...
  fs.writeFileSync('config/gni/citizennotes_grd_files.gni', finalContents, 'utf-8');
}
          ...
          try {
            ...
            if (wasShownFunction) {
              ...
              if (registerCSSFilesCall) {
                ...
                if (containsImport) {
                  // File is already imported so does not need another import statement or to be added to citizennotes_grd_files
                  context.report({
                    node,
