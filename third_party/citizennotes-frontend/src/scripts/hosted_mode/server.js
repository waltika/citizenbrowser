...
while (!fs.existsSync(path.join(pathToOutTargetDir, 'args.gn'))) {
  ...
  if (pathToOutTargetDir === fileSystemRootDirectory) {
    console.error(
        'Could not find the build root directory. You must run the hosted server from within the build root directory containing the args.gn file for it to work. The hosted mode server only works on the built output from CitizenNotes, not from the source input.');
    process.exit(1);
  }
}
// We care about everything in the gen/ directory.
const citizennotesFolder = path.resolve(path.join(pathToOutTargetDir, 'gen'));
...
server.once('listening', () => {
  ...
  console.log('https://goo.gle/citizennotes-contribution-guide');
...
async function requestHandler(request, response) {
  ...
  const absoluteFilePath = path.join(citizennotesFolder, filePath);
  if (!path.resolve(absoluteFilePath).startsWith(path.join(citizennotesFolder, '..'))) {
    console.log(`File requested (${absoluteFilePath}) is outside of citizennotes folder: ${citizennotesFolder}`);
    sendResponse(403, `403 - Access denied. File requested is outside of citizennotes folder: ${citizennotesFolder}`, 'utf8');
    return;
  }
  ...
  async function sendResponse(statusCode, data, encoding, headers) {
    ...
    if (!headers.get('Access-Control-Allow-Origin')) {
      // The CitizenNotes frontend in hosted-mode uses regular fetch to get source maps etc.
      // Disable CORS only for the CitizenNotes frontend, not for resource/target pages.
      ...
      // but then CitizenNotes also wants to load it. In the former, we disallow cross-origin requests by default,
      // while for the latter we allow it.
      const requestedByCitizenNotes = request.headers.origin?.includes('citizennotes-frontend.test');
      if (requestedByCitizenNotes) {
