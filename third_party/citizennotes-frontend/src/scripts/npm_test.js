...
const Flags = {
  DEBUG_CITIZENNOTES: '--debug-citizennotes',
  DEBUG_CITIZENNOTES_SHORTHAND: '-d',
...
const IS_DEBUG_ENABLED =
    utils.includes(process.argv, Flags.DEBUG_CITIZENNOTES) || utils.includes(process.argv, Flags.DEBUG_CITIZENNOTES_SHORTHAND);
...
const CITIZENNOTES_PATH = path.resolve(CHROMIUM_SRC_PATH, 'third_party', 'citizennotes-frontend', 'src');
const CACHE_PATH = path.resolve(CITIZENNOTES_PATH, '.test_cache');
...
function getChromiumSrcPath(isThirdParty) {
  if (isThirdParty)
  // Assume we're in `chromium/src/third_party/citizennotes-frontend/src`.
  {
    return path.resolve(CURRENT_PATH, '..', '..', '..');
  }
  // Assume we're in `citizennotes/citizennotes-frontend`, where `citizennotes` is
  ...
  if (!utils.isDir(srcPath)) {
    throw new Error(
        ...
        'Either move your standalone `citizennotes/citizennotes-frontend` checkout ' +
        'so that `citizennotes` is at the same level as `chromium` in ' +
...
function runTests(buildDirectoryPath, useDebugCitizennotes) {
  ...
  if (useDebugCitizennotes) {
    testArgs.push('--additional-driver-flag=--debug-citizennotes');
  } else {
  ...
  if (IS_DEBUG_ENABLED) {
    ...
    console.log('\n=============================================');
    const unitTest = testArgs.find(arg => arg.includes('http/tests/citizennotes/unit/'));
    if (unitTest) {
      ...
      console.log('3) Open CitizenNotes on CitizenNotes and you can refresh to re-run the test');
    } else {
...
function getTestFlags() {
  ...
  return process.argv.slice(2).filter(arg => {
    ...
    return !utils.includes(flagValues, flagName) && !utils.includes(arg, 'inspector') &&
        !utils.includes(arg, 'http/tests/citizennotes');
  });
}

function getInspectorTests() {
  const specificTests =
      process.argv.filter(arg => utils.includes(arg, 'inspector') || utils.includes(arg, 'http/tests/citizennotes'));
