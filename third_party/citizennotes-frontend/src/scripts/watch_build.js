...
// - `is_debug = true`
// - `citizennotes_skip_typecheck = true`
// flags set.
const assertTargetArgsForWatchBuild = async () => {
  ...
  try {
    ...
    assertTrueArg('citizennotes_skip_typecheck');
  } catch (err) {
    console.error(
        `watch_build needs is_debug and citizennotes_skip_typecheck args to be set to true for target ${target}\n${err}\n`);
    process.exit(1);
  }
};
