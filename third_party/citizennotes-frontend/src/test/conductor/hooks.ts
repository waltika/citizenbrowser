...
import {
  ...
  CitizenNotesFrontendTab,
  type CitizenNotesFrontendReloadOptions,
...
let frontendTab: CitizenNotesFrontendTab;
...
async function loadTargetPageAndFrontend(testServerPort: number) {
  ...
  // CitizenNotes Frontend in hosted mode, or the component docs in docs test mode.
  let frontend: puppeteer.Page;

  if (TEST_SERVER_TYPE === 'hosted-mode') {
    /**
     * In hosted mode we run the CitizenNotes and test against it.
     */
    frontendTab = await CitizenNotesFrontendTab.create({
      browser,
      testServerPort,
...
export async function reloadCitizenNotes(options?: CitizenNotesFrontendReloadOptions) {
...
export function getCitizenNotesFrontendHostname(): string {
