...
import {CitizenNotesFrontendTab} from './frontend_tab.js';
import {TargetTab} from './target_tab.js';

interface FrontendTargetTabs {
  frontend: CitizenNotesFrontendTab;
  target: TargetTab;
}
...
/**
 * A pool of CitizenNotes frontend tab plus target tab pairs. Every time a pair
...
export class FrontendTargetPool {
  ...
  private async addTabPairToPool(): Promise<void> {
    ...
    const frontend = await CitizenNotesFrontendTab.create(
