...
This directory hosts the end-to-end tests we run on CitizenNotes. These tests open a target page and a CitizenNotes frontend page, for which the CitizenNotes frontend connects to the target page over CDP. We use [Puppeteer] to talk over CDP and all functionality of Puppeteer is available to you as well when writing end-to-end tests. We use [Mocha] as testing framework.
...
1. Verify that core user stories are working as intended and are not broken by a particular CitizenNotes frontend change.
1. Serve as documentation and reference point for how CitizenNotes is intended to be used.
...
### Debug tests with CitizenNotes

Running `npm run debug-e2etest` also allows debugging a test with CitizenNotes. Please note that there are two different targets that can be inspected, and the way they are inspected differs slightly:

- You can debug the "CitizenNotes under test" with CitizenNotes-on-CitizenNotes. Use the standard CitizenNotes key combination to open another CitizenNotes instance while you look at the "CitizenNotes under test". You can set breakpoints and inspect the status of the "CitizenNotes under test" this way.
- You can debug the puppeteer side by inspecting the Node.js process that runs the e2e suite. Either open `chrome://inspect` or click the Node.js icon in any open CitizenNotes window to connect to the puppeteer process. You can step through the puppeteer test code this way.
...
```sh
git cl try -B citizennotes-frontend/try -b e2e_stressor_linux -b e2e_stressor_win64 -b e2e_stressor_mac -p e2e_env='{"TEST_PATTERNS":"network/network-datagrid_test.ts","ITERATIONS":20}'
```

or multiple test files:

```sh
git cl try -B citizennotes-frontend/try -b e2e_stressor_linux -b e2e_stressor_win64 -b e2e_stressor_mac -p e2e_env='{"TEST_PATTERNS":"network/network-datagrid_test.ts,network/network_test.ts","ITERATIONS":20}'
```
...
```sh
git cl try -B citizennotes-frontend/try \
...
To that end, the "what" from the "how" are separate in end-to-end tests. The "what" is the actual end-to-end test. The "how" are functions in [helpers](helpers/) that implement the interaction with CitizenNotes to perform a particular action.
...
Secondly, the function has a `waitFor`, which waits for the command menu (found by the `QUICK_OPEN_SELECTOR`) to appear. For every action that is performed in CitizenNotes, there must be a corresponding user-visible change in the UI. This means that you always have to wait for something to happen and you can't assume that, as soon as you have performed an action, the UI has updated accordingly.

**Note: Because of the async rendering of CitizenNotes, content might not be strictly visible when DOM Nodes are appended to the DOM.** As such, be aware of the functionality you are testing and relying on, as it could render differently than you originally assumed.
...
1. Be aware of the async rendering of CitizenNotes
...
The former are implemented in [helpers](helpers/), written by the CitizenNotes maintainers and are specific to the implementation of the CitizenNotes frontend. The latter are implemented in [../shared](../shared/), written by the Puppeteer maintainers and are predominantly CitizenNotes-agnostic (apart from the handling of a target page + CDP connection).
