...
const getFieldValuesTextContent = async () => {
  const fieldValues = await getTrimmedTextContent('citizennotes-report-value');
  if (fieldValues[0]) {
...
describe('The Application Tab', async () => {
  ...
  describe('', async () => {
    ...
    it('shows details for opened windows in the frame tree', async () => {
      ...
      // window.open above would put CitizenNotes in the background stopping updates
  ...
  it('shows dedicated workers in the frame tree', async () => {
    ...
    await click('#tab-resources');
    await doubleClickSourceTreeItem(TOP_FRAME_SELECTOR);
    // CitizenNotes is not ready yet when the worker is being initially attached.
    // We therefore need to reload the page to see the worker in CitizenNotes.
    await target.reload();
