...
describe('The Extension API', async () => {
  it('can open wasm resources with offset', async () => {
    ...
    await extension.waitForFunction(async (resource: string) => {
      const resources =
          await new Promise<Chrome.CitizenNotes.Resource[]>(r => window.chrome.citizennotes.inspectedWindow.getResources(r));
      return resources.find(r => r.url === resource);
    }, undefined, resource);

    // Accepts a wasm offset as column
    await extension.evaluate(resource => window.chrome.citizennotes.panels.openResource(resource, 0, 0x4b), resource);
    await waitForHighlightedLine(0x4b);

    // Selects the right wasm line on an inexact match
    await extension.evaluate(resource => window.chrome.citizennotes.panels.openResource(resource, 0, 0x4e), resource);
    await waitForHighlightedLine(0x4d);

    // Accepts a missing columnNumber
    await extension.evaluate(resource => window.chrome.citizennotes.panels.openResource(resource, 0), resource);
    await waitForHighlightedLine(0);

    // Accepts a wasm offset as column and a callback
    {
      const r = await extension.evaluate(
          resource => new Promise(r => window.chrome.citizennotes.panels.openResource(resource, 0, 0x4b, () => r(1))),
          resource);
    ...
    // Is backwards compatible: accepts a callback with a missing columnNumber
    {
      const r = await extension.evaluate(
          ...
          resource => new Promise(r => window.chrome.citizennotes.panels.openResource(resource, 0, () => r(1))), resource);
      assert.deepEqual(r, 1);
    }
    await waitForHighlightedLine(0);
  });
  ...
  it('can open page resources with column numbers', async () => {
    ...
    // Accepts a missing columnNumber
    await extension.evaluate(resource => window.chrome.citizennotes.panels.openResource(resource, 2), resource);
    await waitForHighlightedLine(3);

    // Accepts a column number
    {
      await extension.evaluate(resource => window.chrome.citizennotes.panels.openResource(resource, 29, 160), resource);
    ...
    {
      const r = await extension.evaluate(
          resource => new Promise(r => window.chrome.citizennotes.panels.openResource(resource, 1, 2000, () => r(1))),
          resource);
      assert.deepEqual(r, 1);
    ...
    // Is backwards compatible: accepts a callback with a missing columnNumber
    {
      const r = await extension.evaluate(
          // @ts-expect-error Legacy API
          resource => new Promise(r => window.chrome.citizennotes.panels.openResource(resource, 2, () => r(1))), resource);
      assert.deepEqual(r, 1);
      await waitForHighlightedLine(3);
    }
  });
