...
import {getResourcesPathWithCitizenNotesHostname, loadExtension} from '../helpers/extension-helpers.js';
import {
...
declare global {
  let chrome: Chrome.CitizenNotes.Chrome;
  interface Window {
...
declare function RegisterExtension(
    pluginImpl: Partial<Chrome.CitizenNotes.LanguageExtensionPlugin>, name: string,
    // eslint-disable-next-line @typescript-eslint/naming-convention
...
// This testcase reaches into CitizenNotes internals to install the extension plugin. At this point, there is no sensible
...
describe('The Debugger Language Plugins', async () => {
  ...
  it('can show C filenames after loading the module', async () => {
    ...
    const extension = await loadExtension(
        'TestExtension', `${getResourcesPathWithCitizenNotesHostname()}/extensions/language_extensions.html`);
    await extension.evaluate(() => {
      // A simple plugin that resolves to a single source file
      class SingleFilePlugin {
        async addRawModule(rawModuleId: string, symbols: string, rawModule: Chrome.CitizenNotes.RawModule) {
  ...
  it('use correct code offsets to interpret raw locations', async () => {
    ...
    await extension.evaluate((mappings: LabelMapping[]) => {
      class LocationMappingPlugin {
        ...
        async rawLocationToSourceLocation(rawLocation: Chrome.CitizenNotes.RawLocation) {
          if (this.module) {
  ...
  it('resolve locations for breakpoints correctly', async () => {
    ...
    await extension.evaluate((mappings: LabelMapping[]) => {
      ...
      class LocationMappingPlugin {
        ...
        async sourceLocationToRawLocation(sourceLocation: Chrome.CitizenNotes.SourceLocation):
    ...
    const toolbarLink = await waitFor('.toolbar-item .citizennotes-link');
  ...
  it('shows top-level and nested variables', async () => {
    ...
    await extension.evaluateHandle(() => {
      class VariableListingPlugin {
        private modules:
            Map<string,
                {rawLocationRange?: Chrome.CitizenNotes.RawLocationRange, sourceLocation?: Chrome.CitizenNotes.SourceLocation}>;
        constructor() {
        ...
        async listVariablesInScope(rawLocation: Chrome.CitizenNotes.RawLocation) {
    ...
    await extension.evaluate(() => {
      class InliningPlugin {
        private modules: Map<string, {
          rawLocationRange?: Chrome.CitizenNotes.RawLocationRange,
          sourceLocations?: Chrome.CitizenNotes.SourceLocation[],
        }>;
        constructor() {
          this.modules = new Map();
        }
        ...
        async getFunctionInfo(rawLocation: Chrome.CitizenNotes.RawLocation) {
          const {rawLocationRange} = this.modules.get(rawLocation.rawModuleId) || {};
          if (rawLocationRange && rawLocationRange.startOffset <= rawLocation.codeOffset &&
              rawLocation.codeOffset < rawLocationRange.endOffset) {
  ...
  it('falls back to wasm function names when inline info not present', async () => {
    ...
    await extension.evaluate(() => {
      class InliningPlugin {
        ...
        async listVariablesInScope(_rawLocation: Chrome.CitizenNotes.RawLocation) {
          return [];
        }
      }
  ...
  it('shows variable values with the evaluate API', async () => {
    ...
    await extension.evaluate(() => {
      class EvalPlugin {
        ...
        async evaluate(expression: string, _context: Chrome.CitizenNotes.RawLocation, _stopId: unknown):
        ...
        async getProperties(objectId: string): Promise<Chrome.CitizenNotes.PropertyDescriptor[]> {
  ...
  it('shows variable value in popover', async () => {
    ...
    await extension.evaluate(() => {
      class VariableListingPlugin {
        ...
        evaluate(expression: string, _context: Chrome.CitizenNotes.RawLocation, _stopId: unknown):
            Promise<Chrome.CitizenNotes.RemoteObject|null> {
  ...
  it('can access wasm data directly', async () => {
    ...
    const mem = await extension.evaluate(async(): Promise<number[]> => {
      const buffer = await chrome.citizennotes.languageServices.getWasmLinearMemory(0, 10, 0n);
    ...
    const global = await extension.evaluate(() => chrome.citizennotes.languageServices.getWasmGlobal(0, 0n));
    assert.deepEqual(global, {type: 'i32', value: 0xdad});

    const local = await extension.evaluate(() => chrome.citizennotes.languageServices.getWasmLocal(0, 0n));
    assert.deepEqual(local, {type: 'i32', value: 4});

    const local2 = await extension.evaluate(() => chrome.citizennotes.languageServices.getWasmLocal(1, 0n));
    ...
    const fail = await extension.evaluate(() => chrome.citizennotes.languageServices.getWasmLocal(1, 0n));
    ...
    // const local2Set = await extension.evaluate(() => chrome.citizennotes.languageServices.getWasmLocal(1, 1n));
