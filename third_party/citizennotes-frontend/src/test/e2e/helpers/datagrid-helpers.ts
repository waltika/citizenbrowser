...
export async function getDataGridRows(
  ...
  const dataGrid = !root ? await waitFor('citizennotes-data-grid') : root;
...
export async function getDataGrid(root?: ElementHandle) {
  const dataGrid = await waitFor('citizennotes-data-grid', root);
  if (!dataGrid) {
...
export async function getDataGridController() {
  const dataGrid = await waitFor('citizennotes-data-grid-controller');
  if (!dataGrid) {
