...
const COMPUTED_PROPERTY_SELECTOR = 'citizennotes-computed-style-property';
...
const ADORNER_SELECTOR = 'citizennotes-adorner';
...
export const waitForDomNodeToBeVisible = async (elementSelector: string) => {
  ...
  // CitizenNotes will force Blink to make the hover shown, so we have
...
export const getComputedStyleProperties = async () => {
  ...
  for (const prop of allProperties) {
    ...
    const traceElements = await prop.$$('pierce/citizennotes-computed-style-trace');
...
export const getStyleRuleWithSourcePosition = (styleSelector: string, sourcePosition?: string) => {
  ...
  return waitForFunction(async () => {
    ...
    if (candidate) {
      const sourcePositionElement = await candidate.$('.styles-section-subtitle .citizennotes-link');
...
export const getBreadcrumbsTextContent = async ({expectedNodeCount}: {expectedNodeCount: number}) => {
  const crumbsSelector = 'li.crumb > a > citizennotes-node-text';
  await waitForFunction(async () => {
...
export const getSelectedBreadcrumbTextContent = async () => {
  const selectedCrumb = await waitFor('li.crumb.selected > a > citizennotes-node-text');
...
export const clickOnFirstLinkInStylesPanel = async () => {
  ...
  await click('div.styles-section-subtitle span.citizennotes-link', {root: stylesPane});
};
