...
import {getBrowserAndPages, getCitizenNotesFrontendHostname, getResourcesPath, waitFor} from '../../shared/helper.js';
...
export function getResourcesPathWithCitizenNotesHostname() {
  return getResourcesPath(getCitizenNotesFrontendHostname());
}

export async function loadExtension(name: string, startPage?: string) {
  startPage = startPage || `${getResourcesPathWithCitizenNotesHostname()}/extensions/empty_extension.html`;
  ...
  async function doLoad(frontend: puppeteer.Page, extensionInfo: {startPage: string, name: string}) {
    ...
    try {
      await frontend.evaluate(extensionInfo => {
        ...
        const extensionIFrames = document.body.querySelectorAll(`[data-citizennotes-extension="${extensionInfo.name}"]`);
      ...
      const iframe = await waitFor(`[data-citizennotes-extension="${name}"]`);
