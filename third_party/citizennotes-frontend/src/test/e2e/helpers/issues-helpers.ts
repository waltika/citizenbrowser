...
export const HIDE_ISSUES_MENU = 'citizennotes-hide-issues-menu';
    ...
    return await table.evaluate(table => {
      ...
      for (const tableRow of table.childNodes) {
        ...
        for (const cell of tableRow.childNodes) {
          const requestLinkIcon = cell instanceof HTMLElement && cell.querySelector('citizennotes-request-link-icon');
          if (requestLinkIcon) {
