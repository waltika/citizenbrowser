...
export const DEBUGGER_PAUSED_EVENT = 'CitizenNotes.DebuggerPaused';
 ...
 * CitizenNotes uses the escaped snippet name for the ARIA label. `createNewSnippet`
...
export async function refreshCitizenNotesAndRemoveBackendState(target: puppeteer.Page) {
  // Navigate to a different site to make sure that back-end state will be removed.
  await target.goto('about:blank');
  await reloadCitizenNotes({selectedPanel: {name: 'sources'}});
}
