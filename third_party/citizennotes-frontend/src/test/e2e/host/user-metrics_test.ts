...
describe('User Metrics', () => {
  it('dispatches dock and undock events', async () => {
    ...
    await assertHistogramEventsInclude([
      {
        actionName: 'CitizenNotes.ActionTaken',
  ...
  it('dispatches a metric event the console drawer', async () => {
    ...
    await assertHistogramEventsInclude([
      {
        actionName: 'CitizenNotes.PanelShown',
        actionCode: 10,  // drawer-console-view.
      },
      {
        actionName: 'CitizenNotes.KeyboardShortcutFired',
  ...
  it('dispatches events for view shown at launch', async () => {
    await reloadCitizenNotes({selectedPanel: {name: 'timeline'}});
  ...
  it('dispatches events for drawer shown at launch', async () => {
    await reloadCitizenNotes({drawerShown: true});
  ...
  it('dispatches events for opening issues drawer via hamburger menu', async () => {
    ...
    await assertHistogramEventsInclude([
      {
        actionName: 'CitizenNotes.IssuesPanelOpenedFrom',
  ...
  it('dispatches an event when the keybindSet setting is changed', async () => {
    ...
    await assertHistogramEventsInclude([
      ...
      {
        actionName: 'CitizenNotes.KeybindSetSettingChanged',
        actionCode: 1,  // vsCode
      },
    ]);
  });
  ...
  it('dispatches closed panel events for views', async () => {
    ...
    await assertHistogramEventsInclude([
      ...
      {
        actionName: 'CitizenNotes.PanelClosed',
        actionCode: 16,  // Security
      },
    ]);
  });
  ...
  it('dispatches an event when experiments are enabled and disabled', async () => {
    ...
    await assertHistogramEventsInclude([
      ...
      {
        actionName: 'CitizenNotes.ExperimentEnabled',
        actionCode: 0,  // Allow extensions to load custom stylesheets
      },
      {
        actionName: 'CitizenNotes.ExperimentDisabled',
        actionCode: 0,  // Allow extensions to load custom stylesheets
      },
    ]);
  });
  ...
  it('dispatches an event when experiments are initialized at launch', async () => {
    await assertHistogramEventsInclude([
      {
        actionName: 'CitizenNotes.ExperimentEnabledAtLaunch',
        actionCode: 52,  // Enabled by default: cssTypeComponentLength
      },
      {
        actionName: 'CitizenNotes.ExperimentDisabledAtLaunch',
  ...
  it('tracks panel loading', async () => {
    ...
    assert.include(events.map(e => e.histogramName), 'CitizenNotes.Launch.Timeline');
  });

  it('records the selected language', async () => {
    await assertHistogramEventsInclude([
      {
        actionName: 'CitizenNotes.Language',
        actionCode: 17,  // en-US
      },
    ]);
  });
  ...
  it('records the sync setting', async () => {
    await assertHistogramEventsInclude([{
      actionName: 'CitizenNotes.SyncSetting',
...
describe('User Metrics for sidebar panes', () => {
  it('dispatches sidebar panes events for navigating Elements Panel sidebar panes', async () => {
    ...
    await assertHistogramEventsInclude([
      {
        actionName: 'CitizenNotes.Elements.SidebarTabShown',
  ...
  it('should not dispatch sidebar panes events for navigating to the same pane', async () => {
    ...
    assert.notInclude(eventNames, 'CitizenNotes.Elements.SidebarTabShown');
  });
  ...
  it('dispatches sidebar panes events for switching to \'Workspace\' tab in the \'Sources\' panel', async () => {
    ...
    await assertHistogramEventsInclude([
      {
        actionName: 'CitizenNotes.Sources.SidebarTabShown',
...
describe('User Metrics for Issue Panel', () => {
  ...
  it('dispatches an event when a LowTextContrastIssue is created', async () => {
    ...
    await assertHistogramEventsInclude([
      {
        actionName: 'CitizenNotes.IssueCreated',
  ...
  it('dispatch events when a link to an element is clicked', async () => {
    ...
    await assertHistogramEventsInclude([
      ...
      {
        actionName: 'CitizenNotes.IssuesPanelIssueExpanded',
        actionCode: 4,  // ContentSecurityPolicy
      },
      {
        actionName: 'CitizenNotes.IssuesPanelResourceOpened',
...
describe('User Metrics for the Page Resource Loader', () => {
  it('dispatches an event when a source map is loaded', async () => {
    ...
    await waitForHistogramEvent(
        {
          actionName: 'CitizenNotes.DeveloperResourceLoaded',
          actionCode: 0,  // LoadThroughPageViaTarget
        },
    );
    await waitForHistogramEvent(
        {
          actionName: 'CitizenNotes.DeveloperResourceScheme',
        },
    );
  });
});
...
describe('User Metrics for clicking stylesheet request initiators', () => {
  ...
  it('dispatches an event when clicked in the console', async () => {
    async function clickOnLinkWithText(text: string, root?: puppeteer.JSHandle) {
      ...
      assert.isTrue(
          await element.evaluate(e => e.classList.contains('citizennotes-link')),
          'Clicked element was not a citizennotes link');
    }
  ...
  it('dispatches an event when clicked in the Network panel', async () => {
    async function clickOnInitiatorLink(resource: string) {
      ...
      const element = await click(`[title="${resourceURL}"] ~ .initiator-column .citizennotes-link`);
      assert.isTrue(
          await element.evaluate(e => e.classList.contains('citizennotes-link')),
          'Clicked element was not a citizennotes link');
    }
