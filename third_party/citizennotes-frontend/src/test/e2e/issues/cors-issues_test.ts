...
describe('CORS issues', async () => {
  it('should display CORS violations with the correct affected resources', async () => {
    ...
    await target.evaluate(async () => {
      // @ts-ignore
      await window.doCorsFetches(`https://citizennotes.oopif.test:${document.location.port}`);
    });
    ...
    await target.evaluate(async () => {
      try {
        const url = new URL('./issues/origin-wildcard.rawresponse', document.location.toString())
                        .toString()
                        .replace('localhost', 'citizennotes.oopif.test');
  ...
  it('should display CORS issues that are disallowed by the mode', async () => {
    ...
    await target.evaluate(async () => {
      try {
        const url = new URL('/', document.location.toString()).toString().replace('localhost', 'citizennotes.oopif.test');
    ...
    await target.evaluate(async () => {
      try {
        const url = new URL('/', document.location.toString())
                        .toString()
                        .replace('https://localhost', 'webdav://citizennotes.oopif.test');
        await fetch(url);
      } catch (e) {
      }
    });
