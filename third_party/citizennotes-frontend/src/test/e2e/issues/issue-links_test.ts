...
describe('Issue links in the console tab', async () => {
  it('should reveal the right issue', async () => {
    ...
    const issueLinkIcon = await waitFor('citizennotes-issue-link-icon');
    const citizennotesIcon = await waitFor('citizennotes-icon', issueLinkIcon);
    ...
    const issueTitleFromLink = await waitForFunction(async () => {
      const title = await citizennotesIcon.evaluate(el => (el as HTMLElement).title);
    ...
    await click('citizennotes-issue-link-icon');
