...
describe('Navigation', async function() {
  ...
  it('successfully returns a Lighthouse report with CitizenNotes throttling', async () => {
    await navigateToLighthouseTab('lighthouse/hello.html');

    await setThrottlingMethod('citizennotes');

    await clickStartButton();

    const {lhr, reportEl} = await waitForResult();

    assert.strictEqual(lhr.configSettings.throttlingMethod, 'citizennotes');

    // [crbug.com/1347220] CitizenNotes throttling can force resources to load slow enough for these audits to fail sometimes.
    const flakyAudits = [
  ...
  it('successfully returns a Lighthouse report when settings changed', async () => {
    await setCitizenNotesSettings({language: 'es'});
    await navigateToLighthouseTab('lighthouse/hello.html');
    await registerServiceWorker();
