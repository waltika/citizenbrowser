...
describe('The Memory Panel', async function() {
  // These tests render large chunks of data into CitizenNotes and filter/search
  ...
  it('shows object source links in snapshot', async () => {
    ...
    for (const entry of expectedEntries) {
      ...
      const linkText =
          await waitForFunction(async () => element?.evaluate(e => e.querySelector('.citizennotes-link')?.textContent));
