...
const RECORDER_CONTROLLER_TAG_NAME = 'citizennotes-recorder-controller';
...
async function createRecording(name: string, selectorAttribute?: string) {
  ...
  if (selectorAttribute) {
    const input = await waitForAria(
        'SELECTOR ATTRIBUTE https://g.co/citizennotes/recorder#selector',
    );
...
export async function createAndStartRecording(
  ...
  await click('citizennotes-control-button');
  await waitFor('citizennotes-recording-view');
...
export async function clickSelectButtonItem(itemLabel: string, root: string) {
  ...
  const selectMenuButton = await waitFor(
      'citizennotes-select-menu-button',
      selectMenu,
  );
  ...
  const selectMenuItems = await selectMenu.$$('pierce/citizennotes-menu-item');
...
export async function setupRecorderWithScriptAndReplay(
  ...
  await clickSelectButtonItem('Normal (Default)', 'citizennotes-replay-button');
  await onceFinished;
}
