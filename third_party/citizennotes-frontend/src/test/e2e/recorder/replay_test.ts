...
describe('Recorder', function() {
  ...
  describe('Replay', () => {
    ...
    it('should record interactions with OOPIFs', async () => {
      ...
      await setupRecorderWithScriptAndReplay({
        ...
        steps: [
          ...
          {
            type: 'click' as StepType.Click,
            target: `https://citizennotes.oopif.test:${getTestServerPort()}/test/e2e/resources/recorder/iframe1.html`,
            ...
            assertedEvents: [
              {
                type: 'navigation' as AssertedEventType.Navigation,
                title: '',
                url: `https://citizennotes.oopif.test:${getTestServerPort()}/test/e2e/resources/recorder/iframe2.html`,
              },
            ],
          },
        ],
      ...
      const frame = target.frames().find(
          frame => frame.url() ===
              `https://citizennotes.oopif.test:${getTestServerPort()}/test/e2e/resources/recorder/iframe2.html`,
      );
      assert.ok(frame, 'Frame that the target page navigated to is not found');
    });
    ...
    it('should replay when clicked on slow replay', async () => {
      ...
      const onceFinished = onReplayFinished();
      await clickSelectButtonItem('Slow', 'citizennotes-replay-button');
      await onceFinished;
