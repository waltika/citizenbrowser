...
describe('Recorder', function() {
  ...
  describe('UI', () => {
    ...
    describe('Settings', () => {
      it('should change network settings', async () => {
        ...
        const selectMenu = await click(
            '.editable-setting citizennotes-select-menu',
        );
        await waitForAnimationFrame();

        await click('citizennotes-menu-item:nth-child(3)', {root: selectMenu});
        await waitForAnimationFrame();
    ...
    describe('Shortcuts', () => {
      it('should toggle code view with shortcut', async () => {
        let noSplitView = await waitForNone('citizennotes-split-view');
        assert.isTrue(noSplitView);

        await toggleCodeView();
        const splitView = await waitFor('citizennotes-split-view');
        assert.isOk(splitView);

        await toggleCodeView();
        noSplitView = await waitForNone('citizennotes-split-view');
        assert.isTrue(noSplitView);
      });
    });
  });
  ...
  describe('Header', () => {
    ...
    describe('Shortcut Dialog', () => {
      it('should open the shortcut dialog', async () => {
        const {frontend} = getBrowserAndPages();
        await frontend.bringToFront();
        const shortcutDialog = await waitFor('citizennotes-shortcut-dialog');

        await click('citizennotes-button', {root: shortcutDialog});

        const dialog = await waitFor('citizennotes-dialog', shortcutDialog);
        assert.isOk(dialog);
  ...
  describe('Recording list', () => {
    ...
    it('can delete a recording from the list', async () => {
      ...
      await frontend.waitForSelector('pierce/citizennotes-start-view');
    });
  });
});
