...
import {reloadCitizenNotes} from '../../shared/helper.js';
...
describe('The Security Panel', async function() {
  it('is open by default when citizennotes initializes', async () => {
  ...
  it('appears under More tools after being closed', async () => {
    ...
    await reloadCitizenNotes({selectedPanel: {name: 'security'}});
  ...
  it('opens if the query param "panel" is set', async () => {
    await closeSecurityTab();
    await reloadCitizenNotes({queryParams: {panel: 'security'}});
    await securityTabExists();
