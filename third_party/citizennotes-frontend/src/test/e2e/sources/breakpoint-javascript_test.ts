...
import {
  ...
  refreshCitizenNotesAndRemoveBackendState,
  RESUME_BUTTON,
...
describe('The Sources Tab', async function() {
  ...
  it('can hit a breakpoint on the main thread on a fresh CitizenNotes', async () => {
    ...
    await step('Navigate to a different site to refresh citizennotes and remove back-end state', async () => {
      await refreshCitizenNotesAndRemoveBackendState(target);
    });
  ...
  it.skip(
      '[crbug.com/1229541] can hit a breakpoint in an inline script on the main thread on a fresh CitizenNotes',
      async () => {
        await enableExperiment('instrumentationBreakpoints');
        const {frontend, target} = getBrowserAndPages();
  ...
  it('can hit a breakpoint in an inline script with sourceURL comment on the main thread on a fresh CitizenNotes',
     async () => {
       await enableExperiment('instrumentationBreakpoints');
       const {frontend, target} = getBrowserAndPages();
