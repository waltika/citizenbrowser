...
import {type CitizenNotesRecorder} from '../../../../../front_end/panels/recorder/injected/injected.js';
...
describe('Injected', () => {
  ...
  beforeEach(async () => {
    ...
    await frontend.evaluate(() => {
      (window.CitizenNotesRecorder as CitizenNotesRecorder)
          .startRecording(
              {
  ...
  afterEach(async () => {
    const {frontend} = getBrowserAndPages();
    await frontend.evaluate(() => {
      window.CitizenNotesRecorder.stopRecording();
    });
  });
  ...
  it('should get selectors for an element', async () => {
    ...
    const selectors = await frontend.evaluate(() => {
      ...
      return window.CitizenNotesRecorder.recordingClientForTesting.getSelectors(
          target,
      );
    });
  ...
  it('should get selectors for elements with custom selector attributes', async () => {
    ...
    const selectors = await frontend.evaluate(() => {
      ...
      return targets.map(
          window.CitizenNotesRecorder.recordingClientForTesting.getSelectors,
      );
    });
    assertMatchesJSONSnapshot(selectors);
  });
  ...
  describe('CSS selectors', () => {
    it('should query CSS selectors', async () => {
      ...
      const results = await frontend.evaluate(() => {
        return [
          window.CitizenNotesRecorder.recordingClientForTesting
    ...
    it('should return not-optimized CSS selectors for duplicate elements', async () => {
      ...
      const selector = await frontend.evaluate(() => {
        ...
        return window.CitizenNotesRecorder.recordingClientForTesting.getCSSSelector(
            target,
        );
      });
  ...
  describe('Text selectors', () => {
    const getSelectorOfButtonWithLength = (length: number) => {
      ...
      return frontend.evaluate(length => {
        ...
        return window.CitizenNotesRecorder.recordingClientForTesting.getTextSelector(
            target,
        );
      }, length);
    };
