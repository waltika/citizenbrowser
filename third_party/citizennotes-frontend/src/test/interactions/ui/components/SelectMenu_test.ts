...
async function getFocusedItemValue() {
  const focusedItem = await waitFor('citizennotes-menu-item:focus');
...
async function getSelectMenu(options: GetSelectMenuOptions = {}) {
  ...
  if (options.placeholderSelector) {
    ...
    return await waitFor('citizennotes-select-menu', placeholder);
  }
  return await waitFor('citizennotes-select-menu');
}
...
async function testScreenshotOnPlaceholder(placeholderSelector: string, screenshot: string) {
  ...
  const selectMenu = await waitFor('citizennotes-select-menu', dialog);
  const animationEndPromise = waitForDialogAnimationEnd();
  await click('button', {
    root: selectMenu,
  });
...
describe('SelectMenu', () => {
  ...
  it('triggers a selectmenuselected event when clicking an item from the menu', async () => {
    await openMenu();
    const item = await waitFor('citizennotes-select-menu > citizennotes-menu-item:nth-child(1)');
  ...
  it('renders a menu in its correct position (top or bottom)', async () => {
    ...
    await animationEndPromise;
    const regularMenuWrapper = await waitFor('citizennotes-select-menu', placeHolder1);
    ...
    await animationEndPromise;
    const invertedMenuWrapper = await waitFor('citizennotes-select-menu', placeHolder2);
  ...
  it('does not open on click if it\'s disabled', async () => {
    ...
    const selectMenu = await waitFor('citizennotes-select-menu', selectMenuWrapper);

    const selectMenuButton = await waitFor('citizennotes-select-menu-button', selectMenu);
