...
import {type CitizenNotesFrontendReloadOptions} from '../conductor/frontend_tab.js';
import {getCitizenNotesFrontendHostname, reloadCitizenNotes} from '../conductor/hooks.js';
...
async function setExperimentEnabled(experiment: string, enabled: boolean, options?: CitizenNotesFrontendReloadOptions) {
  const {frontend} = getBrowserAndPages();
  await frontend.evaluate(`(async () => {
    const Root = await import('./core/root/root.js');
    Root.Runtime.experiments.setEnabled('${experiment}', ${enabled});
  })()`);
  await reloadCitizenNotes(options);
}

export const enableExperiment = (experiment: string, options?: CitizenNotesFrontendReloadOptions) =>
    setExperimentEnabled(experiment, true, options);

export const disableExperiment = (experiment: string, options?: CitizenNotesFrontendReloadOptions) =>
    setExperimentEnabled(experiment, false, options);

export const setCitizenNotesSettings = async (settings: Record<string, string>) => {
  const {frontend} = getBrowserAndPages();
...
export {getBrowserAndPages, getCitizenNotesFrontendHostname, getTestServerPort, reloadCitizenNotes};
