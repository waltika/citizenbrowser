...
function assertEqualUrlStringString(actual: Platform.CitizenNotesPath.UrlString|null, expected: string, message?: string) {
  assert.strictEqual(actual, expected as Platform.CitizenNotesPath.UrlString, message);
}
...
describe('Parsed URL', () => {
  ...
  it('converts platform path to a URL that does not start with "file://"', () => {
    const platformPathTest = 'usr/lib' as Platform.CitizenNotesPath.RawPathString;
  ...
  it('converts platform path to a URL that starts with "file://"', () => {
    const platformPathTest = 'file://usr/lib' as Platform.CitizenNotesPath.RawPathString;
    const convertedUrl = ParsedURL.rawPathToUrlString(platformPathTest);
    assert.strictEqual(convertedUrl, 'file://usr/lib', 'URL was not converted correctly');
  });

  it('converts path that starts with "file://" to a platform path', () => {
    const pathTest = 'file://usr/lib' as Platform.CitizenNotesPath.UrlString;
  ...
  it('extracts the path from a valid URL', () => {
    const urlTest = 'http://www.example.com/test/path' as Platform.CitizenNotesPath.UrlString;
  ...
  it('returns an empty string as a path if the URL is not valid', () => {
    const urlTest = 'www.example.com/test/path' as Platform.CitizenNotesPath.UrlString;
    const extractedPath = ParsedURL.extractPath(urlTest);
  ...
  it('returns an empty string as a origin if the URL is not valid', () => {
    ...
    const extractedOrigin = ParsedURL.extractOrigin(urlTest as Platform.CitizenNotesPath.UrlString);
  ...
  it('uses the completeURL function to return a data URL as it is', () => {
    ...
    const baseUrlTest = 'www.example.com' as Platform.CitizenNotesPath.UrlString;
  ...
  it('uses the completeURL function to return the href if the base URL is a data URL', () => {
    const hrefTest = 'www.example.com';
    const baseUrlTest = 'data://www.example.com' as Platform.CitizenNotesPath.UrlString;
    const completeUrl = ParsedURL.completeURL(baseUrlTest, hrefTest);
    assertEqualUrlStringString(completeUrl, hrefTest, 'complete URL is not returned correctly');
  });
  ...
  it('uses the completeURL function to return the href with scheme if the base URL was valid and the href scheme was dropped',
     () => {
       const hrefTest = '//www.example.com';
       const baseUrlTest = 'http://www.example.com/' as Platform.CitizenNotesPath.UrlString;
       const completeUrl = ParsedURL.completeURL(baseUrlTest, hrefTest);
  ...
  it('uses the completeURL function to resolve an empty href to a base URL without fragment', () => {
    const hrefTest = '';
    const baseUrlTest = 'http://www.example.com/?testParam=t' as Platform.CitizenNotesPath.UrlString;
    const completeUrl = ParsedURL.completeURL(baseUrlTest, hrefTest);
  ...
  it('returns the correct results for all ported web_tests unit tests', () => {
    assertEqualUrlStringString(
        ParsedURL.completeURL(
            'http://example.com/script.js' as Platform.CitizenNotesPath.UrlString, 'http://example.com/map.json'),
        'http://example.com/map.json');
    assertEqualUrlStringString(
        ParsedURL.completeURL('http://example.com/script.js' as Platform.CitizenNotesPath.UrlString, '/map.json'),
        'http://example.com/map.json');
    assertEqualUrlStringString(
        ParsedURL.completeURL(
            'http://example.com/scripts/script.js' as Platform.CitizenNotesPath.UrlString, '../maps/map.json'),
        'http://example.com/maps/map.json');

    const baseURL = 'http://a/b/c/d;p?q' as Platform.CitizenNotesPath.UrlString;
    ...
    assertEqualUrlStringString(
        ParsedURL.completeURL('http://a/b/c/d;p?q' as Platform.CitizenNotesPath.UrlString, '//secure.com/moo'),
        'http://secure.com/moo');
    assertEqualUrlStringString(
        ParsedURL.completeURL('http://a/b/c/d;p?q' as Platform.CitizenNotesPath.UrlString, 'cat.jpeg'),
        'http://a/b/c/cat.jpeg');
    assertEqualUrlStringString(
        ParsedURL.completeURL('http://example.com/path.css?query#fragment' as Platform.CitizenNotesPath.UrlString, ''),
        'http://example.com/path.css?query');
  });

  it('encodes partial path', () => {
    const pathTest = 'path/with%20escape/and spaces' as Platform.CitizenNotesPath.RawPathString;
  ...
  it('decodes partial path', () => {
    ...
    const encodedPath = 'path/with%2520escape/and%20spaces' as Platform.CitizenNotesPath.EncodedPathString;
  ...
  it('encodes, decodes partial path with email address', () => {
    const pathTest = 'username:password@example.com' as Platform.CitizenNotesPath.RawPathString;  // valid filename on unix
  ...
  it('encodes, decodes partial path', () => {
    const pathTest = 'C:/Program%20Files/Google' as Platform.CitizenNotesPath.RawPathString;
    const encodedPath = ParsedURL.rawPathToEncodedPathString(pathTest);
    const convertedPath = ParsedURL.encodedPathToRawPathString(encodedPath);
    assert.strictEqual(convertedPath, pathTest, 'path was not converted successfully');
  });
  ...
  it('encodes, decodes partial path with whitespace', () => {
    const pathTest = 'C:/Program Files/Google' as Platform.CitizenNotesPath.RawPathString;
    const encodedPath = ParsedURL.rawPathToEncodedPathString(pathTest);
    const convertedPath = ParsedURL.encodedPathToRawPathString(encodedPath);
    assert.strictEqual(convertedPath, pathTest, 'path was not converted successfully');
  });
  ...
  it('converts relative platform path and base URL to URL', () => {
    const baseUrl = 'http://localhost:8080/my%20folder/old%20path' as Platform.CitizenNotesPath.UrlString;
    const relativePath = 'new spaced%20name' as Platform.CitizenNotesPath.RawPathString;
  ...
  it('converts URL to a platform path that includes drive letter and spaces on Windows', () => {
    const urlTest = 'file:///C:/Program%20Files/Google' as Platform.CitizenNotesPath.UrlString;
  ...
  it('converts URL to a platform path that includes spaces and percents', () => {
    const urlTest = 'file:///home/user/with%20space/with%2520escape' as Platform.CitizenNotesPath.UrlString;
  ...
  it('converts Windows platform path with spaces and percents to file url', () => {
    const urlTest = 'C:\\Program Files\\with%20escape' as Platform.CitizenNotesPath.RawPathString;
  ...
  it('converts platform path with variety of special characters to URL and back consistently with Chrome', () => {
    const platformPathTest =
        '/home/a:b@c(d, e+f)=&g;#h$' as Platform.CitizenNotesPath.RawPathString;  // Valid filename on unix
    const urlTest = 'file:///home/a:b@c(d,%20e+f)=&g%3B%23h$' as
        Platform.CitizenNotesPath.UrlString;  // URL in Chrome address bar if you open that file
