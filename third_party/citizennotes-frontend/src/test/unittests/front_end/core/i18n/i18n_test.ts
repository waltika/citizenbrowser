...
describe('getLocalizedLanguageRegion', () => {
  function createMockCitizenNotesLocale(locale: string): i18n.CitizenNotesLocale.CitizenNotesLocale {
    return {locale, forceFallbackLocale: () => {}} as i18n.CitizenNotesLocale.CitizenNotesLocale;
  }

  it('build the correct language/region string', () => {
    assert.strictEqual(
        i18n.i18n.getLocalizedLanguageRegion('de-AT', createMockCitizenNotesLocale('en-US')),
        'German (Austria) - Deutsch (Österreich)');
    assert.strictEqual(
        i18n.i18n.getLocalizedLanguageRegion('de', createMockCitizenNotesLocale('en-US')), 'German - Deutsch');
    assert.strictEqual(
        i18n.i18n.getLocalizedLanguageRegion('en-US', createMockCitizenNotesLocale('de')), 'Englisch (USA) - English (US)');
  });

  it('uses english for the target locale if the languages match', () => {
    assert.strictEqual(
        i18n.i18n.getLocalizedLanguageRegion('de-AT', createMockCitizenNotesLocale('de')),
        'Deutsch (Österreich) - German (Austria)');
    assert.strictEqual(i18n.i18n.getLocalizedLanguageRegion('de', createMockCitizenNotesLocale('de')), 'Deutsch - German');
  });
});
...
describe('fetchAndRegisterLocaleData', () => {
  ...
  const bundled = 'citizennotes://citizennotes/bundled/citizennotes_app.html';
  const version = '@ffe848af6a5df4fa127e2929331116b7f9f1cb30';
  const remoteOrigin = 'https://chrome-citizennotes-frontend.appspot.com/';
  ...
  it('fetches non-bundled locale files from the remote service endpoint', async () => {
    ...
    assert.isTrue(
        fetchStub.calledWith(
            'citizennotes://citizennotes/remote/serve_file/@ffe848af6a5df4fa127e2929331116b7f9f1cb30/core/i18n/locales/de.json'),
