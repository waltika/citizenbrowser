...
describeWithMockConnection('SourceMapManager', () => {
  it('uses url for a worker\'s source maps from frame', async () => {
    ...
    const frameUrl = 'https://frame-host/index.html' as Platform.CitizenNotesPath.UrlString;
    const scriptUrl = 'https://script-host/script.js' as Platform.CitizenNotesPath.UrlString;
    const sourceUrl = 'script.js' as Platform.CitizenNotesPath.UrlString;
    const sourceMapUrl = 'script.js.map' as Platform.CitizenNotesPath.UrlString;
    ...
    assert.strictEqual(sourceMap?.url(), 'https://frame-host/script.js.map' as Platform.CitizenNotesPath.UrlString);
    assert.deepEqual(
        sourceMap?.sourceURLs(), ['https://frame-host/original-script.js' as Platform.CitizenNotesPath.UrlString]);
  });

  it('can handle source maps in a data URL frame', async () => {
    setupPageResourceLoaderForSourceMap(content);
    const sourceUrl = 'script.js' as Platform.CitizenNotesPath.UrlString;
    const sourceMapUrl = `data:test/html;base64,${btoa(content)}` as Platform.CitizenNotesPath.UrlString;
    const frameSource =
        '<script>0\n//# sourceURL=' + sourceUrl + '\n//# sourceMappingURL=' + sourceMapUrl + '</script>';
    const frameUrl = `data:test/html;base64,${btoa(frameSource)}` as Platform.CitizenNotesPath.UrlString;
    const scriptUrl = 'https://script-host/script.js' as Platform.CitizenNotesPath.UrlString;
    ...
    const sourceMap = await sourceMapManager.sourceMapForClientPromise(script);
    assert.deepEqual(sourceMap?.sourceURLs(), ['/original-script.js' as Platform.CitizenNotesPath.UrlString]);
  });
});

describe('SourceMapManager', () => {
  const sourceURL = 'http://localhost/foo.js' as Platform.CitizenNotesPath.UrlString;
