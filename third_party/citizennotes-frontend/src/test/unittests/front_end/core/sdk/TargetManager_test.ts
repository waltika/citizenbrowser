...
describeWithMockConnection('TargetManager', () => {
  ...
  it('notifies about inspected URL change', () => {
    ...
    targets.forEach(t => t.setInspectedURL(`https://a.com/${t.id()}` as Platform.CitizenNotesPath.UrlString));
    assert.isTrue(inspectedURLChangedHostApi.notCalled && inspectedURLChangedEventListener.notCalled);
    ...
    targets.forEach(t => t.setInspectedURL(`https://b.com/${t.id()}` as Platform.CitizenNotesPath.UrlString));
    ...
    targets.forEach(t => t.setInspectedURL(`https://c.com/${t.id()}` as Platform.CitizenNotesPath.UrlString));
