...
describeWithMockConnection('Target', () => {
  ...
  it('notifies about inspected URL change', () => {
    ...
    subframeTarget.setInspectedURL('https://example.com/' as Platform.CitizenNotesPath.UrlString);
    assert.isTrue(inspectedURLChanged.calledOnce);

    mainFrameTargetWithoutTab.setInspectedURL('https://example.com/' as Platform.CitizenNotesPath.UrlString);
    assert.isTrue(inspectedURLChanged.calledTwice);

    mainFrameTargetUnderTab.setInspectedURL('https://example.com/' as Platform.CitizenNotesPath.UrlString);
