...
export async function initializeGlobalLocaleVars() {
  // Expose the locale.
  i18n.CitizenNotesLocale.CitizenNotesLocale.instance({
    create: true,
    data: {
      navigatorLanguage: 'en-US',
      settingLanguage: 'en-US',
      lookupClosestCitizenNotesLocale: () => 'en-US',
    },
  });

  // Load the strings from the resource file.
  const locale = i18n.CitizenNotesLocale.CitizenNotesLocale.instance().locale;
  // proxied call.
  try {
...
export function deinitializeGlobalLocaleVars() {
  i18n.CitizenNotesLocale.CitizenNotesLocale.removeInstance();
}
