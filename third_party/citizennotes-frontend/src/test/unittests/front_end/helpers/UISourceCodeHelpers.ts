...
class TestPlatformFileSystem extends Persistence.PlatformFileSystem.PlatformFileSystem {
  ...
  constructor(path: Platform.CitizenNotesPath.UrlString, type: string, mimeType: string, autoMapping: boolean) {
    super(path, type);
...
export function createFileSystemUISourceCode(options: {
  ...
  const fileSystemPath = (options.fileSystemPath || '') as Platform.CitizenNotesPath.UrlString;
...
export function setupMockedUISourceCode(url: string = 'https://example.com/') {
  ...
  const urlStringTagExample = url as Platform.CitizenNotesPath.UrlString;
