...
describeWithMockConnection('CompilerScriptMapping', () => {
  ...
  const waitForUISourceCodeAdded =
      (url: string, target: SDK.Target.Target): Promise<Workspace.UISourceCode.UISourceCode> =>
          debuggerWorkspaceBinding.waitForUISourceCodeAdded(url as Platform.CitizenNotesPath.UrlString, target);
  ...
  it('correctly removes UISourceCodes when detaching a sourcemap', async () => {
    ...
    assert.isNull(
        workspace.uiSourceCodeForURL(`${sourceRoot}/a.ts` as Platform.CitizenNotesPath.UrlString),
        '`a.ts` should not be around anymore');
    assert.isNull(
        workspace.uiSourceCodeForURL(`${sourceRoot}/b.ts` as Platform.CitizenNotesPath.UrlString),
