...
describeWithMockConnection('ResourceUtils', () => {
  const displayNameForURL = (targetFactory: () => SDK.Target.Target) => {
    ...
    const INSPECTED_URL_PATH_COMPONENTS = ['', 'foo', 'bar'] as Platform.CitizenNotesPath.UrlString[];
    const INSPECTED_URL_PATH = INSPECTED_URL_PATH_COMPONENTS.join('/') as Platform.CitizenNotesPath.UrlString;
    const INSPECTED_URL =
        INSPECTED_URL_SCHEME + INSPECTED_URL_DOMAIN + INSPECTED_URL_PATH as Platform.CitizenNotesPath.UrlString;

    const RESOURCE_URL = INSPECTED_URL + '?RESOURCE_URL' as Platform.CitizenNotesPath.UrlString;
    const RESOURCE = {displayName: 'RESOURCE_DISPLAY_NAME'} as SDK.Resource.Resource;
    const UI_SOURCE_CODE_URL = INSPECTED_URL + '?UI_SOURCE_CODE_URL' as Platform.CitizenNotesPath.UrlString;
    ...
    const INVALID_URL = ':~INVALID_URL~:' as Platform.CitizenNotesPath.UrlString;
    ...
    it('returns path relative to the last main URL component if it matches and does not have query string',
       async () => {
         assert.strictEqual(
             Bindings.ResourceUtils.displayNameForURL(INSPECTED_URL + QUERY_STRING as Platform.CitizenNotesPath.UrlString),
             INSPECTED_URL_PATH_COMPONENTS.slice(-1)[0] + QUERY_STRING as Platform.CitizenNotesPath.UrlString);
         assert.strictEqual(
             Bindings.ResourceUtils.displayNameForURL(INSPECTED_URL + OTHER_PATH as Platform.CitizenNotesPath.UrlString),
             INSPECTED_URL_PATH_COMPONENTS.slice(-1)[0] + OTHER_PATH as Platform.CitizenNotesPath.UrlString);
       });

    it('returns path relative to the main URL domain if it matches and does have query string', async () => {
      target.setInspectedURL(INSPECTED_URL + QUERY_STRING as Platform.CitizenNotesPath.UrlString);
      assert.strictEqual(
          Bindings.ResourceUtils.displayNameForURL(INSPECTED_URL + QUERY_STRING as Platform.CitizenNotesPath.UrlString),
          INSPECTED_URL_PATH + QUERY_STRING as Platform.CitizenNotesPath.UrlString);
      assert.strictEqual(
          Bindings.ResourceUtils.displayNameForURL(INSPECTED_URL + OTHER_PATH as Platform.CitizenNotesPath.UrlString),
          INSPECTED_URL_PATH + OTHER_PATH as Platform.CitizenNotesPath.UrlString);
    });

    it('returns path relative to the main URL domain if only domain matches', async () => {
      assert.strictEqual(
          Bindings.ResourceUtils.displayNameForURL(
              INSPECTED_URL_SCHEME + INSPECTED_URL_DOMAIN + OTHER_PATH as Platform.CitizenNotesPath.UrlString),
          OTHER_PATH as Platform.CitizenNotesPath.UrlString);
    });
    ...
    it('returns path relative to the main URL domain if path partially matches', async () => {
      assert.strictEqual(
          ...
          '/' + INSPECTED_URL_PATH_COMPONENTS[1] + '/' as Platform.CitizenNotesPath.UrlString);
    });

    it('returns main URL domain if it matches and the path is empty', async () => {
      assert.strictEqual(
          Bindings.ResourceUtils.displayNameForURL(
              INSPECTED_URL_SCHEME + INSPECTED_URL_DOMAIN + '/' as Platform.CitizenNotesPath.UrlString),
          INSPECTED_URL_DOMAIN + '/' as Platform.CitizenNotesPath.UrlString);
    });

    it('strips scheme if domain does not match main URL', async () => {
      assert.strictEqual(
          Bindings.ResourceUtils.displayNameForURL(
              INSPECTED_URL_SCHEME + OTHER_DOMAIN + OTHER_PATH as Platform.CitizenNotesPath.UrlString),
          OTHER_DOMAIN + OTHER_PATH as Platform.CitizenNotesPath.UrlString);
    });
