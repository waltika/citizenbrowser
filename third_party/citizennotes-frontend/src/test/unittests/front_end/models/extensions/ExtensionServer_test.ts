...
import {describeWithCitizennotesExtension} from './helpers.js';
import {type Chrome} from '../../../../../extension-api/ExtensionAPI.js';
import {createTarget, expectConsoleLogs} from '../../helpers/EnvironmentHelpers.js';

describeWithCitizennotesExtension('Extensions', {}, context => {
  it('are initialized after the target is initialized and navigated to a non-privileged URL', async () => {
    ...
    assert.isUndefined(context.chrome.citizennotes);

    const addExtensionStub = sinon.stub(Extensions.ExtensionServer.ExtensionServer.instance(), 'addExtension');
    createTarget().setInspectedURL('http://example.com' as Platform.CitizenNotesPath.UrlString);
  ...
  it('are not initialized before the target is initialized and navigated to a non-privileged URL', async () => {
    ...
    const addExtensionStub = sinon.stub(Extensions.ExtensionServer.ExtensionServer.instance(), 'addExtension');
    createTarget().setInspectedURL('chrome://version' as Platform.CitizenNotesPath.UrlString);
  ...
  it('defers loading extensions until after navigation from a privileged to a non-privileged host', async () => {
    ...
    target.setInspectedURL('chrome://abcdef' as Platform.CitizenNotesPath.UrlString);
...
describeWithCitizennotesExtension('Extensions', {}, context => {
  ...
  it('can register a recorder extension for export', async () => {
    ...
    await context.chrome.citizennotes?.recorder.registerRecorderExtensionPlugin(extensionPlugin, 'Test', 'text/javascript');
    ...
    await context.chrome.citizennotes?.recorder.unregisterRecorderExtensionPlugin(extensionPlugin);
  });
  ...
  it('can register a recorder extension for replay', async () => {
    ...
    const extensionPlugin = new RecorderPlugin();
    await context.chrome.citizennotes?.recorder.registerRecorderExtensionPlugin(extensionPlugin, 'Replay');
  ...
  it('can create and show a panel for Recorder', async () => {
    const panel = await new Promise<Chrome.CitizenNotes.ExtensionPanel>(
        resolve => context.chrome.citizennotes?.panels.create('Test', 'test.png', 'test.html', resolve));
    class RecorderPlugin {
      replay(_recording: object) {
        panel?.show();
      }
  ...
  it('can create and show a view for Recorder', async () => {
    const view = await context.chrome.citizennotes?.recorder.createView('Test', 'test.html');
    class RecorderPlugin {
      replay(_recording: object) {
        view?.show();
      }
  ...
  it('can not show a view for Recorder without using the replay trigger', async () => {
    ...
    await new Promise(resolve => context.chrome.citizennotes?.inspectedWindow.eval('1', undefined, resolve));
...
const allowedUrl = 'http://example.com' as Platform.CitizenNotesPath.UrlString;
const blockedUrl = 'http://web.dev' as Platform.CitizenNotesPath.UrlString;
const hostsPolicy = {
...
describeWithCitizennotesExtension('Runtime hosts policy', {hostsPolicy}, context => {
  expectConsoleLogs({error: ['Extension server error: Operation failed: Permission denied']});

  for (const protocol of ['citizennotes', 'chrome', 'chrome-untrusted', 'chrome-error', 'chrome-search']) {
    it(`blocks API calls on blocked protocols: ${protocol}`, async () => {
      assert.isUndefined(context.chrome.citizennotes);
      const target = createTarget({type: SDK.Target.Type.Frame});
      const addExtensionStub = sinon.stub(Extensions.ExtensionServer.ExtensionServer.instance(), 'addExtension');

      target.setInspectedURL(`${protocol}://foo` as Platform.CitizenNotesPath.UrlString);
      assert.isTrue(addExtensionStub.notCalled);
      assert.isUndefined(context.chrome.citizennotes);
    });
  }
  ...
  it('allows API calls on allowlisted hosts', async () => {
    ...
    target.setInspectedURL(allowedUrl);
    {
      const result = await new Promise<object>(cb => context.chrome.citizennotes?.network.getHAR(cb));
  ...
  it('allows API calls on non-blocked hosts', async () => {
    const target = createTarget({type: SDK.Target.Type.Frame});
    target.setInspectedURL('http://example.com2' as Platform.CitizenNotesPath.UrlString);
    {
      const result = await new Promise<object>(cb => context.chrome.citizennotes?.network.getHAR(cb));
      // eslint-disable-next-line rulesdir/compare_arrays_with_assert_deepequal
      assert.hasAnyKeys(result, ['entries']);
  ...
  it('does not include blocked hosts in the HAR entries', async () => {
    ...
    {
      const result = await new Promise<object>(cb => context.chrome.citizennotes?.network.getHAR(cb)) as HAR.Log.LogDTO;
  ...
  function setUpFrame(
      name: string, url: Platform.CitizenNotesPath.UrlString, parentFrame?: SDK.ResourceTreeModel.ResourceTreeFrame,
      executionContextOrigin?: Platform.CitizenNotesPath.UrlString) {
    ...
    if (executionContextOrigin) {
      executionContextOrigin = new URL(executionContextOrigin).origin as Platform.CitizenNotesPath.UrlString;
  ...
  it('blocks evaluation on blocked subframes', async () => {
    ...
    const result = await new Promise<{result: unknown, error?: {details: unknown[]}}>(
        r => context.chrome.citizennotes?.inspectedWindow.eval(
  ...
  it('doesn\'t block evaluation on blocked sub-executioncontexts with useContentScriptContext', async () => {
    ...
    const parentFrameUrl = allowedUrl;
    const childFrameUrl = `${allowedUrl}/2` as Platform.CitizenNotesPath.UrlString;
  ...
  async function createUISourceCode(
      project: Bindings.ContentProviderBasedProject.ContentProviderBasedProject, url: Platform.CitizenNotesPath.UrlString) {
    const mimeType = 'text/html';
  ...
  it('blocks getting resource contents on blocked urls', async () => {
    ...
    Platform.assertNotNullOrUndefined(context.chrome.citizennotes);
    const resources =
        await new Promise<Chrome.CitizenNotes.Resource[]>(r => context.chrome.citizennotes?.inspectedWindow.getResources(r));
  ...
  function createRequest(
      ...
      requestId: Protocol.Network.RequestId, url: Platform.CitizenNotesPath.UrlString): void {
  ...
  it('does not include blocked hosts in onRequestFinished event listener', async () => {
    ...
    context.chrome.citizennotes?.network.onRequestFinished.addListener(
...
describe('ExtensionServer', () => {
  it('can correctly expand resource paths', async () => {
    ...
    const extensionOrigin = 'chrome://abcdef' as Platform.CitizenNotesPath.UrlString;
    const almostOrigin = `${extensionOrigin}/` as Platform.CitizenNotesPath.UrlString;
    const expectation = `${extensionOrigin}/foo` as Platform.CitizenNotesPath.UrlString;
    assert.strictEqual(
        undefined,
  ...
  it('cannot inspect chrome webstore URLs', () => {
    ...
    for (const url of blockedUrls as Platform.CitizenNotesPath.UrlString[]) {
      assert.isFalse(Extensions.ExtensionServer.ExtensionServer.canInspectURL(url), url);
    }
    for (const url of allowedUrls as Platform.CitizenNotesPath.UrlString[]) {
