...
describe('HAR.Log', () => {
  it('blocked time when no response received is returned in milliseconds (crbug.com/1145177)', async () => {
    ...
    const request = SDK.NetworkRequest.NetworkRequest.create(
        requestId, 'p0.com' as Platform.CitizenNotesPath.UrlString, Platform.CitizenNotesPath.EmptyUrlString, null, null, null);
  ...
  it('_initiator.requestId is exported', async () => {
    const requestId = 'r0' as Protocol.Network.RequestId;
    const request = SDK.NetworkRequest.NetworkRequest.create(
        requestId, 'p0.com' as Platform.CitizenNotesPath.UrlString, Platform.CitizenNotesPath.EmptyUrlString, null, null,
