...
const setUpEnvironmentWithUISourceCode =
    (url: string, resourceType: Common.ResourceType.ResourceType, project?: Workspace.Workspace.Project) => {
      ...
      const uiSourceCode =
          new Workspace.UISourceCode.UISourceCode(project, url as Platform.CitizenNotesPath.UrlString, resourceType);
...
describeWithMockConnection('NetworkPersistenceManager', () => {
  ...
  it('merges request headers with override without overlap', async () => {
    const interceptedRequest = {
      ...
      responseHeaders: [
        {name: 'server', value: 'CitizenNotes mock server'},
      ],
  ...
  it('translates URLs into raw and encoded paths', async () => {
    ...
    toTest.forEach(testStrings => {
      assert.deepEqual(
          networkPersistenceManager.rawPathFromUrl(testStrings.url as Platform.CitizenNotesPath.UrlString),
          testStrings.raw);
      assert.deepEqual(
          networkPersistenceManager.encodedPathFromUrl(testStrings.url as Platform.CitizenNotesPath.UrlString),
          testStrings.encoded);
    });
  });
...
describeWithMockConnection('NetworkPersistenceManager', () => {
  ...
  it('updates active state when target detach and attach', async () => {
    ...
    const {project} =
        createFileSystemUISourceCode({url: 'file:///tmp' as Platform.CitizenNotesPath.UrlString, mimeType: 'text/plain'});
...
describe('NetworkPersistenceManager', () => {
  ...
  it('generates header patterns for global header overrides', async () => {
    ...
    const {uiSourceCode} = createFileSystemUISourceCode({
      url: 'file:///path/to/overrides/.headers' as Platform.CitizenNotesPath.UrlString,
      content: headers,
      mimeType: 'text/plain',
      fileSystemPath: 'file:///path/to/overrides',
    });
  ...
  it('generates header patterns for long URLs', async () => {
    ...
    const {uiSourceCode} = createFileSystemUISourceCode({
      url: 'file:///path/to/overrides/www.longurls.com/longurls/.headers' as Platform.CitizenNotesPath.UrlString,
      content: headers,
      mimeType: 'text/plain',
      fileSystemPath: 'file:///path/to/overrides',
    });
