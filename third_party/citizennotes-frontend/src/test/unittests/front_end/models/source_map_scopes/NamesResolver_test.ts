...
describeWithMockConnection('NameResolver', () => {
  const URL = 'file:///tmp/example.js' as Platform.CitizenNotesPath.UrlString;
  ...
  it('resolves inner scope clashing names from let -> var transpilation', async () => {
    ...
    // to var-variables to avoid clash, and CitizenNotes then (somewhat questionably) deobfuscates the var variables
    // back to the original names in the function scope (as opposed to the original block scopes). Ideally, CitizenNotes
