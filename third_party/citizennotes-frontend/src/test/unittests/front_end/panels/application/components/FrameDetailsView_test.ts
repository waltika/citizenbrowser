...
describeWithRealConnection('FrameDetailsView', () => {
  it('renders with a title', async () => {
    ...
    const report = getElementWithinComponent(component, 'citizennotes-report', ReportView.ReportView.Report);
  ...
  it('renders report keys and values', async () => {
    ...
    const keys = getCleanTextContentFromElements(component.shadowRoot, 'citizennotes-report-key');
    ...
    const values = getCleanTextContentFromElements(component.shadowRoot, 'citizennotes-report-value');
    ...
    const stackTrace = getElementWithinComponent(
        component, 'citizennotes-resources-stack-trace', ApplicationComponents.StackTrace.StackTrace);
    assertShadowRoot(stackTrace.shadowRoot);
    const expandableList =
        getElementWithinComponent(stackTrace, 'citizennotes-expandable-list', ExpandableList.ExpandableList.ExpandableList);
    assertShadowRoot(expandableList.shadowRoot);

    const stackTraceRows = getElementsWithinComponent(
        expandableList, 'citizennotes-stack-trace-row', ApplicationComponents.StackTrace.StackTraceRow);
    ...
    const adScriptLink = component.shadowRoot.querySelector('citizennotes-report-value.ad-script-link');
