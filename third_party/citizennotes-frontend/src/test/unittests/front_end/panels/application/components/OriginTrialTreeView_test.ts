...
async function renderOriginTrialTreeViewTreeOutline(
  ...
  const treeOutline: OriginTrialTreeOutline =
      getElementWithinComponent<ApplicationComponents.OriginTrialTreeView.OriginTrialTreeView, OriginTrialTreeOutline>(
          component, 'citizennotes-tree-outline', TreeOutline.TreeOutline.TreeOutline);
...
function extractBadgeTextFromTreeNode(node: HTMLLIElement): string[] {
  return [...node.querySelectorAll('citizennotes-resources-origin-trial-tree-view-badge')].map(badgeElement => {
    assertShadowRoot(badgeElement.shadowRoot);
    const adornerElement = badgeElement.shadowRoot.querySelector('citizennotes-adorner');
...
describeWithLocale('OriginTrialTreeView', () => {
  ...
  it('renders token fields', async () => {
    ...
    const rowsComponent = tokenFieldsNode.nodeElement.querySelector('citizennotes-resources-origin-trial-token-rows');
