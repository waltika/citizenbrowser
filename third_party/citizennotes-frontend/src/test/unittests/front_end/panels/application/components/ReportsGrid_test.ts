...
const renderReportsGrid =
  ...
  const datagrid = getElementWithinComponent(controller, 'citizennotes-data-grid', DataGrid.DataGrid.DataGrid);
...
const getHeaderText = (cell: HTMLTableCellElement): string => {
  const ret = cell.textContent?.trim() ||
      cell.querySelector('citizennotes-resources-reports-grid-status-header')?.shadowRoot?.textContent?.trim();
