...
function mockBuildStackTraceRows(
  ...
  return stackTrace.callFrames.map(
      callFrame => ({
        ...
        link: Components.Linkifier.Linkifier.linkifyURL(callFrame.url as Platform.CitizenNotesPath.UrlString),
        rowCountHide: false,
      }));
}
...
describeWithLocale('StackTrace', () => {
  ...
  it('generates rows from stack trace data', () => {
    ...
    const expandableList =
        getElementWithinComponent(component, 'citizennotes-expandable-list', ExpandableList.ExpandableList.ExpandableList);
  ...
  it('hides hidden rows behind "show all" button', async () => {
    ...
    const stackTraceLinkButton = getElementWithinComponent(
        expandableList, 'citizennotes-stack-trace-link-button', ApplicationComponents.StackTrace.StackTraceLinkButton);
