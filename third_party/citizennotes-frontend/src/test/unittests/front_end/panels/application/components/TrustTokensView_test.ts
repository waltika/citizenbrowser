...
function getInternalDataGridShadowRoot(component: ApplicationComponents.TrustTokensView.TrustTokensView): ShadowRoot {
  const dataGridController = getElementWithinComponent(
      component, 'citizennotes-data-grid-controller', DataGrid.DataGridController.DataGridController);
  const dataGrid = getElementWithinComponent(dataGridController, 'citizennotes-data-grid', DataGrid.DataGrid.DataGrid);
...
describeWithMockConnection('TrustTokensView', () => {
  ...
  it('hides trust token table when there are no trust tokens', async () => {
    ...
    const nullGridElement = component.shadowRoot.querySelector('citizennotes-data-grid-controller');
  ...
  it('calls the delete handler with the right issuer when the delete button is clicked in a row', async () => {
    ...
    const deleteButtonComponent = deleteCell.querySelector('citizennotes-trust-tokens-delete-button');
