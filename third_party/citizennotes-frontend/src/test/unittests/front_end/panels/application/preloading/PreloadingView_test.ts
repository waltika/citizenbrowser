...
class NavigationEmulator {
  ...
  async openCitizenNotes(): Promise<void> {
...
describeWithMockConnection('PreloadingRuleSetView', async () => {
  it('renders grid and details', async () => {
    ...
    await emulator.openCitizenNotes();
...
`);
    ...
    const report =
        getElementWithinComponent(preloadingDetailsComponent, 'citizennotes-report', ReportView.ReportView.Report);

    const keys = getCleanTextContentFromElements(report, 'citizennotes-report-key');
    const values = getCleanTextContentFromElements(report, 'citizennotes-report-value');
    ...
    const buttons = report.querySelectorAll('citizennotes-report-value:nth-of-type(2) citizennotes-button');
...
async function testWarnings(
  ...
  const infobar = infobarContainer.querySelector('citizennotes-resources-preloading-disabled-infobar');
    ...
    await testWarnings(
        ...
        'Speculative loading is force-enabled', [
          [
            ...
            'Prefetch is forced-enabled because CitizenNotes is open. When CitizenNotes is closed, prefetch will be disabled because this browser session is part of a holdback group used for performance comparisons.',
          ],
        ]);
  });
    ...
    await testWarnings(
        ...
        'Speculative loading is force-enabled', [
          [
            ...
            'Prerendering is forced-enabled because CitizenNotes is open. When CitizenNotes is closed, prerendering will be disabled because this browser session is part of a holdback group used for performance comparisons.',
          ],
        ]);
  });
