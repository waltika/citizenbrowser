...
function assertDiff(
  ...
  const controller = getElementWithinComponent(
      gridComponent, 'citizennotes-data-grid-controller', DataGrid.DataGridController.DataGridController);
  const grid = getElementWithinComponent(controller, 'citizennotes-data-grid', DataGrid.DataGrid.DataGrid);
...
describeWithEnvironment('MismatchedPreloadingGrid', async () => {
  ...
  it('renderes edit diff', async () => {
    const data: PreloadingComponents.MismatchedPreloadingGrid.MismatchedPreloadingGridData = {
      pageURL: 'https://example.com/prefetched.html?q=1' as Platform.CitizenNotesPath.UrlString,
      rows: [{
