...
describeWithMockConnection('ConsoleView', () => {
  ...
  async function canSaveToFile(targetFactory: () => SDK.Target.Target) {
    ...
    const FILENAME = `${URL_HOST}-${TIMESTAMP}.log` as Platform.CitizenNotesPath.RawPathString;
  ...
  describe('self-XSS warning', () => {
    ...
    it('shows', async () => {
      ...
      assert.strictEqual(
          ...
          'Warning: Don’t paste code into the CitizenNotes Console that you don’t understand or haven’t reviewed yourself. This could allow attackers to steal your identity or take control of your computer. Please type ‘allow pasting’ below to allow pasting.');
    });
