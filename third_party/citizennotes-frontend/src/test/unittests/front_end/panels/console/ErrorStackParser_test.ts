...
describe('ErrorStackParser', () => {
  ...
  beforeEach(() => {
    ...
    runtimeModel = sinon.createStubInstance(SDK.RuntimeModel.RuntimeModel, {
      target: sinon.createStubInstance(SDK.Target.Target, {
        inspectedURL: 'http://www.example.org' as Platform.CitizenNotesPath.UrlString,
      }),
  ...
  it('detects URLs with parens', () => {
    const url =
        'http://localhost:5173/src/routes/(v2-routes)/project/+layout.ts?ts=12345' as Platform.CitizenNotesPath.UrlString;
  ...
  it('correctly handles eval frames', () => {
    const url = 'http://www.chromium.org/foo.js' as Platform.CitizenNotesPath.UrlString;
  ...
  it('uses the inspected target URL to complete relative URLs', () => {
    ...
    assertNotNullOrUndefined(frames);
    assert.strictEqual(frames[1].link?.url, 'http://www.example.org/testing.js' as Platform.CitizenNotesPath.UrlString);
  });
  ...
  it('uses the inspected target URL to complete relative URLs in eval frames', () => {
    ...
    assert.lengthOf(frames, 3);
    assert.deepStrictEqual(frames[2].link, {
      url: 'http://www.example.org/inspected-page.html' as Platform.CitizenNotesPath.UrlString,
  ...
  it('uses the inspected target URL to complete relative URLs with parens', () => {
    ...
    assert.deepStrictEqual(frames[1].link, {
      url: 'http://www.example.org/(abc)/foo.js' as Platform.CitizenNotesPath.UrlString,
      prefix: '        at foo (',
      suffix: ')',
