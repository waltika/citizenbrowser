...
describeWithMockConnection('NetworkLogView', () => {
  const tests = (targetFactory: () => SDK.Target.Target) => {
    ...
    it('can create curl command parameters when some headers do not have value', async () => {
      const request = createNetworkRequest('https://www.example.com/file.html' as Platform.CitizenNotesPath.UrlString, {
        requestHeaders: [
    ...
    const tests = (inScope: boolean) => () => {
      ...
      it('can export all as HAR', async () => {
        ...
        target.setInspectedURL(`http://${URL_HOST}/foo` as Platform.CitizenNotesPath.UrlString);
        const FILENAME = `${URL_HOST}.har` as Platform.CitizenNotesPath.RawPathString;
      ...
      it('can import and filter from HAR', async () => {
        const URL_1 = 'http://example.com/' as Platform.CitizenNotesPath.UrlString;
        const URL_2 = 'http://example.com/favicon.ico' as Platform.CitizenNotesPath.UrlString;
        function makeHarEntry(url: Platform.CitizenNotesPath.UrlString) {
          return {
    ...
    it('hide Chrome extension requests from checkbox', async () => {
      ...
      assert.deepEqual(
          rootNode.children.map(n => (n as Network.NetworkDataGridNode.NetworkNode).request()?.url()),
          ['chrome-extension://url1' as Platform.CitizenNotesPath.UrlString, 'url2' as Platform.CitizenNotesPath.UrlString]);

      clickCheckbox(hideExtCheckbox);
      assert.deepEqual(
          rootNode.children.map(n => (n as Network.NetworkDataGridNode.NetworkNode).request()?.url()),
          ['url2' as Platform.CitizenNotesPath.UrlString]);
    ...
    it('can filter requests with blocked response cookies from checkbox', async () => {
      ...
      assert.deepEqual(rootNode.children.map(n => (n as Network.NetworkDataGridNode.NetworkNode).request()?.url()), [
        'url1' as Platform.CitizenNotesPath.UrlString,
      ]);
    ...
    it('can filter requests with blocked response cookies from dropdown', async () => {
      ...
      assert.deepEqual(
          rootNode.children.map(n => (n as Network.NetworkDataGridNode.NetworkNode).request()?.url()),
          ['url1' as Platform.CitizenNotesPath.UrlString, 'url2' as Platform.CitizenNotesPath.UrlString]);
    ...
    function createOverrideRequests() {
      const urlNotOverridden = 'url-not-overridden' as Platform.CitizenNotesPath.UrlString;
      const urlHeaderOverridden = 'url-header-overridden' as Platform.CitizenNotesPath.UrlString;
      const urlContentOverridden = 'url-content-overridden' as Platform.CitizenNotesPath.UrlString;
      const urlHeaderAndContentOverridden = 'url-header-und-content-overridden' as Platform.CitizenNotesPath.UrlString;
...
function getSoftMenu(): HTMLElement {
  const container = document.querySelector('div[data-citizennotes-glass-pane]');
