...
async function contentData(): Promise<SDK.NetworkRequest.ContentData> {
  const content = '<style> p { color: red; }</style><link rel="stylesheet" ref="http://citizennotes-frontend.test/style">';
...
describeWithLocale('RequestPreviewView', () => {
  it('prevents previewed html from making same-site requests', async () => {
    const request = SDK.NetworkRequest.NetworkRequest.create(
        ...
        'http://citizennotes-frontend.test/content' as Platform.CitizenNotesPath.UrlString,
