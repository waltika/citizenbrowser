...
async function editHeaderRow(
  ...
  assertShadowRoot(component.shadowRoot);
  const rows = component.shadowRoot.querySelectorAll('citizennotes-header-section-row');
  ...
  const editableComponent = row.shadowRoot.querySelector(`${selector} citizennotes-editable-span`);
...
async function removeHeaderRow(
  ...
  assertShadowRoot(component.shadowRoot);
  const row = component.shadowRoot.querySelectorAll('citizennotes-header-section-row')[index];
...
async function setupHeaderEditingWithRequest(
  ...
  const networkPersistenceManager =
      await createWorkspaceProject('file:///path/to/overrides' as Platform.CitizenNotesPath.UrlString, [
        {
          name: '.headers',
  ...
  if (project) {
    const uiSourceCode = project.uiSourceCodeForURL(
        'file:///path/to/overrides/www.example.com/.headers' as Platform.CitizenNotesPath.UrlString);
    if (uiSourceCode) {
...
function checkHeaderSectionRow(
  ...
  const nameEditableComponent =
      row.shadowRoot.querySelector<NetworkComponents.EditableSpan.EditableSpan>('.header-name citizennotes-editable-span');
  if (isNameEditable) {
  ...
  const valueEditableComponent =
      row.shadowRoot.querySelector<NetworkComponents.EditableSpan.EditableSpan>('.header-value citizennotes-editable-span');
  if (isValueEditable) {
...
describeWithEnvironment('ResponseHeaderSection', () => {
  ...
  it('renders detailed reason for blocked requests', async () => {
    ...
    const row = component.shadowRoot.querySelectorAll('citizennotes-header-section-row')[1];
    assertElement(row, HTMLElement);
    assertShadowRoot(row.shadowRoot);
  ...
  it('displays info about blocked "Set-Cookie"-headers', async () => {
    ...
    const row = component.shadowRoot.querySelector('citizennotes-header-section-row');
    assertElement(row, HTMLElement);
    assertShadowRoot(row.shadowRoot);
    ...
    const icon = row.shadowRoot.querySelector('citizennotes-icon');
  ...
  it('marks overridden headers', async () => {
    const request = {
      sortedResponseHeaders: [
        ...
        {name: 'duplicate-both-with-mismatch', value: 'CitizenNotes'},
    ...
    checkRow(rows[3].shadowRoot, 'duplicate-both-with-mismatch:', 'CitizenNotes', true);
  ...
  it('can remove header overrides', async () => {
    ...
    const {component, spy} = await setupHeaderEditing(headerOverridesFileContent, actualHeaders, originalHeaders);
    assertShadowRoot(component.shadowRoot);
    let rows = component.shadowRoot.querySelectorAll('citizennotes-header-section-row');
    ...
    rows = component.shadowRoot.querySelectorAll('citizennotes-header-section-row');
    assert.strictEqual(rows.length, 3);
    checkHeaderSectionRow(rows[0], 'added:', 'foo', true, false, true);
    checkHeaderSectionRow(rows[1], 'cache-control:', 'max-age=9999', true, false, true);
  ...
  it('renders headers as (not) editable depending on overall overrides setting', async () => {
    const request = SDK.NetworkRequest.NetworkRequest.create(
        ...
        'https://www.example.com/index.html' as Platform.CitizenNotesPath.UrlString, '' as Platform.CitizenNotesPath.UrlString,
        null, null, null);
    ...
    rows = component2.shadowRoot.querySelectorAll('citizennotes-header-section-row');
    assert.strictEqual(rows.length, 2);
    ...
    rows = component3.shadowRoot.querySelectorAll('citizennotes-header-section-row');
    assert.strictEqual(rows.length, 2);
    checkHeaderSectionRow(rows[0], 'server:', 'overridden server', true, false, true);
    checkHeaderSectionRow(rows[1], 'header-name:', 'header value', true, true, true);
  });
  ...
  it('persists edits to header overrides and resurfaces them upon component (re-)creation', async () => {
    ...
    const rows = component2.shadowRoot.querySelectorAll('citizennotes-header-section-row');
    assert.strictEqual(rows.length, 2);
