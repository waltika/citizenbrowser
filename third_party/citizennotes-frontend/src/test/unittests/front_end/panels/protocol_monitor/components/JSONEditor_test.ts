...
describeWithEnvironment('JSONEditor', () => {
  ...
  const renderSuggestionBox = async(
    ...
    const inputs = jsonEditor.renderRoot.querySelectorAll('citizennotes-suggestion-input');
    // inputs[0] corresponds to the citizennotes-suggestion-input of the command
    ...
    const suggestionBox = suggestionInput.renderRoot.querySelector('citizennotes-suggestion-box');
  ...
  const serializePopupContent = (): string|null|undefined => {
    const container = document.body.querySelector<HTMLDivElement>('[data-citizennotes-glass-pane]');
    const hintDetailView = container?.shadowRoot?.querySelector('citizennotes-css-hint-details-view');
  ...
  const renderEditorForCommand = async(command: string, parameters: {[paramName: string]: unknown}): Promise<{
    ...
    const inputs = shadowRoot.querySelectorAll('citizennotes-suggestion-input');
  ...
  const renderParamsWithDefaultValues =
    ...
    const setDefaultValueButton = jsonEditor.renderRoot.querySelector('citizennotes-button');
    ...
    const input = jsonEditor.renderRoot.querySelectorAll('citizennotes-suggestion-input');
  ...
  const renderWarningIcon =
    ...
    // inputs[0] corresponds to the citizennotes-suggestion-input of the command
    const input = jsonEditor.renderRoot.querySelectorAll('citizennotes-suggestion-input')[1];
    if (!input) {
    ...
    input.blur();
    await jsonEditor.updateComplete;
    const warningIcon = jsonEditor.renderRoot.querySelector('citizennotes-icon');
    if (!warningIcon) {
  ...
  describe('Suggestion box', () => {
    ...
    it('should show the suggestion box for enum parameters nested inside arrays', async () => {
      ...
      const addParamButton = jsonEditor.renderRoot.querySelector('citizennotes-button[title="Add a parameter"]');
      if (!addParamButton) {
        throw new Error('No button');
      }
    ...
    it('should show the keys with default values when clicking of plus button for optional object parameters',
       async () => {
         ...
         const showDefaultValuesButton =
             jsonEditor.renderRoot.querySelector('citizennotes-button[title="Add a parameter"]');
         ...
         const numberOfInputs = jsonEditor.renderRoot.querySelectorAll('citizennotes-suggestion-input').length - 1;
  ...
  describe('Reset to default values', () => {
    ...
    it('should reset the value of array parameter to empty array when clicking on clear button', async () => {
      ...
      const setDefaultValueButton =
          jsonEditor.renderRoot.querySelector('citizennotes-button[title="Reset to default value"]');
    ...
    it('should reset the value of optional object parameter to undefined after clicking on clear button', async () => {
      ...
      const showDefaultValuesButton = jsonEditor.renderRoot.querySelector('citizennotes-button[title="Add a parameter"]');
      if (!showDefaultValuesButton) {
        throw new Error('No button');
      }
      ...
      await renderHoveredElement(param);
      const clearButton = jsonEditor.renderRoot.querySelector('citizennotes-button[title="Reset to default value"]');
  ...
  describe('Delete and add for array parameters', () => {
    it('should delete the specified array parameter by clicking the "Delete" button', async () => {
      ...
      const deleteButtons = shadowRoot.querySelectorAll('citizennotes-button[title="Delete parameter"]');
  ...
  describe('Send parameters in a correct format', () => {
    ...
    it('should return the parameters in a format understandable by the ProtocolMonitor when sending a command via the send button',
       async () => {
         ...
         const toolbar = jsonEditor.renderRoot.querySelector('citizennotes-pm-toolbar');
         if (!toolbar) {
  ...
  it('should not display parameters if a command is unknown', async () => {
    ...
    const inputs = jsonEditor.renderRoot.querySelectorAll('citizennotes-suggestion-input');
    const addButtons = jsonEditor.renderRoot.querySelectorAll('citizennotes-button[title="Add a parameter"]');
  ...
  it('checks that the selection of a target works', async () => {
    ...
    const shadowRoot = jsonEditor.renderRoot;
    const selectMenu = shadowRoot.querySelector('citizennotes-select-menu');
  ...
  it('should return the parameters in a format understandable by the ProtocolMonitor when sending a command with object parameter that has no typeRef found in map',
     async () => {
       ...
       const addParamButton = jsonEditor.renderRoot.querySelector('citizennotes-button[title="Add custom property"]');
       if (!addParamButton) {
         throw new Error('No button');
       }
       ...
       await jsonEditor.updateComplete;
       const editors = shadowRoot.querySelectorAll('citizennotes-suggestion-input');
