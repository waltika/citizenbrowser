...
describeWithEnvironment('RecordingView', () => {
  ...
  async function waitForTextEditor(
    ...
    const textEditor = view.shadowRoot?.querySelector('citizennotes-text-editor');
  ...
  function hoverOverScrollStep(
    ...
    const steps = view.shadowRoot?.querySelectorAll('citizennotes-step-view') || [];
  ...
  async function waitForSplitViewToDissappear(
    ...
    await getEventPromise(view, 'code-generated');
    const splitView = view.shadowRoot?.querySelector('citizennotes-split-view');
