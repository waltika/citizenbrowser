...
describeWithLocale('StepEditor', () => {
  async function renderEditor(
    ...
    const editor = document.createElement('citizennotes-recorder-step-editor');
  ...
  function getInputByAttribute(
    ...
    const input = editor.renderRoot.querySelector(
        `.attribute[data-attribute="${attribute}"] citizennotes-suggestion-input`,
    );
    if (!input) {
      throw new Error(`${attribute} citizennotes-suggestion-input not found`);
    }
  ...
  async function addOptionalField(
    ...
    const button = editor.renderRoot.querySelector(
        `citizennotes-button.add-row[data-attribute="${attribute}"]`,
    );
  ...
  async function deleteOptionalField(
    ...
    const button = editor.renderRoot.querySelector(
        `citizennotes-button.delete-row[data-attribute="${attribute}"]`,
    );
    assertElement(button, HTMLElement);
    button.click();
    await triggerMicroTaskQueue();
  ...
  async function clickFrameLevelButton(
    ...
    const button = editor.renderRoot.querySelector(
        `.attribute[data-attribute="frame"] citizennotes-button${className}`,
    );
    assertElement(button, HTMLElement);
    button.click();
    await editor.updateComplete;
  ...
  async function clickSelectorLevelButton(
    ...
    const button = editor.renderRoot.querySelector(
        `[data-selector-path="${path.join('.')}"] citizennotes-button${className}`,
    );
    assertElement(button, HTMLElement);
    button.click();
    await editor.updateComplete;
    ...
    });
    {
      ...
      assert.isTrue(
          editor.shadowRoot?.activeElement?.matches(
              'citizennotes-suggestion-input[data-path="frame.1"]',
              ),
      );
    }
    {
      ...
      assert.isTrue(
          editor.shadowRoot?.activeElement?.matches(
              'citizennotes-suggestion-input[data-path="frame.0"]',
              ),
      );
    }
  });
  ...
  it('should add/remove selector parts', async () => {
...

    {
      ...
      assert.isTrue(
          editor.shadowRoot?.activeElement?.matches(
              'citizennotes-suggestion-input[data-path="selectors.0.1"]',
              ),
      );
    }

    {
      ...
      assert.isTrue(
          editor.shadowRoot?.activeElement?.matches(
              'citizennotes-suggestion-input[data-path="selectors.0.0"]',
              ),
      );
    }
  });
  ...
  it('should add/remove selectors', async () => {
    ...
    });
    {
      ...
      assert.isTrue(
          editor.shadowRoot?.activeElement?.matches(
              'citizennotes-suggestion-input[data-path="selectors.1.0"]',
              ),
      );
    }
    {
  ...
  it('clears text selection when navigating away from citizennotes-suggestion-input', async () => {
    const editor = await renderEditor({type: Models.Schema.StepType.Scroll});

    // Clicking on the type citizennotes-suggestion-input should select the entire text in the field.
    const input = getInputByAttribute(editor, 'type');
    input.focus();
    input.click();
