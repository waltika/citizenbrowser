...
describe('HeadersView', async () => {
      ...
      headerOverrides: [
        {
          ...
          headers: [
            {
              name: 'server',
              value: 'CitizenNotes Unit Test Server',
            },
            {
  ...
  it('displays data and allows editing', async () => {
    ...
    assert.deepEqual(rows, [
      'Apply to:*',
      'server:CitizenNotes Unit Test Server',
    ...
    assert.strictEqual(learnMoreLink.title, 'https://goo.gle/citizennotes-override');
    ...
    assert.deepEqual(rows, [
      'Apply to:index.html',
      'content-type:CitizenNotes Unit Test Server',
  ...
  it('resets edited value to previous state on Escape key', async () => {
    const editor = await renderEditor();
    assertShadowRoot(editor.shadowRoot);
    assert.deepEqual(getSingleRowContent(editor.shadowRoot, 1), 'server:CitizenNotes Unit Test Server');
    ...
    assert.deepEqual(getSingleRowContent(editor.shadowRoot, 1), 'discard_me_2:CitizenNotes Unit Test Server');
