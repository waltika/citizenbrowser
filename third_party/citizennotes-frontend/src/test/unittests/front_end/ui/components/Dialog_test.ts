...
class DialogExampleWindowBoundsServiceFactory implements WindowBoundsService.WindowBoundsService.WindowBoundsService {
  ...
  getCitizenNotesBoundingElement(): HTMLElement {
...
describe('Dialog', () => {
  describe('positioning', () => {
    ...
    it.skip(
        ...
        async () => {
          const citizennotesDialog = new Dialogs.Dialog.Dialog();
          const WINDOW_WIDTH = 500;
          // This container will be set as the dialog's "window", or the representation
          // of CitizenNotes bounding element.
          ...
          citizennotesDialog.origin = Dialogs.Dialog.MODAL;

          // Set the dialog's "window" to be the container element we just created.
          citizennotesDialog.windowBoundsService = new DialogExampleWindowBoundsServiceFactory(container);

          host.addEventListener('click', () => citizennotesDialog.setDialogVisible(true));
          citizennotesDialog.addEventListener('clickoutsidedialog', () => citizennotesDialog.setDialogVisible(false));

          container.appendChild(host);
          container.appendChild(citizennotesDialog);
          Helpers.renderElementIntoDOM(container);
          await coordinator.done();
          citizennotesDialog.appendChild(content);

          // Open the dialog and check its position.
          Helpers.dispatchClickEvent(host);
          await coordinator.done();
          const dialog = citizennotesDialog.shadowRoot?.querySelector('dialog');
          if (!dialog) {
    ...
    describe('with an anchor and possible overflow', () => {
      ...
      beforeEach(async () => {
        ...
        citizennotesDialog.origin = host;
        citizennotesDialog.horizontalAlignment = Dialogs.Dialog.DialogHorizontalAlignment.CENTER;
      ...
      it('sets the max width and height correctly when the dialog\'s content dimensions exceed the viewport and the dialog is anchored to the left',
         async () => {
           citizennotesDialog.horizontalAlignment = Dialogs.Dialog.DialogHorizontalAlignment.LEFT;
           // Open the dialog and check its position.
           Helpers.dispatchClickEvent(host);
           await coordinator.done();
           const dialog = citizennotesDialog.shadowRoot?.querySelector('dialog');
      ...
      it('sets the max width and height correctly when the dialog\'s content dimensions exceed the viewport and the dialog is anchored to the right',
         async () => {
           citizennotesDialog.horizontalAlignment = Dialogs.Dialog.DialogHorizontalAlignment.RIGHT;
           await coordinator.done();
           // Open the dialog and check its position.
           Helpers.dispatchClickEvent(host);
           await coordinator.done();
      ...
      it('sets the dialog\'s horizontal position correctly to prevent overlap with CitizenNotes when alinged to the left.',
         async () => {
           citizennotesDialog.horizontalAlignment = Dialogs.Dialog.DialogHorizontalAlignment.LEFT;
      ...
      it('sets the dialog\'s horizontal position correctly to prevent overlap with CitizenNotes when alinged to the right.',
         async () => {
           citizennotesDialog.horizontalAlignment = Dialogs.Dialog.DialogHorizontalAlignment.RIGHT;
      ...
      it('sets the dialog\'s horizontal position correctly to prevent overlapping with CitizenNotes on the right when alinged to the center.',
         async () => {
           const containerWidth = container.clientWidth;
      ...
      it('sets the dialog\'s horizontal position correctly to prevent overlapping with CitizenNotes on the left when alinged to the center.',
         async () => {
  ...
  describe('closing the dialog with the ESC key', () => {
    let citizennotesDialog: Dialogs.Dialog.Dialog;
    beforeEach(async () => {
      citizennotesDialog = new Dialogs.Dialog.Dialog();
    ...
    it('closes the dialog by default when the ESC key is pressed', async () => {
      let dialog = citizennotesDialog.shadowRoot?.querySelector('dialog[open]');
      if (!dialog) {
        assert.fail('Dialog not found');
        return;
      }
      ...
      Helpers.dispatchKeyDownEvent(dialog, {key: Platform.KeyboardUtilities.ESCAPE_KEY, bubbles: true, composed: true});
      await coordinator.done();
      dialog = citizennotesDialog.shadowRoot?.querySelector('dialog[open]');
      if (dialog) {
    ...
    it('closes the dialog by default when the ESC key is pressed anywhere within the citizennotes bounding element',
       async () => {
         let dialog = citizennotesDialog.shadowRoot?.querySelector('dialog[open]');
         if (!dialog) {
           assert.fail('Dialog not found');
         ...
         const boundingElement = citizennotesDialog.windowBoundsService.getCitizenNotesBoundingElement();
         Helpers.dispatchKeyDownEvent(
    ...
    it('does not close the dialog when the ESC key is pressed if the closeOnESC prop is set to false', async () => {
      let dialog = citizennotesDialog.shadowRoot?.querySelector('dialog[open]');
      citizennotesDialog.closeOnESC = false;
      if (!dialog) {
        assert.fail('Dialog not found');
        return;
      }
