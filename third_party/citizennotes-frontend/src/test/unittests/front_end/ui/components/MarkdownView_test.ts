...
describeWithEnvironment('MarkdownView', async () => {
  describe('renderToken', async () => {
    ...
    it('renders link with valid key', () => {
      ...
      assert.isTrue(renderResult.includes('<citizennotes-markdown-link'));
    });
    ...
    it('renders icon with valid key', () => {
      ...
      assert.isTrue(renderResult.includes('<citizennotes-markdown-image'));
    });
...
`;
  ...
  const renderString =
      (string: string, selector: 'p'|'code'|'citizennotes-code-block',
