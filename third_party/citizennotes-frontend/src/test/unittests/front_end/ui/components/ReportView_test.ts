...
describe('ReportView', () => {
  ...
  describe('row', () => {
    it('renders the elements provided for the "key" and "value" slot', () => {
      ...
      LitHtml.render(
          LitHtml.html`
        <citizennotes-report-key>This is the key</citizennotes-report-key>
        <citizennotes-report-value>This is the value</citizennotes-report-value>
      `,
          report);
      renderElementIntoDOM(report);

      const slot = getElementWithinComponent(report, 'slot', HTMLSlotElement);
      const keyElement = report.querySelector('citizennotes-report-key');
      const valueElement = report.querySelector('citizennotes-report-value');
