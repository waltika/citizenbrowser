...
describeWithLocale('Feedback button', () => {
  it('calls out to the Host API to open the link in a new tab', async () => {
    ...
    component.data = {
      feedbackUrl: 'https://feedbackurl.com' as Platform.CitizenNotesPath.UrlString,
    };

    renderElementIntoDOM(component);
    await coordinator.done();
    assertShadowRoot(component.shadowRoot);
    const button = component.shadowRoot.querySelector('citizennotes-button');
    assertElement(button, HTMLElement);
    dispatchClickEvent(button);
    assert.strictEqual(openInNewTabStub.callCount, 1);
    assert.isTrue(
        openInNewTabStub.firstCall.calledWith('https://feedbackurl.com' as Platform.CitizenNotesPath.UrlString),
