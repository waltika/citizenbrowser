...
describeWithEnvironment('SourceFrame', () => {
  ...
  it('shows self-XSS warning which the user can disable', async () => {
    ...
    const sourceFrame = await createSourceFrame('Example');
    const codeMirror =
        sourceFrame.element.querySelector('citizennotes-text-editor')?.shadowRoot?.querySelector('.cm-content') || null;
    ...
    const dialogShadowRoot = document.body.querySelector<HTMLDivElement>('[data-citizennotes-glass-pane]')
    ...
    dispatchPasteEvent(codeMirror, {clipboardData: dt, bubbles: true});
    await new Promise(resolve => setTimeout(resolve, 0));
    const dialogContainer = document.body.querySelector<HTMLDivElement>('[data-citizennotes-glass-pane]');
